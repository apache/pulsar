...
public abstract class DeviationShedder implements LoadSheddingStrategy {
    ...
    // Get the threshold above which the standard deviation of a broker is large enough to warrant unloading bundles.
    protected abstract double getDeviationThreshold(ServiceConfiguration conf);

    /**
     * Recommend that all of the returned bundles be unloaded based on observing obsessive standard deviations
    ...
    public Map<String, String> selectBundlesForUnloading(final LoadData loadData, final ServiceConfiguration conf) {
        ...
        // While the most loaded broker is above the standard deviation threshold, continue to move bundles.
        while ((metricTreeSetCache.last().getKey() - mean) / standardDeviation > deviationThreshold) {
            ...
            while (!(bundleTreeSetCache.isEmpty() || selected)) {
                ...
                // When the bundle is moved, we want the now least loaded server to have lower overall load than the
                // most loaded server does not. Thus, we will only consider moving the bundle if this condition
