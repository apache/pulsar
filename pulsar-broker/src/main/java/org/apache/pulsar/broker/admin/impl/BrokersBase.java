...
public class BrokersBase extends AdminResource {
    ...
    public void getActiveBrokers(@Suspended final AsyncResponse asyncResponse, @PathParam("cluster") String cluster) {
    ...
    public void getLeaderBroker(@Suspended final AsyncResponse asyncResponse) {
        validateSuperUserAccessAsync()
                .thenAccept(__ -> {
                    LeaderBroker leaderBroker = pulsar().getLeaderElectionService()
                            .getCurrentLeader()
    ...
    @ApiOperation(
            value = "Get the list of namespaces served by the specific broker",
            response = NamespaceOwnershipStatus.class,
    ...
    public void getOwnedNamespaces(
        ...
        validateSuperUserAccessAsync()
                ...
                .exceptionally(ex -> {
                    ...
                    if (!isRedirectException(ex)) {
                        LOG.error(
                                "[{}] Failed to get the namespace ownership status. cluster={}, broker={}",
                                clientAppId(),
                                cluster,
                                broker);
                    }
    ...
    @ApiOperation(
            value = "Delete dynamic ServiceConfiguration into metadata only."
    ...
    public void getDynamicConfigurationName(@Suspended AsyncResponse asyncResponse) {
        validateSuperUserAccessAsync()
                .thenAccept(
                        __ -> asyncResponse.resume(pulsar().getBrokerService().getDynamicConfiguration()))
                .exceptionally(ex -> {
    ...
    public void getRuntimeConfiguration(@Suspended AsyncResponse asyncResponse) {
        validateSuperUserAccessAsync()
                .thenAccept(
                        __ -> asyncResponse.resume(pulsar().getBrokerService().getRuntimeConfiguration()))
                .exceptionally(ex -> {
    ...
    private synchronized CompletableFuture<Void> persistDynamicConfigurationAsync(
        ...
        if (!pulsar().getBrokerService().validateDynamicConfiguration(configName, configValue)) {
            return FutureUtil.failedFuture(
                    new RestException(Status.PRECONDITION_FAILED, " Invalid dynamic-config value"));
        }
        if (pulsar().getBrokerService().isDynamicConfiguration(configName)) {
            ...
            return FutureUtil.failedFuture(
                    new RestException(Status.PRECONDITION_FAILED, "Can't update non-dynamic configuration"));
        }
    }
    ...
    public void healthCheck(
            @Suspended AsyncResponse asyncResponse, @QueryParam("topicVersion") TopicVersion topicVersion) {
        validateSuperUserAccessAsync()
    ...
    private void checkDeadlockedThreads() {
        ...
        if (threadIds != null && threadIds.length > 0) {
            ...
            String threadNames = Arrays.stream(threadInfos)
                    .map(threadInfo -> threadInfo.getThreadName() + "(tid=" + threadInfo.getThreadId() + ")")
    ...
    private CompletableFuture<Void> internalRunHealthCheck(TopicVersion topicVersion) {
        ...
        return pulsar().getBrokerService().getTopic(topicName, true).thenCompose(topicOptional -> {
            if (!topicOptional.isPresent()) {
                LOG.error(
                        "[{}] Fail to run health check while get topic {}. because get null value.",
                        clientAppId(),
                        topicName);
                throw new RestException(
                        Status.NOT_FOUND, String.format("Topic [%s] not found after create.", topicName));
            }
            PulsarClient client;
            try {
     ...
     */
    private CompletableFuture<Void> closeAndReCheck(
            Producer<String> producer, Reader<String> reader, Topic topic, String subscriptionName) {
            ...
            if (readerFuture.isCompletedExceptionally()) {
                ...
                if (subscription != null) {
                    ...
                    subscription.deleteForcefully().exceptionally(ex -> {
                        LOG.error("[{}] Force delete subscription fail" + " while health check", clientAppId(), ex);
                        return null;
                    });
                }
            } else {
    ...
    private CompletableFuture<Void> doShutDownBrokerGracefullyAsync(
