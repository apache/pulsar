...
public class TransactionAggregator {
    ...
    private static final FastThreadLocal<ManagedLedgerStats> localManageLedgerStats = new FastThreadLocal<>() {
        @Override
    ...
    public static void generate(PulsarService pulsar, PrometheusMetricStreams stream, boolean includeTopicMetrics) {
                                ...
                                if (topic instanceof PersistentTopic) {
                                    topic.getSubscriptions().values().forEach(subscription -> {
                                        try {
                                            ...
                                            if (!isEventSystemTopic(TopicName.get(subscription
                                                            .getTopic()
                                                            .getName()))
        ...
        pulsar.getTransactionMetadataStoreService()
                ...
                .forEach((transactionCoordinatorID, transactionMetadataStore) -> {
                    ...
                    printTransactionCoordinatorStats(
                            stream,
                            cluster,
    ...
    private static void generateManageLedgerStats(
            ManagedLedger managedLedger,
    ...
    private static void printManageLedgerStats(
            PrometheusMetricStreams stream,
            String cluster,
            String namespace,
            String topic,
            String subscription,
            ManagedLedgerStats stats) {

        writeMetric(stream, "pulsar_storage_size", stats.storageSize, cluster, namespace, topic, subscription);
        writeMetric(
                stream,
                "pulsar_storage_logical_size",
                stats.storageLogicalSize,
                cluster,
                namespace,
                topic,
                subscription);
        writeMetric(stream, "pulsar_storage_backlog_size", stats.backlogSize, cluster, namespace, topic, subscription);
        writeMetric(
                stream,
        ...
        writeMetric(
                stream, "pulsar_storage_write_rate", stats.storageWriteRate, cluster, namespace, topic, subscription);
        writeMetric(stream, "pulsar_storage_read_rate", stats.storageReadRate, cluster, namespace, topic, subscription);
        ...
        writeMetric(stream, "pulsar_entry_size_le_128", entrySizeBuckets[0], cluster, namespace, topic, subscription);
        writeMetric(stream, "pulsar_entry_size_le_512", entrySizeBuckets[1], cluster, namespace, topic, subscription);
        writeMetric(stream, "pulsar_entry_size_le_1_kb", entrySizeBuckets[2], cluster, namespace, topic, subscription);
        writeMetric(stream, "pulsar_entry_size_le_2_kb", entrySizeBuckets[3], cluster, namespace, topic, subscription);
        writeMetric(stream, "pulsar_entry_size_le_4_kb", entrySizeBuckets[4], cluster, namespace, topic, subscription);
        writeMetric(stream, "pulsar_entry_size_le_16_kb", entrySizeBuckets[5], cluster, namespace, topic, subscription);
        writeMetric(
                stream, "pulsar_entry_size_le_100_kb", entrySizeBuckets[6], cluster, namespace, topic, subscription);
        writeMetric(stream, "pulsar_entry_size_le_1_mb", entrySizeBuckets[7], cluster, namespace, topic, subscription);
        writeMetric(
                stream, "pulsar_entry_size_le_overflow", entrySizeBuckets[8], cluster, namespace, topic, subscription);
        writeMetric(
                stream,
    ...
    static void printTransactionCoordinatorStats(
            PrometheusMetricStreams stream,
            String cluster,
            AggregatedTransactionCoordinatorStats stats,
            long coordinatorId) {
        writeMetric(stream, "pulsar_txn_active_count", stats.actives, cluster, coordinatorId);
        writeMetric(stream, "pulsar_txn_committed_total", stats.committedCount, cluster, coordinatorId);
        writeMetric(stream, "pulsar_txn_aborted_total", stats.abortedCount, cluster, coordinatorId);
        writeMetric(stream, "pulsar_txn_created_total", stats.createdCount, cluster, coordinatorId);
        writeMetric(stream, "pulsar_txn_timeout_total", stats.timeoutCount, cluster, coordinatorId);
        writeMetric(stream, "pulsar_txn_append_log_total", stats.appendLogCount, cluster, coordinatorId);
        ...
        writeMetric(stream, "pulsar_txn_execution_latency_le_300000", latencyBuckets[10], cluster, coordinatorId);
        writeMetric(stream, "pulsar_txn_execution_latency_le_1500000", latencyBuckets[11], cluster, coordinatorId);
        writeMetric(stream, "pulsar_txn_execution_latency_le_3000000", latencyBuckets[12], cluster, coordinatorId);
        writeMetric(stream, "pulsar_txn_execution_latency_le_overflow", latencyBuckets[13], cluster, coordinatorId);
    }

    private static void writeMetric(
            PrometheusMetricStreams stream, String metricName, double value, String cluster, long coordinatorId) {
