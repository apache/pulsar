...
public interface TransactionBufferSnapshotIndexService {

    /**
     * Create a transaction buffer snapshot index writer.
     *
     * @param topicName {@link TopicName} the topic name
     *
     * @return {@link CompletableFuture<Writer>} return the future of writer
     */
    CompletableFuture<Writer<TransactionBufferSnapshotIndexes>> createWriter(TopicName topicName);

    /**
     * Create a transaction buffer snapshot index reader.
     *
     * @param topicName {@link TopicName} the topic name
     *
     * @return {@link CompletableFuture<Writer>} return the future of reader
     */
    CompletableFuture<Reader<TransactionBufferSnapshotIndexes>> createReader(TopicName topicName);

    /**
     * Remove a topic client from cache.
     *
     * @param topicName {@link TopicName} the topic name
     * @param transactionBufferSnapshotIndexSystemTopicClient
     * {@link TransactionBufferSnapshotIndexSystemTopicClient} the topic client
     *
     */
    void removeClient(TopicName topicName, TransactionBufferSnapshotIndexSystemTopicClient
