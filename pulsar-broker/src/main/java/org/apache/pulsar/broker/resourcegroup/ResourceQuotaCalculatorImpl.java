    ...
    public long computeLocalQuota(long confUsage, long myUsage, long[] allUsages) throws PulsarAdminException {
        ...
        if (confUsage < 0) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        "Configured usage ({}) is not set; returning a special value ({}) for calculated quota",
                        confUsage,
                        retVal);
            }
            return retVal;
        ...
        if (myUsage < 0 || totalUsage < 0) {
            String errMesg = String.format("Local usage (%d) or total usage (%d) is negative", myUsage, totalUsage);
            log.error(errMesg);
        ...
        if (totalUsage == 0) {
            if (log.isDebugEnabled()) {
                log.debug(
                        "computeLocalQuota: totalUsage is zero; "
        ...
        if (myUsage > totalUsage) {
            String errMesg = String.format("Local usage (%d) is greater than total usage (%d)", myUsage, totalUsage);
        ...
        log.info(
                "computeLocalQuota: myUsage={}, totalUsage={}, myFraction={}; newQuota returned={} [long: {}]",
                myUsage,
                totalUsage,
                myUsageFraction,
                calculatedQuota,
