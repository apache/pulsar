...
public class SchemaRegistryServiceImpl implements SchemaRegistryService {
    ...
    public CompletableFuture<SchemaAndMetadata> getSchema(String schemaId, SchemaVersion version) {
        ...
        if (version == SchemaVersion.Latest) {
            ...
            completableFuture = trimDeletedSchemaAndGetList(schemaId)
                    .thenApply(metadataList -> metadataList.stream()
                            .filter(schemaAndMetadata ->
    ...
    private CompletableFuture<List<CompletableFuture<SchemaAndMetadata>>> convertToSchemaAndMetadata(
            String schemaId, List<CompletableFuture<StoredSchema>> schemas) {
        List<CompletableFuture<SchemaAndMetadata>> list = schemas.stream()
                .map(future -> future.thenCompose(stored -> Functions.bytesToSchemaInfo(stored.data)
                        .thenApply(Functions::schemaInfoToSchema)
                        .thenApply(schema -> new SchemaAndMetadata(schemaId, schema, stored.version))))
                .collect(Collectors.toList());
        if (log.isDebugEnabled()) {
    ...
    public CompletableFuture<SchemaVersion> putSchemaIfAbsent(
        ...
        schemaStorage
                .put(schemaId, schemasFuture -> schemasFuture
                        .thenCompose(schemaFutureList -> trimDeletedSchemaAndGetList(
                                ...
                                .thenCompose(schemaVersion -> {
                                    ...
                                    if (schemaAndMetadataList.size() != 0) {
                                        if (isTransitiveStrategy(strategy)) {
                                            checkCompatibilityFuture = checkCompatibilityWithAll(
                                                    schemaId, schema, strategy, schemaAndMetadataList);
                                        } else {
                                    ...
                                    return checkCompatibilityFuture.thenCompose(v -> {
                                        byte[] context = hashFunction
                                                .hashBytes(schema.getData())
                                                .asBytes();
    ...
    public CompletableFuture<SchemaVersion> deleteSchema(String schemaId, String user, boolean force) {
        ...
        return schemaStorage.put(schemaId, deletedEntry, new byte[] {}).whenComplete((v, t) -> {
            if (t != null) {
    ...
    public CompletableFuture<SchemaVersion> deleteSchemaStorage(String schemaId, boolean forcefully) {
        ...
        return schemaStorage.delete(schemaId, forcefully).whenComplete((v, t) -> {
            if (t != null) {
                this.stats.recordDelFailed(schemaId);
    ...
    private void checkCompatible(
            SchemaAndMetadata existingSchema, SchemaData newSchema, SchemaCompatibilityStrategy strategy)
        ...
        if (!newHash.equals(existingHash)) {
            compatibilityChecks
                    .getOrDefault(newSchema.getType(), SchemaCompatibilityCheck.DEFAULT)
            ...
            for (SchemaAndMetadata schemaAndMetadata : schemaAndMetadataList) {
                if (newHash.equals(SchemaHash.of(schemaAndMetadata.schema))) {
                    return completedFuture(
                            ((LongSchemaVersion) schemaStorage.versionFromBytes(schemaAndMetadata.version.bytes()))
                                    .getVersion());
                }
            }
        ...
        if (isUsingAvroSchemaParser(schemaData.getType())) {
            ...
            for (SchemaAndMetadata schemaAndMetadata : schemaAndMetadataList) {
                if (isUsingAvroSchemaParser(schemaAndMetadata.schema.getType())) {
                    ...
                    if (Arrays.equals(
                                    hashFunction
                                            .hashBytes(schemaAndMetadata.schema.getData())
                                            .asBytes(),
    ...
    private CompletableFuture<Void> checkCompatibilityWithLatest(
            String schemaId, SchemaData schema, SchemaCompatibilityStrategy strategy) {
        if (SchemaCompatibilityStrategy.ALWAYS_COMPATIBLE == strategy) {
            return CompletableFuture.completedFuture(null);
        }
    ...
    private CompletableFuture<Void> checkCompatibilityWithAll(
            String schemaId, SchemaData schema, SchemaCompatibilityStrategy strategy) {
    ...
    private CompletableFuture<Void> checkCompatibilityWithAll(
        ...
        if (strategy == SchemaCompatibilityStrategy.ALWAYS_COMPATIBLE) {
            ...
            if (breakSchema == null) {
                try {
                    compatibilityChecks
                            .getOrDefault(schema.getType(), SchemaCompatibilityCheck.DEFAULT)
                            .checkCompatible(
                                    schemaAndMetadataList.stream()
                ...
                result.completeExceptionally(new IncompatibleSchemaException(String.format(
                        "Incompatible schema: exists schema type %s, new schema type %s",
    ...
    private CompletableFuture<List<SchemaAndMetadata>> trimDeletedSchemaAndGetList(
            String schemaId, CompletableFuture<List<CompletableFuture<SchemaAndMetadata>>> schemaFutureList) {
        ...
        schemaFutureList.thenCompose(FutureUtils::collect).handle((schemaList, ex) -> {
            ...
            if (ex != null) {
                ...
                deleteSchemaStorage(schemaId, true).handle((sv, th) -> {
                    log.info(
                            "Clean up non-recoverable schema {}. Deletion of schema {} {}",
                            rc.getMessage(),
                            schemaId,
                            (th == null
                                    ? "successful"
                                    : "failed, " + th.getCause().getMessage()));
    ...
    interface Functions {
        ...
        static List<SchemaRegistryFormat.SchemaInfo.KeyValuePair> toPairs(Map<String, String> map) {
            ...
            for (Map.Entry<String, String> entry : map.entrySet()) {
                ...
                pairs.add(builder.setKey(entry.getKey())
