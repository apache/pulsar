...
class TopicStats {
    ...
    public static void printTopicStats(PrometheusMetricStreams stream, TopicStats stats,
                                       ...
                                       String[] customMetricAndValues) {
        writeMetric(stream, "pulsar_subscriptions_count", stats.subscriptionsCount,
            cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        ...
        writeMetric(stream, "pulsar_storage_logical_size",
            ...
            splitTopicAndPartitionIndexLabel, customMetricAndValues);
        ...
        writeMetric(stream, "pulsar_storage_offloaded_size", stats.managedLedgerStats
            .offloadedStorageUsed, cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        ...
        writeBacklogQuotaMetric(stream, "pulsar_storage_backlog_quota_exceeded_evictions_total",
            ...
            splitTopicAndPartitionIndexLabel, BacklogQuotaType.destination_storage, customMetricAndValues);
        writeBacklogQuotaMetric(stream, "pulsar_storage_backlog_quota_exceeded_evictions_total",
            stats.timeBasedBacklogQuotaExceededEvictionCount, cluster, namespace, topic,
            splitTopicAndPartitionIndexLabel, BacklogQuotaType.message_age, customMetricAndValues);
        ...
        for (TopicMetricBean topicMetricBean : stats.bucketDelayedIndexStats.values()) {
            String[] extraLabels = mergeArrays(customMetricAndValues, topicMetricBean.labelsAndValues);
        ...
        writeMetric(stream, "pulsar_storage_write_latency_le_0_5",
            latencyBuckets[0], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        writeMetric(stream, "pulsar_storage_write_latency_le_1",
            latencyBuckets[1], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        writeMetric(stream, "pulsar_storage_write_latency_le_5",
            latencyBuckets[2], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        writeMetric(stream, "pulsar_storage_write_latency_le_10",
            latencyBuckets[3], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        writeMetric(stream, "pulsar_storage_write_latency_le_20",
            latencyBuckets[4], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        writeMetric(stream, "pulsar_storage_write_latency_le_50",
            latencyBuckets[5], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        writeMetric(stream, "pulsar_storage_write_latency_le_100",
            latencyBuckets[6], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        writeMetric(stream, "pulsar_storage_write_latency_le_200",
            latencyBuckets[7], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        writeMetric(stream, "pulsar_storage_write_latency_le_1000",
            latencyBuckets[8], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        writeMetric(stream, "pulsar_storage_write_latency_overflow",
            latencyBuckets[9], cluster, namespace, topic, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        ...
        stats.subscriptionStats.forEach((sub, subsStats) -> {
            writeSubscriptionMetric(stream, "pulsar_subscription_back_log", subsStats.msgBacklog,
                cluster, namespace, topic, sub, splitTopicAndPartitionIndexLabel, customMetricAndValues);
            ...
            String[] throttledLabels1 =
                mergeArrays(customMetricAndValues, "subscription", sub, "reason", "subscription");
            String[] throttledLabels2 = mergeArrays(customMetricAndValues, "subscription", sub, "reason", "topic");
            String[] throttledLabels3 = mergeArrays(customMetricAndValues, "subscription", sub, "reason", "broker");
            ...
            for (TopicMetricBean topicMetricBean : subsStats.bucketDelayedIndexStats.values()) {
                ...
                String[] extraLabels = mergeArrays(customMetricAndValues, labelsAndValues);
                writeTopicMetric(stream, topicMetricBean.name, topicMetricBean.value, cluster, namespace,
                    topic, splitTopicAndPartitionIndexLabel, extraLabels);
            }
            ...
            subsStats.consumerStat.forEach((c, consumerStats) -> {
                writeConsumerMetric(stream, "pulsar_consumer_msg_rate_redeliver", consumerStats.msgRateRedeliver,
                    cluster, namespace, topic, sub, c, splitTopicAndPartitionIndexLabel, customMetricAndValues);
        ...
        if (!stats.replicationStats.isEmpty()) {
            stats.replicationStats.forEach((remoteCluster, replStats) -> {
                writeMetric(stream, "pulsar_replication_rate_in", replStats.msgRateIn,
                    cluster, namespace, topic, remoteCluster, splitTopicAndPartitionIndexLabel, customMetricAndValues);
    ...
    private static void writeBacklogQuotaMetric(PrometheusMetricStreams stream, String metricName, Number value,
        ...
        String[] extraLabels = mergeArrays(customMetricAndValues, "quota_type", quotaTypeLabelValue);
        writeTopicMetric(stream, metricName, value, cluster, namespace, topic, splitTopicAndPartitionIndexLabel,
            extraLabels);
    }
    ...
    private static void writeMetric(PrometheusMetricStreams stream, String metricName, Number value, String cluster,
                                    ...
                                    boolean splitTopicAndPartitionIndexLabel,
                                    String[] customMetricAndValues) {
        String[] extraLabels = mergeArrays(customMetricAndValues, "remote_cluster", remoteCluster);
        writeTopicMetric(stream, metricName, value, cluster, namespace, topic, splitTopicAndPartitionIndexLabel,
            extraLabels);
    }
    ...
    private static void writeProducerMetric(PrometheusMetricStreams stream, String metricName, Number value,
                                            ...
                                            String[] customMetricAndValues) {
        String[] extraLabels =
            mergeArrays(customMetricAndValues, "producer_name", producer, "producer_id", String.valueOf(producerId));
        writeTopicMetric(stream, metricName, value, cluster, namespace, topic, splitTopicAndPartitionIndexLabel,
            extraLabels);
    }
    ...
    private static void writeSubscriptionMetric(PrometheusMetricStreams stream, String metricName, Number value,
                                                ...
                                                boolean splitTopicAndPartitionIndexLabel,
                                                String[] customMetricAndValues) {
        String[] extraLabels = mergeArrays(customMetricAndValues, "subscription", subscription);
        writeTopicMetric(stream, metricName, value, cluster, namespace, topic, splitTopicAndPartitionIndexLabel,
            extraLabels);
    }
    ...
    private static void writeConsumerMetric(PrometheusMetricStreams stream, String metricName, Number value,
                                            ...
                                            String[] customMetricAndValues) {
        String[] extraLabels =
            mergeArrays(customMetricAndValues, "subscription", subscription, "consumer_name", consumer.consumerName(),
