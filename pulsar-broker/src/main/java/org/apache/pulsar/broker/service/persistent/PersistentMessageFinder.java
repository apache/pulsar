...
public class PersistentMessageFinder implements AsyncCallbacks.FindEntryCallback {
        ...
        if (messageFindInProgressUpdater.compareAndSet(this, FALSE, TRUE)) {
            ...
            Pair<PositionImpl, PositionImpl> range =
                    getFindPositionRange(cursor.getManagedLedger().getLedgersInfo().values(),
                            (PositionImpl) cursor.getManagedLedger().getLastConfirmedEntry(), timestamp,
    ...
    public static Pair<PositionImpl, PositionImpl> getFindPositionRange(Iterable<LedgerInfo> ledgerInfos,
                                                                PositionImpl lastConfirmedEntry, long targetTimestamp,
        ...
        for (LedgerInfo info : ledgerInfos) {
            ...
            if (closeTimestamp <= targetTimestampMin) {
                ...
                end = new PositionImpl(info.getLedgerId(), info.getEntries() - 1);
                break;
            }
        }
        ...
        if (lastLedgerInfo != null && secondToLastLedgerInfo != null
            ...
            PositionImpl firstPositionInLedger = new PositionImpl(lastLedgerInfo.getLedgerId(), 0);
