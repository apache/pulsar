...
public class DefaultSchemaRegistryService implements SchemaRegistryService {
    ...
    public CompletableFuture<Schema> getSchema(String schemaId) {
        return getSchemaLocator(schemaId).thenCompose(locator -> {
            ...
            return readSchemaEntry(schemaLocator.getPosition())
                .thenApply(DefaultSchemaRegistryService::schemaEntryToSchema);
        });
    }
    ...
    public CompletableFuture<Long> putSchema(Schema schema) {
        return getSchemaLocator(schema.id).thenCompose(optionalSchemaLocator -> {
            ...
            return createLedger().thenCompose(
                ledgerHandle -> addEntry(ledgerHandle, newSchemaEntry(locator, schema)).thenCompose(
                    position -> {
                        SchemaRegistryFormat.PositionInfo positionInfo =
                            buildPositionInfo(ledgerHandle, position);
                        ...
                        return putSchemaLocator(schema.id,
                            SchemaRegistryFormat.SchemaLocator.newBuilder()
                                .setInfo(buildSchemaInfo(schema))
                                .setPosition(positionInfo)
                                .addAllIndex(buildIndex(locator, positionInfo, nextVersion))
    ...
    private CompletableFuture<SchemaRegistryFormat.SchemaEntry> findSchemaEntry(List<SchemaRegistryFormat.IndexEntry> index, long version) {
        if (index.isEmpty()) {
            return completedFuture(null);
        }
    ...
    private CompletableFuture<SchemaRegistryFormat.SchemaEntry> readSchemaEntry(SchemaRegistryFormat.PositionInfo position) {
        return openLedger(position.getLedgerId())
            .thenCompose((ledger) -> parseSchemaEntry(ledger, position.getEntryId()))
            .thenCompose(DefaultSchemaRegistryService::parseSchemaEntry);
    }

    static private CompletableFuture<SchemaRegistryFormat.SchemaEntry> parseSchemaEntry(LedgerEntry ledgerEntry) {
    ...
    private static SchemaRegistryFormat.SchemaInfo.SchemaType convertFromDomainType(SchemaType type) {
        switch (type) {
            case AVRO:
    ...
    private static SchemaType convertToDomainType(SchemaRegistryFormat.SchemaInfo.SchemaType type) {
        switch (type) {
            case AVRO:
    ...
    private static Schema schemaEntryToSchema(SchemaRegistryFormat.SchemaEntry entry) {
    ...
    private static SchemaRegistryFormat.SchemaEntry newSchemaEntry(SchemaRegistryFormat.SchemaLocator locator, Schema schema) {
    ...
    private static SchemaRegistryFormat.SchemaInfo buildSchemaInfo(Schema schema) {
    ...
    private static SchemaRegistryFormat.PositionInfo buildPositionInfo(LedgerHandle ledgerHandle, long entryId) {
    ...
    private static Iterable<SchemaRegistryFormat.IndexEntry> buildIndex(
    ...
    private CompletableFuture<Optional<Entry<SchemaRegistryFormat.SchemaLocator, Stat>>> getSchemaLocator(String schema) {
        return localZkCache.getEntryAsync("/schemas/" + schema, new SchemaLocatorDeserializer());
    }
    ...
    private CompletableFuture<LedgerHandle> createLedger() {
        ...
        bookKeeper.asyncCreateLedger(0, 0, BookKeeper.DigestType.CRC32, null,
    ...
    private CompletableFuture<LedgerHandle> openLedger(Long ledgerId) {
        final CompletableFuture<LedgerHandle> future = new CompletableFuture<>();
        bookKeeper.asyncOpenLedger(ledgerId, BookKeeper.DigestType.CRC32, null,
            (i, ledgerHandle, o) -> future.complete(ledgerHandle), null
        );
        return future;
    }

    private CompletableFuture<LedgerEntry> parseSchemaEntry(LedgerHandle ledger, long entry) {
