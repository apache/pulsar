...
public class AvgShedder implements LoadSheddingStrategy, ModularLoadManagerStrategy {
    ...
    public Multimap<String, String> findBundlesForUnloading(LoadData loadData, ServiceConfiguration conf) {
        ...
        final double minMsgThreshold = conf.getLoadBalancerBundleUnloadMinMsgThreshold();

        final double maxUnloadPercentage = conf.getLoadBalancerMaxUnloadPercentage();
        final double maxRemainPercentage = conf.getLoadBalancerMaxRemainPercentage();
        ...
        for (Pair<String, String> pair : pairs) {
            ...
            MutableDouble trafficRemainNotOffload = new MutableDouble(0);
            ...
            if (msgRequiredFromUnloadedBundles > minMsgThreshold) {
                ...
                trafficRemainNotOffload.setValue(msgRequiredFromUnloadedBundles * maxRemainPercentage);
                ...
                trafficRemainNotOffload.setValue(throuputRequiredFromUnloadedBundles * maxRemainPercentage);
            } else {
                log.info(
            ...
            loadData.getBundleDataForLoadShedding().entrySet().stream().filter(e ->
                ...
                if (traffic > 0 && traffic < trafficMarkedToOffload.getValue()
                        && trafficMarkedToOffload.getValue() > trafficRemainNotOffload.getValue()) {
    ...
    private Double calculateScores(String broker, LocalBrokerData localBrokerData, final ServiceConfiguration conf) {
        double score = localBrokerData.getMaxResourceUsageWithWeightWithinLimit(
