...
public class MLPendingAckStore extends PendingAckStoreState implements PendingAckStore {
    ...
    public MLPendingAckStore(ManagedLedgerFactory managedLedgerFactory, String topicName, String subName) {
        super(State.None);
    ...
    public void replayAsync(PendingAckReplyCallBack pendingAckReplyCallBack) {
        new Thread(() -> new PendingAckReplayer(pendingAckReplyCallBack).start()).start();
    ...
    public CompletableFuture<Void> append(TxnID txnID, PositionImpl position, AckType ackType) {
        if (!checkIfReady()) {
            log.error("MLPendingAckStore haven't init success before ack " +
                    "with transaction! TopicName : {}, SubName: {}", topicName, subName);
            return FutureUtil.failedFuture
                    (new TransactionAckConflictException("MLPendingAckStore haven't init success! " +
                            "TopicName : " + topicName + ", SubName: " + subName));
        }
    ...
    public CompletableFuture<Void> deleteTxn(TxnID txnID, AckType ackType) {
        ...
        if (ackType == AckType.Cumulative) {
            if (pendingCumulativeAckPosition != null && pendingCumulativeAckPosition.getKey() != null
                ...
                completableFuture.completeExceptionally(new TransactionAckConflictException("Pending cumulative ack " +
            ...
            if (pendingIndividualAckPersistentMap.containsKey(txnID)) {
                ...
                completableFuture.completeExceptionally(new TransactionAckConflictException("Pending ack store delete " +
    ...
    class PendingAckReplayer {
        ...
        PendingAckReplayer(PendingAckReplyCallBack pendingAckReplyCallBack) {
