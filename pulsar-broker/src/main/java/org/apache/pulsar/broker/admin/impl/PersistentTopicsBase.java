...
public class PersistentTopicsBase extends AdminResource {
    ...
    protected void internalGetBacklog(AsyncResponse asyncResponse, boolean authoritative) {
        ...
        ret.thenCompose(__ -> namespaceResources().getPoliciesAsync(namespaceName))
           .thenCompose(__ -> {
               ...
               if (offlineTopicStats != null) {
                   ...
                   if (TimeUnit.MINUTES.convert(elapsedMs, TimeUnit.MILLISECONDS) < OFFLINE_TOPIC_STAT_TTL_MINS) {
                       asyncResponse.resume(offlineTopicStats);
                       return null;
                   }
               }
          ...
          .thenAccept(config -> {
              ...
              try {
                  ...
                  asyncResponse.resume(offlineTopicBacklog);
              } catch (Exception ex) {
        ...
        .exceptionally(ex -> {
            if (ex instanceof MetadataStoreException.NotFoundException) {
                ...
                log.error("[{}] Failed TODO {}", clientAppId(), topicName, ex);
                asyncResponse.resume(new RestException(ex));
            }
            return null;
        });
