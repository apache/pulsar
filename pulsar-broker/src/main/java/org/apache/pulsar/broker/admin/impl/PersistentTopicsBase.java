...
public class PersistentTopicsBase extends AdminResource {
        ...
        return validateNamespaceOperationAsync(namespaceName, NamespaceOperation.GET_TOPICS)
                ...
                .thenApply(topics -> topics.stream()
                        .filter(topic -> {
                            ...
                            if (bundle.isPresent()) {
                                NamespaceBundle b = pulsar().getNamespaceService()
    ...
    protected CompletableFuture<List<String>> internalGetListAsync() {
        return validateNamespaceOperationAsync(namespaceName, NamespaceOperation.GET_TOPICS)
                ...
                .thenCompose(__ -> topicResources().listPersistentTopicsAsync(namespaceName))
                .thenApply(topics -> topics.stream()
                        .filter(topic -> !isTransactionInternalName(TopicName.get(topic)))
                        .collect(Collectors.toList()));
    }
    ...
    protected CompletableFuture<List<String>> internalGetPartitionedTopicListAsync() {
        return validateNamespaceOperationAsync(namespaceName, NamespaceOperation.GET_TOPICS)
                ...
                .thenCompose(namespaceExists -> {
                    ...
                    if (!namespaceExists) {
                        log.warn(
                                "[{}] Failed to get partitioned topic list {}: Namespace does not exist",
                                clientAppId(),
                                namespaceName);
                        throw new RestException(Status.NOT_FOUND, "Namespace does not exist");
                    } else {
    ...
    private CompletableFuture<Void> grantPermissionsAsync(TopicName topicUri, String role, Set<AuthAction> actions) {
        ...
        if (null != authService) {
            return authService
                    .grantPermissionAsync(topicUri, actions, role, null /*additional auth-data json*/)
                    .thenAccept(__ -> log.info(
                            "[{}] Successfully granted access for role {}: {} - topic {}",
                            clientAppId(),
                            role,
                            actions,
                            topicUri))
                    .exceptionally(ex -> {
                        ...
                        if (realCause instanceof MetadataStoreException.NotFoundException
                            ...
                            log.warn(
                                    "[{}] Failed to set permissions for topic {}: Namespace does not exist",
                                    clientAppId(),
                                    topicUri,
                                    realCause);
                            ...
                            log.warn(
                                    "[{}] Failed to set permissions for topic {}: {}",
                                    clientAppId(),
                                    topicUri,
                            ...
                            log.error(
                                    "[{}] Failed to get permissions for topic {}", clientAppId(), topicUri, realCause);
    ...
    protected void internalGrantPermissionsOnTopic(
            final AsyncResponse asyncResponse, String role, Set<AuthAction> actions) {
        // This operation should be reading from zookeeper and it should be allowed without having admin privileges
        validateAdminAccessForTenantAsync(namespaceName.getTenant())
                .thenCompose(__ -> validatePoliciesReadOnlyAccessAsync()
                        .thenCompose(unused1 -> grantPermissionsAsync(topicName, role, actions)
                                .thenAccept(unused -> asyncResponse.resume(
                ...
                .thenCompose(__ -> getPartitionedTopicMetadataAsync(topicName, true, false)
                        .thenCompose(metadata -> {
                            ...
                            return future.thenComposeAsync(
                                            unused -> getAuthorizationService().revokePermissionAsync(topicName, role))
                                    .thenAccept(unused -> asyncResponse.resume(
                                            Response.noContent().build()));
                        }))
                .exceptionally(ex -> {
    ...
    protected CompletableFuture<Void> internalCreateNonPartitionedTopicAsync(
        ...
        return ret.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative))
                .thenCompose(__ -> validateNamespaceOperationAsync(
                        topicName.getNamespaceObject(), NamespaceOperation.CREATE_TOPIC))
                .thenCompose(__ -> getPartitionedTopicMetadataAsync(topicName, false, false))
                .thenAccept(partitionMetadata -> {
                    if (partitionMetadata.partitions > 0) {
                        log.warn(
                                "[{}] Partitioned topic with the same name already exists {}",
                                clientAppId(),
                                topicName);
                ...
                .thenAccept(
                        __ -> log.info("[{}] Successfully created non-partitioned topic {}", clientAppId(), topicName));
    }
     ...
     */
    protected @Nonnull CompletableFuture<Void> internalUpdatePartitionedTopicAsync(
            int expectPartitions, boolean updateLocal, boolean force) {
                ...
                .thenCompose(partitionedTopicMetadata -> {
                    ...
                    if (currentMetadataPartitions <= 0) {
                        throw new RestException(
                                Status.CONFLICT /* Unprocessable entity*/,
                    ...
                    if (expectPartitions < currentMetadataPartitions) {
                        throw new RestException(
                                ...
                                String.format(
                                        "Expect partitions %s can't less than current partitions %s.",
                                        expectPartitions, currentMetadataPartitions));
                    }
                    int brokerMaximumPartitionsPerTopic =
                            pulsarService.getConfiguration().getMaxNumPartitionsPerPartitionedTopic();
                    if (brokerMaximumPartitionsPerTopic != 0 && expectPartitions > brokerMaximumPartitionsPerTopic) {
                        throw new RestException(
                                422 /* Unprocessable entity*/,
                                String.format(
                                        "Expect partitions %s grater than maximum partitions per topic %s",
                    ...
                    if (!force) {
                        checkFuture = admin.topics()
                                .getListAsync(topicName.getNamespace(), topicName.getDomain())
                                .thenAccept(topics -> {
                                    final List<TopicName> existingPartitions = topics.stream()
                                            .map(TopicName::get)
                                            .filter(candidateTopicName -> candidateTopicName
                                    ...
                                    if (maximumPartitionIndex.isPresent()
                                        ...
                                        List<String> unexpectedPartitions = existingPartitions.stream()
                                                .filter(candidateTopicName -> candidateTopicName.getPartitionIndex()
                    ...
                    return checkFuture
                            .thenCompose(topics -> {
                                        ...
                                        (expectPartitions == currentMetadataPartitions)
                                                ...
                                                : namespaceResources()
                                                        ...
                                                        .updatePartitionedTopicAsync(
                                                                topicName,
                                                                m -> new PartitionedTopicMetadata(
                                                                        expectPartitions, m.properties));
                                return updateMetadataFuture
                                        ...
                                        .thenCompose(__ -> admin.topics()
                                                .getStatsAsync(topicName
                                                        .getPartition(0)
                                                        .toString())
                                                .thenCompose(stats -> {
                                                    List<CompletableFuture<Void>> futures = stats
                                                            .getSubscriptions()
                                                            .entrySet()
                                                                ...
                                                                for (int i = 0; i < expectPartitions; i++) {
                                                                    innerFutures.add(admin.topics()
                                                                            ...
                                                                            .exceptionally(ex -> {
                                                                                ...
                                                                                // Ignore subscription already exist
                                                                                // exception
                                                                                return null;
                                                                            }));
                                                                }
                            ...
                            .thenCompose(__ -> {
                                ...
                                return namespaceResources()
                                        ...
                                        .thenCompose(policies -> {
                                            ...
                                            boolean containsCurrentCluster = replicationClusters.contains(
                                                    pulsar().getConfig().getClusterName());
                                            if (!containsCurrentCluster) {
                                                log.error(
                                                        "[{}] local cluster is not part of replicated cluster for namespace {}",
                                                        clientAppId(),
                                                        topicName);
                                                throw new RestException(
                                                        422, "Local cluster is not part of replicate cluster list");
                                            }
                                            ...
                                            List<CompletableFuture<Void>> futures = replicationClusters.stream()
                                                    .map(replicationCluster -> admin.clusters()
                                                            .getClusterAsync(replicationCluster)
                                                            .thenCompose(clusterData -> pulsarService
                                                                    .getBrokerService()
                ...
                .thenAccept(metadata -> {
                    if (metadata != null) {
                        tryCreatePartitionsAsync(metadata.partitions)
                                ...
                                .exceptionally(e -> {
                                    log.error(
                                            "[{}] Failed to create partitions for topic {}", clientAppId(), topicName);
    ...
    protected CompletableFuture<Void> internalSetDelayedDeliveryPolicies(
            DelayedDeliveryPolicies deliveryPolicies, boolean isGlobal) {
    ...
    protected PartitionedTopicMetadata internalGetPartitionedMetadata(
    ...
    protected CompletableFuture<Void> internalUpdatePropertiesAsync(
            boolean authoritative, Map<String, String> properties) {
        ...
        return validateTopicOwnershipAsync(topicName, authoritative)
                        ...
                        return pulsar().getBrokerService()
                                ...
                                .thenCompose(metadata -> {
                                    ...
                                    return namespaceResources()
                                            ...
                                            .updatePartitionedTopicAsync(
                                                    topicName,
                                                    p -> new PartitionedTopicMetadata(
                                                            p.partitions,
                                                            p.properties == null
                                                                    ? properties
                                                                    : MapUtils.putAll(
                ...
                .thenAccept(__ -> log.info(
                        "[{}] [{}] update properties success with properties {}",
                        clientAppId(),
                        topicName,
                        properties));
    }
    ...
    private CompletableFuture<Void> internalUpdateNonPartitionedTopicProperties(Map<String, String> properties) {
        ...
        pulsar().getBrokerService().getTopicIfExists(topicName.toString()).thenAccept(opt -> {
            if (!opt.isPresent()) {
                throw new RestException(Status.NOT_FOUND, getTopicNotFoundErrorMessage(topicName.toString()));
            }
            ManagedLedger managedLedger = ((PersistentTopic) opt.get()).getManagedLedger();
            managedLedger.asyncSetProperties(
                    properties,
    ...
    protected CompletableFuture<Void> internalRemovePropertiesAsync(boolean authoritative, String key) {
        return validateTopicOwnershipAsync(topicName, authoritative)
                ...
                .thenAccept(__ ->
                        log.info("[{}] remove [{}] properties success with key {}", clientAppId(), topicName, key));
    }
    ...
    private CompletableFuture<Void> internalRemoveNonPartitionedTopicProperties(String key) {
        ...
        pulsar().getBrokerService().getTopicIfExists(topicName.toString()).thenAccept(opt -> {
            ...
            ManagedLedger managedLedger = ((PersistentTopic) opt.get()).getManagedLedger();
            managedLedger.asyncDeleteProperty(
                    key,
                    new AsyncCallbacks.UpdatePropertiesCallback() {
    ...
    protected CompletableFuture<Void> internalCheckTopicExists(TopicName topicName) {
        return pulsar().getNamespaceService().checkTopicExists(topicName).thenAccept(exist -> {
            if (!exist) {
                throw new RestException(Status.NOT_FOUND, getTopicNotFoundErrorMessage(topicName.toString()));
            }
        });
    ...
    protected void internalDeletePartitionedTopic(AsyncResponse asyncResponse, boolean authoritative, boolean force) {
        validateTopicOwnershipAsync(topicName, authoritative)
                .thenCompose(__ -> validateNamespaceOperationAsync(
                        topicName.getNamespaceObject(), NamespaceOperation.DELETE_TOPIC))
                .thenCompose(
                        __ -> pulsar().getBrokerService()
                                .fetchPartitionedTopicMetadataAsync(topicName)
                                .thenCompose(partitionedMeta -> {
                                    ...
                                    return internalRemovePartitionsAuthenticationPoliciesAsync()
                                            .thenCompose(
                                                    unused -> internalRemovePartitionsTopicAsync(numPartitions, force));
                                })
                        // Only tries to delete the znode for partitioned topic when all its partitions are successfully
                        // deleted
                        )
                .thenCompose(__ -> getPulsarResources()
                ...
                .exceptionally(ex -> {
                    ...
                    if (realCause instanceof PreconditionFailedException) {
                        asyncResponse.resume(new RestException(
                                Status.PRECONDITION_FAILED, "Topic has active producers/subscriptions"));
                        ...
                        asyncResponse.resume(new RestException(new RestException(
                                Status.NOT_FOUND, getPartitionedTopicNotFoundErrorMessage(topicName.toString()))));
    ...
    private CompletableFuture<Void> internalRemovePartitionsTopicAsync(int numPartitions, boolean force) {
        return pulsar().getPulsarResources()
                ...
                .runWithMarkDeleteAsync(
                        topicName, () -> internalRemovePartitionsTopicNoAutoCreationDisableAsync(numPartitions, force));
    }

    private CompletableFuture<Void> internalRemovePartitionsTopicNoAutoCreationDisableAsync(
            int numPartitions, boolean force) {
        return FutureUtil.waitForAll(IntStream.range(0, numPartitions)
                .mapToObj(i -> {
                    ...
                    try {
                        ...
                        log.error(
                                "[{}] Failed to get admin client while delete partition {}",
                                clientAppId(),
                                topicNamePartition,
                                ex);
    ...
    private CompletableFuture<Void> internalRemovePartitionsAuthenticationPoliciesAsync() {
        CompletableFuture<Void> future = new CompletableFuture<>();
        getAuthorizationService().removePermissionsAsync(topicName).whenComplete((r, ex) -> {
            if (ex != null) {
                Throwable realCause = FutureUtil.unwrapCompletionException(ex);
                if (realCause instanceof MetadataStoreException.NotFoundException) {
                    log.warn("Namespace policies of {} not found", topicName.getNamespaceObject());
                    ...
                    log.error("Failed to delete authentication policies for partitioned topic {}", topicName, ex);
                    future.completeExceptionally(realCause);
                }
            } else {
                    ...
                    if (topicName.isPartitioned()) {
                        ...
                        getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                                        ...
                                        for (int i = 0; i < meta.partitions; i++) {
                                            ...
                                            try {
                                                futures.add(pulsar().getAdminClient()
                                                        .topics()
                                                        .unloadAsync(topicNamePartition.toString()));
                                            } catch (Exception e) {
                                                log.error(
                                                        "[{}] Failed to unload topic {}",
                                                        clientAppId(),
                                                        topicNamePartition,
                                                        e);
                                        ...
                                        FutureUtil.waitForAll(futures).handle((result, exception) -> {
                                            if (exception != null) {
                                                ...
                                                if (th instanceof NotFoundException) {
                                                    asyncResponse.resume(
                                                            new RestException(Status.NOT_FOUND, th.getMessage()));
                                ...
                                .exceptionally(ex -> {
                                    ...
                                    if (!isRedirectException(ex)) {
                                        log.error(
                                                "[{}] Failed to get partitioned metadata while unloading topic {}",
                                                clientAppId(),
                                                topicName,
                                                ex);
                                    }
    ...
    protected CompletableFuture<DelayedDeliveryPolicies> internalGetDelayedDeliveryPolicies(
    ...
    protected CompletableFuture<OffloadPoliciesImpl> internalGetOffloadPolicies(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal).thenApply(op -> {
            ...
            if (applied) {
                ...
                offloadPolicies = OffloadPoliciesImpl.mergeConfiguration(
                        offloadPolicies,
                        namespacePolicy,
    ...
    protected CompletableFuture<InactiveTopicPolicies> internalGetInactiveTopicPolicies(
        ...
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(op -> op.map(TopicPolicies::getInactiveTopicPolicies).orElseGet(() -> {
                    if (applied) {
    ...
    protected CompletableFuture<Integer> internalGetMaxUnackedMessagesOnSubscription(
            boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal).thenApply(op -> op.map(
    ...
    protected CompletableFuture<Void> internalSetMaxUnackedMessagesOnSubscription(
            Integer maxUnackedNum, boolean isGlobal) {
        if (maxUnackedNum != null && maxUnackedNum < 0) {
            throw new RestException(Status.PRECONDITION_FAILED, "maxUnackedNum must be 0 or more");
        }
    ...
    private void internalUnloadTransactionCoordinatorAsync(AsyncResponse asyncResponse, boolean authoritative) {
        validateTopicOwnershipAsync(topicName, authoritative)
                .thenCompose(__ -> validateTopicOperationAsync(topicName, TopicOperation.UNLOAD)
                        .thenCompose(v -> pulsar().getTransactionMetadataStoreService()
                ...
                .exceptionally(ex -> {
                    // If the exception is not redirect exception we need to log it.
                    if (!isRedirectException(ex)) {
                        log.error("[{}] Failed to unload tc {},{}", clientAppId(), topicName.getPartitionIndex(), ex);
                    }
                    resumeAsyncResponseExceptionally(asyncResponse, ex);
                    return null;
                });
    ...
    protected void internalGetSubscriptions(AsyncResponse asyncResponse, boolean authoritative) {
        ...
        future.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative)
                                ...
                                getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                                        .thenAccept(partitionMetadata -> {
                                            if (partitionMetadata.partitions > 0
                                                ...
                                                try {
                                                    final Set<String> subscriptions = Collections.newSetFromMap(
                                                            new ConcurrentHashMap<>(partitionMetadata.partitions));
                                                    final List<CompletableFuture<Object>> subscriptionFutures =
                                                            new ArrayList<>();
                                                    if (topicName.getDomain() == TopicDomain.persistent) {
                                                        ...
                                                        FutureUtil.waitForAll(new ArrayList<>(existsFutures.values()))
                                                                .thenApply(unused2 -> existsFutures.entrySet().stream()
                                                                        .filter(e -> e.getValue()
                                                                                .join())
                                                                        .map(item -> topicName
                                                                                .getPartition(item.getKey())
                                                                                .toString())
                                                                        .collect(Collectors.toList()))
                                                                .thenAccept(topics -> {
                                                                    if (log.isDebugEnabled()) {
                                                                ...
                                                                .thenAccept(unused3 -> resumeAsyncResponse(
                                                                        asyncResponse,
                                                                        subscriptions,
                                                                        subscriptionFutures));
                                                    } else {
                                                            ...
                                                            CompletableFuture<List<String>> subscriptionsAsync =
                                                                    pulsar().getAdminClient()
                                                                            ...
                                                                            .getSubscriptionsAsync(topicName
                                                                                    .getPartition(i)
                                                                                    .toString());
                                                            subscriptionFutures.add(subscriptionsAsync.thenApply(
                                                                    subscriptions::addAll));
                                                        }
                                                        resumeAsyncResponse(
                                                                asyncResponse, subscriptions, subscriptionFutures);
                                                    }
                                                } catch (Exception e) {
                                                    log.error(
                                                            "[{}] Failed to get list of subscriptions for {}",
                                                            clientAppId(),
                                                            topicName,
                                                            e);
                                        ...
                                        .exceptionally(ex -> {
                                            ...
                                            if (!isRedirectException(ex)) {
                                                log.error(
                                                        "[{}] Failed to get partitioned topic metadata while get"
                        ...
                        .exceptionally(ex -> {
                            ...
                            if (!isRedirectException(ex)) {
                                log.error(
                                        "[{}] Failed to validate the global namespace/topic ownership while get subscriptions"
                                                + " for topic {}",
                                        clientAppId(),
                                        topicName,
                                        ex);
    ...
    private void internalGetSubscriptionsForNonPartitionedTopic(AsyncResponse asyncResponse) {
        getTopicReferenceAsync(topicName)
                .thenAccept(topic -> asyncResponse.resume(
                        new ArrayList<>(topic.getSubscriptions().keys())))
                .exceptionally(ex -> {
                    // If the exception is not redirect exception we need to log it.
                    if (!isRedirectException(ex)) {
    ...
    protected CompletableFuture<? extends TopicStats> internalGetStatsAsync(
        ...
        return future.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative))
                ...
                .thenCompose(__ -> getTopicReferenceAsync(topicName))
                .thenCompose(topic ->
                        topic.asyncGetStats(getPreciseBacklog, subscriptionBacklogSize, getEarliestTimeInBacklog));
    }

    protected CompletableFuture<PersistentTopicInternalStats> internalGetInternalStatsAsync(
    ...
    protected void internalGetManagedLedgerInfo(AsyncResponse asyncResponse, boolean authoritative) {
        ...
        future.thenAccept(__ -> {
                    ...
                    if (topicName.isPartitioned()) {
                        ...
                        getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                                .thenAccept(partitionMetadata -> {
                                    if (partitionMetadata.partitions > 0) {
                                        ...
                                        for (int i = 0; i < partitionMetadata.partitions; i++) {
                                            TopicName topicNamePartition = topicName.getPartition(i);
                                            try {
                                                final ObjectReader managedLedgerInfoReader =
                                                        objectReader().forType(ManagedLedgerInfo.class);
                                                futures.add(pulsar().getAdminClient()
                                                        .topics()
                                                        ...
                                                        .thenApply((response) -> {
                                                            try {
                                                                return Pair.of(
                                                                        ...
                                                                        managedLedgerInfoReader.readValue(response));
                                                ...
                                                log.error(
                                                        "[{}] Failed to get admin client while get managed info for {}",
                                                        clientAppId(),
                                                        topicNamePartition,
                                                        e);
                                                throw new RestException(e);
                                        ...
                                        FutureUtil.waitForAll(futures).whenComplete((result, exception) -> {
                                            if (exception != null) {
                                                ...
                                                if (t instanceof NotFoundException) {
                                                    ...
                                                    log.error(
                                                            "[{}] Failed to get managed info for {}",
                                                            clientAppId(),
                                                            topicName,
                                                            t);
                                                ...
                                                for (CompletableFuture<Pair<String, ManagedLedgerInfo>> infoFuture :
                                                        futures) {
                                                    Pair<String, ManagedLedgerInfo> info = infoFuture.getNow(null);
                                                    partitionedManagedLedgerInfo.partitions.put(
                                ...
                                .exceptionally(ex -> {
                                    ...
                                    if (!isRedirectException(ex)) {
                                        log.error(
                                                "[{}] Failed to get partitioned metadata while get managed info for {}",
                                                clientAppId(),
                                                topicName,
                                                ex);
                                    }
                ...
                .exceptionally(ex -> {
                    ...
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to validate the global namespace ownership while get managed info for {}",
                                clientAppId(),
                                topicName,
                                ex);
                    }
    ...
    protected void internalGetPartitionedStats(
            AsyncResponse asyncResponse,
            boolean authoritative,
        ...
        future.thenCompose(__ -> getPartitionedTopicMetadataAsync(topicName, authoritative, false))
                .thenAccept(partitionMetadata -> {
                    ...
                    for (int i = 0; i < partitionMetadata.partitions; i++) {
                        ...
                        topicStatsFutureList.add(pulsar().getNamespaceService()
                                .isServiceUnitOwnedAsync(partition)
                                .thenCompose(owned -> {
                                    if (owned) {
                                        return getTopicReferenceAsync(partition)
                                                .thenApply(ref -> ref.getStats(
                                                        getPreciseBacklog,
                    ...
                    FutureUtil.waitForAll(topicStatsFutureList).handle((result, exception) -> {
                        ...
                        for (int i = 0; i < topicStatsFutureList.size(); i++) {
                            ...
                            if (statFuture.isDone() && !statFuture.isCompletedExceptionally()) {
                                try {
                                    ...
                                    if (perPartition) {
                                        stats.getPartitions()
                                                .put(topicName.getPartition(i).toString(), statFuture.get());
                                    }
                                } catch (Exception e) {
                                    asyncResponse.resume(new RestException(e));
                                    return null;
                        ...
                        if (perPartition && stats.partitions.isEmpty()) {
                            try {
                                boolean pathExists = namespaceResources()
                                        .getPartitionedTopicResources()
                                ...
                                if (pathExists) {
                                    ...
                                    asyncResponse.resume(new RestException(
                                            Status.NOT_FOUND, "Internal topics have not been generated yet"));
                                    return null;
                                }
                            } catch (Exception e) {
                                asyncResponse.resume(new RestException(e));
    ...
    protected void internalGetPartitionedStatsInternal(AsyncResponse asyncResponse, boolean authoritative) {
        ...
        future.thenCompose(__ -> getPartitionedTopicMetadataAsync(topicName, authoritative, false))
                .thenAccept(partitionMetadata -> {
                    ...
                    for (int i = 0; i < partitionMetadata.partitions; i++) {
                        try {
                            topicStatsFutureList.add(pulsar().getAdminClient()
                                    .topics()
                                    .getInternalStatsAsync(
                                            (topicName.getPartition(i).toString()), false));
                        } catch (PulsarServerException e) {
                            asyncResponse.resume(new RestException(e));
                            return;
                        }
                    ...
                    FutureUtil.waitForAll(topicStatsFutureList).handle((result, exception) -> {
                        ...
                        for (int i = 0; i < topicStatsFutureList.size(); i++) {
                            ...
                            if (statFuture.isDone() && !statFuture.isCompletedExceptionally()) {
                                try {
                                    stats.partitions.put(
                                            topicName.getPartition(i).toString(), statFuture.get());
                                } catch (Exception e) {
                                    asyncResponse.resume(new RestException(e));
                                    return null;
                                }
    ...
    protected CompletableFuture<Void> internalDeleteSubscriptionAsync(
        ...
        return future.thenCompose((__) -> validateTopicOperationAsync(topicName, TopicOperation.UNSUBSCRIBE, subName))
                .thenCompose(__ -> {
                    if (topicName.isPartitioned()) {
                        ...
                        return getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                                .thenCompose(partitionMetadata -> {
                                    if (partitionMetadata.partitions > 0) {
                                        ...
                                        for (int i = 0; i < partitionMetadata.partitions; i++) {
                                            TopicName topicNamePartition = topicName.getPartition(i);
                                            futures.add(adminClient
                                                    .topics()
                                                    .deleteSubscriptionAsync(
                                                            topicNamePartition.toString(), subName, false));
                                        }
                                    ...
                                    return internalDeleteSubscriptionForNonPartitionedTopicAsync(
                                            subName, authoritative, force);
                                });
                    }
                });
    }
    ...
    private CompletableFuture<Void> internalDeleteSubscriptionForNonPartitionedTopicAsync(
            String subName, boolean authoritative, boolean force) {
        return validateTopicOwnershipAsync(topicName, authoritative)
                .thenCompose(__ -> getTopicReferenceAsync(topicName))
                .thenCompose((topic) -> {
                    ...
                    if (sub == null) {
                        throw new RestException(
                                Status.NOT_FOUND, getSubNotFoundErrorMessage(topicName.toString(), subName));
                    }
                    return force ? sub.deleteForcefully() : sub.delete();
                });
    }

    private void internalAnalyzeSubscriptionBacklogForNonPartitionedTopic(
            AsyncResponse asyncResponse, String subName, Optional<Position> position, boolean authoritative) {
        validateTopicOwnershipAsync(topicName, authoritative)
                ...
                .thenAccept((AnalyzeBacklogResult rawResult) -> {
                    ...
                    if (rawResult.getFirstPosition() != null) {
                        result.setFirstMessageId(rawResult.getFirstPosition().getLedgerId()
                                + ":"
                    ...
                    log.info(
                            "[{}] analyzeBacklog topic {} subscription {} result {}",
                            clientAppId(),
                            subName,
                            topicName,
                            result);
                ...
                .exceptionally(ex -> {
                    ...
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to analyze subscription backlog {} {}",
                                clientAppId(),
                                topicName,
                                subName,
                                cause);
    ...
    private void internalUpdateSubscriptionPropertiesForNonPartitionedTopic(
            AsyncResponse asyncResponse,
            String subName,
    ...
    private void internalGetSubscriptionPropertiesForNonPartitionedTopic(
            AsyncResponse asyncResponse, String subName, boolean authoritative) {
        validateTopicOwnershipAsync(topicName, authoritative)
                .thenCompose(__ -> validateTopicOperationAsync(topicName, TopicOperation.CONSUME, subName))
                .thenCompose(__ -> getTopicReferenceAsync(topicName))
    ...
    protected void internalDeleteSubscriptionForcefully(
            AsyncResponse asyncResponse, String subName, boolean authoritative) {
        CompletableFuture<Void> future;
        if (topicName.isGlobal()) {
            future = validateGlobalNamespaceOwnershipAsync(namespaceName);
        ...
        future.thenAccept(__ -> {
                    if (topicName.isPartitioned()) {
                        internalDeleteSubscriptionForNonPartitionedTopicForcefully(
                                asyncResponse, subName, authoritative);
                    } else {
                        getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                                .thenAccept(partitionMetadata -> {
                                    if (partitionMetadata.partitions > 0) {
                                        ...
                                        for (int i = 0; i < partitionMetadata.partitions; i++) {
                                            ...
                                            try {
                                                futures.add(pulsar().getAdminClient()
                                                        .topics()
                                                        .deleteSubscriptionAsync(
                                                                topicNamePartition.toString(), subName, true));
                                            } catch (Exception e) {
                                                log.error(
                                                        "[{}] Failed to delete subscription forcefully {} {}",
                                                        clientAppId(),
                                                        topicNamePartition,
                                                        subName,
                                                        e);
                                ...
                                .exceptionally(ex -> {
                                    ...
                                    if (!isRedirectException(ex)) {
                                        log.error(
                                                "[{}] Failed to delete subscription forcefully {} from topic {}",
                                                clientAppId(),
                                                subName,
                                                topicName,
                                                ex);
    ...
    private void internalDeleteSubscriptionForNonPartitionedTopicForcefully(
            AsyncResponse asyncResponse, String subName, boolean authoritative) {
        validateTopicOwnershipAsync(topicName, authoritative)
    ...
    protected void internalSkipAllMessages(AsyncResponse asyncResponse, String subName, boolean authoritative) {
        ...
        future.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative))
                                    ...
                                    if (partitionMetadata.partitions > 0) {
                                        ...
                                        for (int i = 0; i < partitionMetadata.partitions; i++) {
                                            ...
                                            try {
                                                futures.add(pulsar().getAdminClient()
                                                        .topics()
                                                        .skipAllMessagesAsync(topicNamePartition.toString(), subName));
                                            } catch (Exception e) {
                                                log.error(
                                                        "[{}] Failed to skip all messages {} {}",
                                                        clientAppId(),
                                                        topicNamePartition,
                                                        subName,
                                                        e);
                                        ...
                                        return internalSkipAllMessagesForNonPartitionedTopicAsync(
                                                asyncResponse, subName);
                                    }
                                });
                    }
                })
                .exceptionally(ex -> {
                    ...
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to skip all messages for subscription {} on topic {}",
                                clientAppId(),
                                subName,
                                topicName,
                                ex);
    ...
    private CompletableFuture<Void> internalSkipAllMessagesForNonPartitionedTopicAsync(
            AsyncResponse asyncResponse, String subName) {
    ...
    protected void internalSkipMessages(
            AsyncResponse asyncResponse, String subName, int numMessages, boolean authoritative) {
        CompletableFuture<Void> future;
        if (topicName.isGlobal()) {
            future = validateGlobalNamespaceOwnershipAsync(namespaceName);
        } else {
                ...
                .thenCompose(__ -> getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                        .thenCompose(partitionMetadata -> {
                            ...
                            return getTopicReferenceAsync(topicName).thenCompose(t -> {
                                PersistentTopic topic = (PersistentTopic) t;
                                if (topic == null) {
                                    throw new RestException(new RestException(
    ...
    protected void internalExpireMessagesForAllSubscriptions(
            AsyncResponse asyncResponse, int expireTimeInSeconds, boolean authoritative) {
        CompletableFuture<Void> future;
        if (topicName.isGlobal()) {
            future = validateGlobalNamespaceOwnershipAsync(namespaceName);
        } else {
        ...
        future.thenCompose(__ -> getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                        .thenAccept(partitionMetadata -> {
                            if (topicName.isPartitioned()) {
                                internalExpireMessagesForAllSubscriptionsForNonPartitionedTopic(
                                        asyncResponse, partitionMetadata, expireTimeInSeconds, authoritative);
                            } else {
                                if (partitionMetadata.partitions > 0) {
                                    ...
                                    for (int i = 0; i < partitionMetadata.partitions; i++) {
                                        ...
                                        try {
                                            ...
                                            log.error(
                                                    "[{}] Failed to expire messages up to {} on {}",
                                                    clientAppId(),
                                                    expireTimeInSeconds,
                                                    topicNamePartition,
                                                    e);
                ...
                .exceptionally(ex -> {
                    ...
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to expire messages for all subscription on topic {}",
                                clientAppId(),
                                topicName,
                                ex);
                    }
    ...
    private void internalExpireMessagesForAllSubscriptionsForNonPartitionedTopic(
            AsyncResponse asyncResponse,
        ...
        validateTopicOwnershipAsync(topicName, authoritative)
                ...
                .thenCompose(__ -> getTopicReferenceAsync(topicName).thenAccept(t -> {
                    ...
                    for (int i = 0; i < subNames.size(); i++) {
                        try {
                            futures.add(internalExpireMessagesByTimestampForSinglePartitionAsync(
                                    partitionMetadata, subNames.get(i), expireTimeInSeconds));
                        } catch (Exception e) {
                            log.error(
                                    "[{}] Failed to expire messages for all subscription up to {} on {}",
                                    clientAppId(),
                                    expireTimeInSeconds,
                                    topicName,
                                    e);
    ...
    private CompletableFuture<Void> internalResetCursorForPartitionedTopic(
            String subName, long timestamp, boolean authoritative) {
        return getPartitionedTopicMetadataAsync(topicName, authoritative, false).thenCompose(partitionMetadata -> {
            ...
            if (numPartitions > 0) {
                ...
                return future.whenComplete((r, ex) -> {
                    ...
                    if (failureCount.get() == numPartitions) {
                        ...
                        log.warn(
                                "[{}] [{}] Partial errors for reset cursor on subscription {} to time {}",
                                clientAppId(),
                                topicName,
                                subName,
                                timestamp,
    ...
    private CompletableFuture<Void> internalResetCursorForNonPartitionedTopic(
            String subName, long timestamp, boolean authoritative) {
        return validateTopicOwnershipAsync(topicName, authoritative)
                .thenCompose(__ -> validateTopicOperationAsync(topicName, TopicOperation.RESET_CURSOR, subName))
                .thenCompose(__ -> {
                    log.info(
                            "[{}] [{}] Received reset cursor on subscription {} to time {}",
                            clientAppId(),
                            topicName,
                            subName,
                            timestamp);
    ...
    protected void internalCreateSubscription(
            AsyncResponse asyncResponse,
        ...
        ret.thenAccept(__ -> {
                    final MessageIdImpl targetMessageId =
                            messageId == null ? (MessageIdImpl) MessageId.latest : messageId;
                    log.info(
                            "[{}][{}] Creating subscription {} at message id {} with properties {}",
                            clientAppId(),
                            topicName,
                            subscriptionName,
                            targetMessageId,
                    ...
                    if (topicName.isPartitioned()) {
                        ...
                        pulsar().getBrokerService()
                                ...
                                .thenCompose(allowAutoTopicCreation -> getPartitionedTopicMetadataAsync(
                                                topicName, authoritative, allowAutoTopicCreation)
                                        .thenAccept(partitionMetadata -> {
                                            final int numPartitions = partitionMetadata.partitions;
                                            if (numPartitions > 0) {
                                                final CompletableFuture<Void> future = new CompletableFuture<>();
                                                ...
                                                for (int i = 0; i < numPartitions; i++) {
                                                    ...
                                                    try {
                                                        ...
                                                        log.warn(
                                                                "[{}] [{}] Failed to create subscription {} at message id {}",
                                                                clientAppId(),
                                                                topicNamePartition,
                                                                subscriptionName,
                                                                targetMessageId,
                                                ...
                                                future.whenComplete((r, ex) -> {
                                                    if (ex != null) {
                                                        if (ex instanceof PulsarAdminException) {
                                                            asyncResponse.resume(
                                                                    new RestException((PulsarAdminException) ex));
                                                            return;
                                                        } else {
                                                            asyncResponse.resume(new RestException(ex));
                                                            return;
                                                    ...
                                                    if (partitionException.get() != null) {
                                                        ...
                                                        if (partitionException.get() instanceof PulsarAdminException) {
                                                            asyncResponse.resume(new RestException(
                                                                    (PulsarAdminException) partitionException.get()));
                                                            return;
                                                        } else {
                                                            asyncResponse.resume(
                                                                    new RestException(partitionException.get()));
                                                            return;
                                                        }
                                                    }
                                ...
                                .exceptionally(ex -> {
                                    ...
                                    if (!isRedirectException(ex)) {
                                        log.error(
                                                "[{}] Failed to create subscription {} on topic {}",
                                                clientAppId(),
                                                subscriptionName,
                                                topicName,
                                                ex);
    ...
    private void internalCreateSubscriptionForNonPartitionedTopic(
        ...
        validateTopicOwnershipAsync(topicName, authoritative)
                ...
                .thenCompose(isAllowAutoTopicCreation ->
                        pulsar().getBrokerService().getTopic(topicName.toString(), isAllowAutoTopicCreation))
                .thenApply(optTopic -> {
                    if (optTopic.isPresent()) {
                        return optTopic.get();
                    } else {
                        throw new RestException(
                                Status.PRECONDITION_FAILED, "Topic does not exist and cannot be auto-created");
                    }
                })
                .thenCompose(topic -> {
                ...
                .thenRun(() -> {
                    log.info(
                            "[{}][{}] Successfully created subscription {} at message id {}",
                            clientAppId(),
                            topicName,
                            subscriptionName,
                            targetMessageId);
                ...
                .exceptionally(ex -> {
                    ...
                    if (t instanceof SubscriptionInvalidCursorPosition) {
                        ...
                        asyncResponse.resume(new RestException(
                                Status.PRECONDITION_FAILED, "Failed for Subscription Busy: " + t.getMessage()));
                    } else {
                        resumeAsyncResponseExceptionally(asyncResponse, t);
                    }
                    return null;
    ...
    protected void internalUpdateSubscriptionProperties(
            AsyncResponse asyncResponse,
            String subName,
            Map<String, String> subscriptionProperties,
            boolean authoritative) {
        ...
        future.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative))
                .thenAccept(__ -> {
                    if (topicName.isPartitioned()) {
                        internalUpdateSubscriptionPropertiesForNonPartitionedTopic(
                                asyncResponse, subName, subscriptionProperties, authoritative);
                    } else {
                        getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                                .thenAcceptAsync(
                                        partitionMetadata -> {
                                            if (partitionMetadata.partitions > 0) {
                                                ...
                                                for (int i = 0; i < partitionMetadata.partitions; i++) {
                                                    ...
                                                    try {
                                                        ...
                                                        log.error(
                                                                "[{}] Failed to update properties for subscription {} {}",
                                                                clientAppId(),
                                                                topicNamePartition,
                                                                subName,
                                                                e);
                                ...
                                .exceptionally(ex -> {
                                    log.error(
                                            "[{}] Failed to update properties for subscription {} from topic {}",
                                            clientAppId(),
                                            subName,
                                            topicName,
                                            ex);
                ...
                .exceptionally(ex -> {
                    ...
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to update subscription {} from topic {}",
                                clientAppId(),
                                subName,
                                topicName,
                                ex);
    ...
    protected void internalAnalyzeSubscriptionBacklog(
            AsyncResponse asyncResponse, String subName, Optional<Position> position, boolean authoritative) {
        CompletableFuture<Void> future;
        if (topicName.isGlobal()) {
            future = validateGlobalNamespaceOwnershipAsync(namespaceName);
        ...
        future.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative))
                ...
                .thenAccept(__ -> {
                    internalAnalyzeSubscriptionBacklogForNonPartitionedTopic(
                            asyncResponse, subName, position, authoritative);
                })
                .exceptionally(ex -> {
                    // If the exception is not redirect exception we need to log it.
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to analyze back log of subscription {} from topic {}",
                                clientAppId(),
                                subName,
                                topicName,
                                ex);
    ...
    protected void internalGetSubscriptionProperties(
            AsyncResponse asyncResponse, String subName, boolean authoritative) {
        CompletableFuture<Void> future;
        if (topicName.isGlobal()) {
            future = validateGlobalNamespaceOwnershipAsync(namespaceName);
        ...
        future.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative))
                .thenAccept(__ -> {
                    if (topicName.isPartitioned()) {
                        internalGetSubscriptionPropertiesForNonPartitionedTopic(asyncResponse, subName, authoritative);
                    } else {
                        getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                                .thenAcceptAsync(
                                        partitionMetadata -> {
                                            if (partitionMetadata.partitions > 0) {
                                                final List<CompletableFuture<Map<String, String>>> futures =
                                                        new ArrayList<>();
                                                ...
                                                FutureUtil.waitForAll(futures).handle((result, exception) -> {
                                                    if (exception != null) {
                                                        ...
                                                        if (t instanceof NotFoundException) {
                                                            ...
                                                            log.error(
                                                                    "[{}] Failed to get properties for subscription {} {}",
                                                                    clientAppId(),
                                                                    topicName,
                                                                    subName,
                                                                    t);
                                                    ...
                                                    asyncResponse.resume(Response.ok(aggregatedResult)
                                                            .build());
                                                    return null;
                                                });
                                            } else {
                                                internalGetSubscriptionPropertiesForNonPartitionedTopic(
                                                        asyncResponse, subName, authoritative);
                                            }
                                        },
                                        pulsar().getExecutor())
    ...
    protected void internalResetCursorOnPosition(
            AsyncResponse asyncResponse,
            String subName,
            boolean authoritative,
            MessageIdImpl messageId,
                    ...
                    .thenCompose(topicMetadata -> {
                        if (topicMetadata.partitions > 0) {
                            log.warn(
                                    ...
                                    topicName,
                                    subName);
                            throw new CompletionException(new RestException(
                                    Status.METHOD_NOT_ALLOWED,
        ...
        future.thenAccept(__ -> {
                    log.info(
                            "[{}][{}] received reset cursor on subscription {} to position {}",
                            clientAppId(),
                            topicName,
                            subName,
                            messageId);
                    validateTopicOwnershipAsync(topicName, authoritative)
                            ...
                            .thenAccept(topic -> {
                                ...
                                batchSizeFuture
                                        .thenAccept(bi -> {
                                            ...
                                            sub.resetCursor(seekPosition)
                                                    .thenRun(() -> {
                                                        log.info(
                                                                "[{}][{}] successfully reset cursor on subscription {}"
                                                                        + " to position {}",
                                                                clientAppId(),
                                                                topicName,
                                                                subName,
                                                    ...
                                                    .exceptionally(ex -> {
                                                        Throwable t = (ex instanceof CompletionException
                                                                ...
                                                                : ex);
                                                        log.warn(
                                                                "[{}][{}] Failed to reset cursor on subscription {}"
                                                                        + " to position {}",
                                                                clientAppId(),
                                                                topicName,
                                                                subName,
                            ...
                            .exceptionally(ex -> {
                                ...
                                if (!isRedirectException(ex)) {
                                    log.warn(
                                            "[{}][{}] Failed to reset cursor on subscription {} to position {}",
                                            clientAppId(),
                                            topicName,
                                            subName,
                                            messageId,
    ...
    private void getEntryBatchSize(
    ...
    private PositionImpl calculatePositionAckSet(
            boolean isExcluded, int batchSize, int batchIndex, MessageIdImpl messageId) {
        PositionImpl seekPosition;
        if (batchSize > 0) {
            long[] ackSet;
                    ...
                    if (topicName.isPartitioned()) {
                        ...
                        return getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                                .thenAccept(topicMetadata -> {
                                    if (topicMetadata.partitions > 0) {
                                        log.warn(
                                                "[{}] Not supported getMessageById operation on partitioned-topic {}",
                                                clientAppId(),
                                                topicName);
                                        throw new RestException(
                                                Status.METHOD_NOT_ALLOWED,
                ...
                .thenCompose(topic -> {
                    ...
                    return ledger.asyncFindPosition(entry -> {
                                try {
                                    ...
                                    log.error(
                                            "[{}] Error deserializing message for message position find", topicName, e);
                                } finally {
                                    entry.release();
                                }
                                return false;
    ...
    protected CompletableFuture<Response> internalPeekNthMessageAsync(
            String subName, int messagePosition, boolean authoritative) {
        CompletableFuture<Void> ret;
        // If the topic name is a partition name, no need to get partition topic metadata again
        if (!topicName.isPartitioned()) {
            ret = getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                    .thenCompose(topicMetadata -> {
                        if (topicMetadata.partitions > 0) {
                            throw new RestException(
                                    Status.METHOD_NOT_ALLOWED, "Peek messages on a partitioned topic is not allowed");
                        }
                        return CompletableFuture.completedFuture(null);
                    });
        } else {
        ...
        return ret.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative))
                ...
                .thenCompose(topic -> {
                    ...
                    if (!(topic instanceof PersistentTopic)) {
                        log.error(
                                "[{}] Not supported operation of non-persistent topic {} {}",
                                clientAppId(),
                                topicName,
                                subName);
                        throw new RestException(
                                Status.METHOD_NOT_ALLOWED, "Peek messages on a non-persistent topic is not allowed");
                    } else {
                ...
                .thenCompose(entry -> {
                    try {
                        ...
                        log.error(
                                "[{}] Failed to peek message at position {} from {} {}",
                                clientAppId(),
                                messagePosition,
                                topicName,
                                subName,
    ...
    protected CompletableFuture<Response> internalExamineMessageAsync(
            String initialPosition, long messagePosition, boolean authoritative) {
        CompletableFuture<Void> ret;
        if (topicName.isGlobal()) {
            ret = validateGlobalNamespaceOwnershipAsync(namespaceName);
        } else {
        ...
        return ret.thenCompose(__ -> getTopicReferenceAsync(topicName))
                .thenCompose(topic -> {
                    if (!(topic instanceof PersistentTopic)) {
                        log.error("[{}] Not supported operation of non-persistent topic {} ", clientAppId(), topicName);
                        throw new RestException(
                                Status.METHOD_NOT_ALLOWED, "Examine messages on a non-persistent topic is not allowed");
                    }
                    try {
                        ...
                        long messageToSkip = initialPositionLocal.equals("earliest")
                                ? messagePositionLocal
                        ...
                        persistentTopic.asyncReadEntry(
                                readPosition,
                                new AsyncCallbacks.ReadEntryCallback() {
                                    @Override
                                    public void readEntryComplete(Entry entry, Object ctx) {
                        ...
                        log.error(
                                "[{}] Failed to examine message at position {} from {} due to {}",
                                clientAppId(),
                                messagePosition,
                                topicName,
                                exception);
    ...
    private Response generateResponseWithEntry(Entry entry) throws IOException {
        ...
        if (metadata.hasNumMessagesInBatch()) {
            ...
            responseBuilder.header("X-Pulsar-batch-size", totalSize - metadata.getSerializedSize());
        }
        ...
        if (metadata.hasSchemaVersion()) {
            responseBuilder.header(
                    "X-Pulsar-Base64-schema-version", Base64.getEncoder().encodeToString(metadata.getSchemaVersion()));
        }
        ...
        if (metadata.hasOrderingKey()) {
            responseBuilder.header(
                    "X-Pulsar-Base64-ordering-key", Base64.getEncoder().encodeToString(metadata.getOrderingKey()));
        }
        ...
        ByteBuf data = PulsarByteBufAllocator.DEFAULT.heapBuffer(
                uncompressedPayload.readableBytes(), uncompressedPayload.readableBytes());
    ...
    protected CompletableFuture<Map<BacklogQuota.BacklogQuotaType, BacklogQuota>> internalGetBacklogQuota(
            boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal).thenApply(op -> {
            Map<BacklogQuota.BacklogQuotaType, BacklogQuota> quotaMap = op.map(TopicPolicies::getBackLogQuotaMap)
                    .map(map -> {
            ...
            if (applied && quotaMap.isEmpty()) {
                ...
                if (quotaMap.isEmpty()) {
                    for (BacklogQuota.BacklogQuotaType backlogQuotaType : BacklogQuota.BacklogQuotaType.values()) {
                        quotaMap.put(backlogQuotaType, namespaceBacklogQuota(namespaceName, backlogQuotaType));
                    }
                }
            }
            return quotaMap;
    ...
    protected void internalGetBacklogSizeByMessageId(
            AsyncResponse asyncResponse, MessageIdImpl messageId, boolean authoritative) {
        CompletableFuture<Void> ret;
        // If the topic name is a partition name, no need to get partition topic metadata again
        if (!topicName.isPartitioned()) {
            ret = getPartitionedTopicMetadataAsync(topicName, authoritative, false)
                    .thenCompose(topicMetadata -> {
                        if (topicMetadata.partitions > 0) {
                            log.warn(
                                    "[{}] Not supported calculate backlog size operation on partitioned-topic {}",
                                    clientAppId(),
                                    topicName);
                            asyncResponse.resume(new RestException(
                                    Status.METHOD_NOT_ALLOWED,
        ...
        future.thenAccept(__ -> validateTopicOwnershipAsync(topicName, authoritative)
                ...
                .exceptionally(ex -> {
                    ...
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to validate global namespace ownership "
    ...
    protected CompletableFuture<Void> internalSetBacklogQuota(
            BacklogQuota.BacklogQuotaType backlogQuotaType, BacklogQuotaImpl backlogQuota, boolean isGlobal) {
        BacklogQuota.BacklogQuotaType finalBacklogQuotaType =
                backlogQuotaType == null ? BacklogQuota.BacklogQuotaType.destination_storage : backlogQuotaType;
        ...
        return validateTopicPolicyOperationAsync(topicName, PolicyName.REPLICATION, PolicyOperation.WRITE)
                ...
                .thenCompose(__ -> getTopicPoliciesAsyncWithRetry(topicName).thenCompose(op -> {
                    ...
                    return pulsar().getTopicPoliciesService()
                            ...
                            .thenRun(() -> {
                                log.info(
                                        "[{}] Successfully set replication clusters for namespace={}, "
                                                + "topic={}, clusters={}",
                                        clientAppId(),
                                        namespaceName,
                                        topicName.getLocalName(),
    ...
    protected CompletableFuture<Boolean> internalGetDeduplication(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(op -> op.map(TopicPolicies::getDeduplicationEnabled).orElseGet(() -> {
                    if (applied) {
    ...
    protected CompletableFuture<Void> internalSetMessageTTL(Integer ttlInSecond, boolean isGlobal) {
        ...
        if (ttlInSecond != null && ttlInSecond < 0) {
            return FutureUtil.failedFuture(
                    new RestException(Status.PRECONDITION_FAILED, "Invalid value for message TTL"));
        }
        ...
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal).thenCompose(op -> {
            ...
            return pulsar().getTopicPoliciesService()
                    .updateTopicPoliciesAsync(topicName, topicPolicies)
                    .thenRun(() -> log.info(
                            "[{}] Successfully set topic message ttl: namespace={}, topic={}, ttl={}",
                            clientAppId(),
                            namespaceName,
                            topicName.getLocalName(),
                            ttlInSecond));
    ...
    private CompletableFuture<RetentionPolicies> getRetentionPoliciesAsync(
            TopicName topicName, TopicPolicies topicPolicies) {
    ...
    protected CompletableFuture<PersistencePolicies> internalGetPersistence(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(op -> op.map(TopicPolicies::getPersistence).orElseGet(() -> {
                    if (applied) {
                        PersistencePolicies namespacePolicy = getNamespacePolicies(namespaceName).persistence;
    ...
    protected CompletableFuture<Void> internalSetPersistence(
            PersistencePolicies persistencePolicies, boolean isGlobal) {
    ...
    protected CompletableFuture<Integer> internalGetMaxProducers(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(op -> op.map(TopicPolicies::getMaxProducerPerTopic).orElseGet(() -> {
                    if (applied) {
    ...
    protected CompletableFuture<Void> internalSetMaxProducers(Integer maxProducers, boolean isGlobal) {
        if (maxProducers != null && maxProducers < 0) {
            throw new RestException(Status.PRECONDITION_FAILED, "maxProducers must be 0 or more");
        }
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal).thenCompose(op -> {
            TopicPolicies topicPolicies = op.orElseGet(TopicPolicies::new);
    ...
    protected CompletableFuture<Void> internalSetMaxSubscriptionsPerTopic(
            Integer maxSubscriptionsPerTopic, boolean isGlobal) {
    ...
    protected CompletableFuture<DispatchRateImpl> internalGetReplicatorDispatchRate(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(
                        op -> op.map(TopicPolicies::getReplicatorDispatchRate).orElseGet(() -> {
                            if (applied) {
                                DispatchRateImpl namespacePolicy = getNamespacePolicies(namespaceName)
                                        .replicatorDispatchRate
                                        .get(pulsar().getConfiguration().getClusterName());
    ...
    protected CompletableFuture<Void> internalSetReplicatorDispatchRate(
    ...
    protected CompletableFuture<Void> preValidation(boolean authoritative) {
        ...
        if (topicName.isPartitioned()) {
            return FutureUtil.failedFuture(new RestException(
                    Status.PRECONDITION_FAILED, "Not allowed to set/get topic policy for a partition"));
        }
        CompletableFuture<Void> ret;
        if (topicName.isGlobal()) {
            ret = validateGlobalNamespaceOwnershipAsync(namespaceName);
        ...
        return ret.thenCompose(__ -> checkTopicExistsAsync(topicName)).thenCompose(exist -> {
            if (!exist) {
                throw new RestException(Status.NOT_FOUND, getTopicNotFoundErrorMessage(topicName.toString()));
            } else {
                return getPartitionedTopicMetadataAsync(topicName, false, false).thenCompose(metadata -> {
                    if (metadata.partitions > 0) {
                        return validateTopicOwnershipAsync(
                                TopicName.get(topicName.toString() + PARTITIONED_TOPIC_SUFFIX + 0), authoritative);
                    } else {
                        return validateTopicOwnershipAsync(topicName, authoritative);
                    }
                });
    ...
    protected CompletableFuture<Integer> internalGetMaxConsumers(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(op -> op.map(TopicPolicies::getMaxConsumerPerTopic).orElseGet(() -> {
                    if (applied) {
    ...
    protected CompletableFuture<Void> internalSetMaxConsumers(Integer maxConsumers, boolean isGlobal) {
        if (maxConsumers != null && maxConsumers < 0) {
            throw new RestException(Status.PRECONDITION_FAILED, "maxConsumers must be 0 or more");
        }
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal).thenCompose(op -> {
            TopicPolicies topicPolicies = op.orElseGet(TopicPolicies::new);
    ...
    protected CompletableFuture<MessageId> internalTerminateAsync(boolean authoritative) {
        if (SystemTopicNames.isSystemTopic(topicName)) {
            return FutureUtil.failedFuture(
                    new RestException(Status.METHOD_NOT_ALLOWED, "Termination of a system topic is not allowed"));
        }
        ...
        return ret.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative))
                ...
                .thenAccept(partitionMetadata -> {
                    if (partitionMetadata.partitions > 0) {
                        throw new RestException(
                                Status.METHOD_NOT_ALLOWED, "Termination of a partitioned topic is not allowed");
                    }
                })
                .thenCompose(__ -> getTopicReferenceAsync(topicName))
                .thenCompose(topic -> {
                    if (!(topic instanceof PersistentTopic)) {
                        throw new RestException(
                                Status.METHOD_NOT_ALLOWED, "Termination of a non-persistent topic is not allowed");
                    }
    ...
    protected void internalTerminatePartitionedTopic(AsyncResponse asyncResponse, boolean authoritative) {
        ...
        future.thenCompose(__ -> validateTopicOperationAsync(topicName, TopicOperation.TERMINATE)
                        ...
                        .thenAccept(partitionMetadata -> {
                            if (partitionMetadata.partitions == 0) {
                                String msg =
                                        "Termination of a non-partitioned topic is not allowed using partitioned-terminate"
                                                + ", please use terminate commands";
                                log.error("[{}] [{}] {}", clientAppId(), topicName, msg);
                                asyncResponse.resume(new RestException(Status.METHOD_NOT_ALLOWED, msg));
                                return;
    ...
    protected void internalExpireMessagesByTimestamp(
            AsyncResponse asyncResponse, String subName, int expireTimeInSeconds, boolean authoritative) {
        CompletableFuture<Void> future;
        if (topicName.isGlobal()) {
            future = validateGlobalNamespaceOwnershipAsync(namespaceName);
        } else {
                                        ...
                                        .thenCompose(partitionMetadata -> {
                                            if (topicName.isPartitioned()) {
                                                return internalExpireMessagesByTimestampForSinglePartitionAsync(
                                                        ...
                                                        .thenAccept(unused3 -> asyncResponse.resume(Response.noContent()
                                                                .build()));
                                            } else {
                                                if (partitionMetadata.partitions > 0) {
                                                    return CompletableFuture.completedFuture(null)
                                                    ...
                                                    return internalExpireMessagesByTimestampForSinglePartitionAsync(
                                                                    partitionMetadata, subName, expireTimeInSeconds)
                                                            .thenAccept(
                                                                    unused -> asyncResponse.resume(Response.noContent()
                                                                            .build()));
                                                }
                                            }
                                        })))
    ...
    private CompletableFuture<Void> internalExpireMessagesByTimestampForSinglePartitionAsync(
        ...
        if (!topicName.isPartitioned() && partitionMetadata.partitions > 0) {
            ...
            getTopicReferenceAsync(topicName)
                    .thenAccept(t -> {
                        if (t == null) {
                            resultFuture.completeExceptionally(new RestException(
                                    Status.NOT_FOUND, getTopicNotFoundErrorMessage(topicName.toString())));
                            return;
                        }
                        if (!(t instanceof PersistentTopic)) {
                        ...
                        if (messageExpirer.expireMessages(expireTimeInSeconds)) {
                            log.info(
                                    "[{}] Message expire started up to {} on {} {}",
                                    clientAppId(),
                                    expireTimeInSeconds,
                                    topicName,
                                    subName);
                            ...
                            if (log.isDebugEnabled()) {
                                log.debug(
                                        "Expire message by timestamp not issued on topic {} for subscription {} "
    ...
    protected void internalExpireMessagesByPosition(
            AsyncResponse asyncResponse,
            String subName,
            boolean authoritative,
            MessageIdImpl messageId,
        ...
        future.thenCompose(__ -> validateTopicOwnershipAsync(topicName, authoritative))
                ...
                .thenCompose(__ -> {
                    log.info(
                            "[{}][{}] Received expire messages on subscription {} to position {}",
                            clientAppId(),
                            topicName,
                            subName,
                            messageId);
                    return internalExpireMessagesNonPartitionedTopicByPosition(
                            asyncResponse, subName, messageId, isExcluded, batchIndex);
                })
                .exceptionally(ex -> {
                    // If the exception is not redirect exception we need to log it.
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to expire messages up to {} on subscription {} to position {}",
                                clientAppId(),
                                topicName,
                                subName,
                                messageId,
    ...
    private CompletableFuture<Void> internalExpireMessagesNonPartitionedTopicByPosition(
            AsyncResponse asyncResponse, String subName, MessageIdImpl messageId, boolean isExcluded, int batchIndex) {
        return getTopicReferenceAsync(topicName)
                .thenAccept(t -> {
                    PersistentTopic topic = (PersistentTopic) t;
                    if (topic == null) {
                    ...
                    try {
                        ...
                        batchSizeFuture
                                .thenAccept(bi -> {
                                    ...
                                    try {
                                        if (messageExpirer.expireMessages(position)) {
                                            ...
                                            if (log.isDebugEnabled()) {
                                                log.debug(
                                                        "Expire message by position not issued on topic {} for subscription {} "
                                                                + "due to ongoing message expiration not finished or subscription almost "
                                                                + "catch up.",
                                                        topicName,
                                                        subName);
                                            ...
                                            throw new RestException(
                                                    ...
                                                    "Expire message by position not issued on topic "
                                                            + topicName + " for subscription " + subName
                                                            + " due to ongoing"
                                ...
                                .exceptionally(e -> {
                                    ...
                                    if (throwable instanceof RestException) {
                                        log.warn(
                                                "[{}] Failed to expire messages up to {} on {} with subscription {}: {}",
                                                clientAppId(),
                                                messageId,
                                                topicName,
                                                subName,
    ...
    protected void internalTriggerCompaction(AsyncResponse asyncResponse, boolean authoritative) {
        ...
        future.thenAccept(__ -> {
                                    ...
                                    if (numPartitions > 0) {
                                        ...
                                        for (int i = 0; i < numPartitions; i++) {
                                            ...
                                            try {
                                                ...
                                                log.error(
                                                        "[{}] Failed to trigger compaction on topic {}",
                                                        clientAppId(),
                                                        topicNamePartition,
                                                        e);
                                                asyncResponse.resume(new RestException(e));
                ...
                .exceptionally(ex -> {
                    ...
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to validate global namespace ownership to trigger compaction on topic {}",
                                clientAppId(),
                                topicName,
                                ex);
                    }
        ...
        validateTopicOwnershipAsync(topicName, authoritative)
                ...
                .thenAccept(topic -> {
                    try {
                        ...
                        resumeAsyncResponseExceptionally(
                                asyncResponse, new RestException(Status.CONFLICT, e.getMessage()));
                        return;
                    } catch (Exception e) {
    ...
    public CompletableFuture<PartitionedTopicMetadata> getPartitionedTopicMetadata(
                                ...
                                .exceptionally(ex -> {
                                    ...
                                    if (throwable2 instanceof RestException) {
                                        ...
                                        authorizationFuture.completeExceptionally(
                                                new PulsarClientException(String.format(
                                                        "Authorization failed %s on topic %s with error %s",
        ...
        authorizationFuture
                .thenCompose(__ -> checkLocalOrGetPeerReplicationCluster(
                        pulsar, topicName.getNamespaceObject(), SystemTopicNames.isSystemTopic(topicName)))
                .thenCompose(res ->
                        pulsar.getBrokerService().fetchPartitionedTopicMetadataCheckAllowAutoCreationAsync(topicName))
                .thenAccept(metadata -> {
                    if (log.isDebugEnabled()) {
                        log.debug(
                                "[{}] Total number of partitions for topic {} is {}",
                                clientAppId,
                                topicName,
                                metadata.partitions);
                    }
    ...
    public static CompletableFuture<PartitionedTopicMetadata> unsafeGetPartitionedTopicMetadataAsync(
        ...
        checkLocalOrGetPeerReplicationCluster(pulsar, topicName.getNamespaceObject(), isSystemTopic(topicName))
                ...
                .thenAccept(metadata -> {
                    if (log.isDebugEnabled()) {
                        log.debug("Total number of partitions for topic {} is {}", topicName, metadata.partitions);
                    }
                    metadataFuture.complete(metadata);
                })
                .exceptionally(ex -> {
    ...
    private CompletableFuture<Topic> getTopicReferenceAsync(TopicName topicName) {
        return pulsar().getBrokerService()
                .getTopicIfExists(topicName.toString())
                .thenCompose(optTopic -> optTopic.map(CompletableFuture::completedFuture)
    ...
    private CompletableFuture<Topic> topicNotFoundReasonAsync(TopicName topicName) {
        if (!topicName.isPartitioned()) {
            return FutureUtil.failedFuture(
                    new RestException(Status.NOT_FOUND, getTopicNotFoundErrorMessage(topicName.toString())));
        }

        return getPartitionedTopicMetadataAsync(TopicName.get(topicName.getPartitionedTopicName()), false, false)
                .thenAccept(partitionedTopicMetadata -> {
                    if (partitionedTopicMetadata == null || partitionedTopicMetadata.partitions == 0) {
                        final String topicErrorType =
                                partitionedTopicMetadata == null ? "has no metadata" : "has zero partitions";
                        throw new RestException(
                                Status.NOT_FOUND,
                                String.format(
    ...
    private Subscription getSubscriptionReference(String subName, PersistentTopic topic) {
        try {
            Subscription sub = topic.getSubscription(subName);
            if (sub == null) {
                sub = topic.createSubscription(subName, InitialPosition.Earliest, false, null)
                        .get();
            }
    ...
    protected CompletableFuture<Void> internalValidateClientVersionAsync() {
        ...
        if (userAgent.contains(DEPRECATED_CLIENT_VERSION_PREFIX)) {
            try {
                ...
                String[] splits =
                        tokens.length > 1 ? tokens[1].split("-")[0].trim().split("\\.") : null;
    ...
    private CompletableFuture<Void> validateNonPartitionTopicNameAsync(String topicName) {
        ...
        if (topicName.contains(PARTITIONED_TOPIC_SUFFIX)) {
            try {
                ...
                long suffix = Long.parseLong(topicName.substring(partitionIndex + PARTITIONED_TOPIC_SUFFIX.length()));
                TopicName partitionTopicName =
                        TopicName.get(domain(), namespaceName, topicName.substring(0, partitionIndex));
                ret = getPartitionedTopicMetadataAsync(partitionTopicName, false, false)
                        .thenAccept(metadata -> {
                            ...
                            if (metadata.partitions > 0 && suffix >= (long) metadata.partitions) {
                                log.warn(
                                        "[{}] Can't create topic {} with \"-partition-\" followed by"
    ...
    protected CompletableFuture<Void> internalTrimTopic(AsyncResponse asyncResponse, boolean authoritative) {
        if (!topicName.isPersistent()) {
            ...
            asyncResponse.resume(
                    new RestException(Status.METHOD_NOT_ALLOWED, "Trim on a non-persistent topic is not allowed"));
            return null;
        }
        if (topicName.isPartitioned()) {
            return validateTopicOperationAsync(topicName, TopicOperation.TRIM_TOPIC)
                    .thenCompose((x) -> trimNonPartitionedTopic(asyncResponse, topicName, authoritative));
        }
        return validateTopicOperationAsync(topicName, TopicOperation.TRIM_TOPIC)
    ...
    private CompletableFuture<Void> trimNonPartitionedTopic(
            AsyncResponse asyncResponse, TopicName topicName, boolean authoritative) {
        return validateTopicOwnershipAsync(topicName, authoritative)
                .thenCompose(__ -> getTopicReferenceAsync(topicName))
                .thenCompose(topic -> {
                    if (!(topic instanceof PersistentTopic persistentTopic)) {
                        log.info("[{}] Trim on a non-persistent topic {} is not allowed", clientAppId(), topicName);
                        asyncResponse.resume(new RestException(
                                Status.METHOD_NOT_ALLOWED, "Trim on a non-persistent topic is not allowed"));
                        return CompletableFuture.completedFuture(null);
                    }
    ...
    private CompletableFuture<Void> trimPartitionedTopic(
            AsyncResponse asyncResponse, PartitionedTopicMetadata metadata) {
    ...
    protected CompletableFuture<DispatchRateImpl> internalGetDispatchRate(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(op -> op.map(TopicPolicies::getDispatchRate).orElseGet(() -> {
                    if (applied) {
                        DispatchRateImpl namespacePolicy = getNamespacePolicies(namespaceName)
                                .topicDispatchRate
                                .get(pulsar().getConfiguration().getClusterName());
    ...
    protected CompletableFuture<DispatchRate> internalGetSubscriptionDispatchRate(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(
                        op -> op.map(TopicPolicies::getSubscriptionDispatchRate).orElseGet(() -> {
                            if (applied) {
                                DispatchRateImpl namespacePolicy = getNamespacePolicies(namespaceName)
    ...
    protected CompletableFuture<DispatchRate> internalGetSubscriptionLevelDispatchRate(
            String subName, boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal).thenCompose(otp -> {
    ...
    protected CompletableFuture<Void> internalSetSubscriptionLevelDispatchRate(
            String subName, DispatchRateImpl dispatchRate, boolean isGlobal) {
    ...
    protected CompletableFuture<Long> internalGetCompactionThreshold(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(op -> op.map(TopicPolicies::getCompactionThreshold).orElseGet(() -> {
                    if (applied) {
    ...
    protected CompletableFuture<SubscribeRate> internalGetSubscribeRate(boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(op -> op.map(TopicPolicies::getSubscribeRate).orElseGet(() -> {
                    if (applied) {
    ...
    protected void handleTopicPolicyException(String methodName, Throwable thr, AsyncResponse asyncResponse) {
        ...
        if (!(cause instanceof WebApplicationException)
            ...
            log.error("[{}] Failed to perform {} on topic {}", clientAppId(), methodName, topicName, cause);
        }
        resumeAsyncResponseExceptionally(asyncResponse, cause);
    }
    ...
    protected void internalSetReplicatedSubscriptionStatus(
            AsyncResponse asyncResponse, String subName, boolean authoritative, boolean enabled) {
        log.info(
                "[{}] Attempting to change replicated subscription status to {} - {} {}",
                clientAppId(),
                enabled,
                topicName,
                subName);
        ...
        if (!topicName.isGlobal()) {
            asyncResponse.resume(new RestException(
                    Status.METHOD_NOT_ALLOWED, "Cannot enable/disable replicated subscriptions on non-global topics"));
            return;
        }
        ...
        CompletableFuture<Void> validateFuture = validateTopicOperationAsync(
                        topicName, TopicOperation.SET_REPLICATED_SUBSCRIPTION_STATUS, subName)
        ...
        if (topicName.isPartitioned()) {
            resultFuture =
                    validateFuture.thenAccept(__ -> internalSetReplicatedSubscriptionStatusForNonPartitionedTopic(
                            asyncResponse, subName, authoritative, enabled));
        } else {
                        ...
                        if (partitionMetadata.partitions > 0) {
                            ...
                            for (int i = 0; i < partitionMetadata.partitions; i++) {
                                ...
                                try {
                                    ...
                                    log.warn(
                                            "[{}] Failed to change replicated subscription status to {} - {} {}",
                                            clientAppId(),
                                            enabled,
                                            topicNamePartition,
                                            subName,
                            ...
                            FutureUtil.waitForAll(futures).handle((result, exception) -> {
                                if (exception != null) {
                                    ...
                                    if (t instanceof NotFoundException) {
                                        asyncResponse.resume(
                                                new RestException(Status.NOT_FOUND, "Topic or subscription not found"));
                                        return null;
                                    } else if (t instanceof PreconditionFailedException) {
                                        asyncResponse.resume(new RestException(
                                                Status.PRECONDITION_FAILED,
    ...
    private void internalSetReplicatedSubscriptionStatusForNonPartitionedTopic(
        ...
        validateTopicOwnershipAsync(topicName, authoritative)
                ...
                .thenAccept(topic -> {
                    ...
                    if (topic instanceof PersistentTopic && sub instanceof PersistentSubscription) {
                        if (!((PersistentSubscription) sub).setReplicated(enabled)) {
                            asyncResponse.resume(new RestException(
                                    Status.INTERNAL_SERVER_ERROR, "Failed to update cursor properties"));
                            return;
                        }

                        ((PersistentTopic) topic).checkReplicatedSubscriptionControllerState();
                        log.info(
                                "[{}] Changed replicated subscription status to {} - {} {}",
                                clientAppId(),
                                enabled,
                                topicName,
                                subName);
                ...
                .exceptionally(ex -> {
                    ...
                    if (!isRedirectException(ex)) {
                        log.error(
                                "[{}] Failed to set replicated subscription status on {} {}",
                                clientAppId(),
                                topicName,
                                subName,
                                ex);
    ...
    protected void internalGetReplicatedSubscriptionStatus(
            AsyncResponse asyncResponse, String subName, boolean authoritative) {
        ...
        if (!topicName.isGlobal()) {
            asyncResponse.resume(new RestException(
                    Status.METHOD_NOT_ALLOWED, "Cannot get replicated subscriptions on non-global topics"));
            return;
        }
        ...
        if (topicName.isPartitioned()) {
            resultFuture =
                    validateFuture.thenAccept(__ -> internalGetReplicatedSubscriptionStatusForNonPartitionedTopic(
                            asyncResponse, subName, authoritative));
        } else {
            resultFuture = validateFuture
                    .thenCompose(__ -> getPartitionedTopicMetadataAsync(topicName, authoritative, false))
                        ...
                        if (partitionMetadata.partitions > 0) {
                            ...
                            for (int i = 0; i < partitionMetadata.partitions; i++) {
                                ...
                                futures.add(pulsar().getNamespaceService()
                                        ...
                                        .thenCompose(owned -> {
                                            if (owned) {
                                                return getReplicatedSubscriptionStatusFromLocalBroker(
                                                        partition, subName);
                                            } else {
                                                try {
                                                    return pulsar().getAdminClient()
                                                            ...
                                                            .getReplicatedSubscriptionStatusAsync(
                                                                    partition.toString(), subName)
                                                            .whenComplete((__, throwable) -> {
                                                                if (throwable != null) {
                                                                    log.error(
                                                                            "[{}] Failed to get replicated subscriptions on"
                                                                                    + " {} {}",
                                                                            clientAppId(),
                                                                            partition,
                                                                            subName,
                                                    ...
                                                    log.warn(
                                                            "[{}] Failed to get replicated subscription status on {} {}",
                                                            clientAppId(),
                                                            partition,
                                                            subName,
                                                            e);
    ...
    private CompletableFuture<Map<String, Boolean>> getReplicatedSubscriptionStatusFromLocalBroker(
            TopicName localTopicName, String subName) {
        return getTopicReferenceAsync(localTopicName).thenCompose(topic -> {
            Subscription sub = topic.getSubscription(subName);
            if (sub == null) {
                return FutureUtil.failedFuture(new RestException(
                        Status.NOT_FOUND, getSubNotFoundErrorMessage(localTopicName.toString(), subName)));
            }
            if (topic instanceof PersistentTopic && sub instanceof PersistentSubscription) {
    ...
    protected CompletableFuture<SchemaCompatibilityStrategy> internalGetSchemaCompatibilityStrategy(boolean applied) {
        ...
        return validateTopicPolicyOperationAsync(
                        topicName, PolicyName.SCHEMA_COMPATIBILITY_STRATEGY, PolicyOperation.READ)
    ...
    protected CompletableFuture<Void> internalSetSchemaCompatibilityStrategy(SchemaCompatibilityStrategy strategy) {
        return validateTopicPolicyOperationAsync(
                        topicName, PolicyName.SCHEMA_COMPATIBILITY_STRATEGY, PolicyOperation.WRITE)
                .thenCompose((__) -> getTopicPoliciesAsyncWithRetry(topicName).thenCompose(op -> {
                    TopicPolicies topicPolicies = op.orElseGet(TopicPolicies::new);
    ...
    protected CompletableFuture<Boolean> internalGetSchemaValidationEnforced(boolean applied) {
        return getTopicPoliciesAsyncWithRetry(topicName).thenApply(op -> op.map(
        ...
        return validateTopicPolicyOperationAsync(topicName, PolicyName.ENTRY_FILTERS, PolicyOperation.READ)
                .thenCompose(__ -> {
                    ...
                    if (!pulsar().getConfiguration().isAllowOverrideEntryFilters()) {
                        return CompletableFuture.completedFuture(new EntryFilters(
        ...
        for (String shadowTopic : shadowTopics) {
            try {
                ...
                if (!shadowTopicName.isPersistent()) {
                    return FutureUtil.failedFuture(new RestException(
                            Status.PRECONDITION_FAILED, "Only persistent topic can be set as shadow topic"));
                }
                futures.add(pulsar().getNamespaceService()
                        .checkTopicExists(shadowTopicName)
                        .thenAccept(isExists -> {
                            if (!isExists) {
                                throw new RestException(
                                        Status.PRECONDITION_FAILED, "Shadow topic [" + shadowTopic + "] not exists.");
                            }
                        }));
            } catch (IllegalArgumentException e) {
                return FutureUtil.failedFuture(
                        new RestException(Status.FORBIDDEN, "Invalid shadow topic name: " + shadowTopic));
            }
        }
        return FutureUtil.waitForAll(futures);
    }
    ...
    protected CompletableFuture<Void> internalSetShadowTopic(List<String> shadowTopics) {
        if (!topicName.isPersistent()) {
            return FutureUtil.failedFuture(new RestException(
                    Status.PRECONDITION_FAILED, "Only persistent source topic is supported with shadow topics."));
        }
    ...
    protected CompletableFuture<AutoSubscriptionCreationOverride> internalGetAutoSubscriptionCreation(
            boolean applied, boolean isGlobal) {
        return getTopicPoliciesAsyncWithRetry(topicName, isGlobal)
                .thenApply(op -> op.map(TopicPolicies::getAutoSubscriptionCreationOverride)
                        .orElseGet(() -> {
                            if (applied) {
                                AutoSubscriptionCreationOverride namespacePolicy =
                                        getNamespacePolicies(namespaceName).autoSubscriptionCreationOverride;
                                return namespacePolicy == null
