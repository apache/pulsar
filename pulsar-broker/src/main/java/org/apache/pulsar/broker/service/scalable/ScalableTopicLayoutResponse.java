...
/**
 * Response sent to clients containing the full DAG state along with broker addresses
 * for each active segment and the controller broker address.
 *
 * <p>This is used for both the initial lookup response and subsequent pushed updates
 * via the DAG watch session.
 *
 * @param epoch                     the current layout epoch
 * @param segments                  full DAG of segments (active + sealed)
 * @param segmentBrokerAddresses    broker service URL for each active segment (segmentId to URL)
 * @param segmentBrokerAddressesTls optional TLS broker URL for each active segment
