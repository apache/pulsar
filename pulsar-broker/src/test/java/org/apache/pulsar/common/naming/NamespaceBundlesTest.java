...
public class NamespaceBundlesTest {
  ...
  public void testConstructor() throws Exception {
    ...
    NamespaceBundles bundles =
        new NamespaceBundles(
            NamespaceName.get("pulsar/use/ns2"), factory, Optional.empty(), partitions);
    ...
    assertEquals(
        bundleList.get(1),
        factory.getBundle(
            nsFld, Range.range(0x10000000L, BoundType.CLOSED, 0x40000000L, BoundType.OPEN)));
    assertEquals(
        bundleList.get(2),
        factory.getBundle(
            nsFld, Range.range(0x40000000L, BoundType.CLOSED, 0xffffffffL, BoundType.CLOSED)));
  }
  ...
  public void testFindBundle() throws Exception {
    ...
    NamespaceBundles bundles =
        new NamespaceBundles(
            NamespaceName.get("pulsar/global/ns1"), factory, Optional.empty(), partitions);
    ...
    try {
      bundles =
          new NamespaceBundles(topicName.getNamespaceObject(), factory, Optional.empty(), newPar);
      bundles.findBundle(topicName);
  ...
  public void testSplitBundles() throws Exception {
    ...
    Pair<NamespaceBundles, List<NamespaceBundle>> splitChildBundles =
        splitBundlesUtilFactory(
            utilityFactory, nsname, bundles2, testChildBundle, numberSplitBundles);
  ...
  public void testSplitBundleInTwo() throws Exception {
    ...
    Pair<NamespaceBundles, List<NamespaceBundle>> splitChildBundles =
        splitBundlesUtilFactory(
            ...
            NO_BUNDLES);
    assertBundles(
        utilityFactory, nsname, splitBundles.getRight().get(0), splitChildBundles, NO_BUNDLES);
    ...
    splitChildBundles =
        splitBundlesUtilFactory(
            ...
            NO_BUNDLES);
    assertBundles(
        utilityFactory, nsname, splitBundles.getRight().get(1), splitChildBundles, NO_BUNDLES);
  }
  ...
  public void testSplitBundleByFixBoundary() throws Exception {
    ...
    try {
      factory.splitBundles(
          bundleToSplit, 0, Collections.singletonList(bundleToSplit.getLowerEndpoint()));
    } catch (IllegalArgumentException e) {
      // No-op
    }
    try {
      factory.splitBundles(
          bundleToSplit, 0, Collections.singletonList(bundleToSplit.getUpperEndpoint()));
    } catch (IllegalArgumentException e) {
      // No-op
    }
    ...
    Pair<NamespaceBundles, List<NamespaceBundle>> splitBundles =
        factory.splitBundles(bundleToSplit, 0, Collections.singletonList(fixBoundary)).join();
    assertEquals(
        splitBundles.getRight().get(0).getLowerEndpoint(), bundleToSplit.getLowerEndpoint());
    assertEquals(
        splitBundles.getRight().get(1).getLowerEndpoint().longValue(),
        bundleToSplit.getLowerEndpoint() + fixBoundary);
  }

  private void validateSplitBundlesRange(
      NamespaceBundle fullBundle, List<NamespaceBundle> splitBundles) {
