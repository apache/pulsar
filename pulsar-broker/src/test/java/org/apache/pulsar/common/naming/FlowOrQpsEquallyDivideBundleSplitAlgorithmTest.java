...
public class FlowOrQpsEquallyDivideBundleSplitAlgorithmTest {
  ...
  public void testSplitBundleByFlowOrQps() {
    FlowOrQpsEquallyDivideBundleSplitAlgorithm algorithm =
        new FlowOrQpsEquallyDivideBundleSplitAlgorithm();
    ...
    mockTopics.forEach(
        (topic) -> {
          ...
          hashAndMsgMap.put(
              hashValue, topicStatsMap.get(topic).msgRateIn + topicStatsMap.get(topic).msgRateOut);
          hashAndThroughput.put(
              hashValue,
              topicStatsMap.get(topic).msgThroughputIn + topicStatsMap.get(topic).msgThroughputOut);
        });

    List<Long> splitPositions =
        algorithm
            .getSplitBoundary(
                new FlowOrQpsEquallyDivideBundleSplitOption(
