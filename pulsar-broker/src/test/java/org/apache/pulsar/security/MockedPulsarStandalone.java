...
public abstract class MockedPulsarStandalone implements AutoCloseable {
  ...
  @Getter private final ServiceConfiguration serviceConfiguration = new ServiceConfiguration();
  ...
  private static final String BROKER_INTERNAL_CLIENT_TOKEN =
      Jwts.builder().claim("sub", BROKER_INTERNAL_CLIENT_SUBJECT).signWith(SECRET_KEY).compact();
  protected static final String SUPER_USER_SUBJECT = "super-user";
  protected static final String SUPER_USER_TOKEN =
      Jwts.builder().claim("sub", SUPER_USER_SUBJECT).signWith(SECRET_KEY).compact();
  protected static final String NOBODY_SUBJECT = "nobody";
  protected static final String NOBODY_TOKEN =
      Jwts.builder().claim("sub", NOBODY_SUBJECT).signWith(SECRET_KEY).compact();
  ...
  protected void configureTokenAuthentication() {
    ...
    serviceConfiguration.setAuthenticationProviders(
        Set.of(AuthenticationProviderToken.class.getName()));
    // internal client
  ...
  protected void configureDefaultAuthorization() {
    ...
    serviceConfiguration.setSuperUserRoles(
        Set.of(SUPER_USER_SUBJECT, BROKER_INTERNAL_CLIENT_SUBJECT));
  }
  ...
  protected void loadECTlsCertificateWithKeyStore() {
    ...
    serviceConfiguration.setBrokerClientAuthenticationPlugin(
        AuthenticationKeyStoreTls.class.getName());
    final Map<String, String> brokerClientAuthParams = new HashMap<>();
  ...
  protected void enableTlsAuthentication() {
    serviceConfiguration.setAuthenticationEnabled(true);
    serviceConfiguration.setAuthenticationProviders(
        Sets.newHashSet(AuthenticationProviderTls.class.getName()));
  }

  @SneakyThrows
  protected void start() {
    this.pulsarTestContext =
  ...
  private void setupDefaultTenantAndNamespace() throws Exception {
    ...
    if (!serviceInternalAdmin.tenants().getTenants().contains(DEFAULT_TENANT)) {
      serviceInternalAdmin
          ...
          .createTenant(
              DEFAULT_TENANT,
              TenantInfo.builder().allowedClusters(Sets.newHashSet(TEST_CLUSTER_NAME)).build());
    }
    if (!serviceInternalAdmin
        .namespaces()
        .getNamespaces(DEFAULT_TENANT)
