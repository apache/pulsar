...
public class TlsWithECCertificateFileTest extends MockedPulsarStandalone {
  ...
  public void testConnectionFailWithoutCertificate() {
    ...
    final PulsarClient client =
        PulsarClient.builder().serviceUrl(getPulsarService().getBrokerServiceUrlTls()).build();
    @Cleanup
    final Producer<byte[]> producer = client.newProducer().topic("should_be_failed").create();
  }

  @Test
  @SneakyThrows
  public void testConnectionSuccessWithCertificate() {
    final AuthenticationTls authentication =
        new AuthenticationTls(TLS_EC_CLIENT_CERT_PATH, TLS_EC_CLIENT_KEY_PATH);
    ...
    final PulsarClient client =
        PulsarClient.builder()
            ...
            .tlsTrustCertsFilePath(TLS_EC_TRUSTED_CERT_PATH)
            .build();
    @Cleanup final Producer<byte[]> producer = client.newProducer().topic(topicName).create();
    @Cleanup
