...
public class ManagedLedgerInterceptorImplTest extends MockedBookKeeperTestCase {
  ...
  public void testAddBrokerEntryMetadata() throws Exception {
    ...
    assertEquals(
        19, ((ManagedLedgerInterceptorImpl) ledger.getManagedLedgerInterceptor()).getIndex());
  ...
  public void testMessagePayloadProcessor() throws Exception {
    ...
    ManagedLedgerInterceptor interceptor =
        new ManagedLedgerInterceptorImpl(new HashSet(), processors);
  ...
  public static long calculatePreciseSize(ManagedLedgerImpl ledger) {
    return ledger.getLedgersInfo().values().stream()
            .map(info -> info.getSize())
            .reduce((l1, l2) -> l1 + l2)
            .orElse(0L)
        + ledger.getCurrentLedgerSize();
  }
  ...
  public void testFindPositionByIndex() throws Exception {
    ...
    for (int i = 0; i < maxEntriesPerLedger; i++) {
      firstLedgerId =
          ledger
              .addEntry("dummy-entry".getBytes(StandardCharsets.UTF_8), MOCK_BATCH_SIZE)
              .getLedgerId();
    }

    assertEquals(
        ((ManagedLedgerInterceptorImpl) ledger.getManagedLedgerInterceptor()).getIndex(), 9);

    Position position = null;
    for (int index = 0;
        index <= ((ManagedLedgerInterceptorImpl) ledger.getManagedLedgerInterceptor()).getIndex();
        index++) {
    ...
    assertEquals(
        ((ManagedLedgerInterceptorImpl) ledger.getManagedLedgerInterceptor()).getIndex(), 19);
    ...
    assertEquals(
        ((ManagedLedgerInterceptorImpl) ledger.getManagedLedgerInterceptor()).getIndex(), 29);
  ...
  private class MockManagedLedgerInterceptorImpl extends ManagedLedgerInterceptorImpl {
    ...
    public void beforeAddEntry(AddEntryOperation op, int numberOfMessages) {
      ...
      op.setData(
          Commands.addBrokerEntryMetadata(
              op.getData(), brokerEntryMetadataInterceptors, numberOfMessages));
      if (op != null) {
  ...
  public void testManagedLedgerPayloadInputProcessorFailure() throws Exception {
    ...
    config.setManagedLedgerInterceptor(
        new ManagedLedgerInterceptorImpl(
            Collections.emptySet(),
            Set.of(
                new ManagedLedgerPayloadProcessor() {
                  @Override
                  public Processor inputProcessor() {
                    return new Processor() {
                      @Override
