...
public class BacklogQuotaManagerTest {
    ...
    try {
      ...
      pulsar =
          new PulsarService(
              ...
              Optional.empty(),
              exitCode -> {},
              BrokerOpenTelemetryTestUtil.getOpenTelemetrySdkBuilderConsumer(
      ...
      prometheusMetricsClient =
          new PrometheusMetricsClient("127.0.0.1", pulsar.getListenPortHTTP().get());

      admin
          .clusters()
          .createCluster("usc", ClusterData.builder().serviceUrl(adminUrl.toString()).build());
      admin
          .tenants()
          .createTenant(
  ...
  void createNamespaces() throws PulsarAdminException {
    ...
    admin
        .namespaces()
        .setNamespaceReplicationClusters("prop/quotaholdasync", Sets.newHashSet("usc"));
  }
  ...
  public void testBacklogQuotaWithReader() throws Exception {
    assertEquals(admin.namespaces().getBacklogQuotaMap("prop/ns-quota"), new HashMap<>());
    admin
        .namespaces()
        .setBacklogQuota(
            "prop/ns-quota",
    ...
    try (PulsarClient client =
        PulsarClient.builder().serviceUrl(adminUrl.toString()).statsInterval(0, SECONDS).build()) {
  ...
  private TopicStats getTopicStats(String topic1, boolean getPreciseBacklog)
    ...
    TopicStats stats =
        admin
            .topics()
            .getStats(
                topic1, GetStatsOptions.builder().getPreciseBacklog(getPreciseBacklog).build());
    return stats;
  }
      ...
      Awaitility.await()
          .untilAsserted(
              () -> {
                ...
                assertEquals(
                    internalStats.ledgers.get(0).ledgerId, finalMessageId.getLedgerId() + 1);
              });
  ...
  @Test
  public void backlogsStatsPrecise()
    ...
    try (PulsarClient client =
      ...
      log.info(
          "Subscription 1 moved 1 message. Now subscription 2 is the oldest. Oldest message:"
      ...
      Metric backlogAgeMetric =
          metrics.findSingleMetricByNameAndLabels(
              "pulsar_storage_backlog_age_seconds", Pair.of("topic", topic1));
      assertThat(backlogAgeMetric.tags)
          .containsExactly(
      ...
      long actualAge =
          (long)
              metrics
                  .findByNameAndLabels("pulsar_storage_backlog_age_seconds", "topic", topic1)
                  .get(0)
                  .value;
  ...
  @Test
  public void backlogsStatsPreciseWithNoBacklog()
      throws PulsarAdminException, PulsarClientException, InterruptedException {
    config.setPreciseTimeBasedBacklogQuotaCheck(true);
  ...
  @Test
  public void backlogsStatsNotPrecise()
      throws PulsarAdminException, PulsarClientException, InterruptedException {
    config.setPreciseTimeBasedBacklogQuotaCheck(false);
    ...
    try (PulsarClient client =
      ...
      // We have only one ledger, and it is not closed yet, so we can't tell the age until it is
      // closed
      assertThat(topicStats.getOldestBacklogMessageSubscriptionName()).isNull();
  ...
  @Test
  public void backlogsStatsNotPreciseWithNoBacklog()
      throws PulsarAdminException, PulsarClientException, InterruptedException {
    config.setPreciseTimeBasedBacklogQuotaCheck(false);
    config.setExposePreciseBacklogInPrometheus(false);
    config.setManagedLedgerMaxEntriesPerLedger(6);
  ...
  private void unloadAndLoadTopic(String topic, Producer producer)
   ...
   */
  private String waitForMarkDeletePositionToChange(
      String topic, String subscriptionName, String previousMarkDeletePosition) {
        ...
        .until(
            ...
            markDeletePosition ->
                markDeletePosition != null
                    && !markDeletePosition.equals(previousMarkDeletePosition));
  }
  ...
  /**
   * Time based backlog quota won't affect reader since broker doesn't keep track of consuming
   * position for reader and can't do message age check against the quota.
   *
   * @throws Exception
   */
  @Test
  public void testTriggerBacklogTimeQuotaWithReader() throws Exception {
    ...
    try (PulsarClient client =
      ...
      // and cause back pressure action to be token. Since broker don't keep track consuming
      // position for reader.
      assertEquals(
          nonDurableSubscriptionBacklog,
          numMsgs,
  ...
  public void testConsumerBacklogEvictionSizeQuota() throws Exception {
    ...
    TopicStats stats = getTopicStats(topic1);
    assertTrue(
        stats.getBacklogSize() < 10 * 1024, "Storage size is [" + stats.getStorageSize() + "]");
    ...
    assertMetricLongSumValue(
        metrics, OpenTelemetryTopicStats.BACKLOG_QUOTA_LIMIT_SIZE, attributes, backlogSizeLimit);
    assertMetricLongSumValue(
        metrics,
        OpenTelemetryTopicStats.BACKLOG_EVICTION_COUNTER,
        Attributes.builder()
            .putAll(attributes)
  ...
  public void testConsumerBacklogEvictionTimeQuota() throws Exception {
    ...
    PersistentTopic topic1Reference =
        (PersistentTopic) pulsar.getBrokerService().getTopicReference(topic1).get();
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              assertEquals(
                  stats2.getSubscriptions().get(subName1).getMsgBacklog(),
                  ml.getCurrentLedgerEntries());
              assertEquals(
                  stats2.getSubscriptions().get(subName2).getMsgBacklog(),
                  ml.getCurrentLedgerEntries());
            });
    ...
    assertMetricLongGaugeValue(
        metrics, OpenTelemetryTopicStats.BACKLOG_QUOTA_LIMIT_TIME, attributes, backlogTimeLimit);
    assertMetricLongSumValue(
        metrics,
        OpenTelemetryTopicStats.BACKLOG_EVICTION_COUNTER,
        Attributes.builder()
    ...
    assertMetricLongGaugeValue(
        metrics,
        OpenTelemetryTopicStats.BACKLOG_QUOTA_AGE,
        attributes,
  ...
  public void testConsumerBacklogEvictionTimeQuotaWithPartEviction() throws Exception {
    ...
    TopicStats stats2 = getTopicStats(topic1);
    // The first 5 messages should be expired due to limit time is 5 seconds, and the last 9 message
    // should not.
    Awaitility.await()
        .untilAsserted(
            () -> {
    ...
    admin
        ...
        .setBacklogQuota(
            ...
            BacklogQuota.builder()
                ...
                .retentionPolicy(BacklogQuota.RetentionPolicy.consumer_backlog_eviction)
                .build());
    @Cleanup PulsarClient client = PulsarClient.builder().serviceUrl(adminUrl.toString()).build();
  ...
  private Producer<byte[]> createProducer(PulsarClient client, String topic)
    ...
    return client.newProducer().enableBatching(false).sendTimeout(2, SECONDS).topic(topic).create();
  }
  ...
  public void testConsumerBacklogEvictionWithAckTimeQuota() throws Exception {
    ...
    for (int i = 0; i < numMsgs; i++) {
      // pause before acknowledging the 11. message so that 2 first ledgers (5 msgs/ledger) will
      // expire before the
      // last ledger
      if (i == 10) {
    ...
    Awaitility.await()
        ...
        .untilAsserted(
            () -> {
              ...
              long msgBacklog =
                  getTopicStats(topic1).getSubscriptions().get(subName2).getMsgBacklog();
  ...
  public void testConcurrentAckAndEviction() throws Exception {
    ...
    PulsarClient client2 =
        PulsarClient.builder().serviceUrl(adminUrl.toString()).statsInterval(0, SECONDS).build();
    Consumer<byte[]> consumer1 =
        client2.newConsumer().topic(topic1).subscriptionName(subName1).subscribe();
    Consumer<byte[]> consumer2 =
        client2.newConsumer().topic(topic1).subscriptionName(subName2).subscribe();
  ...
  public void testEvictionMulti() throws Exception {
    ...
    TopicStats stats = getTopicStats(topic1);
    assertTrue(
        stats.getBacklogSize() <= 15 * 1024, "Storage size is [" + stats.getStorageSize() + "]");
  }

  @Test
  public void testAheadProducerOnHold() throws Exception {
    assertEquals(admin.namespaces().getBacklogQuotaMap("prop/quotahold"), new HashMap<>());
    admin
        .namespaces()
        .setBacklogQuota(
            "prop/quotahold",
  ...
  public void testProducerException() throws Exception {
    ...
    try {
      ...
      assertTrue(
          ce instanceof PulsarClientException.ProducerBlockedQuotaExceededException
  ...
  public void testBacklogQuotaInGB(boolean backlogQuotaSizeGB) throws Exception {
    ...
    if (backlogQuotaSizeGB) {
      config.setBacklogQuotaDefaultLimitGB(
          ((double) backlogQuotaByte) / BacklogQuotaImpl.BYTES_IN_GIGABYTE);
    } else {
      config.setBacklogQuotaDefaultLimitBytes(backlogQuotaByte);
    }
    config.setBacklogQuotaDefaultRetentionPolicy(
