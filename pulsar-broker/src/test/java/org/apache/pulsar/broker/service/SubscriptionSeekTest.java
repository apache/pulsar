...
public class SubscriptionSeekTest extends BrokerTestBase {
  ...
  @Test
  public void testSeekForBatchByAdmin()
      throws PulsarClientException, ExecutionException, InterruptedException, PulsarAdminException {
    final String topicName =
        "persistent://prop/use/ns-abcd/testSeekForBatchByAdmin-" + UUID.randomUUID().toString();
    String subscriptionName = "my-subscription-batch";
    ...
    admin
        .topics()
        .resetCursor(topicName, subscriptionName, messageIds.get(messageIds.size() - 1), true);
    // Wait consumer reconnect
    Awaitility.await().until(consumer::isConnected);
    ...
    admin
        .topics()
        .resetCursor(topicName, subscriptionName, messageIds.get(messageIds.size() - 1), false);
    // Wait consumer reconnect
    Awaitility.await().until(consumer::isConnected);
    received = consumer.receive();
    ...
    admin
        .topics()
        .resetCursor(topicName, subscriptionName, new BatchMessageIdImpl(-1, -1, -1, 10), true);
    // Wait consumer reconnect
    Awaitility.await().until(consumer::isConnected);
    received = consumer.receive();
    assertEquals(received.getMessageId(), messageIds.get(0));
  ...
  public void testConcurrentResetCursor() throws Exception {
    final String topicName =
        "persistent://prop/use/ns-abc/testConcurrentReset_" + System.currentTimeMillis();
  ...
  public void testSeekOnPartitionedTopic() throws Exception {
    ...
    @Cleanup
    org.apache.pulsar.client.api.Consumer<byte[]> consumer =
        pulsarClient.newConsumer().topic(topicName).subscriptionName("my-subscription").subscribe();
  ...
  public void testSeekTime() throws Exception {
    ...
    long resetTimeInMillis =
        TimeUnit.SECONDS.toMillis(RelativeTimeUtil.parseRelativeTimeInSeconds(resetTimeStr));
  ...
  public void testSeekTimeByFunction() throws Exception {
    ...
    assertEquals(
        count, msgNumInPartition0 + msgNumInPartition1 + msgNumInPartition2 + msgNumInPartition3);
  }
    ...
    for (int i = 0; i < partitions; i++) {
      PersistentTopic topicRef =
          (PersistentTopic)
              pulsar
                  .getBrokerService()
                  .getTopicReference(topicName + TopicName.PARTITIONED_TOPIC_SUFFIX + i)
                  .get();
      assertNotNull(topicRef);
      assertEquals(topicRef.getProducers().size(), 1);
      assertEquals(topicRef.getSubscriptions().size(), 1);
  ...
  @Test
  public void testOnlyCloseActiveConsumerForSingleActiveConsumerDispatcherWhenSeek()
      throws Exception {
    final String topicName =
  ...
  public void testSeekByFunction() throws Exception {
    ...
    Reader<String> reader =
        pulsarClient
            .newReader(Schema.STRING)
            .startMessageId(MessageId.earliest)
            .topic(partitionedTopic.getPartition(0).toString())
            .create();
    ...
    MessageId lastMsgInPartition1 =
        admin.topics().getLastMessageId(partitionedTopic.getPartition(1).toString());
    reader.close();
    reader =
        pulsarClient
            .newReader(Schema.STRING)
            .startMessageId(MessageId.earliest)
            .topic(partitionedTopic.getPartition(2).toString())
            .create();
    ...
    assertEquals(
        received.size(), msgNumFromPartition1 + msgNumFromPartition2 + msgNumFromPartition3);
  ...
  public void testSeekByFunctionAndMultiTopic() throws Exception {
    ...
    MessageId msgIdInTopic1Partition0 =
        admin.topics().getLastMessageId(topic.getPartition(0).toString());
    MessageId msgIdInTopic1Partition2 =
        admin.topics().getLastMessageId(topic.getPartition(2).toString());
    ...
    org.apache.pulsar.client.api.Consumer<String> consumer =
        pulsarClient
            .newConsumer(Schema.STRING)
            .startMessageIdInclusive()
            .topics(Arrays.asList(topicName, topicName2))
            .subscriptionName("my-sub")
            .subscribe();
    consumer.seek(
        (partitionedTopic) -> {
    ...
    assertEquals(
        count, (msgInTopic1Partition0 + msgInTopic1Partition1 + msgInTopic1Partition2) * 2);
  }
  ...
  public void testSeekWillNotEncounteredFencedError() throws Exception {
    ...
    @Cleanup
    org.apache.pulsar.client.api.Consumer<String> consumer =
        client.newConsumer(Schema.STRING).topic(topicName).subscriptionName("s1").subscribe();
