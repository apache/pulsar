...
/**
 * Test for bk bouncy castle version compatible.
 */
@Slf4j
@Test(groups = "broker")
public class BkBouncyCastleVersionCompatibleTest {

    final TestStatsProvider provider = new TestStatsProvider();

    @Test(timeOut = 30000)
    @SneakyThrows
    public void testBouncyCastleVersionCompatible() {
        // Ensure pulsar is using bc-non-fips
        Assert.assertFalse(SecurityUtility.isBCFIPS());
        ...
        conf.setTLSKeyStore(getResourcePath("certificate/bookie.keystore.jks"));
        conf.setTLSKeyStorePasswordPath(getResourcePath("certificate/keyStoreAndTrustStorePassword.txt"));
        conf.setTLSTrustStoreType("JKS");
        conf.setTLSTrustStore(getResourcePath("certificate/bookie.truststore.jks"));
        conf.setTLSTrustStorePasswordPath(getResourcePath("certificate/keyStoreAndTrustStorePassword.txt"));

        File journalDir = IOUtils.createTempDir("bookie", "test");
        conf.setJournalDirName(journalDir.getPath());
        File[] ledgerDirs = new File[] { journalDir };
        ...
        BookieServer bookieServer = new BookieServer(conf, provider.getStatsLogger(""), null);
        try {
            bookieServer.start();
        } catch (Throwable e) {
