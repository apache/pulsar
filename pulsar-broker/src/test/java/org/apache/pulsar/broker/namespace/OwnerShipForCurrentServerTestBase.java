...
public abstract class OwnerShipForCurrentServerTestBase {
  ...
  @Setter private int brokerCount = 3;

  @Getter private final List<ServiceConfiguration> serviceConfigurationList = new ArrayList<>();
  @Getter private final List<PulsarService> pulsarServiceList = new ArrayList<>();
  ...
  protected void startBroker() throws Exception {
    for (int i = 0; i < brokerCount; i++) {
      ...
      PulsarTestContext.Builder testContextBuilder = PulsarTestContext.builder().config(conf);
      if (i > 0) {
