...
public class ResourceGroupServiceTest extends MockedPulsarServiceBaseTest {
    ...
    ResourceQuotaCalculator dummyQuotaCalc =
        new ResourceQuotaCalculator() {
          ...
          public long computeLocalQuota(long confUsage, long myUsage, long[] allUsages) {
            // Note: as a side effect of simply incrementing/returning
            // numAnonymousQuotaCalculations, we will also
            // set up the quota of bytes/messages accordingly in various rounds of calling into
            // dummyQuotaCalc();
  ...
  public void measureOpsTime() throws PulsarAdminException {
    ...
    for (int ix = 0; ix < numPerfTestIterations; ix++) {
      for (int monClassIdx = 0;
          monClassIdx < ResourceGroupMonitoringClass.values().length;
          monClassIdx++) {
  ...
  public void testResourceGroupOps() throws PulsarAdminException, InterruptedException {
    ...
    Assert.assertThrows(
        PulsarAdminException.class, () -> rgs.resourceGroupCreate(rgName, rgConfig));

    org.apache.pulsar.common.policies.data.ResourceGroup randomConfig =
        new org.apache.pulsar.common.policies.data.ResourceGroup();
    Assert.assertThrows(
        PulsarAdminException.class, () -> rgs.resourceGroupUpdate(randomRgName, randomConfig));
    ...
    Assert.assertEquals(
        monClassFields.configValuesPerPeriod.bytes, rgConfig.getPublishRateInBytes().longValue());
    Assert.assertEquals(
        monClassFields.configValuesPerPeriod.messages, rgConfig.getPublishRateInMsgs().intValue());
    monClassFields = retRG.monitoringClassFields[ResourceGroupMonitoringClass.Dispatch.ordinal()];
    Assert.assertEquals(
        monClassFields.configValuesPerPeriod.bytes, rgConfig.getDispatchRateInBytes().longValue());
    Assert.assertEquals(
        monClassFields.configValuesPerPeriod.messages, rgConfig.getDispatchRateInMsgs().intValue());
    ...
    for (int monClassIdx = 0;
      ...
      // maxUsageReportSuppressRounds iterations. So, if we run for maxUsageReportSuppressRounds
      // iterations,
    ...
    if (numQuotaCalcsDuringTest == 0) {
      // Quota calculations were not done yet during this test; we expect to see the default
      // "initial" setting.
    ...
    // Calculate the quota synchronously to avoid waiting for a periodic call within
    // ResourceGroupService.
    rgs.calculateQuotaForAllResourceGroups();
    publishQuota = rgs.getPublishRateLimiters(rgName);
    // The bytes/messages are (synthetically) set from numAnonymousQuotaCalculations in the above
    // round of
    // calls, or some later round (since the periodic call to calculateQuotaForAllResourceGroups()
    // would be
    // ongoing). So, we expect bytes/messages setting to be more than 0 and at most
    // numAnonymousQuotaCalculations.
    Assert.assertTrue(
        publishQuota.messages > 0 && publishQuota.messages <= numAnonymousQuotaCalculations);
    Assert.assertTrue(
        publishQuota.bytes > 0 && publishQuota.bytes <= numAnonymousQuotaCalculations);
  ...
  public void testClose() throws Exception {
    ResourceGroupService service =
        new ResourceGroupService(pulsar, TimeUnit.MILLISECONDS, null, null);
    service.close();
