...
public final class TopicPoliciesAuthZTest extends MockedPulsarStandalone {
  ...
  private static final String TENANT_ADMIN_TOKEN =
      Jwts.builder().claim("sub", TENANT_ADMIN_SUBJECT).signWith(SECRET_KEY).compact();
  ...
  public void testRetention() {
    ...
    for (AuthAction action : AuthAction.values()) {
      superUserAdmin
          .namespaces()
          .grantPermissionOnNamespace("public/default", subject, Set.of(action));
      try {
        subAdmin.topicPolicies().getRetention(topic);
        Assert.fail("unexpected behaviour");
      } catch (PulsarAdminException ex) {
  ...
  public void testMaxUnackedMessagesOnConsumer() {
    ...
    superUserAdmin
        .topicPolicies()
        .setMaxUnackedMessagesOnConsumer(topic, definedUnackedMessagesOnConsumer);
  ...
  public void testMaxUnackedMessagesOnSubscription() {
    ...
    superUserAdmin
        .topicPolicies()
        .setMaxUnackedMessagesOnSubscription(topic, definedUnackedMessagesOnConsumer);
    ...
    await()
        .untilAsserted(
            () -> {
              final int unackedMessagesOnConsumer =
                  superUserAdmin.topicPolicies().getMaxUnackedMessagesOnSubscription(topic);
              Assert.assertEquals(unackedMessagesOnConsumer, definedUnackedMessagesOnConsumer);
            });
