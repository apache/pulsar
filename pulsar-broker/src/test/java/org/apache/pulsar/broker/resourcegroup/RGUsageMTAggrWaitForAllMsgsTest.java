...
public class RGUsageMTAggrWaitForAllMsgsTest extends ProducerConsumerBase {
  ...
  protected void setup() throws Exception {
    ...
    ResourceQuotaCalculator dummyQuotaCalc =
    ...
    ResourceUsageTopicTransportManager transportMgr =
        new ResourceUsageTopicTransportManager(pulsar);
    this.rgservice =
        new ResourceGroupService(pulsar, TimeUnit.SECONDS, transportMgr, dummyQuotaCalc);
  ...
  private class ProduceMessages implements Runnable {
    ...
    public void run() {
      ...
      try {
        // The producer will send messages to a specific topic, since it doesn't make sense for a
        // producer
        // to produce a message with vagueness about the destination topic (neither do Pulsar APIs
        // allow it).
        producer = pulsarClient.newProducer().topic(myProduceTopic).create();
      } catch (PulsarClientException p) {
        numExceptions++;
        log.info(
            "Producer={} got exception while building producer: ex={}", producerId, p.getMessage());
      }
      ...
      for (int ix = 0; ix < numMesgsToProduce; ix++) {
        ...
        try {
          mesg =
              String.format("ProducerId=%d, ix=%d, topic=%s", producerId, ix, myProduceTopic)
                  .getBytes();
          ...
          numExceptions++;
          log.error(
              "Producer={} got exception while sending {}-th time: ex={}",
              producerId,
              ix,
              e.getMessage());
        }
      ...
      try {
        ...
        numExceptions++;
        log.error(
            "Producer={} got exception while closing producer: ex={}", producerId, e.getMessage());
      }

      log.info(
          "Producer={} done with topic={}; got {} exceptions",
          producerId,
          myProduceTopic,
          numExceptions);
    }
  ...
  private class ConsumeMessages implements Runnable {
    ...
    ConsumeMessages(
        int consId, int nMesgs, int totalMesgs, SubscriptionType subType, String[] topics) {
      consumerId = consId;
    ...
    public void closeConsumer() {
      try {
        ...
        numExceptions++;
        log.error(
            "Consumer={} got exception while closing consumer: ex={}", consumerId, p.getMessage());
      }
    }

    @Override
    public void run() {
      // Create a consumer and subscription, and space for messages, so that they are held for
      // consumption.
      int recvQueueSize = 0;
      ...
      switch (subscriptionType) {
        default:
          ...
          final String errMesg =
              String.format(
                  "Consumer=%d got unexpected subscription type=%s", consumerId, subscriptionType);
          Assert.fail(errMesg);
          break;
        case Shared:
      ...
      try {
        // The consumer will try to get a message from any of the topics, since Pulsar allows a
        // consumer to
        ...
        numExceptions++;
        log.error(
            "Consumer={} got exception while building consumer: ex={}", consumerId, p.getMessage());
      }
      ...
      while (consumerIsReady && !allMessagesReceived) {
        ...
        try {
          ...
          if (message != null) {
            ...
            String mesg =
                String.format(
                    "Consumer=%d recvd %d-th mesg; id=%s, data=%s",
                    consumerId,
                    recvdNumMsgs,
          ...
          numExceptions++;
          log.error(
              "Consumer={} got exception in while receiving {}-th mesg at consumer: ex={}",
              consumerId,
              recvdNumMsgs,
              p.getMessage());
        }
  ...
  private String TopicToTenantRGName(TopicName topicName) {
    // Under the current topic naming scheme, the tenant-rg name is just the tenant part of the
    // topic.
  ...
  private String TopicToNamespaceRGName(TopicName topicName) {
    // Under the current topic naming  scheme, the namespace-rg name is just the namespace part of
    // the topic.
  ...
  // If some are equal, and others unequal, throw, because this is unexpected in this UT at the
  // moment.
  private boolean tenantRGEqualsNamespaceRG(String[] topicStrings) throws PulsarClientException {
    ...
    if ((numEqualRGs + numUnEqualRGs != numTopics) || (numEqualRGs > 0 && numUnEqualRGs > 0)) {
      String errMesg =
          String.format(
              "Found %s topics with equal RGs and %s with unequal, on %s topics",
  ...
  private void registerTenantsAndNamespaces(String[] topicStrings) throws Exception {
    for (String topicStr : topicStrings) {
      ...
      // The tenant name and namespace name parts of the topic are the same as their corresponding
      // RG-names.
  ...
  // Produce/consume messages on the given topics, and verify that the resource-group stats are
  // updated.
  private void testProduceConsumeUsageOnRG(String[] topicStrings) throws Exception {
    ...
    final SubscriptionType consumeSubscriptionType =
        SubscriptionType.Shared; // Shared, or Exclusive
    ...
    for (int ix = 0; ix < NUM_CONSUMERS; ix++) {
      ...
      ConsumeMessages cm =
          new ConsumeMessages(
              ix,
              NUM_MESSAGES_PER_CONSUMER,
              TotalExpectedMessagesToReceive,
              consumeSubscriptionType,
              topicStrings);
    ...
    do {
      ...
      for (int ix = 0; ix < NUM_CONSUMERS; ix++) {
        ...
        log.debug(
            "consumer={} received {} messages (current total {}, expected {})",
            ix,
            consNumMesgsRecvd,
            recvdNumMsgs,
            TotalExpectedMessagesToReceive);
    ...
    // tenant RG, and again on the namespace RG. This double-counting is avoided if tenant-RG ==
    // NS-RG.
    ...
    this.verifyRGMetrics(
        sentNumBytes, sentNumMsgs, recvdNumBytes, recvdNumMsgs, scaleFactor, true, true);
  ...
  // Verify the app stats with what we see from the broker-service, and the resource-group (which in
  // turn internally
  // derives stats from the broker service)
  private void verifyRGProdConsStats(
      String[] topicStrings,
      int sentNumBytes,
      int sentNumMsgs,
      int recvdNumBytes,
    ...
    // Since the following walk is on topics, keep track of the RGs for which we have already
    // gathered stats,
    ...
    for (Map.Entry<String, TopicStatsImpl> entry : topicStatsMap.entrySet()) {
      ...
      if (Arrays.asList(topicStrings).contains(mapTopicName)) {
        ...
        if (totalInMessages == 0) {
          log.warn(
              "verifyProdConsStats: found no produced mesgs (msgInCounter) on topic {}",
              mapTopicName);
        }
        ...
        if (sentNumMsgs > 0 || recvdNumMsgs > 0) {
          ...
          if (!RGsWithPublishStatsGathered.contains(tenantRGName)) {
            prodCounts =
                this.rgservice.getRGUsage(
                    tenantRGName, ResourceGroupMonitoringClass.Publish, getCumulativeUsageStats);
            totalTenantRGProdCounts =
                ResourceGroup.accumulateBMCount(totalTenantRGProdCounts, prodCounts);
            RGsWithPublishStatsGathered.add(tenantRGName);
          }
          if (!RGsWithDispatchStatsGathered.contains(tenantRGName)) {
            consCounts =
                this.rgservice.getRGUsage(
                    tenantRGName, ResourceGroupMonitoringClass.Dispatch, getCumulativeUsageStats);
            totalTenantRGConsCounts =
                ResourceGroup.accumulateBMCount(totalTenantRGConsCounts, consCounts);
          ...
          if (tenantRGName.compareTo(nsRGName) != 0) {
            if (!RGsWithPublishStatsGathered.contains(nsRGName)) {
              prodCounts =
                  this.rgservice.getRGUsage(
                      nsRGName, ResourceGroupMonitoringClass.Publish, getCumulativeUsageStats);
              totalNsRGProdCounts =
                  ResourceGroup.accumulateBMCount(totalNsRGProdCounts, prodCounts);
              RGsWithPublishStatsGathered.add(nsRGName);
            }
            if (!RGsWithDispatchStatsGathered.contains(nsRGName)) {
              consCounts =
                  this.rgservice.getRGUsage(
                      nsRGName, ResourceGroupMonitoringClass.Dispatch, getCumulativeUsageStats);
              totalNsRGConsCounts =
                  ResourceGroup.accumulateBMCount(totalNsRGConsCounts, consCounts);
  ...
  private void verifyRGMetrics(
    ...
    log.info(
        ...
        totalTenantUnRegisters,
        totalNamespaceRegisters,
        totalNamespaceUnRegisters);
    ...
    // prior to unregister). In other words, their numbers will equal the residuals for the
    // registers.
    ...
    for (ResourceGroupMonitoringClass mc : ResourceGroupMonitoringClass.values()) {
      ...
      log.info(
          "mc={}: totalQuotaBytes={}, totalQuotaMessages={}, "
              + " totalUsedBytes={}, totalUsedMessages={}"
              + " totalUsageReports={}",
          mc.name(),
          totalQuotaBytes[mcIdx],
          totalQuotaMessages[mcIdx],
          totalUsedBytes[mcIdx],
          totalUsedMessages[mcIdx],
          totalUsageReportCounts[mcIdx]);
      ...
      if (checkProduce && mc == ResourceGroupMonitoringClass.Publish) {
        Assert.assertEquals(
            totalUsedMessages[mcIdx] - residualSentNumMessages, sentNumMsgs * scaleFactor);
        Assert.assertTrue(totalUsedBytes[mcIdx] - residualSentNumBytes >= ExpectedNumBytesSent);
      } else if (checkConsume && mc == ResourceGroupMonitoringClass.Dispatch) {
        Assert.assertEquals(
            totalUsedMessages[mcIdx] - residualRecvdNumMessages, recvdNumMsgs * scaleFactor);
        Assert.assertTrue(
            totalUsedBytes[mcIdx] - residualRecvdNumBytes >= ExpectedNumBytesReceived);
      }

      long perClassUsageReports =
          numLocalUsageReports / ResourceGroupMonitoringClass.values().length;
  ...
  private static final int NUM_TOPICS =
      8; // Set == NumProducers, so each producer can send on its own topic
  private static final int NUM_RESOURCE_GROUPS =
      4; // arbitrarily, half of NumTopics, so 2 topics map to each RG
  ...
  // Combinations of tenant and namespace required to test RG use cases when both NS and tenant are
  // under the control
  // of the same RG, vs. cases where they are under the control of distinct RGs.
  // [This is required to test the special case of "tenant and NS refer to the same RG", because in
  // that case
  ...
  private final ResourceGroupUsageStatsType getCumulativeUsageStats =
  ...
  // Keep track of the tenants that have been registered to their RGs, so we don't dup and get
  // exceptions
  HashSet<String> registeredTenants = new HashSet<>();

  // Keep track of the namespaces that have been registered to their RGs, so we don't dup and get
  // exceptions
  ...
  private void prepareForOps() throws PulsarAdminException {
    ...
    admin
        .clusters()
        .createCluster(clusterName, ClusterData.builder().serviceUrl(brokerUrl.toString()).build());
  }
  ...
  private void prepareRGs() throws Exception {
    ...
    for (int ix = 0; ix < NUM_RESOURCE_GROUPS; ix++) {
      ...
      TenantAndNsNameOppositeOrder[ix] =
          RGNames[ix] + "/" + RGNames[NUM_RESOURCE_GROUPS - (ix + 1)];
    }
