...
public class BrokerBookieIsolationTest {
  ...
  public void testBookieIsolation() throws Exception {
    ...
    Set<BookieId> defaultBookies =
        Sets.newHashSet(bookies[0].getBookieId(), bookies[1].getBookieId());
    Set<BookieId> isolatedBookies =
        Sets.newHashSet(bookies[2].getBookieId(), bookies[3].getBookieId());
    ...
    @Cleanup
    PulsarAdmin admin =
        PulsarAdmin.builder().serviceHttpUrl(pulsarService.getWebServiceAddress()).build();
    ...
    assertEquals(
        ...
        BookieAffinityGroupData.builder()
            ...
            .build());
    assertEquals(
        admin.namespaces().getBookieAffinityGroup(ns3),
        BookieAffinityGroupData.builder()
            .bookkeeperAffinityGroupPrimary(tenantNamespaceIsolationGroups)
            .build());
    assertEquals(
        admin.namespaces().getBookieAffinityGroup(ns4),
        BookieAffinityGroupData.builder()
            .bookkeeperAffinityGroupPrimary(tenantNamespaceIsolationGroups)
            .build());
    ...
    assertEquals(
        admin.namespaces().getBundles(ns2).getNumBundles(),
    ...
    PersistentTopic topic1 =
        (PersistentTopic) createTopicAndPublish(pulsarClient, ns1, "topic1", totalPublish);
    PersistentTopic topic2 =
        (PersistentTopic) createTopicAndPublish(pulsarClient, ns2, "topic1", totalPublish);
    PersistentTopic topic3 =
        (PersistentTopic) createTopicAndPublish(pulsarClient, ns3, "topic1", totalPublish);
    PersistentTopic topic4 =
        (PersistentTopic) createTopicAndPublish(pulsarClient, ns4, "topic1", totalPublish);
    ...
    // totalLedgers = totalPublish / totalEntriesPerLedger. (totalPublish = 100,
    // totalEntriesPerLedger = 20.)
    ...
    Map<EnsemblePlacementPolicyConfig, BookKeeper> bkPlacementPolicyToBkClientMap =
        mlFactory.getBkEnsemblePolicyToBookKeeperMap();

    // broker should create only 1 bk-client and factory per isolation-group
    assertEquals(bkPlacementPolicyToBkClientMap.size(), 1);

    // make sure bk-isolation group also configure REPP_DNS_RESOLVER_CLASS as
    // ZkBookieRackAffinityMapping to
    // configure rack-aware policy with in isolated group
    Map<EnsemblePlacementPolicyConfig, BookKeeper> bkMap =
        mlFactory.getBkEnsemblePolicyToBookKeeperMap();
    ...
    assertEquals(
        clientConf.getProperty(REPP_DNS_RESOLVER_CLASS), BookieRackAffinityMapping.class.getName());
  }
  ...
  private LedgerManager getLedgerManager(BookieImpl bookie1) throws IllegalAccessException {
    ...
    SingleDirectoryDbLedgerStorage singleDirectoryDbLedgerStorage =
        ((List<SingleDirectoryDbLedgerStorage>)
                FieldUtils.readDeclaredField(ledgerStorage, "ledgerStorageList", true))
            .get(0);
    GarbageCollectorThread gcThread =
        (GarbageCollectorThread)
            FieldUtils.readDeclaredField(singleDirectoryDbLedgerStorage, "gcThread", true);
    ScanAndCompareGarbageCollector garbageCollector =
        (ScanAndCompareGarbageCollector)
            FieldUtils.readDeclaredField(gcThread, "garbageCollector", true);
  ...
  public void testSetRackInfoAndAffinityGroupDuringProduce() throws Exception {
    ...
    PersistentTopic topic2 =
        (PersistentTopic) pulsarService.getBrokerService().getTopicReference(topicName).get();
    ManagedLedgerImpl ml = (ManagedLedgerImpl) topic2.getManagedLedger();
    Awaitility.await()
        .untilAsserted(
            () ->
                Assert.assertTrue(
                    ml.getConfig().getBookKeeperEnsemblePlacementPolicyProperties().size() > 0));
    ...
    // validate ledgers' ensemble with affinity bookies
    // The second ledger will be created after the first ledger is full and the isolationGroup has
    // not been set.
  ...
  /**
   * It verifies that "ZkIsolatedBookieEnsemblePlacementPolicy" considers secondary affinity-group
   * if primary group doesn't have enough non-faulty bookies.
   *
   * @throws Exception
   */
  @Test
  public void testBookieIsolationWithSecondaryGroup() throws Exception {
    ...
    Set<BookieId> downedBookies =
        Sets.newHashSet(BookieId.parse("1.1.1.1:1111"), BookieId.parse("1.1.1.1:1112"));
    ...
    try {
      PersistentTopic topic4 =
          (PersistentTopic) createTopicAndPublish(pulsarClient, ns4, "topic1", 1);
  ...
  private void assertAffinityBookies(
      LedgerManager ledgerManager, List<LedgerInfo> ledgers1, Set<BookieId> defaultBookies)
      throws Exception {
  ...
  private Topic createTopicAndPublish(
      PulsarClient pulsarClient, String ns, String topicLocalName, int totalPublish)
      throws Exception {
  ...
  private void setDefaultIsolationGroup(
      String brokerBookkeeperClientIsolationGroups,
      ZooKeeper zkClient,
      Set<BookieId> bookieAddresses)
      throws Exception {
    ...
    zkClient.setData(
        BookieRackAffinityMapping.BOOKIE_INFO_ROOT_PATH, jsonMapper.writeValueAsBytes(bookies), -1);
  }
