...
//        ).when(mockZooKeeperChildrenCache).getAsync(anyString());
    ...
    @Test
    public void testGetMessageByIdHeaderTooLarge431() throws Exception {
        TenantInfoImpl tenantInfo = new TenantInfoImpl(Set.of("role1", "role2"), Set.of("test"));
        admin.tenants().createTenant("tenant-xyz", tenantInfo);
        admin.namespaces().createNamespace("tenant-xyz/ns-abc", Set.of("test"));
        final String topicName = "persistent://tenant-xyz/ns-abc/testGetMessageByIdHeaderTooLarge";
        admin.topics().createNonPartitionedTopic(topicName);

        // Create a message with many properties to produce a large response header
        Map<String, String> largeProps = new HashMap<>();
        for (int i = 0; i < 50; i++) {
            largeProps.put("property-key-" + i, "property-value-" + i);
        }
        @Cleanup
        ...
        MessageIdImpl messageId = (MessageIdImpl) producer.newMessage()
                .value("test-data".getBytes()).properties(largeProps).send();

        // Set a very small limit so the header exceeds it
        int originalLimit = pulsar.getConfiguration().getHttpMaxResponseHeaderSize();
        pulsar.getConfiguration().setHttpMaxResponseHeaderSize(1024);
        try {
            PulsarAdminException ex = Assert.expectThrows(PulsarAdminException.class, () ->
                    admin.topics().getMessageById(topicName, messageId.getLedgerId(), messageId.getEntryId()));
            Assert.assertEquals(ex.getStatusCode(), 431);
            String httpError = ex.getHttpError();
            Assert.assertTrue(httpError.contains("exceeds HTTP response header limit"),
                    "Expected error message to contain 'exceeds HTTP response header limit', but got: " + httpError);
            Assert.assertTrue(httpError.contains("httpMaxResponseHeaderSize"),
                    "Expected error message to contain 'httpMaxResponseHeaderSize', but got: " + httpError);
        } finally {
            pulsar.getConfiguration().setHttpMaxResponseHeaderSize(originalLimit);
        }
    }

    @Test
    public void testGetMessageByIdHeaderWithinLimit() throws Exception {
        TenantInfoImpl tenantInfo = new TenantInfoImpl(Set.of("role1", "role2"), Set.of("test"));
        admin.tenants().createTenant("tenant-xyz", tenantInfo);
        admin.namespaces().createNamespace("tenant-xyz/ns-abc", Set.of("test"));
        final String topicName = "persistent://tenant-xyz/ns-abc/testGetMessageByIdHeaderWithinLimit";
        admin.topics().createNonPartitionedTopic(topicName);

        // Create a message with many properties
        Map<String, String> largeProps = new HashMap<>();
        for (int i = 0; i < 50; i++) {
            largeProps.put("property-key-" + i, "property-value-" + i);
        }
        ...
        // Default limit (8192) should be sufficient for 50 properties
        Message<byte[]> message = admin.topics().getMessageById(topicName,
                messageId.getLedgerId(), messageId.getEntryId());
        Assert.assertEquals(message.getData(), "test-data".getBytes());
        Map<String, String> outProps = message.getProperties();
        for (Map.Entry<String, String> entry : largeProps.entrySet()) {
            Assert.assertEquals(outProps.get(entry.getKey()), entry.getValue());
        }
    }
