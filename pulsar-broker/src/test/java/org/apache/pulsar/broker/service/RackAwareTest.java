...
public class RackAwareTest extends BkEnsemblesTestBase {
  ...
  public void testPlacement() throws Exception {
    ...
    for (int i = 0; i < NUM_BOOKIES; i++) {
      ...
      int rackId = i == 0 ? 1 : 2;
      BookieInfo bi =
          BookieInfo.builder().rack("rack-" + rackId).hostname("bookie-" + (i + 1)).build();
  ...
  public void testPlacementMinRackNumsPerWriteQuorum(boolean forceMinRackNums) throws Exception {
    ...
    for (int i = 0; i < NUM_BOOKIES; i++) {
      ...
      int rackId = i == 0 ? 1 : 2;
      BookieInfo bi = BookieInfo.builder().rack("rack-" + 1).hostname("bookie-" + (i + 1)).build();
      log.info("setting rack for bookie at {} -- {}", bookie, bi);
      admin.bookies().updateBookieRackInfo(bookie, group, bi);
    }
  ...
  public void testRackUpdate() throws Exception {
    ...
    for (int i = 0; i < NUM_BOOKIES / 2; i++) {
      String bookie = servers.get(i).getServer().getLocalAddress().toString();
      BookieInfo bi = BookieInfo.builder().rack("rack-0").hostname("bookie-" + (i + 1)).build();
      log.info("setting rack for bookie at {} -- {}", bookie, bi);
      admin.bookies().updateBookieRackInfo(bookie, group, bi);
    }
    ...
    RackawareEnsemblePlacementPolicy ensemblePlacementPolicy =
        (RackawareEnsemblePlacementPolicy) field.get(bkc);
    Field topoField =
    ...
    NetworkTopologyImpl networkTopology =
        (NetworkTopologyImpl) topoField.get(ensemblePlacementPolicy);
