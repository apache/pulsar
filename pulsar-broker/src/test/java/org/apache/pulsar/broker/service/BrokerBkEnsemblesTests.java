...
public class BrokerBkEnsemblesTests extends BkEnsemblesTestBase {
  ...
  /**
   * It verifies that broker deletes cursor-ledger when broker-crashes without closing topic
   * gracefully
   *
   * <pre>
  ...
  public void testCrashBrokerWithoutCursorLedgerLeak() throws Exception {
    ...
    PersistentTopic topic =
        (PersistentTopic) pulsar.getBrokerService().getOrCreateTopic(topic1).get();
    ...
    ConcurrentHashMap<String, CompletableFuture<ManagedLedgerImpl>> ledgers =
        (ConcurrentHashMap<String, CompletableFuture<ManagedLedgerImpl>>) field.get(factory);
    ledgers.clear();
    ...
    final ManagedCursorImpl cursor1 =
        (ManagedCursorImpl) topic.getManagedLedger().getCursors().iterator().next();
  ...
  public void testTruncateCorruptDataLedger() throws Exception {
    ...
    try {
      admin
          ...
          .createTenant(
              tenant,
              new TenantInfoImpl(
                  Sets.newHashSet("role1", "role2"), Sets.newHashSet(config.getClusterName())));
    } catch (Exception e) {
    ...
    Awaitility.await()
        ...
        .untilAsserted(
            () -> {
              LedgerMetadata meta =
                  bookKeeper.getLedgerMetadata(firstLedgerToDelete).exceptionally(e -> null).get();
  ...
  public void testDeleteTopicWithMissingData() throws Exception {
    ...
    assertThrows(
        PulsarAdminException.ServerSideErrorException.class, () -> admin.topics().delete(topic));
  }
