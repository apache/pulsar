...
/**
 * Configuration for what kind of Mockito spy to use on collaborator objects of the PulsarService
 * managed by {@link PulsarTestContext}.
 *
 * <p>This configuration is applied using {@link PulsarTestContext.Builder#spyConfig(SpyConfig)} or
 * by calling {@link PulsarTestContext.Builder#spyByDefault()} to enable spying for all configurable
 * collaborator objects.
 *
 * <p>For verifying interactions with Mockito's {@link Mockito#verify(Object)} method, the spy type
 * must be set to {@link SpyType#SPY_ALSO_INVOCATIONS}. This is because the default spy type {@link
 * SpyType#SPY} does not record invocations.
 */
...
public class SpyConfig {
  ...
  /**
   * Spy configuration for {@link PulsarService#getBookKeeperClient()}. In the test context, this is
   * the same as {@link PulsarTestContext#getBookKeeperClient()} . It is a client that wraps an
   * in-memory mock bookkeeper implementation.
   */
  ...
  /**
   * Create a builder for SpyConfig with the given spy type for all configurable collaborator
   * objects.
   *
