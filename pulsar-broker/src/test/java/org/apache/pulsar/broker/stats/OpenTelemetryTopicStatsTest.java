...
public class OpenTelemetryTopicStatsTest extends BrokerTestBase {
  ...
  public void testMessagingMetrics() throws Exception {
    ...
    assertMetricLongSumValue(
        metrics, OpenTelemetryTopicStats.PRODUCER_COUNTER, attributes, producerCount);
    assertMetricLongSumValue(
        metrics, OpenTelemetryTopicStats.CONSUMER_COUNTER, attributes, consumerCount);
    ...
    assertMetricLongSumValue(
        metrics,
        OpenTelemetryTopicStats.STORAGE_COUNTER,
        attributes,
        actual -> assertThat(actual).isPositive());
    assertMetricLongSumValue(
        metrics,
        OpenTelemetryTopicStats.STORAGE_LOGICAL_COUNTER,
        attributes,
        actual -> assertThat(actual).isPositive());
    assertMetricLongSumValue(
        metrics,
        OpenTelemetryTopicStats.STORAGE_BACKLOG_COUNTER,
        attributes,
        actual -> assertThat(actual).isPositive());
    ...
    assertMetricLongSumValue(
        metrics, OpenTelemetryTopicStats.STORAGE_IN_COUNTER, attributes, messageCount);
  }
  ...
  public void testPublishRateLimitMetric() throws Exception {
    ...
    Awaitility.await()
        .until(() -> Objects.equals(publishRate, admin.topicPolicies().getPublishRate(topicName)));
    ...
    var metrics = pulsarTestContext.getOpenTelemetryMetricReader().collectAllMetrics();
    assertMetricLongSumValue(
        metrics, OpenTelemetryTopicStats.PUBLISH_RATE_LIMIT_HIT_COUNTER, attributes, 1);
  }
}
