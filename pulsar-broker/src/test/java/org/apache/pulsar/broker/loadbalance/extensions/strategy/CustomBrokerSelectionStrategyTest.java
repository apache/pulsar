...
public class CustomBrokerSelectionStrategyTest extends MultiBrokerBaseTest {
  ...
  private static ServiceConfiguration addCustomConfigs(ServiceConfiguration conf) {
    ...
    // Don't consider broker's load so the broker will be selected randomly with the default
    // strategy
  ...
  public void testSingleBrokerSelected() throws Exception {
    ...
    final var producer =
        (PartitionedProducerImpl<byte[]>)
            getAllClients().get(0).newProducer().topic(topic).create();
    ...
    final var port =
        Integer.parseInt(
            connections.stream()
                ...
                .toString()
                .replaceAll(".*:", ""));
    final var expectedPort =
        Stream.concat(Stream.of(pulsar), additionalBrokers.stream())
