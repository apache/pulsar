...
public class ManagedCursorMetricsTest extends MockedPulsarServiceBaseTest {
  ...
  public void testManagedCursorMetrics() throws Exception {
    ...
    /** Trigger the cursor ledger creation. After create cursor. Verify all metrics is zero. */
    ...
    Producer<byte[]> producer =
        this.pulsarClient.newProducer().topic(topicName).enableBatching(false).create();
    ...
    Assert.assertEquals(
        metricsList.get(0).getMetrics().get("brk_ml_cursor_persistLedgerSucceed"), 0L);
    Assert.assertEquals(
        metricsList.get(0).getMetrics().get("brk_ml_cursor_persistLedgerErrors"), 0L);
    Assert.assertEquals(
        metricsList.get(0).getMetrics().get("brk_ml_cursor_persistZookeeperSucceed"), 0L);
    Assert.assertEquals(
        metricsList.get(0).getMetrics().get("brk_ml_cursor_persistZookeeperErrors"), 0L);
    Assert.assertEquals(
    ...
    assertMetricLongSumValue(
        ...
        attributesSet.getAttributesOperationSucceed(),
        0);
    assertMetricLongSumValue(
        otelMetrics,
        OpenTelemetryManagedCursorStats.PERSIST_OPERATION_COUNTER,
        attributesSet.getAttributesOperationFailure(),
        0);
    assertMetricLongSumValue(
        otelMetrics,
    ...
    assertMetricLongSumValue(
        ...
        0);
    /**
     * 1. Send many messages, and only ack half. After the cursor data is written to BK, verify
     * "brk_ml_cursor_persistLedgerSucceed" and "brk_ml_cursor_nonContiguousDeletedMessagesRange".
    ...
    assertMetricLongSumValue(
        ...
        0);
    /**
     * Make BK error, and send many message, then wait cursor persistent finish. After the cursor
     * data is written to ZK, verify "brk_ml_cursor_persistLedgerErrors" and
     * "brk_ml_cursor_persistZookeeperSucceed".
     */
        ...
        .until(
            () -> {
              ...
              LedgerHandle ledgerHandle =
                  (LedgerHandle) FieldUtils.readField(managedCursor, "cursorLedger", true);
              ledgerHandle.close();
    ...
    assertMetricLongSumValue(
        ...
        0);
    /**
     * TODO verify "brk_ml_cursor_persistZookeeperErrors". This is not easy to implement, we can use
     * {@link #mockZooKeeper} to fail ZK, but we cannot identify whether the request is triggered by
     * the "create new ledger then write ZK" or the "persistent cursor then write ZK". The cursor
     * path is "/managed-ledgers/my-namespace/use/my-ns/persistent/my-topic1/my-sub". Maybe we can
  ...
  private ManagedCursor getManagedCursor(String topicName, String subscriptionName)
      throws Exception {
  ...
  public void testCursorReadWriteMetrics() throws Exception {
    ...
    Assert.assertNotEquals(
        metricsList.get(0).getMetrics().get("brk_ml_cursor_writeLedgerSize"), 0L);
    Assert.assertNotEquals(
        metricsList.get(0).getMetrics().get("brk_ml_cursor_writeLedgerLogicalSize"), 0L);
    Assert.assertEquals(metricsList.get(0).getMetrics().get("brk_ml_cursor_readLedgerSize"), 0L);

    Assert.assertNotEquals(
        metricsList.get(1).getMetrics().get("brk_ml_cursor_writeLedgerSize"), 0L);
    Assert.assertNotEquals(
        metricsList.get(1).getMetrics().get("brk_ml_cursor_writeLedgerLogicalSize"), 0L);
    ...
    assertMetricLongSumValue(
        ...
        value -> assertThat(value).isPositive());
    assertMetricLongSumValue(
        otelMetrics, OpenTelemetryManagedCursorStats.INCOMING_BYTE_COUNTER, attributes1, 0);
    ...
    assertMetricLongSumValue(
        ...
        value -> assertThat(value).isPositive());
    assertMetricLongSumValue(
        otelMetrics, OpenTelemetryManagedCursorStats.INCOMING_BYTE_COUNTER, attributes2, 0);
    // cleanup.
    consumer.close();
    consumer2.close();
    producer.close();
