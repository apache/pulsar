...
public class TopicPoliciesTest extends MockedPulsarServiceBaseTest {
    ...
    public void testCustomMetricLabels() throws Exception {
        final String topicName = "persistent://" + myNamespace + "/test-custom-metric-labels";
        admin.topics().createNonPartitionedTopic(topicName);
        ...
        admin.topicPolicies().setCustomMetricLabels(topicName, labels);

        // Verify labels are set
        Awaitility.await().untilAsserted(() -> {
            Map<String, String> retrievedLabels = admin.topicPolicies().getCustomMetricLabels(topicName);
        ...
        // Test 2: Update custom metric labels
        labels.put("sla_tier", "platinum");
        labels.put("data_sensitivity", "high");
        admin.topicPolicies().setCustomMetricLabels(topicName, labels);

        Awaitility.await().untilAsserted(() -> {
            Map<String, String> retrievedLabels = admin.topicPolicies().getCustomMetricLabels(topicName);
            assertEquals(retrievedLabels.size(), 3);
            assertEquals(retrievedLabels.get("sla_tier"), "platinum");
            assertEquals(retrievedLabels.get("app_owner"), "team-a");
            assertEquals(retrievedLabels.get("data_sensitivity"), "high");
        });

        // Test 3: Remove specific labels
        List<String> keysToRemove = Arrays.asList("app_owner");
        admin.topicPolicies().removeCustomMetricLabels(topicName, false, keysToRemove);

        Awaitility.await().untilAsserted(() -> {
            Map<String, String> retrievedLabels = admin.topicPolicies().getCustomMetricLabels(topicName);
            assertEquals(retrievedLabels.size(), 2);
            assertFalse(retrievedLabels.containsKey("app_owner"));
            assertEquals(retrievedLabels.get("sla_tier"), "platinum");
            assertEquals(retrievedLabels.get("data_sensitivity"), "high");
        });

        // Test 4: Remove all labels
        admin.topicPolicies().removeCustomMetricLabels(topicName, true, null);

        Awaitility.await().untilAsserted(() -> {
            Map<String, String> retrievedLabels = admin.topicPolicies().getCustomMetricLabels(topicName);
            assertTrue(retrievedLabels == null || retrievedLabels.isEmpty());
        });
    ...
    @Test
    public void testCustomMetricLabelsValidation() throws Exception {
        final String topicName = "persistent://" + myNamespace + "/test-custom-metric-labels-validation";
        admin.topics().createNonPartitionedTopic(topicName);
        ...
        Map<String, String> invalidKeyLabels = new HashMap<>();
        invalidKeyLabels.put("invalid_key", "value");

        try {
            admin.topicPolicies().setCustomMetricLabels(topicName, invalidKeyLabels);
            fail("Should have thrown exception for invalid key");
        } catch (PulsarAdminException e) {
            assertTrue(e.getMessage().contains("invalid_key") || e.getMessage().contains("not allowed"));
        }

        // Test 2: Value too long
        Map<String, String> longValueLabels = new HashMap<>();
        longValueLabels.put("sla_tier", "this_is_a_very_long_value_that_exceeds_the_maximum_length");

        try {
            admin.topicPolicies().setCustomMetricLabels(topicName, longValueLabels);
            fail("Should have thrown exception for value too long");
        } catch (PulsarAdminException e) {
            assertTrue(e.getMessage().contains("exceeds maximum length") || e.getMessage().contains("too long"));
        }

        // Test 3: Valid labels should work
        Map<String, String> validLabels = new HashMap<>();
        validLabels.put("sla_tier", "gold");
        validLabels.put("app_owner", "team-a");
        admin.topicPolicies().setCustomMetricLabels(topicName, validLabels);
    ...
    @Test
    public void testCustomMetricLabelsFeatureDisabled() throws Exception {
        final String topicName = "persistent://" + myNamespace + "/test-custom-metric-labels-disabled";
        admin.topics().createNonPartitionedTopic(topicName);

        // Disable custom metric labels feature
        conf.setExposeCustomTopicMetricLabelsEnabled(false);

        // Try to set labels when feature is disabled
        Map<String, String> labels = new HashMap<>();
        labels.put("sla_tier", "gold");

        try {
            admin.topicPolicies().setCustomMetricLabels(topicName, labels);
            fail("Should have thrown exception when feature is disabled");
        } catch (PulsarAdminException e) {
            assertTrue(e.getMessage().contains("not enabled") || e.getMessage().contains("disabled"));
        }
    ...
    @Test
    public void testCustomMetricLabelsGlobalPolicy() throws Exception {
        final String topicName = "persistent://" + myNamespace + "/test-custom-metric-labels-global";
        admin.topics().createNonPartitionedTopic(topicName);
        ...
        globalLabels.put("sla_tier", "silver");
        admin.topicPolicies(true).setCustomMetricLabels(topicName, globalLabels);

        // Verify global labels are set
        Awaitility.await().untilAsserted(() -> {
            Map<String, String> retrievedLabels = admin.topicPolicies(true).getCustomMetricLabels(topicName);
            assertNotNull(retrievedLabels);
            assertEquals(retrievedLabels.get("sla_tier"), "silver");
        });

        // Set local labels (should override global)
        Map<String, String> localLabels = new HashMap<>();
        localLabels.put("sla_tier", "gold");
        localLabels.put("app_owner", "team-b");
        admin.topicPolicies(false).setCustomMetricLabels(topicName, localLabels);

        // Verify local labels take precedence
        Awaitility.await().untilAsserted(() -> {
            Map<String, String> retrievedLabels = admin.topicPolicies(false).getCustomMetricLabels(topicName);
            assertNotNull(retrievedLabels);
            assertEquals(retrievedLabels.get("sla_tier"), "gold");
    ...
    @Test
    public void testCustomMetricLabelsPropagation() throws Exception {
        final String topicName = "persistent://" + myNamespace + "/test-custom-metric-labels-propagation";
        admin.topics().createNonPartitionedTopic(topicName);
        ...
        // Wait for policy propagation via admin API
        Awaitility.await().untilAsserted(() -> {
            Map<String, String> retrievedLabels = admin.topicPolicies().getCustomMetricLabels(topicName);
            assertNotNull(retrievedLabels);
            assertEquals(retrievedLabels.size(), 2);
        ...
        // Update labels and verify propagation
        labels.put("sla_tier", "platinum");
        admin.topicPolicies().setCustomMetricLabels(topicName, labels);
