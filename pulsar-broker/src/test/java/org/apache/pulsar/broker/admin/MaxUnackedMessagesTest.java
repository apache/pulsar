...
public class MaxUnackedMessagesTest extends ProducerConsumerBase {
  ...
  public void testMaxUnackedMessagesOnSubscription() throws Exception {
    ...
    // try to consume remaining messages: broker may take time to deliver so, retry multiple time to
    // consume
    // all messages
  ...
  public void testMaxUnackedMessagesOnConsumerApi() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(() -> assertNull(admin.topics().getMaxUnackedMessagesOnConsumer(topicName)));
  ...
  public void testMaxUnackedMessagesOnConsumerAppliedApi() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(
                    admin.namespaces().getMaxUnackedMessagesPerConsumer(myNamespace).intValue(),
                    15));
    ...
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(
                    admin.namespaces().getMaxUnackedMessagesPerConsumer(myNamespace), null));
  ...
  public void testMaxUnackedMessagesOnSubApplied() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(
                    admin.namespaces().getMaxUnackedMessagesPerSubscription(myNamespace),
                    Integer.valueOf(10)));
  ...
  public void testMaxUnackedMessagesOnConsumer() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(
            () -> assertNotNull(admin.topics().getMaxUnackedMessagesOnConsumer(topicName)));
    assertEquals(
        admin.topics().getMaxUnackedMessagesOnConsumer(topicName).intValue(), unackMsgAllowed);
