...
public class TopicPoliciesWithBrokerRestartTest extends MockedPulsarServiceBaseTest {
  ...
  public void testRetentionWithBrokerRestart() throws Exception {
    ...
    // (3) Send messages
    @Cleanup
    final Producer<byte[]> publisher = pulsarClient.newProducer().topic(topicName).create();
        ...
        .untilAsserted(
            () -> {
              ...
              final ManagedLedgerImpl managedLedger1 =
                  (ManagedLedgerImpl) persistentTopic1.getManagedLedger();
              Assert.assertEquals(managedLedger1.getConfig().getRetentionSizeInMB(), 20);
              Assert.assertEquals(
                  managedLedger1.getConfig().getRetentionTimeMillis(),
    ...
    for (int i = 0; i < topicNum; i++) {
      ...
      final PersistentTopic persistentTopicTmp =
          (PersistentTopic) pulsar.getBrokerService().getTopic(shadowTopicNames, true).join().get();
      final ManagedLedgerImpl managedLedgerTemp =
          (ManagedLedgerImpl) persistentTopicTmp.getManagedLedger();
      Assert.assertEquals(managedLedgerTemp.getConfig().getRetentionSizeInMB(), 20);
      Assert.assertEquals(
          managedLedgerTemp.getConfig().getRetentionTimeMillis(), TimeUnit.MINUTES.toMillis(20));
    }
  }
}
