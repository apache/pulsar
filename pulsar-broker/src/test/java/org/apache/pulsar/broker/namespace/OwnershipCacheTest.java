...
public class OwnershipCacheTest {
  ...
  public void testDisableOwnership() throws Exception {
    ...
    NamespaceBundle testBundle =
        new NamespaceBundle(
            NamespaceName.get("pulsar/test/ns-1"),
  ...
  public void testGetOrSetOwner() throws Exception {
    ...
    assertFalse(data1.isDisabled());
    // case 2: the local broker owned the namespace and disabled, getOrSetOwner() should not change
    // it
    ...
    // case 3: some other broker owned the namespace, getOrSetOwner() should return other broker's
    // URL
    // The only chance that we lost an already existing ephemeral node is when the broker dies or
    // unload has
    // succeeded in both cases, the ownerInfoCache will be updated (i.e. invalidated the entry)
    @Cleanup
    MetadataStoreExtended otherStore =
        MetadataStoreExtended.create(
            zookeeperServer.getHostPort(),
            MetadataStoreConfig.builder().sessionTimeoutMillis(5000).build());
    otherStore
  ...
  public void testGetOwner() throws Exception {
    ...
    NamespaceBundle testBundle =
        new NamespaceBundle(
            NamespaceName.get("pulsar/test/ns-3"),
            Range.closedOpen(0L, (long) Integer.MAX_VALUE),
            bundleFactory);
    // case 1: no one owns the namespace
    assertFalse(cache.getOwnerAsync(testBundle).get().isPresent());
  ...
  public void testReestablishOwnership() throws Exception {
    ...
    NamespaceBundle testFullBundle =
        new NamespaceBundle(
            NamespaceName.get("pulsar/test/ns-8"),
            Range.closedOpen(0L, (long) Integer.MAX_VALUE),
            bundleFactory);
