...
/**
 * This is a JDK Proxy based handler that wraps an AutoCloseable instance and prevents it from being
 * closed when the close method is invoked. The {@link #reallyClose(AutoCloseable)} method can be
 * used to close the delegate instance.
 */
public class NonClosingProxyHandler implements InvocationHandler {
  ...
  public static <T extends AutoCloseable> T createNonClosingProxy(
      T delegate, Class<T> interfaceClass) {
  ...
  /**
   * Returns true if the given instance is a proxy instance created by {@link
   * #createNonClosingProxy(AutoCloseable, Class)}
   *
  ...
  /**
   * Calls close on the delegate instance of a proxy that was created by {@link
   * #createNonClosingProxy(AutoCloseable, Class)}
   *
  ...
  /** JDK Proxy InvocationHandler implementation. If the method is close, then it is ignored. */
  @Override
