...
public class TransferShedderTest {
  ...
  public LoadManagerContext setupContext() {
    ...
    topBundlesLoadDataStore.pushAsync(
        "broker1:8080", getTopBundlesLoad("my-tenant/my-namespaceA", 1000000, 2000000));
    topBundlesLoadDataStore.pushAsync(
        "broker2:8080", getTopBundlesLoad("my-tenant/my-namespaceB", 1000000, 3000000));
    topBundlesLoadDataStore.pushAsync(
        "broker3:8080", getTopBundlesLoad("my-tenant/my-namespaceC", 2000000, 4000000));
    topBundlesLoadDataStore.pushAsync(
        "broker4:8080", getTopBundlesLoad("my-tenant/my-namespaceD", 2000000, 6000000));
    topBundlesLoadDataStore.pushAsync(
        "broker5:8080", getTopBundlesLoad("my-tenant/my-namespaceE", 2000000, 7000000));
  ...
  public LoadManagerContext setupContext(int clusterSize) {
    ...
    for (int i = 0; i < clusterSize; i++) {
      ...
      brokerLoadDataStore.pushAsync(
          "broker" + i + ":8080", getCpuLoad(ctx, brokerLoad, "broker" + i + ":8080"));
      int bundleLoad = rand.nextInt(brokerLoad + 1);
      topBundlesLoadDataStore.pushAsync(
          "broker" + i + ":8080",
          getTopBundlesLoad("my-tenant/my-namespace" + i, bundleLoad, brokerLoad - bundleLoad));
    }
    return ctx;
  }
  ...
  public LoadManagerContext setupContextLoadSkewedOverload(int clusterSize) {
    ...
    for (; i < clusterSize - 1; i++) {
      ...
      topBundlesLoadDataStore.pushAsync(
          "broker" + i + ":8080",
          getTopBundlesLoad("my-tenant/my-namespace" + i, 300_000, 700_000));
      brokerLoadDataStore.pushAsync(
          "broker" + i + ":8080", getCpuLoad(ctx, brokerLoad, "broker" + i + ":8080"));
    }
    int brokerLoad = 100;
    topBundlesLoadDataStore.pushAsync(
        "broker" + i + ":8080",
        getTopBundlesLoad("my-tenant/my-namespace" + i, 30_000_000, 70_000_000));
    brokerLoadDataStore.pushAsync(
        "broker" + i + ":8080", getCpuLoad(ctx, brokerLoad, "broker" + i + ":8080"));
  ...
  public LoadManagerContext setupContextLoadSkewedUnderload(int clusterSize) {
    ...
    brokerLoad = 1;
    topBundlesLoadDataStore.pushAsync(
        "broker" + i + ":8080", getTopBundlesLoad("my-tenant/my-namespace" + i, 300_000, 700_000));
    brokerLoadDataStore.pushAsync(
        "broker" + i + ":8080", getCpuLoad(ctx, brokerLoad, "broker" + i + ":8080"));
    return ctx;
  }
  ...
  public BrokerLoadData getCpuLoad(LoadManagerContext ctx, int load, String broker) {
    ...
    if (ctx.topBundleLoadDataStore().get(broker).isPresent()) {
      var throughputOut =
          ctx.topBundleLoadDataStore().get(broker).get().getTopBundlesLoadData().stream()
              .mapToDouble(v -> v.stats().msgThroughputOut)
              .sum();
      loadData.update(usage1, 1, throughputOut, 3, 4, 5, 6, ctx.brokerConfiguration());
    } else {
      loadData.update(usage1, 1, 2, 3, 4, 5, 6, ctx.brokerConfiguration());
    }
    return loadData;
  }
  ...
  public TopBundlesLoadData getTopBundlesLoad(String bundlePrefix, int load1, int load2) {
    ...
    topKBundles.update(
        Map.of(
            bundlePrefix + "/0x00000000_0x0FFFFFFF",
            namespaceBundleStats1,
            bundlePrefix + "/0x0FFFFFFF_0xFFFFFFFF",
            namespaceBundleStats2),
        2);
    return topKBundles.getLoadData();
  }

  public TopBundlesLoadData getTopBundlesLoad(
      String bundlePrefix, int load1, int load2, int load3, int load4, int load5) {
    var namespaceBundleStats1 = new NamespaceBundleStats();
    namespaceBundleStats1.msgThroughputOut = load1;
    var namespaceBundleStats2 = new NamespaceBundleStats();
    namespaceBundleStats2.msgThroughputOut = load2;
  ...
  public TopBundlesLoadData getTopBundlesLoadWithOutSuffix(String namespace, int load1, int load2) {
    var namespaceBundleStats1 = new NamespaceBundleStats();
    ...
    topKBundles.update(
        Map.of(
            namespace + "/0x00000000_0x7FFFFFF",
            namespaceBundleStats1,
            namespace + "/0x7FFFFFF_0xFFFFFFF",
            namespaceBundleStats2),
        2);
    return topKBundles.getLoadData();
  }
  ...
  public LoadManagerContext getContext() {
    ...
    var topBundleLoadDataStore =
        new LoadDataStore<TopBundlesLoadData>() {
  ...
  public void testOutDatedLoadData() throws IllegalAccessException {
    ...
    FieldUtils.writeDeclaredField(
        brokerLoadDataStore.get("broker1:8080").get(), "updatedAt", 0, true);
    FieldUtils.writeDeclaredField(
        brokerLoadDataStore.get("broker2:8080").get(), "updatedAt", 0, true);
    FieldUtils.writeDeclaredField(
        brokerLoadDataStore.get("broker3:8080").get(), "updatedAt", 0, true);
    FieldUtils.writeDeclaredField(
        brokerLoadDataStore.get("broker4:8080").get(), "updatedAt", 0, true);
    FieldUtils.writeDeclaredField(
        brokerLoadDataStore.get("broker5:8080").get(), "updatedAt", 0, true);
  ...
  public void testRecentlyUnloadedBrokers() {
    ...
    var oldTS =
        System.currentTimeMillis()
            - ctx.brokerConfiguration().getLoadBalancerBrokerLoadDataTTLInSeconds() * 1001;
    ...
    expected.add(
        new UnloadDecision(
            new Unload("broker5:8080", bundleE1, Optional.of("broker1:8080")),
            Success,
            Overloaded));
    expected.add(
        new UnloadDecision(
            new Unload("broker4:8080", bundleD1, Optional.of("broker2:8080")),
            Success,
            Overloaded));
  ...
  public void testGetAvailableBrokersFailed() {
    ...
    TransferShedder transferShedder =
        new TransferShedder(
            pulsar, counter, null, isolationPoliciesHelper, antiAffinityGroupPolicyHelper);
    var ctx = setupContext();
    BrokerRegistry registry = ctx.brokerRegistry();
    doReturn(FutureUtil.failedFuture(new TimeoutException()))
        .when(registry)
        .getAvailableBrokerLookupDataAsync();
  ...
  public void testBundlesWithIsolationPolicies() {
    ...
    IsolationPoliciesHelper isolationPoliciesHelper =
        new IsolationPoliciesHelper(allocationPoliciesSpy);
    ...
    TransferShedder transferShedder =
        spy(
            new TransferShedder(
                pulsar, counter, filters, isolationPoliciesHelper, antiAffinityGroupPolicyHelper));

    setIsolationPolicies(
        allocationPoliciesSpy,
        "my-tenant/my-namespaceE",
        Set.of("broker5:8080"),
        Set.of(),
        Set.of(),
        1);
    ...
    expected.add(
        new UnloadDecision(
            new Unload("broker4:8080", bundleD1, Optional.of("broker1:8080")),
            Success,
            Overloaded));
    assertEquals(res, expected);
    assertEquals(counter.getLoadAvg(), setupLoadAvg);
    ...
    expected.add(
        new UnloadDecision(
            new Unload("broker4:8080", bundleD1, Optional.empty()), Success, Overloaded));
    assertEquals(res, expected);
    assertEquals(counter.getLoadAvg(), setupLoadAvg);
    assertEquals(counter.getLoadStd(), setupLoadStd);
    ...
    doReturn(CompletableFuture.completedFuture(true))
        ...
        .areIsolationPoliciesPresentAsync(any());
  ...
  public void testBundlesWithAntiAffinityGroup() throws MetadataStoreException {
    ...
    AntiAffinityGroupPolicyFilter filter =
        new AntiAffinityGroupPolicyFilter(antiAffinityGroupPolicyHelper);
    filters.add(filter);
    ...
    doAnswer(
        ...
        .when(antiAffinityGroupPolicyHelper)
        .filterAsync(any(), any());
    var res = transferShedder.findBundlesForUnloading(ctx, Map.of(), Map.of());
  ...
  public void testIsLoadBalancerSheddingBundlesWithPoliciesEnabled() {
    ...
    for (boolean[] expect : expects) {
      ...
      assertEquals(
          transferShedder.isLoadBalancerSheddingBundlesWithPoliciesEnabled(ctx, namespaceBundle),
          expect[3]);
    }
  }
  ...
  public void testTargetStd() {
    ...
    topBundlesLoadDataStore.pushAsync(
        "broker1:8080", getTopBundlesLoad("my-tenant/my-namespaceA", 30, 30));
    topBundlesLoadDataStore.pushAsync(
        "broker2:8080", getTopBundlesLoad("my-tenant/my-namespaceB", 40, 40));
    topBundlesLoadDataStore.pushAsync(
        "broker3:8080", getTopBundlesLoad("my-tenant/my-namespaceC", 50, 50));
  ...
  public void testSingleTopBundlesLoadData() {
    ...
    var topBundlesLoadDataStore = ctx.topBundleLoadDataStore();
    topBundlesLoadDataStore.pushAsync(
        "broker1:8080", getTopBundlesLoad("my-tenant/my-namespaceA", 1));
    topBundlesLoadDataStore.pushAsync(
        "broker2:8080", getTopBundlesLoad("my-tenant/my-namespaceB", 2));
    topBundlesLoadDataStore.pushAsync(
        "broker3:8080", getTopBundlesLoad("my-tenant/my-namespaceC", 6));
    topBundlesLoadDataStore.pushAsync(
        "broker4:8080", getTopBundlesLoad("my-tenant/my-namespaceD", 10));
    topBundlesLoadDataStore.pushAsync(
        "broker5:8080", getTopBundlesLoad("my-tenant/my-namespaceE", 70));
    var res = transferShedder.findBundlesForUnloading(ctx, Map.of(), Map.of());
  ...
  public void testBundleThroughputLargerThanOffloadThreshold() {
    ...
    var topBundlesLoadDataStore = ctx.topBundleLoadDataStore();
    topBundlesLoadDataStore.pushAsync(
        "broker4:8080", getTopBundlesLoad("my-tenant/my-namespaceD", 1000000000, 1000000000));
    topBundlesLoadDataStore.pushAsync(
        "broker5:8080", getTopBundlesLoad("my-tenant/my-namespaceE", 1000000000, 1000000000));
    var res = transferShedder.findBundlesForUnloading(ctx, Map.of(), Map.of());
  ...
  public void testUnloadBundlesGreaterThanTargetThroughput() throws IllegalAccessException {
    ...
    topBundlesLoadDataStore.pushAsync(
        "broker1:8080",
        getTopBundlesLoad("my-tenant/my-namespaceA", 1000000, 2000000, 3000000, 4000000, 5000000));
    topBundlesLoadDataStore.pushAsync(
        "broker2:8080",
        getTopBundlesLoad(
            "my-tenant/my-namespaceB", 100000000, 180000000, 220000000, 250000000, 250000000));
    ...
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker2:8080",
                "my-tenant/my-namespaceB/0x00000000_0x1FFFFFFF",
                Optional.of("broker1:8080")),
            Success,
            Overloaded));
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker2:8080",
                "my-tenant/my-namespaceB/0x1FFFFFFF_0x2FFFFFFF",
                Optional.of("broker1:8080")),
            Success,
            Overloaded));
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker2:8080",
                "my-tenant/my-namespaceB/0x2FFFFFFF_0x3FFFFFFF",
                Optional.of("broker1:8080")),
            Success,
            Overloaded));
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker1:8080",
                "my-tenant/my-namespaceA/0x00000000_0x1FFFFFFF",
                Optional.of("broker2:8080")),
            Success,
            Overloaded));
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker1:8080",
                "my-tenant/my-namespaceA/0x1FFFFFFF_0x2FFFFFFF",
                Optional.of("broker2:8080")),
            Success,
            Overloaded));
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker1:8080",
                "my-tenant/my-namespaceA/0x2FFFFFFF_0x3FFFFFFF",
                Optional.of("broker2:8080")),
            Success,
            Overloaded));
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker1:8080",
                "my-tenant/my-namespaceA/0x3FFFFFFF_0x4FFFFFFF",
                Optional.of("broker2:8080")),
            Success,
            Overloaded));
    ...
    assertEquals(res, expected);
    var stats =
        (TransferShedder.LoadStats) FieldUtils.readDeclaredField(transferShedder, "stats", true);
  ...
  public void testSkipBundlesGreaterThanTargetThroughputAfterSplit() {
    ...
    var topBundlesLoadDataStore = ctx.topBundleLoadDataStore();
    topBundlesLoadDataStore.pushAsync(
        "broker1:8080", getTopBundlesLoad("my-tenant/my-namespaceA", 1, 500000000));
    topBundlesLoadDataStore.pushAsync(
        "broker2:8080", getTopBundlesLoad("my-tenant/my-namespaceB", 500000000, 500000000));
  ...
  public void testUnloadBundlesLessThanTargetThroughputAfterSplit() throws IllegalAccessException {
    ...
    topBundlesLoadDataStore.pushAsync(
        ...
        getTopBundlesLoad("my-tenant/my-namespaceA", 1000000, 2000000, 3000000, 4000000, 5000000));
    topBundlesLoadDataStore.pushAsync(
        "broker2:8080", getTopBundlesLoad("my-tenant/my-namespaceB", 490000000, 510000000));
    ...
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker2:8080",
                "my-tenant/my-namespaceB/0x00000000_0x0FFFFFFF",
                Optional.of("broker1:8080")),
            Success,
            Overloaded));
    assertEquals(counter.getLoadAvg(), 5.05);
  ...
  @Test
  public void testUnloadBundlesGreaterThanTargetThroughputAfterSplit()
    ...
    var topBundlesLoadDataStore = ctx.topBundleLoadDataStore();
    topBundlesLoadDataStore.pushAsync(
        "broker1:8080", getTopBundlesLoad("my-tenant/my-namespaceA", 2400000, 2400000));
    topBundlesLoadDataStore.pushAsync(
        "broker2:8080", getTopBundlesLoad("my-tenant/my-namespaceB", 5000000, 5000000));
    ...
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker1:8080",
                res.stream()
                    .filter(x -> x.getUnload().sourceBroker().equals("broker1:8080"))
                    .findFirst()
                    .get()
                    .getUnload()
                    .serviceUnit(),
    ...
    expected.add(
        new UnloadDecision(
            new Unload(
                "broker2:8080",
                res.stream()
                    .filter(x -> x.getUnload().sourceBroker().equals("broker2:8080"))
                    .findFirst()
                    .get()
                    .getUnload()
                    .serviceUnit(),
  ...
  public void testMaxNumberOfTransfersPerShedderCycle() {
    ...
    TransferShedder transferShedder = new TransferShedder(counter);
    var ctx = setupContext();
    ctx.brokerConfiguration().setLoadBalancerMaxNumberOfBrokerSheddingPerCycle(10);
    var res = transferShedder.findBundlesForUnloading(ctx, Map.of(), Map.of());
    var expected = new HashSet<UnloadDecision>();
    expected.add(
        new UnloadDecision(
  ...
  public void testLoadBalancerSheddingConditionHitCountThreshold() {
    ...
    var ctx = setupContext();
    int max = 3;
    ctx.brokerConfiguration().setLoadBalancerSheddingConditionHitCountThreshold(max);
    for (int i = 0; i < max; i++) {
      var res = transferShedder.findBundlesForUnloading(ctx, Map.of(), Map.of());
      assertTrue(res.isEmpty());
  ...
  public void testRemainingTopBundles() {
    ...
    var topBundlesLoadDataStore = ctx.topBundleLoadDataStore();
    topBundlesLoadDataStore.pushAsync(
        "broker5:8080", getTopBundlesLoad("my-tenant/my-namespaceE", 2000000, 3000000));
    var res = transferShedder.findBundlesForUnloading(ctx, Map.of(), Map.of());
  ...
  public void testRandomLoad() throws IllegalAccessException {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      var stats =
          (TransferShedder.LoadStats) FieldUtils.readDeclaredField(transferShedder, "stats", true);
      assertTrue(
          stats.std() <= conf.getLoadBalancerBrokerLoadTargetStd()
  ...
  public void testRandomLoadStats() {
    ...
    for (int t = 0; t < 5; t++) {
      ...
      for (int k = 0; k < conf.getLoadBalancerMaxNumberOfBrokerSheddingPerCycle(); k++) {
        ...
        assertEquals(minLoad, loadStore.get(stats.peekMinBroker()).get().getWeightedMaxEMA());
        assertEquals(maxLoad, loadStore.get(stats.pollMaxBroker()).get().getWeightedMaxEMA());
  ...
  public void testHighVarianceLoadStats() {
    ...
    for (int i = 0; i < loads.length; i++) {
      availableBrokers.put("broker" + i + ":8080", mock(BrokerLookupData.class));
      loadStore.pushAsync(
          "broker" + i + ":8080", getCpuLoad(ctx, loads[i], "broker" + i + ":8080"));
    }
    stats.update(loadStore, availableBrokers, Map.of(), conf);
