...
public class InMemTransactionBufferReaderTest {
  ...
  public void testInvalidNumEntriesArgument() {
    try (InMemTransactionBufferReader reader =
        new InMemTransactionBufferReader(
  ...
  private void verifyAndReleaseEntries(
    ...
    for (int i = 0; i < numEntriesToRead; i++) {
      try (TransactionEntry txnEntry = txnEntries.get(i)) {
        ...
        assertEquals(
            new String(ByteBufUtil.getBytes(txnEntry.getEntry().getDataBuffer()), UTF_8),
            "message-" + i);
      }
    }
  }

  private void verifyEntriesReleased(
      SortedMap<Long, ByteBuf> entries, long startSequenceId, int numEntriesToRead) {
    for (int i = 0; i < numEntriesToRead; i++) {
