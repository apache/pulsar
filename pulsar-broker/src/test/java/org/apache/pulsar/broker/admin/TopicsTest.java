...
public class TopicsTest extends MockedPulsarServiceBaseTest {
  ...
  public void testProduceToNonPartitionedTopic() throws Exception {
    admin
        ...
        .createNonPartitionedTopic(
            "persistent://" + testTenant + "/" + testNamespace + "/" + testTopicName);
    ...
    for (int index = 0; index < response.getMessagePublishResults().size(); index++) {
      Assert.assertEquals(
          Integer.parseInt(
              response.getMessagePublishResults().get(index).getMessageId().split(":")[2]),
          -1);
  ...
  public void testProduceToPartitionedTopic() throws Exception {
    ...
    for (int index = 0; index < messagePerPartition.length; index++) {
      // We publish to each partition in round robin mode so each partition should get at most 2
      // message.
  ...
  public void testProduceToPartitionedTopicSpecificPartition() throws Exception {
    admin
        .topics()
        .createPartitionedTopic(
            "persistent://" + testTenant + "/" + testNamespace + "/" + testTopicName, 5);
    AsyncResponse asyncResponse = mock(AsyncResponse.class);
    Schema<String> schema = StringSchema.utf8();
    ProducerMessages producerMessages = new ProducerMessages();
    producerMessages.setKeySchema(
        ...
        .thenAccept(
            topic -> {
              try {
                ...
                verify(asyncResponse, timeout(5000).times(1)).resume(responseCaptor.capture());
                Assert.assertEquals(
                    responseCaptor.getValue().getStatus(), Response.Status.OK.getStatusCode());
                Object responseEntity = responseCaptor.getValue().getEntity();
                Assert.assertTrue(responseEntity instanceof ProducerAcks);
                ProducerAcks response = (ProducerAcks) responseEntity;
                Assert.assertEquals(response.getMessagePublishResults().size(), 4);
                ...
                for (int index = 0; index < response.getMessagePublishResults().size(); index++) {
                  ...
                  if (0 == errorCode) {
                    Assert.assertEquals(
                        Integer.parseInt(
                            response
                                .getMessagePublishResults()
                                .get(index)
                                .getMessageId()
                                .split(":")[2]),
                        -1);
                    Assert.assertTrue(
                        response.getMessagePublishResults().get(index).getMessageId().length() > 0);
                  } else {
                    errorResponse++;
                    Assert.assertEquals(errorCode, 2);
                    Assert.assertEquals(
                        response.getMessagePublishResults().get(index).getErrorMsg(),
                        "org.apache.pulsar.broker.service.BrokerServiceException$"
                            + "TopicFencedException: Fake exception");
                  }
                }
                // Add entry start to fail after 2nd operation, we published 4 msg so expecting 2
                // error response.
  ...
  public void testLookUpWithRedirect() throws Exception {
    ...
    URI requestPath =
        URI.create(pulsar.getWebServiceAddress() + "/topics/my-tenant/my-namespace/my-topic");
    ...
    Assert.assertEquals(
        responseCaptor.getValue().getStatusInfo(), Response.Status.TEMPORARY_REDIRECT);
  ...
  public void testProduceWithJsonSchema() throws Exception {
    ...
    GenericSchema jsonSchema =
        GenericJsonSchema.of(
            JSONSchema.of(SchemaDefinition.builder().withPojo(PC.class).build()).getSchemaInfo());
    ...
    for (int i = 0; i < 2; i++) {
      ...
      PC msgPc =
          ObjectMapperFactory.getMapper()
              .getObjectMapper()
              .treeToValue(
                  ((GenericJsonRecord) jsonSchema.decode(msg.getData())).getJsonNode(), PC.class);
  ...
  public void testProduceWithAvroSchema() throws Exception {
    ...
    GenericSchemaImpl avroSchema =
        GenericAvroSchema.of(
            AvroSchema.of(SchemaDefinition.builder().withPojo(PC.class).build()).getSchemaInfo());
    PC pc = new PC("dell", "alienware", 2021, GPU.AMD, new Seller("WA", "main street", 98004));
    PC anotherPc = new PC("asus", "rog", 2020, GPU.NVIDIA, new Seller("CA", "back street", 90232));
    Consumer consumer =
        pulsarClient
    ...
    for (int i = 0; i < 2; i++) {
      ...
      Assert.assertEquals(
          ((Utf8) avroRecord.getAvroRecord().get("brand")).toString(), expected.get(i).brand);
      Assert.assertEquals(
          ((Utf8) avroRecord.getAvroRecord().get("model")).toString(), expected.get(i).model);
      ...
      Assert.assertEquals(
          ((Utf8) ((GenericRecord) avroRecord.getAvroRecord().get("seller")).get("state"))
              .toString(),
          expected.get(i).seller.state);
      Assert.assertEquals(
          ((Utf8) ((GenericRecord) avroRecord.getAvroRecord().get("seller")).get("street"))
              .toString(),
  ...
  public void testProduceWithRestAndClientThenConsumeWithClient() throws Exception {
    ...
    Schema keyValueSchema =
        KeyValueSchemaImpl.of(
            StringSchema.utf8(), StringSchema.utf8(), KeyValueEncodingType.SEPARATED);
    Producer producer = pulsarClient.newProducer(keyValueSchema).topic(topicName).create();
    Consumer consumer =
        pulsarClient
  ...
  public void testProduceWithInCompatibleSchema() throws Exception {
    ...
    admin.topics().createNonPartitionedTopic(topicName);
    AsyncResponse asyncResponse = mock(AsyncResponse.class);
    Producer producer = pulsarClient.newProducer(StringSchema.utf8()).topic(topicName).create();
