...
public class PersistentDispatcherMultipleConsumersReadLimitsTest extends ProducerConsumerBase {
  ...
  public void testDispatcherMaxReadSizeBytes() throws Exception {
    final String topicName =
        BrokerTestUtil.newUniqueName("persistent://public/default/testDispatcherMaxReadSizeBytes");
    final String subscription = "sub";

    AtomicInteger entriesReadMax = new AtomicInteger(0);
    BrokerTestInterceptor.INSTANCE.applyDispatcherSpyDecorator(
        PersistentDispatcherMultipleConsumers.class,
        spy -> {
          doAnswer(
                  invocation -> {
                    List<Entry> entries = invocation.getArgument(0);
                    PersistentDispatcherMultipleConsumers.ReadType readType =
                        invocation.getArgument(1);
                    int numberOfEntries = entries.size();
                    log.info(
                        "intercepted readEntriesComplete with {} entries, read type {}",
                        numberOfEntries,
                        readType);
