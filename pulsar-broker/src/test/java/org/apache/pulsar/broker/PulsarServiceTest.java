...
public class PulsarServiceTest extends MockedPulsarServiceBaseTest {
    ...
    PulsarService pulsarService =
        spy(
            new PulsarService(
                configuration,
                new WorkerConfig(),
                Optional.of(expectedWorkerService),
                (exitCode) -> {}));
  ...
  /**
   * Verifies that the getWorkerService throws {@link UnsupportedOperationException} when
  ...
  public void testAdvertisedListeners() throws Exception {
    ...
    conf.setAdvertisedListeners(
        "internal:pulsar://gateway:6650, internal:pulsar+ssl://gateway:6651");
  ...
  public void testDynamicBrokerPort() throws Exception {
    ...
    assertEquals(
        pulsar.getBrokerServiceUrl(), "pulsar://localhost:" + pulsar.getBrokerListenPort().get());
    assertEquals(
