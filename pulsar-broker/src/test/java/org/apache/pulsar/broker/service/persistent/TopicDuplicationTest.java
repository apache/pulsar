...
public class TopicDuplicationTest extends ProducerConsumerBase {
    ...
    public void testDuplicationApi() throws Exception {
        ...
        Awaitility.await().until(() -> admin.topics().getMaxUnackedMessagesOnSubscription(topicName) == null);
    ...
    private void checkDeduplicationDisabled(String producerName, MessageDeduplication messageDeduplication)
            throws Exception {
    ...
    private void checkDeduplicationEnabled(String producerName, MessageDeduplication messageDeduplication, long maxSeq)
            throws Exception {
        messageDeduplication
                .checkStatus()
                .whenComplete((res, ex) -> {
    ...
    public void testDuplicationSnapshotApi() throws Exception {
        ...
        Awaitility.await().until(() -> admin.topics().getDeduplicationSnapshotInterval(topicName) != null);
        Assert.assertEquals(
    ...
    public void testTopicPolicyTakeSnapshot() throws Exception {
        ...
        PersistentTopic persistentTopic = (PersistentTopic)
                pulsar.getBrokerService().getTopicIfExists(topicName).get().get();
        long seqId = persistentTopic
                .getMessageDeduplication()
                .highestSequencedPersisted
                .get(producerName);
        PositionImpl position = (PositionImpl) persistentTopic
                .getMessageDeduplication()
                .getManagedCursor()
                .getManagedLedger()
        ...
        Awaitility.await()
                .until(() -> ((PositionImpl) persistentTopic
                                        .getMessageDeduplication()
                                        .getManagedCursor()
        ...
        position = (PositionImpl) persistentTopic
                .getMessageDeduplication()
                .getManagedCursor()
                .getManagedLedger()
                .getLastConfirmedEntry();
        ...
        Awaitility.await().until(() -> (admin.namespaces().getDeduplicationSnapshotInterval(myNamespace) == null));
        producer.newMessage().value("msg").send();
    ...
    public void testNamespacePolicyApi() throws Exception {
        ...
        admin.namespaces()
                .setDeduplicationSnapshotIntervalAsync(myNamespace, 200)
                .get();
        interval = admin.namespaces()
                .getDeduplicationSnapshotIntervalAsync(myNamespace)
                .get();
