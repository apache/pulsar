...
public class HttpTopicLookupv2Test {
  ...
  public void setUp() throws Exception {
    ...
    ClusterData uscData =
        ClusterData.builder().serviceUrl("http://broker.messaging.usc.example.com:8080").build();
    ClusterData uswData =
        ClusterData.builder().serviceUrl("http://broker.messaging.usw.example.com:8080").build();
    doReturn(config).when(pulsar).getConfiguration();

    ClusterResources clusters = mock(ClusterResources.class);
    when(clusters.getClusterAsync("use"))
        .thenReturn(CompletableFuture.completedFuture(Optional.of(useData)));
    when(clusters.getClusterAsync("usc"))
        .thenReturn(CompletableFuture.completedFuture(Optional.of(uscData)));
    when(clusters.getClusterAsync("usw"))
        .thenReturn(CompletableFuture.completedFuture(Optional.of(uswData)));
  ...
  public void testLookupTopicNotExist() throws Exception {
    ...
    doReturn(booleanFuture)
        .when(namespaceService)
        .checkNonPartitionedTopicExists(any(TopicName.class));
  ...
  public void testValidateReplicationSettingsOnNamespace() throws Exception {
    ...
    verify(asyncResponse).resume(arg.capture());
    assertEquals(
        arg.getValue().getResponse().getStatus(), Status.PRECONDITION_FAILED.getStatusCode());
