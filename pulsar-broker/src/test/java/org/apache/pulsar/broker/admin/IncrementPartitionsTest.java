...
public class IncrementPartitionsTest extends MockedPulsarServiceBaseTest {
  ...
  public void testIncrementPartitionsOfTopicWithSubscriptionProperties() throws Exception {
    ...
    assertEquals(
        ...
        List.of("sub-1"));
    TopicStats stats =
        admin.topics().getStats(TopicName.get(partitionedTopicName).getPartition(15).toString());
  ...
  public void testIncrementPartitionsWithNoSubscriptions() throws Exception {
    ...
    @Cleanup
    Producer<String> consumer =
        pulsarClient.newProducer(Schema.STRING).topic(partitionedTopicName).create();
  ...
  public void testIncrementPartitionsWithReaders() throws Exception {
    ...
    assertEquals(
        admin.topics().getPartitionedTopicMetadata(partitionedTopicName.toString()).partitions, 1);

    @Cleanup
    Producer<String> consumer =
        pulsarClient.newProducer(Schema.STRING).topic(partitionedTopicName.toString()).create();
    ...
    assertEquals(
        admin.topics().getPartitionedTopicMetadata(partitionedTopicName.toString()).partitions, 2);

    assertEquals(
        admin.topics().getSubscriptions(partitionedTopicName.getPartition(0).toString()).size(), 1);

    // Partition-1 should not have subscriptions
    assertEquals(
        admin.topics().getSubscriptions(partitionedTopicName.getPartition(1).toString()),
