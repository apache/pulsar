...
public class SubscriptionStatsTest extends ProducerConsumerBase {
  ...
  public void testNonContiguousDeletedMessagesRanges() throws Exception {
    final String topicName =
        "persistent://my-property/my-ns/testNonContiguousDeletedMessagesRanges-"
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              Assert.assertEquals(
                  stats.getSubscriptions().get(subName).getNonContiguousDeletedMessagesRanges(), 1);
  ...
  public Object[][] topicAndSubscription() {
    return new Object[][] {
      {
        "persistent://my-property/my-ns/testSubscriptionStats-" + UUID.randomUUID(),
        "my-sub1",
        true,
        true
      },
      {
        "non-persistent://my-property/my-ns/testSubscriptionStats-" + UUID.randomUUID(),
        "my-sub2",
        true,
        true
      },
  ...
  public void testSubscriptionStatsDispatchThrottled() throws Exception {
    ...
    final String topic =
        "persistent://my-property/my-ns/testSubscriptionStatsDispatchThrottled-"
            + UUID.randomUUID();
    final String subName = "my-sub";
            ...
            .filter(
                m ->
                    m.tags.get("subscription").equals(subName)
                        ...
                        && m.tags.get("reason").equals("subscription"))
    ...
    double subscriptionAllDispatchThrottledMsgCount =
        subscriptionDispatchThrottledMsgCountMetrics.stream()
            .filter(
                m ->
                    m.tags.get("subscription").equals(subName) && m.tags.get("topic").equals(topic))
            .mapToDouble(m -> m.value)
            .sum();
    Assert.assertEquals(
        subscriptionDispatchThrottledMsgCount, subscriptionAllDispatchThrottledMsgCount);
    ...
    double subscriptionAllDispatchThrottledBytesCount =
        subscriptionDispatchThrottledBytesCountMetrics.stream()
            .filter(
                m ->
                    m.tags.get("subscription").equals(subName) && m.tags.get("topic").equals(topic))
            ...
            .sum();
    Assert.assertEquals(
        subscriptionDispatchThrottledBytesCount, subscriptionAllDispatchThrottledBytesCount);
  }

  @Test(dataProvider = "testSubscriptionMetrics")
  public void testSubscriptionStats(
      final String topic, final String subName, boolean enableTopicStats, boolean setFilter)
      throws Exception {
    @Cleanup
    Producer<String> producer =
        pulsarClient.newProducer(Schema.STRING).topic(topic).enableBatching(false).create();
    ...
    if (enableTopicStats) {
      ...
      double throughFilter =
          throughFilterMetrics.stream()
              .filter(
                  m ->
                      m.tags.get("subscription").equals(subName)
                          && m.tags.get("topic").equals(topic))
              .mapToDouble(m -> m.value)
              .sum();
      double filterAccepted =
          acceptedMetrics.stream()
              .filter(
                  m ->
                      m.tags.get("subscription").equals(subName)
                          && m.tags.get("topic").equals(topic))
              .mapToDouble(m -> m.value)
              .sum();
      double filterRejected =
          rejectedMetrics.stream()
              .filter(
                  m ->
                      m.tags.get("subscription").equals(subName)
                          && m.tags.get("topic").equals(topic))
              .mapToDouble(m -> m.value)
              .sum();
      double filterRescheduled =
          rescheduledMetrics.stream()
              .filter(
                  m ->
                      m.tags.get("subscription").equals(subName)
                          && m.tags.get("topic").equals(topic))
      ...
      if (setFilter) {
        ...
        // the filterProcessCount will be not equal with rejectedCount + rescheduledCount +
        // acceptCount
        Assert.assertEquals(
            throughFilter,
  ...
  private void testSubscriptionStatsAdminApi(
      String topic, String subName, boolean setFilter, int acceptCount, int rejectedCount)
      throws Exception {
    ...
    if (setFilter) {
      ...
      if (persistent) {
        ...
        // Only works on the test, if there are some markers, the filterProcessCount will be not
        // equal with rejectedCount + rescheduledCount + acceptCount
        Assert.assertEquals(
