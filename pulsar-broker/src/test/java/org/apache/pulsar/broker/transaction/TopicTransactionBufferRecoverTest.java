...
public class TopicTransactionBufferRecoverTest extends TransactionTestBase {
  ...
  protected void setup() throws Exception {
    conf.getProperties()
        .setProperty("brokerClient_operationTimeoutMs", Integer.valueOf(10 * 1000).toString());
  ...
  private void recoverTest(String testTopic) throws Exception {
    ...
    log.info(
        "Txn1 commit receive message : {}, messageId : {}",
        message.getValue(),
            ...
            () -> {
              for (int i = 0; i < getPulsarServiceList().size(); i++) {
                ...
                CompletableFuture<Optional<Topic>> completableFuture =
                    topics.get("persistent://" + testTopic);
    ...
    if (testTopic.equals(RECOVER_COMMIT)) {
      ...
      for (int i = messageCnt; i > 1; i--) {
        ...
        log.info(
            "Txn2 commit receive message : {}, messageId : {}",
            message.getValue(),
            message.getMessageId());
        consumer.acknowledge(message);
      }
  ...
  private void makeTBSnapshotReaderTimeoutIfFirstRead(TopicName topicName) throws Exception {
    ...
    for (PulsarService pulsarService : pulsarServiceList) {
      ...
      Field field =
          TransactionBufferSnapshotServiceFactory.class.getDeclaredField(
  ...
  private void testTakeSnapshot() throws Exception {
    ...
    assertEquals(
        snapshot.getMaxReadPositionEntryId(), ((MessageIdImpl) messageId2).getEntryId() + 1);
  ...
  private void testTopicTransactionBufferDeleteAbort(Boolean enableSnapshotSegment)
      throws Exception {
    getPulsarServiceList()
        .get(0)
        .getConfig()
        .setTransactionBufferSegmentedSnapshotEnabled(enableSnapshotSegment);
    @Cleanup
    Producer<String> producer =
        pulsarClient
            .newProducer(Schema.STRING)
    ...
    assertTrue(
        ((MessageIdImpl) messageId2).getLedgerId() != ((MessageIdImpl) messageId1).getLedgerId());
    boolean exist = false;
    for (int i = 0; i < getPulsarServiceList().size(); i++) {
      final var topics = getPulsarServiceList().get(i).getBrokerService().getTopics();
      CompletableFuture<Optional<Topic>> completableFuture =
          topics.get("persistent://" + ABORT_DELETE);
      if (completableFuture != null) {
        Optional<Topic> topic = completableFuture.get();
        if (topic.isPresent()) {
          PersistentTopic persistentTopic = (PersistentTopic) topic.get();
  ...
  private void checkSnapshotCount(
      TopicName topicName, boolean hasSnapshot, PersistentTopic persistentTopic, Field field)
      throws Exception {
  ...
  public void testTransactionBufferRecoverThrowException() throws Exception {
    ...
    doReturn(new MockTableView(pulsarServiceList.get(0)))
        .when(systemTopicTxnBufferSnapshotService)
        .getTableView();
    ...
    doReturn(FutureUtil.failedFuture(new PulsarClientException("test")))
        ...
        .getFuture();
    doReturn(failedCountedWriter)
        .when(systemTopicTxnBufferSnapshotService)
        .getReferenceWriter(any());
    checkCloseTopic(
        pulsarClient,
        transactionBufferSnapshotServiceFactoryOriginal,
        transactionBufferSnapshotServiceFactory,
  ...
  public void testTransactionBufferSegmentSystemTopic() throws Exception {
    ...
    SystemTopicTxnBufferSnapshotService<TransactionBufferSnapshotSegment>
        transactionBufferSnapshotSegmentService =
            new TransactionBufferSnapshotServiceFactory(pulsar)
    ...
    AsyncCallbacks.OpenReadOnlyManagedLedgerCallback callback =
        new AsyncCallbacks.OpenReadOnlyManagedLedgerCallback() {
          ...
          public void openReadOnlyManagedLedgerComplete(
              ReadOnlyManagedLedger readOnlyManagedLedger, Object ctx) {
  ...
  public void testSnapshotSegment() throws Exception {
    ...
    // 0. Modify the configurations, enabling the segment snapshot and set the size of the snapshot
    // segment.
    ...
    this.getPulsarServiceList()
        .get(0)
        .getConfig()
        .setTransactionBufferSegmentedSnapshotEnabled(true);
    this.getPulsarServiceList()
        .get(0)
        .getConfig()
    ...
    for (int i = 0; i < 10; i++) {
      ...
      // 2.1 Receive all message before the maxReadPosition to verify the correctness of the max
      // read position.
      int hasReceived = 0;
      while (true) {
      ...
      // 2.2 Commit all ongoing transaction and verify that the consumer can receive all rest
      // message
