...
public class GracefulExecutorServicesShutdownTest {
  ...
  @Test
  public void shouldShutdownExecutorsImmediately()
  ...
  @Test
  public void shouldTerminateExecutorOnTimeout()
      throws ExecutionException, InterruptedException, TimeoutException {
    // given
    GracefulExecutorServicesShutdown shutdown = GracefulExecutorServicesShutdown.initiate();
    ...
    when(executorService.shutdownNow())
        .thenAnswer(
            invocation -> {
              terminated.set(true);
              return null;
            });
    when(executorService.awaitTermination(anyLong(), any()))
        .thenAnswer(
            invocation -> {
  ...
  @Test
  public void shouldWaitForExecutorToTerminate()
      throws ExecutionException, InterruptedException, TimeoutException {
    // given
    GracefulExecutorServicesShutdown shutdown = GracefulExecutorServicesShutdown.initiate();
    shutdown.timeout(Duration.ofMillis(500));
  ...
  @Test
  public void shouldTerminateWhenFutureIsCancelled()
  ...
  public void shouldAcceptNullReferenceAndIgnoreIt() {
    ...
    CompletableFuture<Void> future =
        GracefulExecutorServicesShutdown.initiate().shutdown(executorService).handle();
    assertTrue(future.isDone());
  }
}
