...
public class SLAMonitoringTest {
  ...
  void setup() throws Exception {
    ...
    for (int i = 0; i < BROKER_COUNT; i++) {
      var namespaceName =
          NamespaceService.getSLAMonitorNamespace(
              pulsarServices[i].getBrokerId(), pulsarServices[i].getConfig());
    ...
    try {
      for (int i = 0; i < BROKER_COUNT; i++) {
        ...
        Map<String, NamespaceOwnershipStatus> nsMap =
            pulsarAdmins[i].brokers().getOwnedNamespaces("my-cluster", list.get(0));
      ...
      try {
        String topic =
            String.format(
                ...
                "my-cluster",
                pulsarServices[i].getBrokerId(),
                "my-topic");
        assertEquals(
            pulsarAdmins[0].lookups().lookupTopic(topic),
            "pulsar://"
        ...
        e.printStackTrace();
        fail(
            "SLA Namespace should have been owned by the broker("
                + "pulsar://"
                + pulsarServices[i].getAdvertisedAddress()
                + ":"
                + brokerNativeBrokerPorts[i]
  ...
  public void testUnloadIfBrokerCrashes() {
    ...
    try {
      ...
      assertNotEquals(
          broker,
          "pulsar://"
              + pulsarServices[crashIndex].getAdvertisedAddress()
              + ":"
