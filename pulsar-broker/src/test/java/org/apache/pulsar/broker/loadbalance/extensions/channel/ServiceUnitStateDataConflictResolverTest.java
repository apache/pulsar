...
public class ServiceUnitStateDataConflictResolverTest {
  ...
  public void testStoreType() {
    ...
    assertFalse(strategy.shouldKeepLeft(null, new ServiceUnitStateData(Owned, dst, 1)));
    assertFalse(strategy.shouldKeepLeft(null, new ServiceUnitStateData(Owned, dst, 2)));
    assertFalse(
        strategy.shouldKeepLeft(
            new ServiceUnitStateData(Owned, dst, 1), new ServiceUnitStateData(Owned, dst, 3)));

    strategy.setStorageType(MetadataStore);
    assertFalse(strategy.shouldKeepLeft(null, new ServiceUnitStateData(Owned, dst, 1)));
    assertTrue(strategy.shouldKeepLeft(null, new ServiceUnitStateData(Owned, dst, 2)));
    assertTrue(
        strategy.shouldKeepLeft(
            new ServiceUnitStateData(Owned, dst, 1), new ServiceUnitStateData(Owned, dst, 3)));
  }
  ...
  public void testTransitionsAndBrokers() {
    ...
    assertTrue(
        strategy.shouldKeepLeft(data(Releasing, src, "dst1"), data2(Assigning, src, "dst2")));
    assertTrue(
        strategy.shouldKeepLeft(data(Releasing, src, "dst1"), data2(Assigning, src, "dst2")));
    assertTrue(
        strategy.shouldKeepLeft(data(Releasing, "src1", dst), data2(Assigning, "src2", dst)));
