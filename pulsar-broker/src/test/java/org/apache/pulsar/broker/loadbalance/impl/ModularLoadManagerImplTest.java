...
public class ModularLoadManagerImplTest {
  ...
  private Object invokeSimpleMethod(
      final Object instance, final String methodName, final Object... args) throws Exception {
  ...
  void setup() throws Exception {
    ...
    config1.setLoadBalancerLoadSheddingStrategy(
    ...
    config2.setLoadBalancerLoadSheddingStrategy(
        "org.apache.pulsar.broker.loadbalance.impl.OverloadShedder");
    ...
    config.setLoadBalancerLoadSheddingStrategy(
        "org.apache.pulsar.broker.loadbalance.impl.OverloadShedder");
    ...
    primaryLoadManager =
        (ModularLoadManagerImpl) getField(pulsar1.getLoadManager().get(), "loadManager");
    secondaryLoadManager =
        (ModularLoadManagerImpl) getField(pulsar2.getLoadManager().get(), "loadManager");
  ...
  public void testCandidateConsistency() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(() -> assertFalse(loadData.getBrokerData().containsKey(secondaryBrokerId)));
  ...
  public void testEvenBundleDistribution() throws Exception {
    final NamespaceBundle[] bundles =
        LoadBalancerTestingUtils.makeBundles(nsFactory, "test", "test", "test", 16);
    ...
    // Write long message rate for first bundle to ensure that even bundle distribution is not a
    // coincidence of
    // balancing by message rate. If we were balancing by message rate, one of the brokers should
    // only have this
    // one bundle.
    pulsar1
        .getLocalMetadataStore()
        .getMetadataCache(BundleData.class)
        .create(firstBundleDataPath, bundleData)
        .join();
    for (final NamespaceBundle bundle : bundles) {
      ...
      if ((numAssignedToPrimary + numAssignedToSecondary) % 2 == 0) {
        // On even number of assignments, assert that an equal number of bundles have been assigned
        // between
        // them.
  ...
  public void testBrokerAffinity() throws Exception {
    ...
    admin1
        .clusters()
        .createCluster(
            cluster, ClusterData.builder().serviceUrl(pulsar1.getWebServiceAddress()).build());
    admin1
        .tenants()
        .createTenant(
            tenant,
    ...
    log.debug(
        "leader lookup address - {}, broker1 lookup address - {}",
        leaderBrokerId,
        pulsar1.getBrokerId());
  ...
  /**
   * It verifies that once broker owns max-number of topics: load-manager doesn't allocates new
   * bundles to that broker unless all the brokers are in same state.
   *
   * <pre>
  ...
  public void testMaxTopicDistributionToBroker() throws Exception {
    ...
    final NamespaceBundle[] bundles =
        LoadBalancerTestingUtils.makeBundles(nsFactory, "test", "test", "test", totalBundles);
    final BundleData bundleData = new BundleData(10, 1000);
    ...
    for (int i = 1; i < totalBundles; i++) {
      assertNotEquals(
          primaryLoadManager.selectBrokerForAssignment(bundles[i]), maxTopicOwnedBroker);
    }
  }
  ...
  public void testFilterBundlesWhileWritingToMetadataStore() throws Exception {
    ...
    MetadataCache<BundleData> metadataCache =
        pulsar1.getLocalMetadataStore().getMetadataCache(BundleData.class);
    String protocol = "http://";
    PulsarService leaderBroker =
        pulsarServices.get(
            protocol + pulsar1.getLeaderElectionService().getCurrentLeader().get().getBrokerId());
    ModularLoadManagerImpl loadManager =
        (ModularLoadManagerImpl) getField(leaderBroker.getLoadManager().get(), "loadManager");
    int topK = 1;
    ...
    for (int i = 0; i < totalBundles; i++) {
      ...
      loadData
          .getBrokerData()
          .get(leaderBroker.getWebServiceAddress().substring(protocol.length()))
          .getLocalData()
          .getLastStats()
          .put(bundles[i].toString(), new NamespaceBundleStats());
      metadataCache.create(bundleDataPath, bundleData).join();
    }
    for (int i = 0; i < totalBundles; i++) {
      final String bundleDataPath = String.format("%s/%s", BUNDLE_DATA_BASE_PATH, bundles[i]);
      assertEquals(
          metadataCache.getWithStats(bundleDataPath).get().get().getStat().getVersion(), 0);
    }
    ...
    for (int i = filterBundleCount; i < totalBundles; i++) {
      final String bundleDataPath = String.format("%s/%s", BUNDLE_DATA_BASE_PATH, bundles[i]);
      assertEquals(
          metadataCache.getWithStats(bundleDataPath).get().get().getStat().getVersion(), 1);
    }
  }
  ...
  public void testLoadShedding() throws Exception {
    ...
    doAnswer(
            invocation -> {
              bundleReference.set(
                  invocation.getArguments()[0].toString() + '/' + invocation.getArguments()[1]);
              return null;
            })
        .when(namespacesSpy1)
    ...
    primaryLoadManagerSpy.handleDataNotification(
        new Notification(
            NotificationType.Created, LoadManager.LOADBALANCE_BROKERS_ROOT + "/broker:8080"));
    ...
    primaryLoadManagerSpy.doLoadShedding();
    // Now less expensive bundle will be unloaded (normally other bundle would move off and nothing
    // would be
  ...
  // Test that ModularLoadManagerImpl will determine that writing local data to ZooKeeper is
  ...
  /**
   * It verifies that deletion of broker-znode on broker-stop will invalidate availableBrokerCache
   * list
   */
  @Test
  ...
  public void testNamespaceIsolationPoliciesForPrimaryAndSecondaryBrokers() throws Exception {
    ...
    SimpleResourceAllocationPolicies simpleResourceAllocationPolicies =
        new SimpleResourceAllocationPolicies(pulsar1);
    ServiceUnitId serviceUnit =
        LoadBalancerTestingUtils.makeBundles(nsFactory, tenant, cluster, namespace, 1)[0];
    BrokerTopicLoadingPredicate brokerTopicLoadingPredicate =
        new BrokerTopicLoadingPredicate() {
          @Override
    ...
    newPolicyJson =
        String.format(
            newPolicyJsonTemplate, tenant, cluster, namespace, broker1Host, broker2Host, 2);
  ...
  public void testLoadSheddingWithNamespaceIsolationPolicies() throws Exception {
    ...
    Producer<byte[]> producer =
        pulsarClient.newProducer().topic("persistent://" + namespace + "/my-topic1").create();
    ModularLoadManagerImpl loadManager =
    ...
    String newPolicyJson =
        String.format(newPolicyJsonTemplate, namespace, broker1Host, broker1Host, 1);
    String newPolicyName = "my-ns-isolation-policies";
    ObjectMapper jsonMapper = ObjectMapperFactory.create();
    NamespaceIsolationDataImpl nsPolicyData =
        jsonMapper.readValue(newPolicyJson.getBytes(), NamespaceIsolationDataImpl.class);
    ...
    String newPolicyJson1 =
        String.format(newPolicyJsonTemplate, namespace, brokerHost, brokerHost, 1);
    NamespaceIsolationDataImpl nsPolicyData1 =
        jsonMapper.readValue(newPolicyJson1.getBytes(), NamespaceIsolationDataImpl.class);
  ...
  /**
   * It verifies that pulsar-service fails if load-manager tries to create ephemeral znode for
   * broker which is already created by other zk-session-id.
   *
   * @throws Exception
   */
  @Test
  public void testOwnBrokerZnodeByMultipleBroker() throws Exception {
    ...
    pulsar1
        .getLocalMetadataStore()
        .put(brokerZnode, new byte[0], Optional.empty(), EnumSet.of(CreateOption.Ephemeral))
        .join();
    try {
      pulsar.start();
  ...
  public void testRemoveDeadBrokerTimeAverageData() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(() -> assertTrue(pulsar1.getLeaderElectionService().isLeader()));
  ...
  public void testBundleDataDefaultValue(boolean isV1) throws Exception {
    ...
    final String namespace = "my-ns";
    NamespaceName ns =
        isV1 ? NamespaceName.get(tenant, cluster, namespace) : NamespaceName.get(tenant, namespace);
    admin1
        .clusters()
        .createCluster(
            cluster, ClusterData.builder().serviceUrl(pulsar1.getWebServiceAddress()).build());
    ...
    NamespaceBundle namespaceBundle =
        nsFactory.getBundle(
            ns,
            Range.range(
                Long.decode(bundlesData.getBoundaries().get(0)),
                BoundType.CLOSED,
                Long.decode(bundlesData.getBoundaries().get(1)),
                BoundType.OPEN));
    ...
    admin1
        .resourceQuotas()
        .setNamespaceBundleResourceQuota(ns.toString(), namespaceBundle.getBundleRange(), quota);
    ...
    // The default value of the bundleData be the same as resourceQuota because the resourceQuota is
    // present.
  ...
  public void testRemoveNonExistBundleData()
    ...
    ModularLoadManagerWrapper loadManager2 =
        (ModularLoadManagerWrapper) pulsar2.getLoadManager().get();
    ...
    NamespaceBundles allBundlesAfterSplit =
        pulsar1.getNamespaceService().getNamespaceBundleFactory().getBundles(namespaceName);
