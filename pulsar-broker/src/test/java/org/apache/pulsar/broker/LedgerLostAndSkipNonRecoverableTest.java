...
public class LedgerLostAndSkipNonRecoverableTest extends ProducerConsumerBase {
  ...
  public void testMarkDeletedPositionCanForwardAfterTopicLedgerLost(boolean enabledBatch)
      throws Exception {
    ...
    int sendMessageCount =
        Arrays.asList(sendMessages).stream()
            .flatMap(s -> s.stream())
    ...
    waitMarkDeleteLargeAndEquals(
        topicName, subName, lastPosition.getLedgerId(), lastPosition.getEntryId());
  ...
  private List<MessageIdImpl>[] sendManyMessages(
      String topicName, int ledgerCount, int messageCountPerLedger, int messageCountPerEntry)
      throws Exception {
  ...
  private List<MessageIdImpl> sendManyMessages(
      String topicName, int messageCountPerLedger, int messageCountPerEntry) throws Exception {
    if (messageCountPerEntry == 1) {
  ...
  private List<MessageIdImpl> sendManyBatchedMessages(
      String topicName, int msgCountPerEntry, int entryCount) throws Exception {
    ...
    return messageIdFutures.stream()
        .map(f -> (MessageIdImpl) f.join())
        .collect(Collectors.toList());
  }
  ...
  private List<MessageIdImpl>[] sortMessageId(
      List<MessageIdImpl> messageIds, boolean enabledBatch) {
  ...
  private Consumer<String> createConsumer(String topicName, String subName, boolean enabledBatch)
      throws Exception {
