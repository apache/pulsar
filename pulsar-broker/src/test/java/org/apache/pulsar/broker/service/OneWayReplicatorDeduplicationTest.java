...
public class OneWayReplicatorDeduplicationTest extends OneWayReplicatorTestBase {
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              assertTrue(
                  persistentTopic1.getReplicators().isEmpty()
  ...
  protected Runnable injectReplicatorClientCnx(
    ...
    ClientBuilderImpl clientBuilder2 =
        (ClientBuilderImpl) PulsarClient.builder().serviceUrl(url2.toString());
  ...
  public Object[][] deduplicationArgs() {
    return new Object[][] {
      {
        true /* inject repeated publishing*/,
        1 /* repeated messages window */,
        true /* supportsReplDedupByLidAndEid */,
        false /* multi schemas */
      },
      {
        true /* inject repeated publishing*/,
        2 /* repeated messages window */,
        true /* supportsReplDedupByLidAndEid */,
        false /* multi schemas */
      },
      {
        true /* inject repeated publishing*/,
        3 /* repeated messages window */,
        true /* supportsReplDedupByLidAndEid */,
        false /* multi schemas */
      },
      {
        true /* inject repeated publishing*/,
        4 /* repeated messages window */,
        true /* supportsReplDedupByLidAndEid */,
        false /* multi schemas */
      },
      {
        true /* inject repeated publishing*/,
        5 /* repeated messages window */,
        true /* supportsReplDedupByLidAndEid */,
        false /* multi schemas */
      },
      {
        true /* inject repeated publishing*/,
        10 /* repeated messages window */,
        true /* supportsReplDedupByLidAndEid */,
        false /* multi schemas */
      },
      // ===== multi schema
      ...
      {
        false /* inject repeated publishing*/,
        0 /* repeated messages window */,
        false /* supportsReplDedupByLidAndEid */,
        false /* multi schemas */
      },
      {
        false /* inject repeated publishing*/,
  ...
  public void testDeduplication(
      final boolean injectRepeatedPublish,
      final int repeatedMessagesWindow,
      final boolean supportsReplDedupByLidAndEid,
            ...
            (conf, eventLoopGroup) ->
                new ClientCnx(InstrumentProvider.NOOP, conf, eventLoopGroup) {
                  ...
                  protected ByteBuf newConnectCommand() throws Exception {
                    ...
                    BaseCommand cmd =
                        Commands.newConnectWithoutSerialize(
                  ...
                  public ChannelHandlerContext ctx() {
                    ...
                    Answer injectedAnswer =
                        invocation -> {
                          // Do not repeat the messages re-sending, and clear the previous cached
                          // messages when
                          ...
                          if (cmd.getType().equals(BaseCommand.Type.SEND)) {
                            synchronized (duplicatedMsgs) {
                              if (duplicatedMsgs.size() >= repeatedMessagesWindow) {
                                for (ByteBufPair bufferPair : duplicatedMsgs) {
                                  originalCtx
                                      .channel()
                                      .write(bufferPair, originalCtx.voidPromise());
                            ...
                            ByteBuf newBuffer1 =
                                UnpooledByteBufAllocator.DEFAULT.heapBuffer(buf1.readableBytes());
                            buf1.readBytes(newBuffer1);
                            buf1.readerIndex(bufferIndex1);
                            ByteBuf newBuffer2 =
                                UnpooledByteBufAllocator.DEFAULT.heapBuffer(buf2.readableBytes());
                    ...
                    doAnswer(injectedAnswer)
                        .when(spyContext)
                        .write(any(), any(ChannelPromise.class));
                    doAnswer(injectedAnswer).when(spyContext).writeAndFlush(any());
                    doAnswer(injectedAnswer)
                        .when(spyContext)
                        .writeAndFlush(any(), any(ChannelPromise.class));
                    return spyContext;
                  }
                });
    ...
    final String topicName =
        BrokerTestUtil.newUniqueName("persistent://" + nonReplicatedNamespace + "/tp_");
            ...
            () -> {
              ...
              admin1
                  ...
                  .setSchemaCompatibilityStrategy(
                      topicName, SchemaCompatibilityStrategy.ALWAYS_COMPATIBLE);
              admin2.topicPolicies().setDeduplicationStatus(topicName, true);
              admin2
                  .topicPolicies()
                  .setSchemaCompatibilityStrategy(
                      topicName, SchemaCompatibilityStrategy.ALWAYS_COMPATIBLE);
              MessageDeduplication messageDeduplication1 =
                  persistentTopic1.getMessageDeduplication();
              if (messageDeduplication1 != null) {
                int snapshotInterval1 =
                    WhiteboxImpl.getInternalState(messageDeduplication1, "snapshotInterval");
              ...
              if (messageDeduplication2 != null) {
                int snapshotInterval2 =
                    WhiteboxImpl.getInternalState(messageDeduplication2, "snapshotInterval");
                assertEquals(snapshotInterval2, 10);
              }
              assertEquals(
                  persistentTopic1.getHierarchyTopicPolicies().getDeduplicationEnabled().get(),
                  Boolean.TRUE);
              assertEquals(
                  persistentTopic1
              ...
              assertEquals(
                  persistentTopic2.getHierarchyTopicPolicies().getDeduplicationEnabled().get(),
                  Boolean.TRUE);
              assertEquals(
                  persistentTopic2
                      .getHierarchyTopicPolicies()
    ...
    while (true) {
      ...
      log.info(
          "received msg. source {}, target {}:{}",
          StringUtils.join(msg.getProperties().values(), ":"),
  ...
  public void testDeduplicationNotLostMessage(boolean enabledDeduplication) throws Exception {
    ...
    /**
     * step-2: Inject a mechanism that makes the client connect broke after repeatedly copied M1 and
     * M2.
     */
    final List<ByteBufPair> duplicatedMsgs = new ArrayList<>();
                    ...
                    Answer injectedAnswer =
                        invocation -> {
                          ...
                          if (cmd.getType().equals(BaseCommand.Type.SEND)) {
                            ...
                            synchronized (duplicatedMsgs) {
                              if (newBuffer2.readableBytes() > 0
                                  && msgSent.incrementAndGet() <= repeatMsgIndex) {
                                duplicatedMsgs.add(ByteBufPair.get(newBuffer1, newBuffer2));
                              }
                            }
                            return invocation.callRealMethod();
    ...
    // Let broker persist messages one by one, in other words, it starts to persist the next message
    // after the
    ...
    doAnswer(
        ...
        .when(spyTp2)
        .publishMessage(any(ByteBuf.class), any(Topic.PublishContext.class));
    CompletableFuture<Optional<Topic>> originalTp2 =
        pulsar2
            .getBrokerService()
            .getTopics()
            .put(tp2.getName(), CompletableFuture.completedFuture(Optional.of(spyTp2)));

    /** Step-1: Publishes messages in the source cluster and start replication, */
    ProducerImpl p1 =
        (ProducerImpl) client1.newProducer().topic(topicName).producerName("p1").create();
    ProducerImpl p2 =
        (ProducerImpl) client1.newProducer().topic(topicName).producerName("p2").create();
    ...
    List<String> msgReceived = new ArrayList<>();
    Consumer consumer = client2.newConsumer().topic(topicName).subscriptionName("s1").subscribe();
    while (true) {
  ...
  public void testReplicationLoadSchemaTimeout(boolean enabledDeduplication) throws Exception {
    ...
    Field filedSchemaRegistryService =
        PulsarService.class.getDeclaredField("schemaRegistryService");
    ...
    doAnswer(
        ...
        .when(spySchemaRegistryService)
        .findSchemaVersion(any(String.class), any(SchemaData.class));
    ...
    Runnable taskToClearInjection =
        injectReplicatorClientCnx(
            (conf, eventLoopGroup) ->
                new ClientCnx(InstrumentProvider.NOOP, conf, eventLoopGroup) {
                  @Override
                  protected void handleGetSchemaResponse(
                    ...
                    future.completeExceptionally(
                        new PulsarClientException.TimeoutException("Mocked timeout"));
                  }
