...
public class TransactionBufferClientTest extends TransactionTestBase {
  ...
  protected void setup() throws Exception {
    ...
    admin
        .tenants()
        .createTenant(
            "public", new TenantInfoImpl(Sets.newHashSet("appid1"), Sets.newHashSet(CLUSTER_NAME)));
    admin.namespaces().createNamespace(namespace, 10);
    admin
        .topics()
        .createPartitionedTopic(partitionedTopicName.getPartitionedTopicName(), partitions);
    tbClient =
        TransactionBufferClientImpl.create(
  ...
  @Test
  public void testRecoveryTransactionBufferWhenCommonTopicAndSystemTopicAtDifferentBroker()
      throws Exception {
    for (int i = 0; i < getPulsarServiceList().size(); i++) {
      getPulsarServiceList().get(i).getConfig().setTransactionBufferSegmentedSnapshotEnabled(true);
    }
    String topic1 =
        NAMESPACE1 + "/testRecoveryTransactionBufferWhenCommonTopicAndSystemTopicAtDifferentBroker";
    admin
        .tenants()
        .createTenant(
    ...
    @Cleanup
    PulsarClient localPulsarClient =
    ...
    Transaction transaction =
        localPulsarClient.newTransaction().withTransactionTimeout(5, TimeUnit.HOURS).build().get();
  ...
  public void testTransactionBufferMetrics() throws Exception {
    ...
    for (int i = 0; i < partitions; i++) {
      ...
      Optional<Metric> optional1 =
          commitLatencyCount.stream()
              .filter(metric -> metric.tags.get("topic").equals(topic))
              .findFirst();
  ...
  public void testTransactionBufferClientTimeout() throws Exception {
    ...
    when(mockClient.getConnection(anyString(), anyInt()))
        .thenReturn(CompletableFuture.completedFuture(Pair.of(clientCnx, false)));
    ...
    when(pulsarService.getClient())
        .thenAnswer(
            new Answer<PulsarClient>() {
