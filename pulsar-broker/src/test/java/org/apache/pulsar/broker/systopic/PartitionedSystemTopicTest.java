...
public class PartitionedSystemTopicTest extends BrokerTestBase {
  ...
  public void testAutoCreatedPartitionedSystemTopic() throws Exception {
    ...
    NamespaceEventsSystemTopicFactory systemTopicFactory =
        new NamespaceEventsSystemTopicFactory(pulsarClient);
    TopicPoliciesSystemTopicClient systemTopicClientForNamespace =
        systemTopicFactory.createTopicPoliciesSystemTopicClient(NamespaceName.get(ns));
    ...
    int partitions =
        admin
            ...
            .getPartitionedTopicMetadata(
                String.format(
                    "persistent://%s/%s", ns, SystemTopicNames.NAMESPACE_EVENTS_LOCAL_NAME))
            .partitions;
    List<String> partitionedTopicList =
        admin
            .topics()
            .getPartitionedTopicList(
                ns, ListTopicsOptions.builder().includeSystemTopic(true).build());
    Assert.assertEquals(partitionedTopicList.size(), 1);
    Assert.assertEquals(partitions, PARTITIONS);
    Assert.assertEquals(
        admin
            .topics()
            .getList(ns, null, ListTopicsOptions.builder().includeSystemTopic(true).build())
            .size(),
        PARTITIONS);
    reader.close();
  }
  ...
  public void testConsumerCreationWhenEnablingTopicPolicy() throws Exception {
    ...
    admin
        .tenants()
        .createTenant(tenant, new TenantInfoImpl(new HashSet<>(), Sets.newHashSet("test")));
    ...
    for (int i = 0; i < namespaceCount; i++) {
      ...
      futureList.add(
          pulsarClient.newConsumer().topic(topic).subscriptionName("sub").subscribeAsync());
    }
  ...
  public void testProduceAndConsumeUnderSystemNamespace() throws Exception {
    ...
    Producer<byte[]> producer =
        pulsarClient.newProducer().topic("pulsar/system/__topic-1").create();
  ...
  public void testHealthCheckTopicNotOffload() throws Exception {
    ...
    TopicName topicName =
        TopicName.get("persistent", namespaceName, HealthChecker.HEALTH_CHECK_TOPIC_SUFFIX);
    PersistentTopic persistentTopic =
        (PersistentTopic)
            pulsar.getBrokerService().getTopic(topicName.toString(), true).get().get();
  ...
  public void testSystemNamespaceNotCreateChangeEventsTopic() throws Exception {
    ...
    NamespaceName namespaceName =
        NamespaceService.getHeartbeatNamespaceV2(pulsar.getBrokerId(), pulsar.getConfig());
    TopicName topicName =
        TopicName.get("persistent", namespaceName, SystemTopicNames.NAMESPACE_EVENTS_LOCAL_NAME);
    Optional<Topic> optionalTopic =
        pulsar.getBrokerService().getTopic(topicName.getPartition(1).toString(), false).join();
    Assert.assertTrue(optionalTopic.isEmpty());

    TopicName heartbeatTopicName =
        TopicName.get("persistent", namespaceName, HealthChecker.HEALTH_CHECK_TOPIC_SUFFIX);
  ...
  public void testHeartbeatTopicNotAllowedToSendEvent() throws Exception {
    ...
    for (int partition = 0; partition < PARTITIONS; partition++) {
      pulsar.getBrokerService().getTopic(topicName.getPartition(partition).toString(), true).join();
    }
    Assert.assertThrows(
        PulsarAdminException.ConflictException.class,
        () -> {
  ...
  public void testHeartbeatNamespaceNotCreateTransactionInternalTopic() throws Exception {
    ...
    NamespaceName namespaceName =
        NamespaceService.getHeartbeatNamespaceV2(pulsar.getBrokerId(), pulsar.getConfig());
    TopicName topicName =
        TopicName.get("persistent", namespaceName, SystemTopicNames.TRANSACTION_BUFFER_SNAPSHOT);
    Optional<Topic> optionalTopic =
        pulsar.getBrokerService().getTopic(topicName.getPartition(1).toString(), false).join();
    Assert.assertTrue(optionalTopic.isEmpty());
  ...
  public void testSetBacklogCausedCreatingProducerFailure() throws Exception {
    ...
    String partition0 =
        TopicName.get(String.format("persistent://%s", topic)).getPartition(0).toString();
  ...
  public void testDeleteTopicSchemaAndPolicyWhenTopicIsNotLoaded() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(() -> assertEquals(admin.topicPolicies().getMaxConsumers(topicName), 2));
    ...
    TopicPolicies topicPolicies =
        TopicPolicyTestUtils.getTopicPolicies(
            pulsar.getTopicPoliciesService(), TopicName.get(topicName));
