...
public class PendingAckPersistentTest extends TransactionTestBase {
  ...
  /**
   * Test consumer can be built successfully with retryable exception and get correct error with
   * no-retryable exception.
   *
   * @throws Exception
   */
  @Test(timeOut = 60000, dataProvider = "retryableErrors")
  public void testBuildConsumerEncounterPendingAckInitFailure(Exception retryableError)
      throws Exception {
    ...
    Field transactionPendingAckStoreProviderField =
        PulsarService.class.getDeclaredField("transactionPendingAckStoreProvider");
    ...
    TransactionPendingAckStoreProvider pendingAckStoreProvider =
        (TransactionPendingAckStoreProvider)
            transactionPendingAckStoreProviderField.get(pulsarServiceList.get(0));
    ...
    Consumer<byte[]> consumer3 =
        pulsarClient.newConsumer().subscriptionName("subName3").topic(topic).subscribe();

    // 5. Test no-retryable exception:
    // The consumer building will be failed without retrying.
    when(mockProvider.checkInitializedBefore(any()))
        // The method checkInitializedBefore will fail with a no-retryable exception without
        // retrying.
        .thenReturn(
            FutureUtil.failedFuture(
                new ManagedLedgerException.NonRecoverableLedgerException("mock fail")))
        .thenReturn(CompletableFuture.completedFuture(false));
    ...
    try {
      ...
      Consumer<byte[]> consumer4 =
          pulsarClient.newConsumer().subscriptionName("subName4").topic(topic).subscribe();
      fail();
    ...
    PersistentTopic topic =
        (PersistentTopic)
            getPulsarServiceList()
                .get(0)
                .getBrokerService()
                .getTopic(TopicName.get(PENDING_ACK_REPLAY_TOPIC).toString(), false)
                .get()
                .get();
        ...
        .until(
            () ->
                (managedCursor.getMarkDeletedPosition())
                        .compareTo(managedCursor.getManagedLedger().getLastConfirmedEntry())
                    == -1);
  }
  ...
  public void testPendingAckMetrics() throws Exception {
    ...
    Producer<String> producer =
        pulsarClient.newProducer(Schema.STRING).topic(PENDING_ACK_REPLAY_TOPIC).create();
    ...
    for (Metric metric : abortedCount) {
      if (metric.tags.get("subscription").equals(subName)
          && metric.tags.get("status").equals("succeed")) {
  ...
  public void cumulativePendingAckReplayTest() throws Exception {
    ...
    getPulsarServiceList()
        .get(0)
        .getConfig()
        .setTransactionPendingAckLogIndexMinLag(4 * messageCount + 2);
  ...
  private void testDeleteSubThenDeletePendingAckManagedLedger() throws Exception {
    ...
    String topic =
        TopicName.get(
                TopicDomain.persistent.toString(), NamespaceName.get(NAMESPACE1), "test-delete")
            .toString();
    @Cleanup
    Consumer<byte[]> consumer =
        pulsarClient
    ...
    assertFalse(
        topics.contains(MLPendingAckStore.getTransactionPendingAckStoreSuffix(topic, subName)));
  ...
  private void testDeleteTopicThenDeletePendingAckManagedLedger() throws Exception {
    ...
    assertFalse(
        topics.contains(MLPendingAckStore.getTransactionPendingAckStoreSuffix(topic, subName1)));
    assertFalse(
        topics.contains(MLPendingAckStore.getTransactionPendingAckStoreSuffix(topic, subName2)));
  ...
  public void testDeleteUselessLogDataWhenSubCursorMoved() throws Exception {
    ...
    String topic =
        TopicName.get(
                TopicDomain.persistent.toString(), NamespaceName.get(NAMESPACE1), "test-log-delete")
            .toString();
  ...
  public void testPendingAckLowWaterMarkRemoveFirstTxn() throws Exception {
    String topic =
        TopicName.get(TopicDomain.persistent.toString(), NamespaceName.get(NAMESPACE1), "test")
            .toString();
    ...
    field1.setAccessible(true);
    PendingAckHandleImpl oldPendingAckHandle =
    ...
    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(oldIndividualAckOfTransaction.size(), 0));
  ...
  public void testTransactionConflictExceptionWhenAckBatchMessage() throws Exception {
    ...
    pulsarServiceList
        .get(0)
        .getBrokerService()
        .getManagedLedgerConfig(TopicName.get(topic))
        .get()
    ...
    Transaction transaction =
        pulsarClient.newTransaction().withTransactionTimeout(5, TimeUnit.DAYS).build().get();
  ...
  public void testGetSubPatternTopicFilterTxnInternalTopic() throws Exception {
    ...
    admin
        .namespaces()
        .getTopics(NAMESPACE1)
        .forEach(
            name -> assertFalse(SystemTopicNames.isTransactionInternalName(TopicName.get(name))));
  ...
  public void testGetManagedLegerConfigFailThenUnload() throws Exception {
    String topic =
        TopicName.get(
                TopicDomain.persistent.toString(),
                NamespaceName.get(NAMESPACE1),
                "testGetManagedLegerConfigFailThenUnload")
            .toString();
            ...
            invocation -> {
              if (isGetManagedLedgerConfigFail.get()) {
                return FutureUtil.failedFuture(
                    new BrokerServiceException.ServiceUnitNotReadyException("test"));
              } else {
