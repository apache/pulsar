...
public class WebServiceOriginalClientIPTest extends MockedPulsarServiceBaseTest {
  ...
  public void testClientIPIsPickedFromXForwardedForHeaderAndLogged(boolean tlsEnabled)
      throws Exception {
    String metricsUrl =
        (tlsEnabled ? pulsar.getWebServiceAddressTls() : pulsar.getWebServiceAddress())
            + "/metrics/";
    performLoggingTest(
        consoleCaptor -> {
  ...
  @Test(dataProvider = "tlsEnabled")
  public void testClientIPIsPickedFromForwardedHeaderAndLogged(boolean tlsEnabled)
      throws Exception {
    String metricsUrl =
        (tlsEnabled ? pulsar.getWebServiceAddressTls() : pulsar.getWebServiceAddress())
            + "/metrics/";
    performLoggingTest(
        consoleCaptor -> {
          // Send a GET request to the metrics URL
          ContentResponse response =
              httpClient.newRequest(metricsUrl).header("Forwarded", "for=11.22.33.44:12345").send();
  ...
  @Test(dataProvider = "tlsEnabled")
  public void testClientIPIsPickedFromHAProxyProtocolAndLogged(boolean tlsEnabled)
      throws Exception {
    String metricsUrl =
        (tlsEnabled ? pulsar.getWebServiceAddressTls() : pulsar.getWebServiceAddress())
            + "/metrics/";
        ...
        consoleCaptor -> {
          ...
          ContentResponse response =
              httpClient
                  ...
                  .tag(
                      new V2.Tag(
                          V2.Tag.Command.PROXY,
                          null,
                          V2.Tag.Protocol.STREAM,
                          // source IP and port
                          "99.22.33.44",
                          1234,
