...
public class PersistentFailoverE2ETest extends BrokerTestBase {
  ...
  private void verifyConsumerActive(TestConsumerStateEventListener listener, int partitionId)
      throws Exception {
  ...
  private void verifyConsumerInactive(TestConsumerStateEventListener listener, int partitionId)
      throws Exception {
  ...
  FailoverConsumer createConsumer(
      String topicName, String subName, String listenerName, String consumerName) throws Exception {
  ...
  public void testSimpleConsumerEventsWithoutPartition() throws Exception {
    ...
    // 5. master consumer failure should resend unacked messages and new messages to another
    // consumer
    for (int i = 0; i < 5; i++) {
      msg = firstConsumer.consumer.receive(1, TimeUnit.SECONDS);
      Assert.assertNotNull(msg);
  ...
  public void testSimpleConsumerEventsWithPartition() throws Exception {
    // Resetting ActiveConsumerFailoverDelayTimeMillis else if testActiveConsumerFailoverWithDelay
    // get executed
    ...
    topicRef =
        (PersistentTopic)
            pulsar.getBrokerService().getTopicReference(destName.getPartition(0).toString()).get();
    PersistentDispatcherSingleActiveConsumer disp0 =
        (PersistentDispatcherSingleActiveConsumer)
            topicRef.getSubscription(subName).getDispatcher();
    topicRef =
        (PersistentTopic)
            pulsar.getBrokerService().getTopicReference(destName.getPartition(1).toString()).get();
    PersistentDispatcherSingleActiveConsumer disp1 =
        (PersistentDispatcherSingleActiveConsumer)
            topicRef.getSubscription(subName).getDispatcher();
    topicRef =
        (PersistentTopic)
            pulsar.getBrokerService().getTopicReference(destName.getPartition(2).toString()).get();
    PersistentDispatcherSingleActiveConsumer disp2 =
        (PersistentDispatcherSingleActiveConsumer)
            topicRef.getSubscription(subName).getDispatcher();
    topicRef =
        (PersistentTopic)
            pulsar.getBrokerService().getTopicReference(destName.getPartition(3).toString()).get();
    PersistentDispatcherSingleActiveConsumer disp3 =
        (PersistentDispatcherSingleActiveConsumer)
            topicRef.getSubscription(subName).getDispatcher();
    ...
    Consumer<byte[]> consumer3 =
        consumerBuilder.clone().consumerName("3").receiverQueueSize(1).subscribe();
    Thread.sleep(CONSUMER_ADD_OR_REMOVE_WAIT_TIME);
    while (true) {
      msg = consumer1.receive(1, TimeUnit.SECONDS);
      if (msg == null) {
