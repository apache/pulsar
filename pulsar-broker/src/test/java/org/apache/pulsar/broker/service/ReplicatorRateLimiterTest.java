...
public class ReplicatorRateLimiterTest extends ReplicatorTestBase {
  ...
  public void testReplicatorRateLimiterWithOnlyTopicLevel() throws Exception {
    ...
    final String topicName =
        "persistent://" + namespace + "/testReplicatorRateLimiterWithOnlyTopicLevel";
  ...
  public void testReplicatorRateLimiterWithOnlyNamespaceLevel() throws Exception {
    ...
    final String namespace = "pulsar/replicatorchange-" + System.currentTimeMillis();
    final String topicName =
        "persistent://" + namespace + "/testReplicatorRateLimiterWithOnlyNamespaceLevel";
  ...
  public void testReplicatorRateLimiterWithOnlyBrokerLevel() throws Exception {
    ...
    final String namespace = "pulsar/replicatorchange-" + System.currentTimeMillis();
    final String topicName =
        "persistent://" + namespace + "/testReplicatorRateLimiterWithOnlyBrokerLevel";
            ...
            () -> {
              ...
              assertEquals(
                  admin1
                      .brokers()
                      .getAllDynamicConfigurations()
                      .get("dispatchThrottlingRatePerReplicatorInMsg"),
                  "10");
              assertEquals(
                  admin1
                      .brokers()
                      .getAllDynamicConfigurations()
                      .get("dispatchThrottlingRatePerReplicatorInByte"),
                  "20");
            });
   ...
   *
   * <p>1. verify default replicator not configured. 2. change namespace setting of replicator
   * dispatchRateMsg, verify topic changed. 3. change namespace setting of replicator
   * dispatchRateByte, verify topic changed.
   *
   * @throws Exception
   */
  @Test
   ...
   *
   * <p>1. verify topic replicator get configured. 2. namespace setting of replicator dispatchRate,
   * verify consumer in other cluster could not receive all messages.
   *
   * @throws Exception
   */
  @Test(dataProvider = "dispatchRateType")
  public void testReplicatorRateLimiterMessageNotReceivedAllMessages(
      DispatchRateType dispatchRateType) throws Exception {
    log.info("--- Starting ReplicatorTest::{} --- ", methodName);

    final String namespace =
        "pulsar/replicatorbyteandmsg-"
   ...
   *
   * <p>1. verify topic replicator get configured. 2. namespace setting of replicator dispatchRate,
   * verify consumer in other cluster could receive all messages < message limit. 3. verify consumer
   * in other cluster could not receive all messages > message limit.
   *
   * @throws Exception
   */
  @Test
  ...
  public void testReplicatorRateLimiterByBytes() throws Exception {
    ...
    @Cleanup PulsarClient client1 = PulsarClient.builder().serviceUrl(url1.toString()).build();
    @Cleanup
    Producer<byte[]> producer =
        client1
            .newProducer()
    ...
    @Cleanup PulsarClient client2 = PulsarClient.builder().serviceUrl(url2.toString()).build();
    final AtomicInteger totalReceived = new AtomicInteger(0);
    ...
    Awaitility.await()
        ...
        .untilAsserted(
            () -> {
              // The rate limit occurs in the next reading cycle, so a value fault tolerance needs
              // to be added.
  ...
  private static Optional<DispatchRateLimiter> getRateLimiter(PersistentTopic topic) {
    return topic.getReplicators().values().stream()
        .findFirst()
        .map(Replicator::getRateLimiter)
        .orElseThrow();
  }
