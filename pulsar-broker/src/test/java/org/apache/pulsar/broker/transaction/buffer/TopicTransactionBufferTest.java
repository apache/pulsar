...
public class TopicTransactionBufferTest extends TransactionTestBase {
  ...
  public void testTransactionBufferAppendMarkerWriteFailState() throws Exception {
    ...
    var attributes =
        Attributes.builder()
            ...
            .putAll(
                OpenTelemetryAttributes.TransactionBufferClientOperationStatus.FAILURE.attributes)
            .build();
    ...
    FieldUtils.writeField(
        persistentTopic.getManagedLedger(), "state", ManagedLedgerImpl.State.WriteFailed, true);
    txn.commit().get();
  ...
  public void testCheckDeduplicationFailedWhenCreatePersistentTopic() throws Exception {
              ...
              if (topicKlass.equals(PersistentTopic.class)) {
                ...
                CompletableFuture<Void> f =
                    CompletableFuture.failedFuture(
                        new ManagedLedgerException("This is an exception"));
    ...
    brokerService.createPersistentTopic0(
        topic, true, new CompletableFuture<>(), Collections.emptyMap());
  ...
  /**
   * This test mainly test the following two point: 1. `getLastMessageIds` will get max read
   * position. Send two message |1:0|1:1|; mock max read position as |1:0|; `getLastMessageIds` will
   * get |1:0|. 2. `getLastMessageIds` will wait Transaction buffer recover completely. Mock
   * `checkIfTBRecoverCompletely` return an exception, `getLastMessageIds` will fail too. Mock
   * `checkIfTBRecoverCompletely` return null, `getLastMessageIds` will get correct result.
   */
  @Test
  public void testGetMaxPositionAfterTBReady() throws Exception {
    ...
    when(transactionBuffer.checkIfTBRecoverCompletely())
        ...
        // If the transaction buffer recover successfully, the max read position can be acquired
        // successfully.
    ...
    Mockito.doReturn(transactionBufferProvider)
        .when(getPulsarServiceList().get(0))
    ...
    // 4. Clean resource
    Mockito.doReturn(originalTBProvider)
        .when(getPulsarServiceList().get(0))
        .getTransactionBufferProvider();
  }
   ...
   *
   * <p>1. Only normal messages in the topic. 2. There are ongoing transactions, last message ID
   * will not be updated until transaction end. 3. Aborted transaction will make the last message ID
   * be updated as expected. 4. Committed transaction will make the last message ID be updated as
   * expected.
   */
  @Test
  public void testGetLastMessageIdsWithOngoingTransactions() throws Exception {
    ...
    // Handle the case of the maxReadPosition < lastPosition, but it's an aborted transactional
    // message.
    Transaction txn3 = pulsarClient.newTransaction().build().get();
  ...
  /**
   * produce 3 messages and then trigger a ledger switch, then create a transaction and send a
   * transactional message. As there are messages in the new ledger, the reader should be able to
   * read the messages. But reader.hasMessageAvailable() returns false if the entry id of max read
   * position is -1.
   *
   * @throws Exception
   */
  ...
  public void testGetLastMessageIdsWithOpenTransactionAtLedgerHead() throws Exception {
    String topic =
        "persistent://" + NAMESPACE1 + "/testGetLastMessageIdsWithOpenTransactionAtLedgerHead";
    String subName = "my-subscription";
    @Cleanup Producer<byte[]> producer = pulsarClient.newProducer().topic(topic).create();
    Consumer<byte[]> consumer =
        pulsarClient.newConsumer().topic(topic).subscriptionName(subName).subscribe();
  ...
  private void assertGetLastMessageId(Consumer<?> consumer, MessageIdImpl expected)
      throws Exception {
  ...
  /**
   * This test verifies the state changes of a TransactionBuffer within a topic under different
   * conditions. Initially, the TransactionBuffer is in a NoSnapshot state upon topic creation. It
   * remains in the NoSnapshot state even after a normal message is sent. The state changes to Ready
   * only after a transactional message is sent. The test also ensures that the TransactionBuffer
   * can be correctly recovered after the topic is unloaded.
   */
  @Test
  public void testWriteSnapshotWhenFirstTxnMessageSend() throws Exception {
    ...
    Consumer<String> consumer =
        pulsarClient.newConsumer(Schema.STRING).topic(topic).subscriptionName("my-sub").subscribe();
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              Assert.assertEquals(
                  topicTransactionBuffer.getState(), TopicTransactionBufferState.State.NoSnapshot);
            });
    ...
    // There are 4 message sent to this topic, 2 normal message and 2 transaction message
    // |m1|m2-txn1|m3-txn1|m4|.
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              Assert.assertEquals(
                  topicTransactionBuffer2.getState(), TopicTransactionBufferState.State.Ready);
            });
  ...
  /**
   * Send some messages before transaction buffer ready and then send some messages after
   * transaction buffer ready, these messages should be received in order.
   */
  @Test
  public void testMessagePublishInOrder() throws Exception {
    // 1. Prepare test environment.
    this.pulsarServiceList.forEach(
        pulsarService -> {
          pulsarService.setTransactionBufferProvider(new TransactionBufferTestProvider());
        });
    String topic =
        "persistent://" + NAMESPACE1 + "/testMessagePublishInOrder" + RandomUtils.nextLong();
    admin.topics().createNonPartitionedTopic(topic);
    PersistentTopic persistentTopic =
        (PersistentTopic)
            pulsarServiceList.get(0).getBrokerService().getTopic(topic, false).get().get();
    ...
    // 2. Set a new future in transaction buffer as `transactionBufferFuture` to simulate whether
    // the
    // transaction buffer recover completely.
    TransactionBufferTestImpl topicTransactionBuffer =
        (TransactionBufferTestImpl) persistentTopic.getTransactionBuffer();
  ...
  public void testRefCountWhenAppendBufferToTxn() throws Exception {
    ...
    topicTransactionBuffer
        .appendBufferToTxn(new TxnID(1, 1), 1L, byteBuf1)
