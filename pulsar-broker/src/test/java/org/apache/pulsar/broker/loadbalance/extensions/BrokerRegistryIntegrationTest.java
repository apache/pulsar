...
public class BrokerRegistryIntegrationTest {
  ...
  protected void setup() throws Exception {
    ...
    admin
        ...
        .createTenant(
            "public",
            TenantInfo.builder().allowedClusters(Collections.singleton(clusterName)).build());
    admin.namespaces().createNamespace("public/default");
    brokerRegistry =
        ((ExtensibleLoadManagerWrapper) pulsar.getLoadManager().get()).get().getBrokerRegistry();
  ...
  public void testRecoverFromNodeDeletion() throws Exception {
    ...
    Assert.assertEquals(
        brokerRegistry.getAvailableBrokersAsync().get(), List.of(pulsar.getBrokerId()));
  }
  ...
  public void testRegisterAgain() throws Exception {
    ...
    log.info(
        "Old result: {} {}", new String(oldResult.getValue()), oldResult.getStat().getVersion());
    brokerRegistry.registerAsync().get();
    ...
    Awaitility.await()
        ...
        .untilAsserted(
            () -> {
              ...
              log.info(
                  "New result: {} {}",
                  new String(newResult.getValue()),
                  newResult.getStat().getVersion());
              Assert.assertTrue(
                  newResult.getStat().getVersion() > oldResult.getStat().getVersion());
