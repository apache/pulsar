...
public class AvgShedderTest {
  ...
  public void testHitHighThreshold() {
    ...
    for (int i = 0; i < 11; i++) {
      ...
      // as AvgShedder map BundleData to broker, the hashCode of different BundleData should be
      // different
    ...
    for (String bundle : bundlesToUnload.values()) {
      ...
      assertEquals(
          avgShedder
              .selectBroker(loadData.getBrokerData().keySet(), bundleData, loadData, conf)
              .get(),
          "broker2");
    }
  }
  ...
  public void testHitLowThreshold() {
    ...
    // do shedding for (lowCountForHighThreshold - 1) times, expect to shed nothing because hit
    // count is not enough
  ...
  public void testSheddingMultiplePairs() {
    ...
    // assert that half of the bundles are shed from broker1, and the other half are shed from
    // broker3
    for (String broker : bundlesToUnload.keys()) {
    ...
    // assert that all the bundles from broker1 are shed to broker2, and all the bundles from
    // broker3 are shed to broker4
    for (String bundle : bundlesToUnload.values()) {
      BundleData bundleData = loadData.getBundleData().get(bundle);
