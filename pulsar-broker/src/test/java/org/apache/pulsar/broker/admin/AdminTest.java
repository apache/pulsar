...
public class AdminTest extends MockedPulsarServiceBaseTest {
  ...
  public void internalConfiguration() throws Exception {
    ...
    InternalConfigurationData expectedData =
        new InternalConfigurationData(
  ...
  private static class OldInternalConfigurationData {
    ...
    @Deprecated private String ledgersRootPath;
    private String bookkeeperMetadataServiceUri;
    private String stateStorageServiceUrl;
  }

  /**
   * This test verifies that the model data changes in InternalConfigurationData are
   * retro-compatible. InternalConfigurationData is downloaded from the Function worker from a
   * broker. The broker may be still serve an "old" version of InternalConfigurationData (before
   * https://github.com/apache/pulsar/pull/14384) while the Worker already uses the new one.
   *
   * @throws Exception
   */
  @Test
  public void internalConfigurationRetroCompatibility() throws Exception {
    OldInternalConfigurationData oldDataModel =
        new OldInternalConfigurationData(
    ...
    final Map<String, Object> oldDataJson =
        ObjectMapperFactory.getMapper().getObjectMapper().convertValue(oldDataModel, Map.class);

    final InternalConfigurationData newData =
    ...
    assertEquals(
        newData.getConfigurationMetadataStoreUrl(), oldDataModel.getConfigurationStoreServers());
    assertEquals(newData.getLedgersRootPath(), oldDataModel.getLedgersRootPath());
    assertEquals(
        newData.getBookkeeperMetadataServiceUri(), oldDataModel.getBookkeeperMetadataServiceUri());
  ...
  public void clusters() throws Exception {
    ...
    MetadataCacheImpl<ClusterData> clusterCache =
        (MetadataCacheImpl<ClusterData>)
            pulsar.getPulsarResources().getClusterResources().getCache();
    MetadataCacheImpl isolationPolicyCache =
        (MetadataCacheImpl)
            pulsar.getPulsarResources().getNamespaceResources().getIsolationPolicies().getCache();
      ...
      asyncRequests(
          ctx ->
              clusters.createCluster(
                  ...
                  "test",
                  ClusterDataImpl.builder()
                      .serviceUrl("http://broker.messaging.test.example.com:8080")
                      .build()));
      fail("should have failed");
    } catch (RestException e) {
      assertEquals(e.getResponse().getStatus(), Status.INTERNAL_SERVER_ERROR.getStatusCode());
      ...
      asyncRequests(
          ctx ->
              clusters.updateCluster(
                  ...
                  "test",
                  ClusterDataImpl.builder()
                      .serviceUrl("http://broker.messaging.test.example.com")
                      .build()));
      fail("should have failed");
    } catch (RestException e) {
      assertEquals(e.getResponse().getStatus(), Status.INTERNAL_SERVER_ERROR.getStatusCode());
  ...
  public void properties() throws Throwable {
    ...
    asyncRequests(
        ctx -> clusters.createCluster(ctx, configClusterName, ClusterDataImpl.builder().build()));

    Set<String> allowedClusters = new HashSet<>();
    allowedClusters.add(configClusterName);
    TenantInfoImpl tenantInfo =
        TenantInfoImpl.builder()
    ...
    final TenantInfoImpl newPropertyAdmin =
        TenantInfoImpl.builder()
            .adminRoles(Set.of("role1", "other-role"))
            .allowedClusters(allowedClusters)
            .build();
    response =
        asyncRequests(ctx -> properties.updateTenant(ctx, "test-property", newPropertyAdmin));
    ...
    try {
      response =
          asyncRequests(ctx -> properties.updateTenant(ctx, "xxx-non-existing", newPropertyAdmin));
      fail("should have failed");
    } catch (RestException e) {
      assertEquals(e.getResponse().getStatus(), Status.NOT_FOUND.getStatusCode());
    }
    ...
    // clear caches to load data from metadata-store again
    MetadataCacheImpl<TenantInfo> cache =
    ...
    TenantInfoImpl newPropertyAdmin2 =
        TenantInfoImpl.builder()
            .adminRoles(Set.of("role1", "other-role"))
        ...
        asyncRequests(
            ctx ->
                namespaces.createNamespace(
                    ctx, "my-tenant", "use", "my-namespace", BundlesData.builder().build()));
    ...
    TenantInfoImpl tenantWithEmptyCluster =
        TenantInfoImpl.builder()
            .adminRoles(Set.of("role1", "role2"))
    ...
    try {
      response =
          asyncRequests(
              ctx ->
                  properties.createTenant(ctx, "tenant-config-is-empty", tenantWithEmptyCluster));
      fail("should have failed");
    } catch (RestException e) {
      assertEquals(e.getResponse().getStatus(), Status.PRECONDITION_FAILED.getStatusCode());
    }
    ...
    TenantInfoImpl tenantContainEmptyCluster =
        TenantInfoImpl.builder()
    ...
    try {
      response =
          asyncRequests(
              ctx ->
                  properties.createTenant(
                      ctx, "tenant-config-contain-empty", tenantContainEmptyCluster));
      fail("should have failed");
    } catch (RestException e) {
      assertEquals(e.getResponse().getStatus(), Status.PRECONDITION_FAILED.getStatusCode());
    }
    ...
    for (int tenantSize = tenants.size(); tenantSize < maxTenants; tenantSize++) {
      ...
      Response obj =
          (Response)
              asyncRequests(
                  ctx -> properties.createTenant(ctx, "test-tenant-" + tenantIndex, tenantInfo));
    ...
    try {
      Response obj =
          (Response)
              asyncRequests(
                  ctx -> properties.createTenant(ctx, "test-tenant-" + maxTenants, tenantInfo));
      fail("should have failed");
    } catch (RestException e) {
      assertEquals(e.getResponse().getStatus(), Status.PRECONDITION_FAILED.getStatusCode());
    }
    ...
    try {
      response =
          asyncRequests(
              ctx -> properties.createTenant(ctx, "test-tenant-" + (maxTenants - 1), tenantInfo));
      fail("should have failed");
    } catch (RestException e) {
      assertEquals(e.getResponse().getStatus(), Status.CONFLICT.getStatusCode());
    }
  ...
  public void resourceQuotas() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(
                    defaultBandwidth, resourceQuotas.getDefaultResourceQuota().getBandwidthIn()));
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(
                    defaultBandwidth, resourceQuotas.getDefaultResourceQuota().getBandwidthOut()));
    ...
    doReturn(policies)
        .when(resourceQuotas)
        .getNamespacePolicies(NamespaceName.get(property, cluster, namespace));
    doReturn(CompletableFuture.completedFuture(policies))
        .when(resourceQuotas)
    ...
    try {
      asyncRequests(
          ctx ->
              resourceQuotas.setNamespaceBundleResourceQuota(
    ...
    try {
      asyncRequests(
          ctx ->
              resourceQuotas.removeNamespaceBundleResourceQuota(
    ...
    // create policies
    TenantInfoImpl admin =
        TenantInfoImpl.builder().allowedClusters(Collections.singleton(cluster)).build();
    ClusterDataImpl clusterData =
        ClusterDataImpl.builder().serviceUrl("http://example.pulsar").build();
    ...
    asyncRequests(
        ctx ->
            resourceQuotas.setNamespaceBundleResourceQuota(
                ctx, property, cluster, namespace, bundleRange, quota));
    ResourceQuota bundleQuota =
        (ResourceQuota)
            asyncRequests(
                ctx ->
                    resourceQuotas.getNamespaceBundleResourceQuota(
                        ctx, property, cluster, namespace, bundleRange));
  ...
  public void brokerStats() throws Exception {
    ...
    Map<String, Map<String, PendingBookieOpsStats>> bookieOpsStats =
  ...
  public void persistentTopics() throws Exception {
    ...
    doReturn("client-id").when(resourceQuotas).clientAppId();
    // create policies
    TenantInfo admin = TenantInfo.builder().allowedClusters(Collections.singleton(cluster)).build();
    ...
    persistentTopics.createPartitionedTopic(
        response, property, cluster, namespace, topic, 5, false);
    ...
    assertEquals(
        persistentTopics.getPartitionedTopicMetadata(topicName, true, false).partitions, 5);
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              ArgumentCaptor<Map<String, Set<AuthAction>>> permissionsCaptor1 =
                  ArgumentCaptor.forClass(Map.class);
              persistentTopics.getPermissionsOnTopic(
                  response1, property, cluster, namespace, topic);
  ...
  public void testUpdatePartitionedTopicCoontainedInOldTopic() throws Exception {
    ...
    ArgumentCaptor<Response> responseCaptor = ArgumentCaptor.forClass(Response.class);
    persistentTopics.createPartitionedTopic(
        response1, property, cluster, namespace, partitionedTopicName, 5, false);
    ...
    responseCaptor = ArgumentCaptor.forClass(Response.class);
    persistentTopics.createPartitionedTopic(
        response2, property, cluster, namespace, partitionedTopicName2, 2, false);
    verify(response2, timeout(5000).times(1)).resume(responseCaptor.capture());
    Assert.assertEquals(
        responseCaptor.getValue().getStatus(), Response.Status.NO_CONTENT.getStatusCode());

    persistentTopics.updatePartitionedTopic(
        response2, property, cluster, namespace, partitionedTopicName2, false, false, false, 10);
  }
  ...
  public void test500Error() throws Exception {
    ...
    Assert.assertTrue(
        ((ErrorData) responseCaptor.getValue().getResponse().getEntity())
