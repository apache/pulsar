...
public class ResourceGroupUsageAggregationTest extends ProducerConsumerBase {
  ...
  protected void setup() throws Exception {
    ...
    this.rgs =
        new ResourceGroupService(pulsar, TimeUnit.MILLISECONDS, transportMgr, dummyQuotaCalc);
  }
  ...
  private void testProduceConsumeUsageOnRG(String topicString) throws Exception {
    ...
    try {
      ...
      final String errMsg =
          String.format("Got exception while building consumer: ex=%s", p.getMessage());
      Assert.fail(errMsg);
    }
    ...
    for (int ix = 0; ix < NumMessagesToSend; ix++) {
      ...
      try {
        ...
        final String errMsg =
            String.format("Got exception while sending %s-th time: ex=%s", ix, p.getMessage());
        Assert.fail(errMsg);
      }
    }
    producer.close();
    ...
    while (recvdNumMsgs < sentNumMsgs) {
      try {
        ...
        final String errMesg =
            String.format(
                "Got exception in while receiving %s-th mesg at consumer: ex=%s",
  ...
  // There appears to be a 45-byte message header which is accounted in the stats, additionally to
  // what the
  // application-level sends/receives. Hence, the byte counts are a ">=" check, instead of an
  // equality check.
  private void verifyStats(
    ...
    if (sentNumMsgs > 0 || recvdNumMsgs > 0) {
      rgs
          .aggregateResourceGroupLocalUsages(); // hack to ensure aggregator calculation without
                                                // waiting
      BytesAndMessagesCount prodCounts =
          rgs.getRGUsage(
              rgName, ResourceGroupMonitoringClass.Publish, ResourceGroupUsageStatsType.Cumulative);
      BytesAndMessagesCount consCounts =
          rgs.getRGUsage(
              rgName,
      ...
      // The counts should be equal, since there wasn't any intervening traffic on
      // TEST_PRODUCE_CONSUME_TOPIC.
      BytesAndMessagesCount prodCounts1 =
          rgs.getRGUsage(
              rgName, ResourceGroupMonitoringClass.Publish, ResourceGroupUsageStatsType.Cumulative);
      BytesAndMessagesCount consCounts1 =
          rgs.getRGUsage(
              rgName,
              ResourceGroupMonitoringClass.Dispatch,
              ResourceGroupUsageStatsType.Cumulative);
  ...
  final String PRODUCE_CONSUME_PERSISTENT_TOPIC =
  ...
  private void prepareData() throws PulsarAdminException {
    ...
    admin
        .namespaces()
        .setNamespaceReplicationClusters(TenantAndNsName, Sets.newHashSet(clusterName));
  }
}
