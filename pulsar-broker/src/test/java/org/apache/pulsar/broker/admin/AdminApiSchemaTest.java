...
public class AdminApiSchemaTest extends MockedPulsarServiceBaseTest {
    ...
    public Object[][] schemas() {
        return new Object[][] {
                ...
                { Schema.KeyValue(StringSchema.utf8(), new StringSchema(US_ASCII)) }
        };
    }
    ...
    public void testSchemaInfoWithVersionApi(Schema<?> schema) throws Exception {
        testSchemaInfoWithVersionApi(
                schema, "schematest/test/test-" + schema.getSchemaInfo().getType());
    }
    ...
    public void testPostSchemaCompatibilityStrategy(ApiVersion version) throws PulsarAdminException {
        String namespace = format(
                "%s%s%s",
                "schematest", (ApiVersion.V1.equals(version) ? "/" + cluster + "/" : "/"), "test");
        String topicName = "persistent://" + namespace + "/testStrategyChange";
        SchemaInfo fooSchemaInfo = Schema.AVRO(
                SchemaDefinition.builder().withAlwaysAllowNull(false).withPojo(Foo.class).build())
                .getSchemaInfo();
        ...
        SchemaInfo foo1SchemaInfo = Schema.AVRO(
                SchemaDefinition.builder().withAlwaysAllowNull(false).withPojo(Foo1.class).build())
                .getSchemaInfo();
    ...
    private <T> void testSchemaInfoWithVersionApi(Schema<T> schema, String topicName)
            throws Exception {
        SchemaInfo si = schema.getSchemaInfo();
        admin.schemas().createSchema(topicName, si);
        log.info("Upload schema to topic {} : {}", topicName, si);
    ...
    public void testInvalidSchemaDataException(ApiVersion version) {
        ...
        SchemaInfo schemaInfo = SchemaInfo.builder().schema(new byte[0]).type(SchemaType.AVRO).name("test").build();
        try {
    ...
    public void testGetSchemaCompatibilityStrategyWhenSetSchemaAutoUpdateCompatibilityStrategy()
        ...
        admin
                .namespaces()
                .setSchemaAutoUpdateCompatibilityStrategy(
                        schemaCompatibilityNamespace, SchemaAutoUpdateCompatibilityStrategy.Forward);
        Awaitility.await()
                .untilAsserted(
                        () -> assertEquals(
                                ...
                                admin
                                        .namespaces()
                                        .getSchemaAutoUpdateCompatibilityStrategy(schemaCompatibilityNamespace)));
        ...
        admin
                ...
                .setSchemaCompatibilityStrategy(
                        schemaCompatibilityNamespace, SchemaCompatibilityStrategy.BACKWARD);
        Awaitility.await()
                .untilAsserted(
                        () -> assertEquals(
                                ...
                                admin
                                        .namespaces()
                                        .getSchemaCompatibilityStrategy(schemaCompatibilityNamespace)));
    }
    ...
    public void testGetSchemaCompatibilityStrategyWhenSetBrokerLevelAndSchemaAutoUpdateCompatibilityStrategy()
        ...
        admin
                .namespaces()
                .setSchemaAutoUpdateCompatibilityStrategy(
                        schemaCompatibilityNamespace, SchemaAutoUpdateCompatibilityStrategy.AlwaysCompatible);
        Awaitility.await()
                .untilAsserted(
                        () -> assertEquals(
                                admin.namespaces().getSchemaCompatibilityStrategy(schemaCompatibilityNamespace),
    ...
    public void testCompatibility() throws Exception {
        ...
        IsCompatibilityResponse isCompatibilityResponse = admin.schemas().testCompatibility(topicName,
                postSchemaPayload);

        assertTrue(isCompatibilityResponse.isCompatibility());
        assertEquals(
                isCompatibilityResponse.getSchemaCompatibilityStrategy(),
        ...
        try {
            ...
            assertTrue(
                    e.getMessage()
                            .contains("Incompatible schema: exists schema type STRING, new schema type INT8"));
        }
    }
    ...
    public void testCompatibilityWithEmpty() throws Exception {
        List<Schema<?>> checkSchemas = List.of(
                Schema.STRING,
                Schema.JSON(
                        SchemaDefinition.builder().withPojo(Foo.class).withProperties(PROPS).build()),
                Schema.AVRO(
                        SchemaDefinition.builder().withPojo(Foo.class).withProperties(PROPS).build()),
        ...
        for (Schema<?> schema : checkSchemas) {
            ...
            PostSchemaPayload postSchemaPayload = new PostSchemaPayload(
                    schemaInfo.getType().toString(), schemaInfo.getSchemaDefinition(), new HashMap<>());
            ...
            admin
                    .namespaces()
                    .setSchemaCompatibilityStrategy(
                            schemaCompatibilityNamespace, SchemaCompatibilityStrategy.FULL_TRANSITIVE);
            ...
            admin
                    .namespaces()
                    .setSchemaCompatibilityStrategy(
                            schemaCompatibilityNamespace, SchemaCompatibilityStrategy.FULL);
        }
    }
}
