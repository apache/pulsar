...
public class BrokerAdminClientTlsAuthTest extends MockedPulsarServiceBaseTest {
  ...
  private void buildConf(ServiceConfiguration conf) {
    ...
    String str =
        String.format("tlsCertFile:%s,tlsKeyFile:%s", BROKER_CERT_FILE_PATH, BROKER_KEY_FILE_PATH);
    conf.setBrokerClientAuthenticationParameters(str);
    conf.setBrokerClientAuthenticationPlugin(
  ...
  /**
   * Test case => Use Multiple Brokers => Create a namespace with bundles distributed among these
   * brokers. => Use Tls as authPlugin for everything. => Run list topics command
   *
   * @throws Exception
   */
  @Test
