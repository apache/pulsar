...
public class MessageDuplicationTest extends BrokerTestBase {
  ...
  public void testIsDuplicate() {
    ...
    serviceConfiguration.setBrokerDeduplicationEntriesInterval(
        BROKER_DEDUPLICATION_ENTRIES_INTERVAL);
    serviceConfiguration.setBrokerDeduplicationMaxNumberOfProducers(
        BROKER_DEDUPLICATION_MAX_NUMBER_PRODUCERS);
  ...
  public void testInactiveProducerRemove() throws Exception {
    ...
    MessageDeduplication messageDeduplication =
        spyWithClassAndConstructorArgs(
            MessageDeduplication.class, pulsarService, topic, managedLedger);
    doReturn(true).when(messageDeduplication).isEnabled();
    ...
    Map<String, Long> inactiveProducers =
        (ConcurrentHashMap<String, Long>) field.get(messageDeduplication);
  ...
  public void testIsDuplicateWithFailure() {
    ...
    doReturn(mock(CompactionServiceFactory.class))
        .when(pulsarService)
        .getCompactionServiceFactory();
  ...
  public Topic.PublishContext getPublishContext(
      String producerName, long seqId, long lastSequenceId) {
    return spy(
        new Topic.PublishContext() {
          @Override
          public String getProducerName() {
