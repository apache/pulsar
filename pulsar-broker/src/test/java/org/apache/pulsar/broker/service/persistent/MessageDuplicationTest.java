...
public class MessageDuplicationTest {
    ...
    @Test
    public void testIsDuplicateAcrossProducers() {
        PulsarService pulsarService = mock(PulsarService.class);
        ServiceConfiguration serviceConfiguration = new ServiceConfiguration();
        serviceConfiguration.setBrokerDeduplicationEntriesInterval(BROKER_DEDUPLICATION_ENTRIES_INTERVAL);
        serviceConfiguration.setBrokerDeduplicationMaxNumberOfProducers(BROKER_DEDUPLICATION_MAX_NUMBER_PRODUCERS);
        serviceConfiguration.setReplicatorPrefix(REPLICATOR_PREFIX);
        serviceConfiguration.setBrokerDeduplicationAcrossProducersEnabled(true);
        ...
        Long lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(producerName1);
        assertNull(lastSequenceIdPushed);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        assertNotNull(lastSequenceIdPushed);
        assertEquals(lastSequenceIdPushed.longValue(), 0);
        ...
        byteBuf2 = getMessage(producerName2, 1);
        publishContext2 = getPublishContext(producerName2, 1);
        status = messageDeduplication.isDuplicate(publishContext2, byteBuf2);
        // should expect unknown because highestSequencePersisted is empty
        assertEquals(status, MessageDeduplication.MessageDupStatus.Unknown);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        ...
        byteBuf2 = getMessage(producerName2, 6);
        publishContext2 = getPublishContext(producerName2, 6);
        status = messageDeduplication.isDuplicate(publishContext2, byteBuf2);
        assertEquals(status, MessageDeduplication.MessageDupStatus.NotDup);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        assertNotNull(lastSequenceIdPushed);
        ...
        byteBuf2 = getMessage(producerName2, 0);
        publishContext2 = getPublishContext(producerName2, 0);
        status = messageDeduplication.isDuplicate(publishContext2, byteBuf2);
        // should expect unknown because highestSequencePersisted is empty
        assertEquals(status, MessageDeduplication.MessageDupStatus.Unknown);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        ...
        // update highest sequence persisted
        messageDeduplication.highestSequencedPersisted.put(MessageDeduplication.ALL_PRODUCERS, 6L);
        ...
        // update highest sequence persisted
        messageDeduplication.highestSequencedPushed.put(MessageDeduplication.ALL_PRODUCERS, 0L);
        messageDeduplication.highestSequencedPersisted.put(MessageDeduplication.ALL_PRODUCERS, 0L);
        byteBuf1 = getMessage(producerName1, 0);
        publishContext1 = getPublishContext(producerName1, 1, 6);
        status = messageDeduplication.isDuplicate(publishContext1, byteBuf1);
        assertEquals(status, MessageDeduplication.MessageDupStatus.NotDup);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        assertNotNull(lastSequenceIdPushed);
        ...
        byteBuf2 = getMessage(producerName2, 0);
        publishContext2 = getPublishContext(producerName2, 2, 6);
        status = messageDeduplication.isDuplicate(publishContext2, byteBuf2);
        assertEquals(status, MessageDeduplication.MessageDupStatus.Unknown);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        assertNotNull(lastSequenceIdPushed);
        ...
        publishContext2 = getPublishContext(producerName2, 4, 8);
        status = messageDeduplication.isDuplicate(publishContext2, byteBuf2);
        assertEquals(status, MessageDeduplication.MessageDupStatus.Unknown);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        assertNotNull(lastSequenceIdPushed);
    ...
    @Test
    public void testIsDuplicateAcrossProducersWithFailure() {
        ...
        persistentTopic.addComplete(new PositionImpl(0, 1), publishContext1);
        verify(managedLedger, times(1)).asyncAddEntry(any(ByteBuf.class), any(), any());
        // just to make sure nothing is stored per producer
        Long lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(producerName1);
        assertNull(lastSequenceIdPushed);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        assertNotNull(lastSequenceIdPushed);
        ...
        lastSequenceIdPushed = messageDeduplication.highestSequencedPersisted.get(producerName1);
        assertNull(lastSequenceIdPushed);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPersisted.get(MessageDeduplication.ALL_PRODUCERS);
        assertNotNull(lastSequenceIdPushed);
        assertEquals(lastSequenceIdPushed.longValue(), 0);
        ...
        byteBuf1 = getMessage(producerName1, 3);
        publishContext1 = getPublishContext(producerName1, 3);
        persistentTopic.publishMessage(byteBuf1, publishContext1);
        persistentTopic.addComplete(new PositionImpl(0, 3), publishContext1);
        verify(managedLedger, times(3)).asyncAddEntry(any(ByteBuf.class), any(), any());
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        assertTrue(lastSequenceIdPushed != null);
        assertEquals(lastSequenceIdPushed.longValue(), 3);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPersisted.get(MessageDeduplication.ALL_PRODUCERS);
        assertTrue(lastSequenceIdPushed != null);
        assertEquals(lastSequenceIdPushed.longValue(), 3);
        ...
        persistentTopic.addFailed(new ManagedLedgerException("test"), publishContext1);
        // check highestSequencedPushed is reset
        assertEquals(messageDeduplication.highestSequencedPushed.size(), 1);
        assertEquals(messageDeduplication.highestSequencedPersisted.size(), 1);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPushed.get(MessageDeduplication.ALL_PRODUCERS);
        assertEquals(lastSequenceIdPushed.longValue(), 6);
        lastSequenceIdPushed = messageDeduplication.highestSequencedPersisted.get(MessageDeduplication.ALL_PRODUCERS);
        assertEquals(lastSequenceIdPushed.longValue(), 6);
