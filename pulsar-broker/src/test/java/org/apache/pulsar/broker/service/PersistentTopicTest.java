...
public class PersistentTopicTest extends MockedBookKeeperTestCase {
  ...
  public void setup() throws Exception {
    ...
    doAnswer(invocationOnMock -> CompletableFuture.completedFuture(null))
        ...
        .getManagedLedgerPropertiesAsync(any());
    doAnswer(
            invocation -> {
        ...
        .when(managedLedgerFactory)
        .asyncDelete(any(), any(), any());
    // Mock serviceCnx.
    serverCnx =
        spyWithClassAndConstructorArgsRecordingInvocations(
            ServerCnx.class, pulsarTestContext.getPulsarService());
    ...
    doReturn(CompletableFuture.completedFuture(Optional.of(true)))
        .when(serverCnx)
  ...
  public void testCreateTopic() {
    ...
    CompletableFuture<Void> future =
        brokerService
            ...
            .thenAccept(topic -> assertTrue(topic.toString().contains(topicName)))
            .exceptionally(
                (t) -> {
  ...
  public void testPublishMessage() throws Exception {
    ...
    doAnswer(
            invocationOnMock -> {
              ...
              final AddEntryCallback callback =
                  (AddEntryCallback) invocationOnMock.getArguments()[2];
              final Topic.PublishContext ctx =
                  (Topic.PublishContext) invocationOnMock.getArguments()[3];
    ...
    final Topic.PublishContext publishContext =
  ...
  public void testDispatcherMultiConsumerReadFailed() {
    PersistentTopic topic =
        spyWithClassAndConstructorArgsRecordingInvocations(
            PersistentTopic.class, successTopicName, ledgerMock, brokerService);
    ...
    dispatcher.readEntriesFailed(
        new ManagedLedgerException.InvalidCursorPositionException("failed"), null);
  ...
  public void testDispatcherSingleConsumerReadFailed() {
    ...
    PersistentDispatcherSingleActiveConsumer dispatcher =
        new PersistentDispatcherSingleActiveConsumer(cursor, SubType.Exclusive, 1, topic, null);
    Consumer consumer = mock(Consumer.class);
    dispatcher.readEntriesFailed(
        new ManagedLedgerException.InvalidCursorPositionException("failed"), consumer);
    verify(topic, atLeast(1)).getBrokerService();
  }
  ...
  public void testAddRemoveProducer() {
    ...
    try {
      ...
      fail(
          "Should have failed with naming exception because producer 'null' is already connected to the topic");
    } catch (Exception e) {
  ...
  private Producer getMockedProducerWithSpecificAddress(
      Topic topic, long producerId, InetAddress address) {
  ...
  public void testChangeSubscriptionType() throws Exception {
    ...
    PersistentSubscription sub =
        new PersistentSubscription(topic, "change-sub-type", cursorMock, false);
    ...
    for (SubType subType :
        List.of(SubType.Shared, SubType.Failover, SubType.Key_Shared, SubType.Exclusive)) {
  ...
  public void testAddRemoveConsumerDurableCursor() throws Exception {
    ...
    PersistentTopic topic = new PersistentTopic(successTopicName, ledgerMock, brokerService);
    PersistentSubscription sub =
        new PersistentSubscription(topic, "non-durable-sub", cursorMock, false);
  ...
  private void testMaxConsumersShared() throws Exception {
    ...
    Method addConsumerToSubscription =
        AbstractTopic.class.getDeclaredMethod(
            "addConsumerToSubscription", Subscription.class, Consumer.class);
  ...
  private Consumer getMockedConsumerWithSpecificAddress(
      Topic topic, Subscription sub, long consumerId, InetAddress address) {
  ...
  public void testDeleteTopicRaceConditions() throws Exception {
    ...
    doAnswer(
            invocationOnMock -> {
              Thread.sleep(1000);
              ((DeleteLedgerCallback) invocationOnMock.getArguments()[0])
                  .deleteLedgerComplete(null);
              return null;
            })
        .when(ledgerMock)
        .asyncDelete(any(DeleteLedgerCallback.class), any());
  ...
  void setupMLAsyncCallbackMocks() {
            ...
            new CloseCallback() {
              ...
              public void closeFailed(ManagedLedgerException exception, Object ctx) {
                ...
                closeFuture.completeExceptionally(
                    new BrokerServiceException.PersistenceException(exception));
              }
            },
            null);
    ...
    doAnswer(
        ...
        .when(cursorMock)
        .asyncMarkDelete(any(), any(), any(MarkDeleteCallback.class), any());
  }
  ...
  public void testDeleteTopicDeleteOnMetadataStoreFailed() throws Exception {
    ...
    metadataStore.failConditional(
        new MetadataStoreException("injected error"),
        (op, path) -> op == OperationType.EXISTS && path.equals("/admin/flags/policies-readonly"));
    try {
      topic.delete().get();
      fail();
    } catch (ExecutionException e) {
  ...
  public void testFailoverSubscription() throws Exception {
    ...
    PersistentTopic topic2 =
        new PersistentTopic(successPartitionTopicName, ledgerMock, brokerService);
    ...
    assertEquals(
        topic2.getSubscription(successSubName).getDispatcher().getConsumers().get(0).consumerId(),
        1);
    assertEquals(
        topic2.getSubscription(successSubName).getDispatcher().getConsumers().get(0).consumerName(),
        "C1");
    assertEquals(
        topic2.getSubscription(successSubName).getDispatcher().getConsumers().get(1).consumerId(),
        2);
    assertEquals(
    ...
    assertEquals(
        ...
        "C1");
    assertEquals(
        topic2.getSubscription(successSubName).getDispatcher().getConsumers().get(2).consumerId(),
        2);
    assertEquals(
  ...
  /**
   * NonPersistentReplicator.removeReplicator doesn't remove replicator in atomic way and does in
   * multiple step: 1. disconnect replicator producer
   *
   * <p>2. close cursor
   *
   * <p>3. remove from replicator-list.
   *
   * <p>If we try to startReplicationProducer before step-c finish then it should not avoid
   * restarting repl-producer.
   *
   * @throws Exception
   */
  @Test
  public void testAtomicReplicationRemoval() throws Exception {
    ...
    when(pulsarClientMock.newProducer(any()))
        .thenAnswer(
            invocation -> {
    ...
    PersistentReplicator replicator =
        spy(
            new GeoPersistentReplicator(
                ...
                brokerService,
                (PulsarClientImpl)
                    brokerService.getReplicationClient(
                        remoteCluster,
                        brokerService
                            .pulsar()
                            .getPulsarResources()
    ...
    // step-1 remove replicator : it will disconnect the producer but it will wait for callback to
    // be completed
    ...
    // step-2 now, policies doesn't have removed replication cluster so, it should not invoke
    // "startProducer" of the
    // replicator
    ...
    ArgumentCaptor<DeleteCursorCallback> captor =
        ArgumentCaptor.forClass(DeleteCursorCallback.class);
  ...
  public void testClosingReplicationProducerTwice() throws Exception {
    ...
    doReturn(new CompletableFuture<Producer>())
        .when(clientImpl)
    ...
    verify(clientImpl).createProducerAsync(any(ProducerConfigurationData.class), any(), eq(null));
  ...
  public void testCompactorSubscription() {
    ...
    PersistentSubscription sub =
        new PulsarCompactorSubscription(
  ...
  public void testCompactionTriggeredAfterThresholdFirstInvocation() throws Exception {
    ...
    Compactor compactor =
        ((PulsarCompactionServiceFactory)
                pulsarTestContext.getPulsarService().getCompactionServiceFactory())
            .getCompactor();
  ...
  public void testBacklogCursor() throws Exception {
    ...
    // Open cursor3, add it into activeCursor-container and do not add it into subscription consumer
    // list
    ...
    for (Entry entry : entries1) {
      log.info(
          "Read entry. Position={} Content='{}'", entry.getPosition(), new String(entry.getData()));
      entry.release();
    }
  ...
  public void testCheckInactiveSubscriptions() throws Exception {
    ...
    PersistentSubscription nonDeletableSubscription1 =
        spyWithClassAndConstructorArgsRecordingInvocations(
            PersistentSubscription.class, topic, "nonDeletableSubscription1", cursorMock, false);
    ...
    PersistentSubscription deletableSubscription1 =
        spyWithClassAndConstructorArgsRecordingInvocations(
            PersistentSubscription.class, topic, "deletableSubscription1", cursorMock, false);
    ...
    PersistentSubscription nonDeletableSubscription2 =
        spyWithClassAndConstructorArgsRecordingInvocations(
            PersistentSubscription.class, topic, "nonDeletableSubscription2", cursorMock, true);
    ...
    pulsarTestContext
        .getPulsarResources()
        .getNamespaceResources()
        .createPolicies(TopicName.get(successTopicName).getNamespaceObject(), new Policies());

    pulsarTestContext.getConfig().setSubscriptionExpirationTimeMinutes(5);

    doReturn(System.currentTimeMillis() - TimeUnit.MINUTES.toMillis(6))
        .when(cursorMock)
        .getLastActive();
  ...
  public void testTopicFencingTimeout() throws Exception {
    ...
    ScheduledFuture<?> fencedTopicMonitoringTask =
        (ScheduledFuture<?>) fencedTopicMonitoringTaskField.get(topic);
  ...
  public void testGetDurableSubscription() throws Exception {
    ...
    doAnswer(
            (Answer<Object>)
                invocationOnMock -> {
                  ((AsyncCallbacks.FindEntryCallback) invocationOnMock.getArguments()[2])
                      .findEntryComplete(mockPosition, invocationOnMock.getArguments()[3]);
                  return null;
                })
        .when(mockCursor)
        .asyncFindNewestMatching(any(), any(), any(), any());
    doAnswer(
            (Answer<Object>)
                invocationOnMock -> {
                  ((AsyncCallbacks.ResetCursorCallback) invocationOnMock.getArguments()[1])
                      .resetComplete(null);
                  return null;
                })
        .when(mockCursor)
        .asyncResetCursor(any(), anyBoolean(), any());
    doAnswer(
            (Answer<Object>)
                invocationOnMock -> {
                  ((DeleteCursorCallback) invocationOnMock.getArguments()[1])
    ...
    doAnswer(
            (Answer<Object>)
                invocationOnMock -> {
                  ((OpenCursorCallback) invocationOnMock.getArguments()[4])
                      .openCursorComplete(mockCursor, null);
                  return null;
                })
        .when(mockLedger)
        .asyncOpenCursor(any(), any(), any(), any(), any(), any());
  ...
  public void testKeySharedMetadataExposedToStats() throws Exception {
    PersistentTopic topic = new PersistentTopic(successTopicName, ledgerMock, brokerService);
    PersistentSubscription sub1 =
        new PersistentSubscription(topic, "key-shared-stats1", cursorMock, false);
    PersistentSubscription sub2 =
        new PersistentSubscription(topic, "key-shared-stats2", cursorMock, false);
    PersistentSubscription sub3 =
        new PersistentSubscription(topic, "key-shared-stats3", cursorMock, false);
  ...
  public void testGetReplicationClusters() throws MetadataStoreException {
    ...
    topic.initialize().join();
    assertEquals(
        topic.getHierarchyTopicPolicies().getReplicationClusters().get(), Collections.emptyList());
    ...
    topic.initialize().join();
    assertEquals(
        topic.getHierarchyTopicPolicies().getReplicationClusters().get(), namespaceClusters);
  ...
  public void testSendProducerTxnPrechecks() throws Exception {
    ...
    producer1.publishTxnMessage(new TxnID(1L, 0L), 1, 1, 1, headersAndPayload, 1, false, false);
