...
public class BrokerServiceAutoTopicCreationTest extends BrokerTestBase {
  ...
  public void testAutoPartitionedTopicCreation() throws Exception {
    ...
    for (int i = 0; i < 3; i++) {
      assertTrue(
          admin.namespaces().getTopics("prop/ns-abc").contains(topicString + "-partition-" + i));
    }
  }
  ...
  /**
   * CheckAllowAutoCreation's default value is false. So using getPartitionedTopicMetadata()
   * directly will not produce partitioned topic even if the option to automatically create
   * partitioned topic is configured
   */
  @Test
  ...
  public void testAutoCreationNamespaceAllowOverridesBroker() throws Exception {
    ...
    pulsar
        .getAdminClient()
        .namespaces()
        .setAutoTopicCreation(topicName.getNamespace(), autoTopicCreationOverride);
    Assert.assertEquals(
        pulsar.getAdminClient().namespaces().getAutoTopicCreation(topicName.getNamespace()),
  ...
  public void testNotAllowSubscriptionTopicCreation() throws Exception {
    pulsar.getConfiguration().setAllowAutoTopicCreation(false);
    String topicName =
        "persistent://prop/ns-abc/non-partitioned-topic" + System.currentTimeMillis();
    ...
    try {
      admin
          .topics()
          .createSubscription(topicName + "-partition-0", subscriptionName, MessageId.earliest);
      fail("should fail to create subscription once not allowAutoTopicCreation");
    } catch (Exception e) {
      // expected
    }
    ...
    try {
      ...
      admin
          .topics()
          .createSubscription(
              partitionTopic + "-partition-0", subscriptionName, MessageId.earliest);
    } catch (Exception e) {
      // expected
      fail("should success to create subscription once topic created");
    }
  ...
  public void testAutoCreationOfSystemTopicTransactionBufferSnapshot() throws Exception {
    ...
    final String topicString =
        "persistent://prop/ns-abc/" + SystemTopicNames.TRANSACTION_BUFFER_SNAPSHOT;
    ...
    assertTrue(
        admin
            ...
            .getTopics(
                "prop/ns-abc",
                ListNamespaceTopicsOptions.builder().includeSystemTopic(true).build())
            .contains(topicString));
    assertFalse(admin.topics().getPartitionedTopicList("prop/ns-abc").contains(topicString));
  }
  ...
  public void testAutoCreationOfSystemTopicNamespaceEvents() throws Exception {
    ...
    final String topicString =
        "persistent://prop/ns-abc/" + SystemTopicNames.NAMESPACE_EVENTS_LOCAL_NAME;

    @Cleanup Producer<byte[]> producer = pulsarClient.newProducer().topic(topicString).create();
  ...
  public void testDynamicConfigurationTopicAutoCreationPartitionedWhenDefaultMoreThanMax()
    ...
    String topic =
        "persistent://"
            ...
            + "/test-dynamicConfiguration-topic-auto-creation-"
            + UUID.randomUUID();
    // test enable AllowAutoTopicCreation, partitioned when maxNumPartitionsPerPartitionedTopic <
    ...
    // set maxNumPartitionsPerPartitionedTopic, make maxNumPartitionsPerPartitionedTopic <
    // defaultNumPartitions
            ...
            () ->
                assertEquals(
                    admin
                        ...
                        .get("maxNumPartitionsPerPartitionedTopic"),
                    "1"));
    topic =
        "persistent://"
            + namespaceName
  ...
  public void testExtensibleLoadManagerImplInternalTopicAutoCreations()
    ...
    admin
        .topics()
        .createNonPartitionedTopic(ExtensibleLoadManagerImpl.BROKER_LOAD_DATA_STORE_TOPIC);
    admin
        .topics()
        .createNonPartitionedTopic(ExtensibleLoadManagerImpl.TOP_BUNDLES_LOAD_DATA_STORE_TOPIC);
    ...
    try {
      pulsarClient
          .newProducer()
          .topic(ExtensibleLoadManagerImpl.BROKER_LOAD_DATA_STORE_TOPIC)
          .create();
    ...
    try {
      pulsarClient
          .newProducer()
          .topic(ExtensibleLoadManagerImpl.TOP_BUNDLES_LOAD_DATA_STORE_TOPIC)
          .create();
      Assert.fail("Create should have failed.");
    } catch (PulsarClientException.TopicDoesNotExistException
        | PulsarClientException.NotFoundException e) {
