...
public class ShadowReplicatorTest extends BrokerTestBase {
  ...
  protected void setup() throws Exception {
    ...
    admin
        .tenants()
        .createTenant(
            "prop1", new TenantInfoImpl(Sets.newHashSet("appid1"), Sets.newHashSet("test")));
  ...
  public void testShadowReplication() throws Exception {
    ...
    admin
        .topics()
        .setShadowTopics(sourceTopicName, Lists.newArrayList(shadowTopicName, shadowTopicName2));

    @Cleanup Producer<byte[]> producer = pulsarClient.newProducer().topic(sourceTopicName).create();
    ...
    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(sourceTopic.getShadowReplicators().size(), 2));

    ShadowReplicator replicator =
        (ShadowReplicator) sourceTopic.getShadowReplicators().get(shadowTopicName);
    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(String.valueOf(replicator.getState()), "Started"));
    ...
    Awaitility.await()
        .until(() -> PersistentReplicator.PENDING_MESSAGES_UPDATER.get(replicator) == 0);
  ...
  private static PersistentReplicator getAnyShadowReplicator(
      TopicName topicName, PulsarService pulsar) {
            ...
            () -> {
              for (Map.Entry<String, ? extends ReplicatorStats> subStats :
                  admin
                      ...
                      .entrySet()) {
                assertTrue(
                    subStats.getValue().getReplicationBacklog() == 0, "replication task finished");
              }
            });
  }
  ...
  public void testCounterOfPengdingMessagesCorrect() throws Exception {
    ...
    admin
        .topics()
        .setShadowTopics(
            sourceTopicName.toString(), Lists.newArrayList(shadowTopicName.toString()));
