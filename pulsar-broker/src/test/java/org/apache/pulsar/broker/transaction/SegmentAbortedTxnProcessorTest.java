...
public class SegmentAbortedTxnProcessorTest extends TransactionTestBase {
  ...
  private static final String PROCESSOR_TOPIC =
      "persistent://" + NAMESPACE1 + "/abortedTxnProcessor";
  ...
  protected void setup() throws Exception {
    ...
    assertTrue(
        getSnapshotAbortedTxnProcessor(PROCESSOR_TOPIC)
            instanceof SnapshotSegmentAbortedTxnProcessorImpl);
  }
  ...
  /**
   * Test api: 1. putAbortedTxnAndPosition 2. checkAbortedTransaction 3. takeAbortedTxnsSnapshot 4.
   * recoverFromSnapshot 5. trimExpiredAbortedTxns
   *
   * @throws Exception
   */
  @Test
  public void testPutAbortedTxnIntoProcessor() throws Exception {
    PersistentTopic persistentTopic =
        (PersistentTopic)
            pulsarService.getBrokerService().getTopic(PROCESSOR_TOPIC, false).get().get();
  ...
  private void waitTaskExecuteCompletely(AbortedTxnProcessor processor) throws Exception {
    Field workerField =
        SnapshotSegmentAbortedTxnProcessorImpl.class.getDeclaredField("persistentWorker");
    ...
    Field taskQueueField =
        SnapshotSegmentAbortedTxnProcessorImpl.PersistentWorker.class.getDeclaredField("taskQueue");
  ...
  private void verifyAbortedTxnIDAndSegmentIndex(
      AbortedTxnProcessor processor, int begin, int txnIdSize) throws Exception {
    ...
    Field unsealedSegmentField =
        SnapshotSegmentAbortedTxnProcessorImpl.class.getDeclaredField("unsealedTxnIds");
    Field indexField =
        SnapshotSegmentAbortedTxnProcessorImpl.class.getDeclaredField("segmentIndex");
  ...
  public void testFuturesCanCompleteWhenItIsCanceled() throws Exception {
    ...
    queue.add(
        new MutablePair<>(
            SnapshotSegmentAbortedTxnProcessorImpl.PersistentWorker.OperationType.WriteSegment,
  ...
  public void testClearSnapshotSegments() throws Exception {
    ...
    Field indexWriteFutureField =
        SnapshotSegmentAbortedTxnProcessorImpl.PersistentWorker.class.getDeclaredField(
            "snapshotIndexWriter");
    ...
    // After release, the writer should be closed, call close method again to make sure the writer
    // was closed.
  ...
  public void testTxnSegmentStats() throws Exception {
    ...
    this.pulsarService
        .getConfig()
        .setTransactionBufferSnapshotSegmentSize(8 + topic.length() + SEGMENT_SIZE * 3);
    ...
    TransactionBufferStats statsWithoutSegmentStats =
        transactions.getTransactionBufferStats(topic, false, false);
    ...
    assertEquals(
        statsWithoutSegmentStats.snapshotType, AbortedTxnProcessor.SnapshotType.Segment.toString());
    ...
    assertTrue(
        statsWithoutSegmentStats.segmentsStats.lastTookSnapshotSegmentTimestamp >= testStartTime);

    // Get the transaction buffer stats with segment stats
    TransactionBufferStats statsWithSegmentStats =
        transactions.getTransactionBufferStats(topic, false, true);
    ...
    assertEquals(
        statsWithSegmentStats.segmentsStats.segmentStats.get(0).lastTxnID,
  ...
  private void verifySnapshotSegmentsSize(String topic, int size) throws Exception {
    SystemTopicClient.Reader<TransactionBufferSnapshotSegment> reader =
        pulsarService
            ...
            .createReader(TopicName.get(topic))
            .get();
    int segmentCount = 0;
    while (reader.hasMoreEvents()) {
      @Cleanup("release")
      Message<TransactionBufferSnapshotSegment> message =
          reader.readNextAsync().get(5, TimeUnit.SECONDS);
  ...
  private void doCompaction(TopicName topic) throws Exception {
    PersistentTopic snapshotTopic =
        (PersistentTopic)
            pulsarService.getBrokerService().getTopic(topic.toString(), false).get().get();
  ...
  /**
   * This test verifies the compatibility of the transaction buffer segmented snapshot feature when
   * enabled on an existing topic. It performs the following steps: 1. Creates a topic with
   * segmented snapshot disabled. 2. Sends 10 messages without using transactions. 3. Sends 10
   * messages using transactions and aborts them. 4. Sends 10 messages without using transactions.
   * 5. Verifies that only the non-transactional messages are received. 6. Enables the segmented
   * snapshot feature and sets the snapshot segment size. 7. Unloads the topic. 8. Sends a new
   * message using a transaction and aborts it. 9. Verifies that the topic has exactly one segment.
   * 10. Re-subscribes the consumer and re-verifies that only the non-transactional messages are
   * received.
   */
  @Test
  public void testSnapshotProcessorUpgrade() throws Exception {
    ...
    // Create a topic, send 10 messages without using transactions, and send 10 messages using
    // transactions.
    ...
    assertTrue(
        getSnapshotAbortedTxnProcessor(topicName) instanceof SingleSnapshotAbortedTxnProcessorImpl);
    ...
    assertTrue(
        getSnapshotAbortedTxnProcessor(topicName)
            instanceof SnapshotSegmentAbortedTxnProcessorImpl);
    ...
    // Re-subscribes the consumer and re-verifies that only the non-transactional messages are
    // received.
    consumer.close();
   ...
   * does not create a __transaction_buffer_snapshot topic in the same namespace. The test performs
   * the following steps: 1. Enable the segmented snapshot feature. 2. Create a new namespace. 3.
   * Create a new topic in the namespace. 4. Check that the __transaction_buffer_snapshot topic is
   * not created in the same namespace. 5. Destroy the namespace after the test.
   */
  @Test
  public void testSegmentedSnapshotWithoutCreatingOldSnapshotTopic() throws Exception {
    ...
    String transactionBufferSnapshotTopic =
        "persistent://" + namespaceName + "/" + SystemTopicNames.TRANSACTION_BUFFER_SNAPSHOT;
    try {
  ...
  private AbortedTxnProcessor getSnapshotAbortedTxnProcessor(String topicName) {
    ...
    return WhiteboxImpl.getInternalState(
        persistentTopic.getTransactionBuffer(), "snapshotAbortedTxnProcessor");
  }
