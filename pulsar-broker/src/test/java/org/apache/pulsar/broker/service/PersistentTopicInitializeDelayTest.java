...
public class PersistentTopicInitializeDelayTest extends BrokerTestBase {
  ...
  public void testTopicInitializeDelay() throws Exception {
    admin
        .tenants()
        .createTenant(
            "public", TenantInfo.builder().allowedClusters(Set.of(configClusterName)).build());
    ...
    Awaitility.await()
        .untilAsserted(() -> assertEquals(admin.topicPolicies().getMaxConsumers(topicName), 10));
