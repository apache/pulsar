...
public class GetPartitionMetadataTest extends TestRetrySupport {
  ...
  protected void setup() throws Exception {
    ...
    admin1
        .tenants()
        .createTenant(
            NamespaceName.get(DEFAULT_NS).getTenant(),
  ...
  protected static void doModifyTopicAutoCreation(
      PulsarAdmin admin1,
    ...
    admin1
        .brokers()
        .updateDynamicConfiguration("allowAutoTopicCreation", allowAutoTopicCreation + "");
    admin1
        .brokers()
        .updateDynamicConfiguration("allowAutoTopicCreationType", allowAutoTopicCreationType + "");
    admin1.brokers().updateDynamicConfiguration("defaultNumPartitions", defaultNumPartitions + "");
    Awaitility.await()
        .untilAsserted(
            () -> {
              assertEquals(
                  pulsar1.getConfiguration().isAllowAutoTopicCreation(), allowAutoTopicCreation);
              assertEquals(
                  pulsar1.getConfiguration().getAllowAutoTopicCreationType(),
                  allowAutoTopicCreationType);
              assertEquals(
                  pulsar1.getConfiguration().getDefaultNumPartitions(), defaultNumPartitions);
            });
  }

  protected void modifyTopicAutoCreation(
      boolean allowAutoTopicCreation,
      TopicType allowAutoTopicCreationType,
      int defaultNumPartitions)
      throws Exception {
    doModifyTopicAutoCreation(
        admin1, pulsar1, allowAutoTopicCreation, allowAutoTopicCreationType, defaultNumPartitions);
  }
    ...
    for (PulsarClientImpl client : getClientsToTest()) {
      ...
      Optional<PartitionedTopicMetadata> metadata1 =
          pulsar1
              ...
              .getPartitionedTopicMetadataAsync(TopicName.get(tp), true)
              .join();
  ...
  public void testGetMetadataIfNonPartitionedTopicExists(
      boolean configAllowAutoTopicCreation,
  ...
  public void testAutoCreatePartitionedTopic(boolean isUsingHttpLookup, TopicDomain topicDomain)
      throws Exception {
    modifyTopicAutoCreation(true, TopicType.PARTITIONED, 3);
    ...
    for (PulsarClientImpl client : clientArray) {
      ...
      verifyNonPartitionedTopicNeverCreated(topicNameStr);
      // The API "getPartitionedTopicMetadata" only creates the partitioned metadata, it will not
      // create the
      // partitions.
      verifyPartitionsNeverCreated(topicNameStr);

      // Case-2: topic with suffix "-partition-1".
      final String topicNameStrWithSuffix =
          BrokerTestUtil.newUniqueName(topicDomain.value() + "://" + DEFAULT_NS + "/tp")
              + "-partition-1";
      // Verify: the result of get partitioned topic metadata.
      ...
      // Verify: the behavior of topic creation.
      List<String> partitionedTopics2 = admin1.topics().getPartitionedTopicList("public/default");
      assertFalse(partitionedTopics2.contains(topicNameStrWithSuffix));
      assertFalse(
          partitionedTopics2.contains(
  ...
  @Test(dataProvider = "clients")
  public void testAutoCreateNonPartitionedTopic(boolean isUsingHttpLookup, TopicDomain topicDomain)
      throws Exception {
  ...
  public void testGetMetadataIfNotAllowedCreate(
      boolean configAllowAutoTopicCreation,
      boolean paramMetadataAutoCreationEnabled,
      boolean isUsingHttpLookup)
      throws Exception {
  ...
  /**
   * Regarding the API "get partitioned metadata" about non-persistent topic. The original behavior
   * is: param-auto-create = true, broker-config-auto-create = true HTTP API: default configuration
   * {@link ServiceConfiguration#getDefaultNumPartitions()} binary API: default configuration {@link
   * ServiceConfiguration#getDefaultNumPartitions()} param-auto-create = true,
   * broker-config-auto-create = false HTTP API: {partitions: 0} binary API: {partitions: 0}
   * param-auto-create = false HTTP API: not found error binary API: not support After PIP-344, the
   * behavior will be the same as persistent topics, which was described in PIP-344.
   */
  @Test(dataProvider = "autoCreationParamsNotAllow")
  public void testGetMetadataIfNotAllowedCreateOfNonPersistentTopic(
      boolean configAllowAutoTopicCreation,
      boolean paramMetadataAutoCreationEnabled,
      boolean isUsingHttpLookup)
      throws Exception {
  ...
  public void testNamespaceNotExist(TopicDomain topicDomain) throws Exception {
    ...
    final String topicNameStr =
        BrokerTestUtil.newUniqueName(topicDomain.toString() + "://" + namespaceNotExist + "/tp");
    PulsarClientImpl[] clientArray = getClientsToTest(false);
    for (PulsarClientImpl client : clientArray) {
      try {
        PartitionedTopicMetadata topicMetadata =
            client.getPartitionedTopicMetadata(topicNameStr, true, true).join();
        log.info("Get topic metadata: {}", topicMetadata.partitions);
        fail("Expected a not found ex");
      } catch (Exception ex) {
        Throwable unwrapEx = FutureUtil.unwrapCompletionException(ex);
  ...
  public void testTenantNotExist(TopicDomain topicDomain) throws Exception {
    ...
    for (PulsarClientImpl client : clientArray) {
      try {
        ...
        Throwable unwrapEx = FutureUtil.unwrapCompletionException(ex);
        assertTrue(
            unwrapEx instanceof PulsarClientException.BrokerMetadataException
                || unwrapEx instanceof PulsarClientException.TopicDoesNotExistException);
      }
    }
    // Verify: lookup semaphore has been releases.
