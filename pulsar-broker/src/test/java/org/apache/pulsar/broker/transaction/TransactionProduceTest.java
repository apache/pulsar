...
public class TransactionProduceTest extends TransactionTestBase {
  ...
  private void produceTest(boolean endAction) throws Exception {
    ...
    for (int i = 0; i < TOPIC_PARTITION; i++) {
      ...
      ReadOnlyCursor originTopicCursor = getOriginTopicCursor(topic, i);
      List<Entry> entries =
          originTopicCursor.readEntries((int) originTopicCursor.getNumberOfEntries());
  ...
  @Test
  public void testUpdateLastMaxReadPositionMovedForwardTimestampForTransactionalPublish()
      throws Exception {
    final String topic =
        NAMESPACE1 + "/testUpdateLastMaxReadPositionMovedForwardTimestampForTransactionalPublish";
    PulsarClient pulsarClient = this.pulsarClient;
    Transaction txn =
        pulsarClient.newTransaction().withTransactionTimeout(5, TimeUnit.SECONDS).build().get();
    @Cleanup
    Producer<byte[]> producer =
        pulsarClient.newProducer().topic(topic).sendTimeout(0, TimeUnit.SECONDS).create();
  ...
  private PersistentTopic getTopic(String topic) throws ExecutionException, InterruptedException {
    Optional<Topic> optionalTopic =
        getPulsarServiceList().get(0).getBrokerService().getTopic(topic, true).get();
  ...
  public void ackCommitTest() throws Exception {
    ...
    @Cleanup
    Producer<byte[]> incomingProducer =
        pulsarClient
            .newProducer()
  ...
  public void ackAbortTest() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(
            () ->
                Assert.assertEquals(
                    getPendingAckCount(ACK_ABORT_TOPIC, subscriptionName), incomingMessageCnt));
  ...
  public void testCommitFailure() throws Exception {
    ...
    @Cleanup final Producer<byte[]> producer = pulsarClient.newProducer().topic(topic).create();
    ...
    try {
      ...
      Assert.assertTrue(
          e.getCause() instanceof PulsarClientException.TransactionHasOperationFailedException);
