...
public class ResourceGroupRateLimiterTest extends BrokerTestBase {
  ...
  public void createResourceGroup(
      String rgName, org.apache.pulsar.common.policies.data.ResourceGroup rg)
  ...
  private void testRateLimit()
    ...
    try {
      ...
      final String errMesg =
          String.format("Got exception while building producer: ex=%s", p.getMessage());
      Assert.fail(errMesg);
    }
    ...
    // In the next interval, the above message will be accepted. Wait for one more second (total
    // 2s),
