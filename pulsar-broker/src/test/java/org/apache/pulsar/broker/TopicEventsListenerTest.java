...
public class TopicEventsListenerTest extends BrokerTestBase {
            ...
            (topic, event, stage, t) -> {
              ...
              if (topic.equals(topicNameToWatch)) {
                if (log.isDebugEnabled()) {
                  log.debug(
                      "got event {}__{} for topic {} with detailed stack",
                      event,
                      stage,
                      topic,
                      new Exception("tracing event source"));
                }
    ...
    try (PulsarAdmin admin2 = createPulsarAdmin()) {
      Awaitility.await()
          .untilAsserted(
              () ->
                  assertEquals(
                      admin2.namespaces().getRetention(namespace), new RetentionPolicies(3, 10)));
    }
  ...
  public void testEvents(
      String topicTypePersistence, String topicTypePartitioned, boolean forceDelete)
      throws Exception {
    String topicName =
        topicTypePersistence + "://" + namespace + "/" + "topic-" + UUID.randomUUID();
        ...
        .untilAsserted(
            () ->
                Assert.assertEquals(
                    events.toArray(),
                    new String[] {
                      "DELETE__BEFORE", "UNLOAD__BEFORE", "UNLOAD__SUCCESS", "DELETE__SUCCESS"
                    }));
  }

  @Test(dataProvider = "topicType")
  public void testEventsWithUnload(
      String topicTypePersistence, String topicTypePartitioned, boolean forceDelete)
      throws Exception {
    String topicName =
        topicTypePersistence + "://" + namespace + "/" + "topic-" + UUID.randomUUID();
    ...
    Awaitility.waitAtMost(10, TimeUnit.SECONDS)
        .untilAsserted(
            () ->
                Assert.assertEquals(
                    events.toArray(), new String[] {"UNLOAD__BEFORE", "UNLOAD__SUCCESS"}));
    ...
    Awaitility.waitAtMost(10, TimeUnit.SECONDS)
        .untilAsserted(
            () ->
                Assert.assertEquals(
                    events.toArray(), new String[] {"DELETE__BEFORE", "DELETE__SUCCESS"}));
  }

  @Test(dataProvider = "topicType")
  public void testEventsActiveSub(
      String topicTypePersistence, String topicTypePartitioned, boolean forceDelete)
      throws Exception {
    String topicName =
        topicTypePersistence + "://" + namespace + "/" + "topic-" + UUID.randomUUID();
    ...
    try {
      ...
      assertTrue(
          e.getMessage().contains("Topic has active producers/subscriptions")
    ...
    if (forceDelete) {
      ...
      expectedEvents = new String[] {"DELETE__BEFORE", "DELETE__FAILURE"};
    }

    Awaitility.waitAtMost(10, TimeUnit.SECONDS)
        .untilAsserted(
            () -> {
              // only care about first 4 events max, the rest will be from client recreating deleted
              // topic
              String[] eventsToArray =
                  (events.size() <= 4)
  ...
  public void testTopicAutoGC(String topicTypePersistence, String topicTypePartitioned)
      throws Exception {
    String topicName =
        topicTypePersistence + "://" + namespace + "/" + "topic-" + UUID.randomUUID();
    ...
    try (PulsarAdmin admin2 = createPulsarAdmin()) {
      Awaitility.await()
          .untilAsserted(
              () ->
                  assertEquals(
                      admin2.namespaces().getRetention(namespace), new RetentionPolicies()));
    }
        ...
        .untilAsserted(
            () ->
                Assert.assertEquals(
                    events.toArray(),
                    new String[] {
                      "UNLOAD__BEFORE", "UNLOAD__SUCCESS",
                    }));
  }

  private void createTopicAndVerifyEvents(
      String topicDomain, String topicTypePartitioned, String topicName) throws Exception {
    final String[] expectedEvents;
    if (topicDomain.equalsIgnoreCase("persistent") || topicTypePartitioned.equals("partitioned")) {
      if (topicTypePartitioned.equals("partitioned")) {
        ...
        expectedEvents =
            new String[] {"LOAD__BEFORE", "CREATE__BEFORE", "CREATE__SUCCESS", "LOAD__SUCCESS"};
      }
    } else {
      expectedEvents =
          new String[] {
            // Before https://github.com/apache/pulsar/pull/21995, Pulsar will skip create topic if
            // the topic
            //   was already exists, and the action "check topic exists" will try to load Managed
            // ledger,
    ...
    Awaitility.waitAtMost(10, TimeUnit.SECONDS)
        .untilAsserted(() -> Assert.assertEquals(events.toArray(), expectedEvents));
  }
