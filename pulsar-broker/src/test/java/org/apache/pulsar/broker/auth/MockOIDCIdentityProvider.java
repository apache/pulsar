...
/**
 * Mock OIDC (and therefore OAuth2) server for testing. Note that the client_id is mapped to the
...
public class MockOIDCIdentityProvider {
  ...
  public MockOIDCIdentityProvider(String clientSecret, String audience, long tokenTTLMillis) {
    ...
    server =
        new WireMockServer(
            wireMockConfig().port(0).extensions(new OAuth2Transformer(keyPair, tokenTTLMillis)));
    server.start();
