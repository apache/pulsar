...
public class TenantEndpointsAuthorizationTest extends MockedPulsarStandalone {
  ...
  public void testListTenants() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowTenantOperationAsync(
            isNull(), Mockito.eq(TenantOperation.LIST_TENANTS), any(), any());
    ...
    Assert.assertThrows(
        ...
        () -> nobodyAdmin.tenants().getTenants());
  }
  ...
  public void testGetTenant() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowTenantOperationAsync(
            eq(tenantName), Mockito.eq(TenantOperation.GET_TENANT), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    ...
    Assert.assertThrows(
        PulsarAdminException.NotAuthorizedException.class,
        () -> nobodyAdmin.tenants().getTenantInfo(tenantName));
  }
    ...
    superUserAdmin
        ...
        .updateTenant(
            ...
            TenantInfo.builder()
                ...
                .adminRoles(Set.of("example"))
                .build());
    // test allow broker operation
    verify(spyAuthorizationService)
        .allowTenantOperationAsync(
            eq(tenantName), Mockito.eq(TenantOperation.UPDATE_TENANT), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    ...
    Assert.assertThrows(
        ...
        () ->
            nobodyAdmin
                .tenants()
                .updateTenant(
                    tenantName, TenantInfo.builder().adminRoles(Set.of("example")).build()));
  }
  ...
  public void testDeleteTenant() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowTenantOperationAsync(
            eq(tenantName), Mockito.eq(TenantOperation.DELETE_TENANT), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    ...
    Assert.assertThrows(
        PulsarAdminException.NotAuthorizedException.class,
        () -> nobodyAdmin.tenants().deleteTenant(tenantName));
  }
}
