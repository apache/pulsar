...
public abstract class AbstractDeliveryTrackerTest {
  ...
  @Test(dataProvider = "delayedTracker")
  public void testWithTimer(DelayedDeliveryTracker tracker, NavigableMap<Long, TimerTask> tasks)
      throws Exception {
  ...
  /**
   * Adding a message that is about to expire within the tick time should lead to a rejection from
   * the tracker when isDelayedDeliveryDeliverAtTimeStrict is false.
   */
  @Test(dataProvider = "delayedTracker")
  ...
  /**
   * In this test, the deliverAt time is after now, but the deliverAt time is too early to run
   * another tick, so the tickTimeMillis determines the delay.
   */
  @Test(dataProvider = "delayedTracker")
  public void testAddMessageWithDeliverAtTimeAfterNowBeforeTickTimeFrequencyWithStrict(
      DelayedDeliveryTracker tracker) throws Exception {
    ...
    // Wait longer than the tick time plus the HashedWheelTimer's tick time to ensure that enough
    // time has
  ...
  /**
   * In this test, the deliverAt time is after now, but before the (tickTimeMillis + now). Because
   * there wasn't a recent tick run, the deliverAt time determines the delay.
   */
  @Test(dataProvider = "delayedTracker")
  public void testAddMessageWithDeliverAtTimeAfterNowAfterTickTimeFrequencyWithStrict(
    ...
    // Wait long enough for the runnable to run, but not longer than the tick time. The point is
    // that the delivery
  ...
  /**
   * In this test, the deliverAt time is after now plus tickTimeMillis, so the tickTimeMillis
