...
public class DisabledCreateTopicToRemoteClusterForReplicationTest extends OneWayReplicatorTestBase {
  ...
  public void testCreatePartitionedTopicWithNsReplication() throws Exception {
    ...
    admin1
        .namespaces()
        .setNamespaceReplicationClusters(ns, new HashSet<>(Arrays.asList(cluster1, cluster2)));
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              PersistentTopic topicPart1 =
                  (PersistentTopic) broker1.getTopic(part1, false).join().get();
  ...
  public void testNonPartitionedTopic() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              PersistentTopic topicPart1 =
                  (PersistentTopic) broker1.getTopic(tp, false).join().get();
              assertFalse(topicPart1.getReplicators().isEmpty());
            });
