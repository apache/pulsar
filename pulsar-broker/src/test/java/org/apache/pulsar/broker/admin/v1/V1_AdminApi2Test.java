...
public class V1_AdminApi2Test extends MockedPulsarServiceBaseTest {
  ...
  public void testIncrementPartitionsOfTopic() throws Exception {
    ...
    assertEquals(
        admin.topics().getPartitionedTopicMetadata(partitionedTopicName).partitions,
        startPartitions);

    // create consumer and subscriptions : check subscriptions
    @Cleanup PulsarClient client = PulsarClient.builder().serviceUrl(pulsarUrl.toString()).build();
    ...
    assertEquals(
        admin.topics().getPartitionedTopicMetadata(partitionedTopicName).partitions, newPartitions);
    // (2) No Msg loss: verify new partitions have the same existing subscription names
    final String newPartitionTopicName =
        TopicName.get(partitionedTopicName).getPartition(startPartitions + 1).toString();
    ...
    // (4) verify existing subscription has not lost any message: create new consumer with sub-2: it
    // will load all
    ...
    consumer2 =
        client
            ...
            .subscribe();
    assertEquals(
        new HashSet<>(admin.topics().getSubscriptions(newPartitionTopicName)),
        Set.of(subName1, subName2));
  ...
  public void nonPersistentTopics() throws Exception {
    ...
    Consumer<byte[]> consumer =
        client.newConsumer().topic(persistentTopicName).subscriptionName("my-sub").subscribe();
    ...
    PersistentTopicInternalStats internalStats =
        admin.nonPersistentTopics().getInternalStats(persistentTopicName);
    ...
    assertEquals(
        admin.nonPersistentTopics().getPartitionedTopicMetadata(partitionedTopicName).partitions,
        5);
  }

  private void publishMessagesOnTopic(String topicName, int messages, int startIdx)
      throws Exception {
    Producer<byte[]> producer =
        pulsarClient
            .newProducer()
  ...
  public void testSetPersistencepolicies() throws Exception {
    ...
    assertEquals(
        admin.namespaces().getPersistence(namespace), new PersistencePolicies(3, 3, 3, 10.0));
  ...
  public void testUpdatePersistencePolicyUpdateManagedCursor() throws Exception {
    ...
    assertEquals(
        admin.namespaces().getPersistence(namespace), new PersistencePolicies(3, 3, 3, 50.0));
    ...
    admin
        .namespaces()
        .setPersistence(namespace, new PersistencePolicies(newEnsembleSize, 3, 3, newThrottleRate));

    retryStrategically(
        (test) ->
            managedLedger.getConfig().getEnsembleSize() == newEnsembleSize
                && cursor.getThrottleMarkDelete() != newThrottleRate,
        5,
        200);
  ...
  public void testResetCursorOnPosition(String namespaceName) throws Exception {
    ...
    MessageIdImpl messageId =
        new MessageIdImpl(resetMessageId.getLedgerId(), resetMessageId.getEntryId(), -1);
  ...
  /**
   * It verifies that pulsar with different load-manager generates different load-report and
  ...
  public void testPeerCluster() throws Exception {
    admin
        ...
        .createCluster(
            "us-west1",
            ClusterData.builder()
                .serviceUrl("http://broker.messaging.west1.example.com:8080")
                .build());
    admin
        .clusters()
        .createCluster(
            "us-west2",
            ClusterData.builder()
                .serviceUrl("http://broker.messaging.west2.example.com:8080")
                .build());
    admin
        .clusters()
        .createCluster(
            "us-east1",
            ClusterData.builder()
                .serviceUrl("http://broker.messaging.east1.example.com:8080")
                .build());
    admin
        .clusters()
        .createCluster(
            "us-east2",
            ClusterData.builder()
                .serviceUrl("http://broker.messaging.east2.example.com:8080")
                .build());

    admin.clusters().updatePeerClusterNames("us-west1", Sets.newLinkedHashSet(List.of("us-west2")));
    assertEquals(
        admin.clusters().getCluster("us-west1").getPeerClusterNames(), List.of("us-west2"));
    ...
    assertEquals(
        admin.clusters().getCluster("us-west1").getPeerClusterNames(),
    ...
    try {
      admin
          .clusters()
          .updatePeerClusterNames("us-west1", Sets.newLinkedHashSet(List.of("invalid-cluster")));
      fail("should have failed");
    } catch (PulsarAdminException e) {
    ...
    try {
      admin
          .clusters()
          .updatePeerClusterNames("us-west1", Sets.newLinkedHashSet(List.of("us-west1")));
      fail("should have failed");
    } catch (PulsarAdminException e) {
      assertTrue(e instanceof PreconditionFailedException);
    }
  ...
  public void testReplicationPeerCluster() throws Exception {
    ...
    Set<String> allowedClusters =
        Set.of("us-west1", "us-west2", "us-west3", "us-west4", "us-east1", "us-east2");
    ...
    admin
        .clusters()
        .updatePeerClusterNames("us-west1", Sets.newLinkedHashSet(List.of("us-west2", "us-west3")));
    assertEquals(
        admin.clusters().getCluster("us-west1").getPeerClusterNames(),
  ...
  public void namespaceAntiAffinity() throws PulsarAdminException {
    ...
    List<String> namespaces2 =
        admin.namespaces().getAntiAffinityNamespaces("prop-xyz", "use", "invalid-group");
    ...
    retryStrategically(
        (test) -> {
          ...
          try {
            ...
            return stats.getPublishers().size() > 0
                && stats.getSubscriptions().get(subscriberName) != null
