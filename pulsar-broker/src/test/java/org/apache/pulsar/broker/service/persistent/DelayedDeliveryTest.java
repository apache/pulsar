...
public class DelayedDeliveryTest extends ProducerConsumerBase {
  ...
  public void testDelayedDelivery() throws Exception {
    ...
    for (int i = 0; i < 10; i++) {
      producer.newMessage().value("msg-" + i).deliverAfter(5, TimeUnit.SECONDS).sendAsync();
    }
    ...
    BrokerOpenTelemetryTestUtil.assertMetricLongSumValue(
        metrics, OpenTelemetryTopicStats.DELAYED_SUBSCRIPTION_COUNTER, attributes, 10);
  ...
  public void testEverythingFilteredInMultipleReads() throws Exception {
    ...
    for (int i = 0; i < 10; i++) {
      producer.newMessage().value("msg-" + i).deliverAfter(5, TimeUnit.SECONDS).send();
    }
  ...
  public void testOrderingDispatch() throws PulsarClientException {
    ...
    for (int i = 0; i < N; i++) {
      if (i < N - 1) {
        assertTrue(
            receives.get(i).getMessageId().compareTo(receives.get(i + 1).getMessageId()) < 0);
      }
    }
  }
  ...
  public void testEnableAndDisableTopicDelayedDelivery() throws Exception {
    ...
    assertEquals(
        5000, admin.topics().getDelayedDeliveryPolicy(topicName).getMaxDeliveryDelayInMillis());
  ...
  public void testEnableTopicDelayedDelivery() throws Exception {
    ...
    assertNotNull(msg);
    consumer.acknowledge(msg);
    // 7 Set a very long tick time, so that trackDelayedDelivery will fail. we can receive msg
    // immediately.
    delayedDeliveryPolicies =
        DelayedDeliveryPolicies.builder().tickTime(Integer.MAX_VALUE).active(true).build();
    admin.topics().setDelayedDeliveryPolicy(topicName, delayedDeliveryPolicies);
    // wait for update
    for (int i = 0; i < 50; i++) {
      Thread.sleep(100);
  ...
  @Test
  public void testClearDelayedMessagesWhenClearBacklog()
    ...
    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(dispatcher.getNumberOfDelayedMessages(), 0));
  }
  ...
  public void testDelayedDeliveryWithAllConsumersDisconnecting() throws Exception {
    ...
    Dispatcher dispatcher =
        pulsar
            ...
            .getDispatcher();
    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(dispatcher.getNumberOfDelayedMessages(), 1));

    c1.close();

    // Attach a new consumer. Since there are no consumers connected, this will trigger the cursor
    // rewind
    @Cleanup
    Consumer<String> c2 =
        pulsarClient
  ...
  public void testDelayedDeliveryExceedsMaxDelay() throws Exception {
    ...
    try {
      producer.newMessage().value("msg").deliverAfter(6, TimeUnit.SECONDS).send();
