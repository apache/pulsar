...
public class BrokerInternalClientConfigurationOverrideTest extends BrokerTestBase {
  ...
  public void testPulsarServiceAdminClientConfiguration() throws PulsarServerException {
    ...
    ClientConfigurationData clientConf =
        ((PulsarAdminImpl) pulsar.getAdminClient()).getClientConfigData();
  ...
  public void testNamespaceServicePulsarClientConfiguration() {
    // This data only needs to have the service url for this test.
    ClusterDataImpl data =
        (ClusterDataImpl) ClusterData.builder().serviceUrl("http://localhost:8080").build();
  ...
  public void testOldNamespacePolicy() throws Exception {
    ...
    assertEquals(
        topicRef.topicPolicies.getMaxUnackedMessagesOnSubscription().get(),
        conf.getMaxUnackedMessagesPerSubscription());
    assertEquals(
        topicRef.topicPolicies.getMaxConsumersPerSubscription().get(),
        conf.getMaxConsumersPerSubscription());
    assertEquals(
        topicRef.topicPolicies.getMaxConsumerPerTopic().get(), conf.getMaxConsumersPerTopic());
    assertEquals(
        topicRef.topicPolicies.getMaxProducersPerTopic().get(), conf.getMaxProducersPerTopic());
    assertEquals(
        topicRef.topicPolicies.getMaxSubscriptionsPerTopic().get(),
        conf.getMaxSubscriptionsPerTopic());
    assertEquals(topicRef.topicPolicies.getTopicMaxMessageSize().get(), conf.getMaxMessageSize());
    assertEquals(
        topicRef.topicPolicies.getMaxUnackedMessagesOnConsumer().get(),
