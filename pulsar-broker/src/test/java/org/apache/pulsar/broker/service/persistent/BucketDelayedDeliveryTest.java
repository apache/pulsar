...
public class BucketDelayedDeliveryTest extends DelayedDeliveryTest {
    ...
    public void testBucketDelayedDeliveryWithAllConsumersDisconnecting() throws Exception {
        ...
        List<String> bucketKeys = ((PersistentDispatcherMultipleConsumers) dispatcher)
                .getCursor().getCursorProperties().keySet().stream()
                        .filter(x -> x.startsWith(ManagedCursorImpl.CURSOR_INTERNAL_PROPERTY_PREFIX))
                        .toList();
        ...
        Dispatcher dispatcher2 = pulsar.getBrokerService()
                .getTopicReference(topic)
                .get()
                .getSubscription("sub")
                .getDispatcher();
        List<String> bucketKeys2 = ((PersistentDispatcherMultipleConsumers) dispatcher2)
                .getCursor().getCursorProperties().keySet().stream()
                        .filter(x -> x.startsWith(ManagedCursorImpl.CURSOR_INTERNAL_PROPERTY_PREFIX))
                        .toList();
    ...
    public void testUnsubscribe() throws Exception {
        ...
        List<Long> bucketIds = cursorProperties.entrySet().stream()
                .filter(x -> x.getKey().startsWith(CURSOR_INTERNAL_PROPERTY_PREFIX + "delayed.bucket"))
        ...
        for (Long bucketId : bucketIds) {
            try {
                LedgerHandle ledgerHandle = pulsarTestContext
                        .getBookKeeperClient()
    ...
    public void testBucketDelayedIndexMetrics() throws Exception {
        ...
        bucketsMetrics.stream()
                .filter(metric -> metric.tags.containsKey("subscription"))
                .forEach(metric -> {
                    assertEquals(3, metric.value);
                    bucketsSum.add(metric.value);
                });
        assertEquals(6, bucketsSum.intValue());
        Optional<PrometheusMetricsTest.Metric> bucketsTopicMetric = bucketsMetrics.stream()
                .filter(metric -> !metric.tags.containsKey("subscription"))
                .findFirst();
        ...
        long count = loadedIndexMetrics.stream()
                .filter(metric -> metric.tags.containsKey("subscription"))
                .peek(metric -> {
        ...
        Optional<PrometheusMetricsTest.Metric> loadedIndexTopicMetrics = bucketsMetrics.stream()
                .filter(metric -> !metric.tags.containsKey("subscription"))
                .findFirst();
        ...
        count = snapshotSizeBytesMetrics.stream()
                .filter(metric -> metric.tags.containsKey("subscription"))
                .peek(metric -> {
                ...
                .count();
        assertEquals(2, count);
        Optional<PrometheusMetricsTest.Metric> snapshotSizeBytesTopicMetrics = snapshotSizeBytesMetrics.stream()
                .filter(metric -> !metric.tags.containsKey("subscription"))
                .findFirst();
        ...
        count = opCountMetrics.stream()
                ...
                .count();
        assertEquals(2, count);
        Optional<PrometheusMetricsTest.Metric> opCountTopicMetrics = opCountMetrics.stream()
                .filter(metric -> metric.tags.get("state").equals("succeed")
                        && metric.tags.get("type").equals("create")
                        && !metric.tags.containsKey("subscription"))
                .findFirst();
        ...
        count = opLatencyMetrics.stream()
                .filter(metric -> metric.tags.get("type").equals("create") && metric.tags.containsKey("subscription"))
                .peek(metric -> {
                ...
                .count();
        assertTrue(count >= 2);
        Optional<PrometheusMetricsTest.Metric> opLatencyTopicMetrics = opCountMetrics.stream()
                .filter(metric -> metric.tags.get("type").equals("create") && !metric.tags.containsKey("subscription"))
                .findFirst();
        ...
        Optional<PrometheusMetricsTest.Metric> namespaceMetric =
                namespaceMetricsMap.get("pulsar_delayed_message_index_bucket_total").stream()
                        .findFirst();
