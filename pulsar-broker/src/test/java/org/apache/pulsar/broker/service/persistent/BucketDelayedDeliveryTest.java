...
public class BucketDelayedDeliveryTest extends DelayedDeliveryTest {
  ...
  public void testBucketDelayedDeliveryWithAllConsumersDisconnecting() throws Exception {
    String topic =
        BrokerTestUtil.newUniqueName("persistent://public/default/testDelaysWithAllConsumerDis");
    ...
    Dispatcher dispatcher =
        pulsar
            ...
            .getDispatcher();
    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(dispatcher.getNumberOfDelayedMessages(), 1000));
    List<String> bucketKeys =
        ((AbstractPersistentDispatcherMultipleConsumers) dispatcher)
            .getCursor().getCursorProperties().keySet().stream()
                .filter(x -> x.startsWith(CURSOR_INTERNAL_PROPERTY_PREFIX))
                .toList();
    ...
    List<String> bucketKeys2 =
        ((AbstractPersistentDispatcherMultipleConsumers) dispatcher2)
            .getCursor().getCursorProperties().keySet().stream()
                .filter(x -> x.startsWith(CURSOR_INTERNAL_PROPERTY_PREFIX))
                .toList();

    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(dispatcher2.getNumberOfDelayedMessages(), 1000));
  ...
  public void testUnsubscribe() throws Exception {
    ...
    Map<String, String> cursorProperties =
        ((AbstractPersistentDispatcherMultipleConsumers) dispatcher)
            ...
            .getCursorProperties();
    List<Long> bucketIds =
        cursorProperties.entrySet().stream()
  ...
  public void testBucketDelayedIndexMetrics() throws Exception {
    ...
    for (int i = 0; i < N; i++) {
      producer.newMessage().value("msg-" + i).deliverAfter(3600 + i, TimeUnit.SECONDS).sendAsync();
    }
    producer.flush();
    ...
    count =
        snapshotSizeBytesMetrics.stream()
            .filter(metric -> metric.tags.containsKey("subscription"))
            .peek(
                metric -> {
    ...
    count =
        opCountMetrics.stream()
            .filter(
                metric ->
                    metric.tags.get("state").equals("succeed")
  ...
  public void testDeleteTopicIfCursorPropsEmpty(SubscriptionType subscriptionType)
      throws Exception {
  ...
  /** see: https://github.com/apache/pulsar/pull/21595. */
  @Test(dataProvider = "subscriptionTypes")
  public void testDeletePartitionedTopicIfCursorPropsEmpty(SubscriptionType subscriptionType)
      throws Exception {
    final String topic = BrokerTestUtil.newUniqueName("persistent://my-property/my-ns/tp_");
    final String subscriptionName = "s1";
    // create a topic.
  ...
  /** see: https://github.com/apache/pulsar/pull/21595. */
  @Test(dataProvider = "subscriptionTypes")
  public void testDeleteTopicIfCursorPropsNotEmpty(SubscriptionType subscriptionType)
      throws Exception {
    final String topic = BrokerTestUtil.newUniqueName("persistent://my-property/my-ns/tp_");
    final String subscriptionName = "s1";
    // create a topic.
  ...
  /** see: https://github.com/apache/pulsar/pull/21595. */
  @Test(dataProvider = "subscriptionTypes")
  public void testDeletePartitionedTopicIfCursorPropsNotEmpty(SubscriptionType subscriptionType)
      throws Exception {
    final String topic = BrokerTestUtil.newUniqueName("persistent://my-property/my-ns/tp_");
    final String subscriptionName = "s1";
    // create a topic.
