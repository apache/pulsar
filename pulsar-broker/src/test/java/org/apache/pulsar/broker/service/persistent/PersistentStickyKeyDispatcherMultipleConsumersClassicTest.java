  ...
  public void testSkipRedeliverTemporally() throws InterruptedException {
    ...
    try {
      Field totalAvailablePermitsField =
          PersistentDispatcherMultipleConsumersClassic.class.getDeclaredField(
    ...
    doAnswer(
        ...
        .when(cursorMock)
        .asyncReadEntriesOrWait(anyInt(), anyLong(), any(), any(), any());
    ...
    redeliverEntries.forEach(
        entry -> {
          ...
          // add to redeliver
          persistentDispatcher.addMessageToReplay(
              entry.getLedgerId(), entry.getEntryId(), stickyKeyHash);
        });
