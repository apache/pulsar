...
public class ResourceGroupsTest extends MockedPulsarServiceBaseTest {
  ...
  public void testCrudResourceGroups() throws Exception {
    ...
    resourcegroups.createOrUpdateResourceGroup(
        "test-resourcegroup-one", testResourceGroupOneUpdate);
  ...
  private void prepareData() throws PulsarAdminException {
    admin
        .clusters()
        .createCluster(
            testCluster, ClusterData.builder().serviceUrl(pulsar.getWebServiceAddress()).build());
    admin
        .tenants()
        .createTenant(
            testTenant, new TenantInfoImpl(Set.of("role1", "role2"), Set.of(testCluster)));
  }
}
