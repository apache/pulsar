...
public class PeerReplicatorTest extends ReplicatorTestBase {
  ...
  /**
   * It verifies that lookup/admin requests for global-namespace would be redirected to peer-cluster
   * if local cluster doesn't own it and peer-cluster owns it, else request will be failed.
   *
   * <pre>
  ...
  public void testPeerClusterTopicLookup(String protocol) throws Exception {
    ...
    PersistentTopic topic =
        (PersistentTopic) pulsar1.getBrokerService().getOrCreateTopic(topic1).get();
    assertNotNull(topic);
  ...
  /**
   * Removing local cluster from the replication-cluster should make sure that bundle should not be
   * loaded by the cluster even if owner broker doesn't receive the watch to avoid lookup-conflict
   * between peer-cluster.
   *
   * @throws Exception
   */
  @Test(groups = "broker")
  public void testPeerClusterInReplicationClusterListChange() throws Exception {
    ...
    @Cleanup
    PulsarClient client3 =
        PulsarClient.builder().serviceUrl(serviceUrl).statsInterval(0, TimeUnit.SECONDS).build();
    // set peer-clusters : r3->r1
    admin1.clusters().updatePeerClusterNames("r3", Sets.newLinkedHashSet(List.of("r1")));
