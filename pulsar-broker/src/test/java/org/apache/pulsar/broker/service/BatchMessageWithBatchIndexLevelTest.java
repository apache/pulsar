...
public class BatchMessageWithBatchIndexLevelTest extends BatchMessageTest {
  ...
  public void testBatchMessageAck() {
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              // Since https://github.com/apache/pulsar/pull/23931 improved the mechanism of
  ...
  public void testMixIndexAndNonIndexUnAckMessageCount() throws Exception {
    ...
    assertEquals(
        admin.topics().getStats(topicName).getSubscriptions().get("sub").getUnackedMessages(), 0);
  }
  ...
  public void testUnAckMessagesWhenConcurrentDeliveryAndAck() throws Exception {
    ...
    // When consumer1 is closed, may some messages are in the client memory(it they are being acked
    // now).
    ...
    ConsumerImpl<String> consumer2 =
        (ConsumerImpl<String>) consumerBuilder.consumerName("c2").subscribe();
    ...
    getCursor(topicName, subName)
        .delete(
            AckSetStateUtil.createPositionWithAckSet(
                ledgerId, entryId, bitSetRecyclable.toLongArray()));
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              assertEquals(
                  serviceConsumer2.getAvailablePermits() + consumer2.getAvailablePermits(),
                  receiverQueueSize);
            });
   ...
   */
  private org.apache.pulsar.broker.service.Consumer makeConsumerReceiveMessagesDelay(
      String topic, String sub, String consumerName, CompletableFuture<Void> signal)
      throws Exception {
    PersistentTopic persistentTopic =
        (PersistentTopic) pulsar.getBrokerService().getTopic(topic, false).join().get();
    AbstractPersistentDispatcherMultipleConsumers dispatcher =
    ...
    doAnswer(
            invocation -> {
              ...
              EntryBatchIndexesAcks batchIndexesAcks =
                  (EntryBatchIndexesAcks) invocation.getArguments()[2];
              ...
              RedeliveryTracker redeliveryTracker =
                  (RedeliveryTracker) invocation.getArguments()[6];
              return signal
                  .thenApply(
                      __ ->
                          originalConsumer.sendMessages(
                              entries,
                              batchSizes,
                              batchIndexesAcks,
                              totalMessages,
  ...
  private org.apache.pulsar.broker.service.Consumer getTheUniqueServiceConsumer(
