...
public class BatchMessageWithBatchIndexLevelTest extends BatchMessageTest {
     ...
     * TODO:
     *  - 没有解决 transaction case 的问题。
     *  - 没有解决 ack receipt 接收回调过早地问题。
     *  - 增加 test： broker 关闭 batch ack。
     */
    @Test(timeOut = 60_000, dataProvider = "argsOfTestAcknowledgeConcurrently", invocationCount = 20)
    public void testAcknowledgeConcurrently(boolean enableBatchSend, boolean enableBatchIndexAcknowledgment1,
        ...
        Producer<String> producer = client1.newProducer(Schema.STRING).topic(topic)
                .enableBatching(enableBatchSend).batchingMaxMessages(3).create();
