...
/**
 * This test doesn't test much in and off itself. However it is useful to see which logs are
 * produced when there's an failure or error in authentication.
 */
@Test(groups = "broker")
public class AuthLogsTest extends MockedPulsarServiceBaseTest {
  ...
  public void binaryEndpoint() throws Exception {
    ...
    try (PulsarClient client =
        ...
        Consumer<byte[]> consumer =
            client.newConsumer().topic("foobar").subscriptionName("foobar").subscribe()) {}
  ...
  public void httpEndpoint() throws Exception {
    ...
    try (PulsarAdmin admin =
      ...
      admin.namespaces().getNamespaces("public");
      fail("Should fail auth");
    } catch (ServerSideErrorException ae) {
      /* expected */
    }
