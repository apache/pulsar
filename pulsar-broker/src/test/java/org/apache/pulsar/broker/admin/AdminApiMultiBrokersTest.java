...
public class AdminApiMultiBrokersTest extends MultiBrokerBaseTest {
  ...
  public void testTopicLookup(TopicDomain topicDomain, boolean isPartition) throws Exception {
    ...
    for (PulsarAdmin pulsarAdmin : getAllAdmins()) {
      try {
        ...
        log.error(
            pulsarAdmin.getServiceUrl() + " - Failed to execute lookup for topic {} .", topic, e);
        Assert.fail(
            "Failed to execute lookup by PulsarAdmin(" + pulsarAdmin.getServiceUrl() + ").");
      }
    }
  ...
  public void testForceDeletePartitionedTopicWithSub() throws Exception {
    ...
    final String topic =
        "persistent://tenant-xyz/ns-abc/topic-"
    ...
    if (admin.topics().getList("tenant-xyz/ns-abc").stream().anyMatch(t -> t.contains(topic))) {
      ...
      assertEquals(
          ...
          pulsar
              ...
              .filter(t -> t.contains(topic))
              .count());
      assertFalse(admin.topics().getPartitionedTopicList("tenant-xyz/ns-abc").contains(topic));
    } else {
