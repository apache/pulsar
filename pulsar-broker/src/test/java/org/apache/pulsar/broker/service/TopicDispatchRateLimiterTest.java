...
public class TopicDispatchRateLimiterTest extends BrokerTestBase {
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              assertEquals(
                  admin.topicPolicies().getDispatchRate(topicName).getDispatchThrottlingRateInMsg(),
                  100);
              assertEquals(
                  admin
                      .topicPolicies()
                      .getDispatchRate(topicName)
                      .getDispatchThrottlingRateInByte(),
                  1000L);
            });
  ...
  public void testTopicDispatchThrottledMetrics() throws Exception {
    ...
    double topicAllDispatchThrottledMsgCount =
        subscriptionDispatchThrottledMsgCountMetrics.stream()
    ...
    double topicAllDispatchThrottledBytesCount =
        subscriptionDispatchThrottledBytesCountMetrics.stream()
            .filter(m -> m.tags.get("topic").equals(topic) && m.tags.get("reason").equals("topic"))
            .mapToDouble(m -> m.value)
            .sum();
    Assert.assertEquals(
        subscriptionDispatchThrottledBytesCount, topicAllDispatchThrottledBytesCount);
  }
}
