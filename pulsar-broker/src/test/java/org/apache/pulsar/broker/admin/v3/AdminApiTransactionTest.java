...
public class AdminApiTransactionTest extends MockedPulsarServiceBaseTest {
  ...
  public void testListTransactionCoordinators() throws Exception {
    initTransaction(4);
    final List<TransactionCoordinatorInfo> result =
        admin.transactions().listTransactionCoordinatorsAsync().get();
  ...
  public void testGetTransactionCoordinatorStats() throws Exception {
    ...
    verifyCoordinatorStats(
        transactionCoordinatorstats.state,
        transactionCoordinatorstats.leastSigBits,
        transactionCoordinatorstats.lowWaterMark);
    ...
    verifyCoordinatorStats(
        ...
        transactionCoordinatorstats.leastSigBits,
        transactionCoordinatorstats.lowWaterMark);
    Map<Integer, TransactionCoordinatorStats> stats =
        admin.transactions().getCoordinatorStatsAsync().get();
    ...
    TransactionInBufferStats transactionInBufferStats =
        admin
            ...
            .getTransactionInBufferStatsAsync(
                new TxnID(transaction.getTxnIdMostBits(), transaction.getTxnIdLeastBits()), topic)
            .get();
    Position position =
  ...
  public void testGetTransactionInPendingAckStats() throws Exception {
    ...
    try {
      admin
          .transactions()
          .getTransactionInPendingAckStatsAsync(new TxnID(1, 2), topic, subName)
          .get();
      fail("Should failed here");
    } catch (ExecutionException ex) {
      assertTrue(ex.getCause() instanceof PulsarAdminException.NotFoundException);
    ...
    Consumer<byte[]> consumer =
        pulsarClient.newConsumer(Schema.BYTES).topic(topic).subscriptionName(subName).subscribe();
    producer.sendAsync("Hello pulsar!".getBytes());
    producer.sendAsync("Hello pulsar!".getBytes());
    producer.sendAsync("Hello pulsar!".getBytes());
    producer.sendAsync("Hello pulsar!".getBytes());
    TransactionImpl transaction = (TransactionImpl) getTransaction();
    TransactionInPendingAckStats transactionInPendingAckStats =
        admin
            .transactions()
            .getTransactionInPendingAckStatsAsync(
                new TxnID(transaction.getTxnIdMostBits(), transaction.getTxnIdLeastBits()),
                topic,
                subName)
            .get();
  ...
  public void testGetTransactionMetadata() throws Exception {
    ...
    Consumer<byte[]> consumer1 =
        pulsarClient.newConsumer(Schema.BYTES).topic(topic1).subscriptionName(subName1).subscribe();

    Consumer<byte[]> consumer2 =
        pulsarClient.newConsumer(Schema.BYTES).topic(topic2).subscriptionName(subName2).subscribe();

    Consumer<byte[]> consumer3 =
        pulsarClient.newConsumer(Schema.BYTES).topic(topic2).subscriptionName(subName3).subscribe();

    MessageId messageId1 = producer1.send("Hello pulsar!".getBytes());
    MessageId messageId2 = producer2.send("Hello pulsar!".getBytes());
    MessageId messageId3 =
        producer1.newMessage(transaction).value("Hello pulsar!".getBytes()).send();
    MessageId messageId4 =
        producer2.newMessage(transaction).value("Hello pulsar!".getBytes()).send();
    ...
    Map<String, TransactionInBufferStats> producedPartitions =
        transactionMetadata.producedPartitions;
    Map<String, Map<String, TransactionInPendingAckStats>> ackedPartitions =
    ...
    assertEquals(
        ackedPartitions.get(topic1).get(subName1).cumulativeAckPosition, position1.toString());
    assertEquals(
        ackedPartitions.get(topic2).get(subName2).cumulativeAckPosition, position2.toString());
    assertEquals(
        ackedPartitions.get(topic2).get(subName3).cumulativeAckPosition, position2.toString());
  }
  ...
  public void testGetTransactionBufferStats() throws Exception {
    ...
    Producer<byte[]> producer =
        pulsarClient
            ...
            .create();
    Consumer<byte[]> consumer1 =
        pulsarClient.newConsumer(Schema.BYTES).topic(topic).subscriptionName(subName1).subscribe();

    Consumer<byte[]> consumer2 =
        pulsarClient.newConsumer(Schema.BYTES).topic(topic).subscriptionName(subName2).subscribe();
    long currentTime = System.currentTimeMillis();
    MessageId messageId = producer.newMessage(transaction).value("Hello pulsar!".getBytes()).send();
    ...
    assertEquals(
        ...
        PositionFactory.create(
                ...
                ((MessageIdImpl) messageId).getEntryId() + 1)
            .toString());
  ...
  public void testGetPendingAckStats(String ackType) throws Exception {
    ...
    try {
      admin.transactions().getPendingAckStatsAsync(topic, subName).get();
      fail("Should failed here");
    } catch (ExecutionException ex) {
      assertTrue(ex.getCause() instanceof PulsarAdminException.NotFoundException);
      PulsarAdminException.NotFoundException cause =
  ...
  public void testGetSlowTransactions() throws Exception {
    ...
    Map<String, TransactionMetadata> transactionMetadataMap =
        admin.transactions().getSlowTransactionsAsync(30, TimeUnit.SECONDS).get();
  ...
  public void testGetCoordinatorInternalStats() throws Exception {
    ...
    Transaction transaction =
        pulsarClient.newTransaction().withTransactionTimeout(60, TimeUnit.SECONDS).build().get();

    TransactionCoordinatorInternalStats stats =
        admin.transactions().getCoordinatorInternalStatsAsync(0, true).get();
    ...
    assertEquals(
        TopicName.get(
                ...
                MLTransactionLogImpl.TRANSACTION_LOG_PREFIX + "0")
  ...
  public void testGetPendingAckInternalStats() throws Exception {
    ...
    managedLedgerInternalStats.cursors.forEach(
        (s, cursorStats) -> {
          assertEquals(s, SystemTopicNames.PENDING_ACK_STORE_CURSOR_NAME);
          assertEquals(
              cursorStats.readPosition, finalManagedLedgerInternalStats.lastConfirmedEntry);
        });
        ...
        .untilAsserted(
            () -> {
              ...
              TransactionBufferInternalStats stats =
                  admin.transactions().getTransactionBufferInternalStatsAsync(topic2, true).get();
              ...
              verifyManagedLedgerInternalStats(
                  stats.singleSnapshotSystemTopicInternalStats.managedLedgerInternalStats,
                  internalStats);
              assertTrue(
                  stats.singleSnapshotSystemTopicInternalStats.managedLedgerName.contains(
                      SystemTopicNames.TRANSACTION_BUFFER_SNAPSHOT));
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              verifyManagedLedgerInternalStats(
                  stats.segmentInternalStats.managedLedgerInternalStats, internalStats);
              assertTrue(
              ...
              verifyManagedLedgerInternalStats(
                  stats.segmentIndexInternalStats.managedLedgerInternalStats, internalStats);
              assertTrue(
  ...
  public void testUpdateTransactionCoordinatorNumber() throws Exception {
    ...
    try {
      ...
      assertEquals(
          pulsarAdminException.getCause().getMessage(),
  ...
  public void testGetRecoveryTime() throws Exception {
    ...
    @Cleanup
    Consumer<byte[]> consumer =
        pulsarClient.newConsumer().subscriptionName(subName).topic(topic).subscribe();

    @Cleanup
    Producer<byte[]> producer =
        pulsarClient.newProducer().sendTimeout(0, TimeUnit.SECONDS).topic(topic).create();
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              TransactionBufferStats transactionBufferStats =
                  admin.transactions().getTransactionBufferStats(topic);
    ...
    transactionPendingAckStats = admin.transactions().getPendingAckStats(topic, subName);
    ...
    PositionInPendingAckStats result =
        admin
            ...
            .getPositionStatsInPendingAck(
                topic, subName, messageId.getLedgerId(), messageId.getEntryId(), null);
  ...
  public void testGetPositionStatsInPendingAckStatsFroBatch() throws Exception {
    ...
    PositionInPendingAckStats positionStatsInPendingAckStats =
        admin
            .transactions()
            .getPositionStatsInPendingAck(
                topic, subscriptionName, messageId.getLedgerId(), messageId.getEntryId(), 1);
    ...
    positionStatsInPendingAckStats =
        admin
            .transactions()
            .getPositionStatsInPendingAck(
                topic, subscriptionName, messageId.getLedgerId(), messageId.getEntryId(), 2);
    assertEquals(
        positionStatsInPendingAckStats.state, PositionInPendingAckStats.State.NotInPendingAck);
    positionStatsInPendingAckStats =
        admin
            .transactions()
            .getPositionStatsInPendingAck(
                topic, subscriptionName, messageId.getLedgerId(), messageId.getEntryId(), 10);
    assertEquals(
        positionStatsInPendingAckStats.state, PositionInPendingAckStats.State.InvalidPosition);
  }
  ...
  public void testPeekMessageForSkipTxnMarker() throws Exception {
    ...
    List<Message<byte[]>> peekMsgs =
        admin
            .topics()
            .peekMessages(topic, "t-sub", n, false, TransactionIsolationLevel.READ_UNCOMMITTED);
  ...
  public void testPeekMessageFoReadCommittedMessages() throws Exception {
    ...
    // peek n message, all messages value should be "msg"
    {
      List<Message<byte[]>> peekMsgs =
          admin
              .topics()
              .peekMessages(topic, "t-sub", n, false, TransactionIsolationLevel.READ_COMMITTED);
      assertEquals(peekMsgs.size(), n);
      for (Message<byte[]> peekMsg : peekMsgs) {
        assertEquals(new String(peekMsg.getValue()), "msg");
      }
    ...
    // peek 3 * n message, and still get n message, all messages value should be "msg"
    {
      List<Message<byte[]>> peekMsgs =
          admin
              .topics()
              .peekMessages(topic, "t-sub", 2 * n, false, TransactionIsolationLevel.READ_COMMITTED);
      assertEquals(peekMsgs.size(), n);
      for (Message<byte[]> peekMsg : peekMsgs) {
        assertEquals(new String(peekMsg.getValue()), "msg");
      }
  ...
  public void testPeekMessageForShowAllMessages() throws Exception {
    ...
    List<Message<byte[]>> peekMsgs =
        admin
            .topics()
            .peekMessages(topic, "t-sub", 5 * n, true, TransactionIsolationLevel.READ_UNCOMMITTED);
  ...
  private Transaction getTransaction() throws Exception {
    return pulsarClient.newTransaction().withTransactionTimeout(5, TimeUnit.SECONDS).build().get();
  }

  private static void verifyManagedLedgerInternalStats(
      ManagedLedgerInternalStats managedLedgerInternalStats, long totalSize) {
  ...
  private static void verifyManagedLedgerInternalStats(
      ManagedLedgerInternalStats internalStats, ManagedLedgerInternalStats persistentTopicStats) {
    ...
    assertEquals(
        persistentTopicStats.lastLedgerCreationFailureTimestamp,
