...
public class PersistentQueueE2ETest extends BrokerTestBase {
  ...
  // this test is good to have to see the distribution, but every now and then it gets slightly
  // different than the
  // expected numbers. keeping this disabled to not break the build, but nevertheless this gives
  // good insight into
  ...
  public void testSharedSingleAckedNormalTopic() throws Exception {
    ...
    PulsarClient newPulsarClient =
        newPulsarClient(lookupUrl.toString(), 0); // Creates new client connection
    ConsumerBuilder<byte[]> consumerBuilder2 =
        newPulsarClient
            .newConsumer()
            .topic(topicName)
            .subscriptionName(subscriptionName)
