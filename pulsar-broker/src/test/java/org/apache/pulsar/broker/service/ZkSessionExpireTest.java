...
public class ZkSessionExpireTest extends NetworkErrorTestBase {
  ...
  private void setupWithSettings(java.util.function.Consumer<ServiceConfiguration> settings)
      throws Exception {
  ...
  public void testTopicUnloadAfterSessionRebuild(boolean enableSystemTopic, Class loadManager)
    ...
    // Inject a prefer mechanism, so that all topics will be assigned to broker1, which can be
    // injected a ZK
            ...
            () -> {
              ...
              log.info(
                  ...
                  pulsar2
                      ...
                      .collect(Collectors.toList()));
              log.info(
                  "future: {}, isDone: {}, isCompletedExceptionally: {}",
                  future,
                  future == null ? "null" : future.isDone(),
                  future,
                  future == null ? "null" : future.isCompletedExceptionally());
              assertTrue(
                  future == null
