...
public abstract class OneWayReplicatorTestBase extends TestRetrySupport {
  ...
  protected final String sourceClusterAlwaysSchemaCompatibleNamespace =
      defaultTenant + "/always-compatible";
  ...
  protected void createDefaultTenantsAndClustersAndNamespace() throws Exception {
    ...
    admin1
        ...
        .createTenant(
            defaultTenant,
            new TenantInfoImpl(Collections.emptySet(), Sets.newHashSet(cluster1, cluster2)));
  ...
  protected void cleanupTopics(String namespace, CleanupTopicAction cleanupTopicAction)
      throws Exception {
    if (usingGlobalZK) {
    ...
    admin1
        .namespaces()
        .setNamespaceReplicationClusters(namespace, Sets.newHashSet(cluster1, cluster2));
    waitChangeEventsInit(namespace);
  }
  ...
  protected void setConfigDefaults(
    ...
    config.setConfigurationMetadataStoreUrl(
        "zk:127.0.0.1:" + brokerConfigZk.getZookeeperPort() + "/foo");
  ...
  protected void cleanupPulsarResources() throws Exception {
    ...
    admin1
        .namespaces()
        .setNamespaceReplicationClusters(replicatedNamespace, Sets.newHashSet(cluster1));
    if (!usingGlobalZK) {
      admin2
          .namespaces()
          .setNamespaceReplicationClusters(replicatedNamespace, Sets.newHashSet(cluster2));
    }
        ...
        .until(
            () -> {
              ...
              try {
                ...
                PersistentTopic persistentTopic =
                    (PersistentTopic)
                        pulsar1.getBrokerService().getTopic(partition0, false).join().get();
                if (persistentTopic.getReplicators().size() > 0) {
                  return true;
                }
              } catch (Exception ex) {
  ...
  protected void setTopicLevelClusters(
      String topic, List<String> clusters, PulsarAdmin admin, PulsarService pulsar)
      throws Exception {
            ...
            () -> {
              TopicPolicies policies =
                  TopicPolicyTestUtils.getTopicPolicies(
                      pulsar.getTopicPoliciesService(), topicName);
              assertEquals(new HashSet<>(policies.getReplicationClusters()), expected);
              if (partitions == 0) {
                checkNonPartitionedTopicLevelClusters(
                    topicName.toString(), clusters, admin, pulsar.getBrokerService());
              } else {
  ...
  protected void checkNonPartitionedTopicLevelClusters(
      String topic, List<String> clusters, PulsarAdmin admin, BrokerService broker)
      throws Exception {
    ...
    Set<String> act =
        new HashSet<>(
            TopicPolicyTestUtils.getTopicPolicies(persistentTopic).getReplicationClusters());
  ...
  protected int ensurePartitionsAreSame(String topic) throws Exception {
    ...
    if (isPartitionedTopic1 != isPartitionedTopic2) {
      throw new IllegalArgumentException(
          String.format(
              "Can not delete topic." + " isPartitionedTopic1: %s, isPartitionedTopic2: %s",
    ...
    if (partitions1 != partitions2) {
      throw new IllegalArgumentException(
          String.format(
              "Can not delete topic." + " partitions1: %s, partitions2: %s",
  ...
  protected void waitReplicatorStopped(
      PulsarService sourceCluster, PulsarService targetCluster, String topicName) {
    Awaitility.await()
            ...
            () -> {
              ...
              assertTrue(
                  persistentTopic1.getReplicators().isEmpty()
                      || !persistentTopic1
                          .getReplicators()
                          .get(targetCluster.getConfig().getClusterName())
                          .isConnected());
            });
  }
}
