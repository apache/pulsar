...
public class TransactionBufferHandlerImplTest {
  ...
  public void testRequestCredits() throws PulsarServerException {
    ...
    when(namespaceService.getBundleAsync(any()))
        .thenReturn(CompletableFuture.completedFuture(mock(NamespaceBundle.class)));
    Optional<NamespaceEphemeralData> opData = Optional.empty();
    when(namespaceService.getOwnerAsync(any()))
        .thenReturn(CompletableFuture.completedFuture(opData));
    ...
    TransactionBufferHandlerImpl handler =
        spy(new TransactionBufferHandlerImpl(pulsarService, null, 1000, 3000));
    doNothing().when(handler).endTxn(any());
    doReturn(CompletableFuture.completedFuture(mock(ClientCnx.class)))
        .when(handler)
        .getClientCnx(anyString());
    for (int i = 0; i < 500; i++) {
  ...
  public void testMinRequestCredits() throws PulsarServerException {
    ...
    when(pulsarService.getClient()).thenReturn(pulsarClient);
    TransactionBufferHandlerImpl handler =
        spy(new TransactionBufferHandlerImpl(pulsarService, null, 50, 3000));
