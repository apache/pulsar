...
public class SharedConsumerAssignorTest {
  ...
  public void prepareData() {
    ...
    // | 5        | A-1  | 2        | 3                |
    // | 6        | B-1  | 1        | 2               |
    // P.S. In the table above, The uuid represents the "<producer-name>-<sequence-id>" for
    // non-chunks
    assertEquals(
  ...
  public void testSingleConsumerMultiAssign() {
    ...
    assertEquals(
        result.getOrDefault(consumer, Collections.emptyList()), entryAndMetadataList.subList(0, 5));
    ...
    assertEquals(
        result.getOrDefault(consumer, Collections.emptyList()), entryAndMetadataList.subList(5, 6));
    ...
    assertEquals(
        result.getOrDefault(consumer, Collections.emptyList()), entryAndMetadataList.subList(6, 7));
  ...
  public void testMultiConsumerWithSmallPermits() {
    ...
    // First consumerA received 3 entries and the available permits became 0. Then the consumer is
    // switched to
    // consumerB and then B-0 and B-1-0-2 were received by it.
    // When consumerB tried to receive A-1-2-3, since the uuid "A-1" was already assigned to
    // consumerA, consumerB
    // is not able to receive A-1-2-3.
    // However, since consumerA has no more permits, A-1-2-3 cannot be assigned to consumerA as
    // well.
    ...
    // Since uuid "A-1" is still cached, A-1-2-3 will be dispatched to consumerA even if consumerB
    // is the first
  ...
  private static List<String> toString(final List<EntryAndMetadata> entryAndMetadataList) {
    return entryAndMetadataList.stream()
        .map(EntryAndMetadata::toString)
  ...
  static class MockProducer {
    ...
    void sendMessage() {
      entryAndMetadataList.add(
          createEntryAndMetadata(
              entryId.getAndIncrement(), createMetadata(name, sequenceId++, null, null)));
    }
  ...
  private static EntryAndMetadata createEntryAndMetadata(
      final long entryId, final MessageMetadata metadata) {
