...
public class MessagePublishBufferThrottleTest extends BrokerTestBase {
  ...
  public void testBlockByPublishRateLimiting() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(() -> assertRateLimitCounter(ConnectionRateLimitOperationName.PAUSED, 1));

    CompletableFuture<Void> flushFuture = producer.flushAsync();

    // Block by publish rate.
    // After 1 second, the message buffer throttling will be lifted, but the rate limiting will
    // still be in place.
  ...
  private void assertRateLimitCounter(
      ConnectionRateLimitOperationName connectionRateLimitState, int expectedCount) {
    var metrics = pulsarTestContext.getOpenTelemetryMetricReader().collectAllMetrics();
    if (expectedCount == 0) {
      assertThat(metrics)
          .noneSatisfy(
