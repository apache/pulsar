...
public class ZooKeeperSessionExpireRecoveryTest extends MockedPulsarServiceBaseTest {
    ...
    public void testSessionExpired() throws Exception {
        admin.clusters().createCluster("my-cluster", ClusterData.builder().serviceUrl("http://test-url").build());
        ...
        try {
            admin.clusters().createCluster("my-cluster-2", ClusterData.builder().serviceUrl("http://test-url").build());
            fail("Should have failed, because global zk is down");
        } catch (PulsarAdminException e) {
            // Ok
        }

        admin.clusters().createCluster("cluster-2", ClusterData.builder().serviceUrl("http://test-url").build());
    }
}
