...
public class TransactionalReplicateSubscriptionTest extends ReplicatorTestBase {
  ...
  protected void createTransactionCoordinatorAssign(
      int numPartitionsOfTC, PulsarService pulsarService) throws MetadataStoreException {
    pulsarService
        .getPulsarResources()
  ...
  public void testReplicatedSubscribeAndSwitchToStandbyClusterWithTransaction() throws Exception {
    ...
    admin1
        .topics()
        .createSubscription(
            topicName, subscriptionName, MessageId.earliest, isReplicatedSubscription);
    ...
    Consumer<String> consumer1 =
        client1
            ...
            .subscribe();
    Transaction txn1 =
        client1.newTransaction().withTransactionTimeout(5, TimeUnit.SECONDS).build().get();
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              assertTrue(
                  replicators != null && replicators.size() == 1, "Replicator should started");
              assertTrue(
                  replicators.values().iterator().next().isConnected(),
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              assertNotNull(
                  topic2.getSubscriptions().get(subscriptionName), "Subscription should created");
            });
