...
public class ClusterMigrationTest {
  ...
  public static Object[][] loadManagerClassName() {
    return new Object[][] {
      {ModularLoadManagerImpl.class.getName()}, {ExtensibleLoadManagerImpl.class.getName()}
    };
  }
  ...
  public void setup() throws Exception {
    ...
    updateTenantInfo(
        ...
        "pulsar",
        new TenantInfoImpl(
            Sets.newHashSet("appid1", "appid2", "appid3"), Sets.newHashSet("r1", "r3")));
    updateTenantInfo(
        admin3,
        "pulsar",
        new TenantInfoImpl(
    ...
    admin1
        .namespaces()
        .setNamespaceReplicationClusters(namespaceNotToMigrate, Sets.newHashSet("r1", "r3"));
    ...
    updateTenantInfo(
        ...
        "pulsar",
        new TenantInfoImpl(
            Sets.newHashSet("appid1", "appid2", "appid3"), Sets.newHashSet("r2", "r4")));
    updateTenantInfo(
        admin4,
        "pulsar",
        new TenantInfoImpl(
    ...
    admin2
        .namespaces()
        .setNamespaceReplicationClusters(namespaceNotToMigrate, Sets.newHashSet("r2", "r4"));
    ...
    assertEquals(
        admin1.clusters().getCluster("r1").getBrokerServiceUrl(), pulsar1.getBrokerServiceUrl());
    assertEquals(
        admin2.clusters().getCluster("r2").getBrokerServiceUrl(), pulsar2.getBrokerServiceUrl());
    assertEquals(
        admin3.clusters().getCluster("r3").getBrokerServiceUrl(), pulsar3.getBrokerServiceUrl());
    assertEquals(
        admin4.clusters().getCluster("r4").getBrokerServiceUrl(), pulsar4.getBrokerServiceUrl());

    sleep(100);
    log.info("--- ReplicatorTestBase::setup completed ---");
  }

  protected void updateTenantInfo(PulsarAdmin admin, String tenant, TenantInfoImpl tenantInfo)
      throws Exception {
  ...
  /**
   * Test producer/consumer migration: using persistent/non-persistent topic and all types of
   * subscriptions (1) Producer1 and consumer1 connect to cluster-1 (2) Close consumer1 to build
   * backlog and publish messages using producer1 (3) Migrate topic to cluster-2 (4) Validate
   * producer-1 is connected to cluster-2 (5) create consumer1, drain backlog and migrate and
   * reconnect to cluster-2 (6) Create new consumer2 with different subscription on cluster-1, which
   * immediately migrate and reconnect to cluster-2 (7) Create producer-2 directly to cluster-2 (8)
   * Create producer-3 on cluster-1 which should be redirected to cluster-2 (8) Publish messages
   * using producer1, producer2, and producer3 (9) Consume all messages by both consumer1 and
   * consumer2 (10) Create Producer/consumer on non-migrated cluster and verify their connection
   * with cluster-1 (11) Restart Broker-1 and connect producer/consumer on cluster-1
   *
   * @throws Exception
   */
  @Test
  public void testClusterMigration() throws Exception {
    ...
    final String topicName =
        BrokerTestUtil.newUniqueName("persistent://" + namespace + "/migrationTopic");
    ...
    AbstractTopic topic1 =
        (AbstractTopic) pulsar1.getBrokerService().getTopic(topicName, false).getNow(null).get();
    ...
    Producer<byte[]> producer2 =
        client2
            ...
            .messageRoutingMode(MessageRoutingMode.SinglePartition)
            .create();
    AbstractTopic topic2 =
        (AbstractTopic) pulsar2.getBrokerService().getTopic(topicName, false).getNow(null).get();
    assertFalse(topic2.getProducers().isEmpty());

    ClusterUrl migratedUrl =
        new ClusterUrl(
            pulsar2.getWebServiceAddress(),
            pulsar2.getWebServiceAddressTls(),
    ...
    consumerM =
        client1
            ...
            .subscribe();
    retryStrategically(
        (t) -> pulsar2.getBrokerService().getTopicReference(newTopicName).isPresent(), 5, 100);
    ...
    retryStrategically(
        (t) ->
            pulsar2.getBrokerService().getTopicReference(newTopicName).isPresent()
                && pulsar2
                    .getBrokerService()
                    .getTopicReference(newTopicName)
                    .get()
                    .getSubscription("sM")
                    .getConsumers()
                    .isEmpty(),
    ...
    Producer<byte[]> producerDiff =
        client1
            ...
            .topic(diffTopic)
            .enableBatching(false)
            .producerName("cluster1-d")
            .messageRoutingMode(MessageRoutingMode.SinglePartition)
            .create();
    AbstractTopic topicDiff =
        (AbstractTopic) pulsar2.getBrokerService().getTopic(diffTopic, false).getNow(null).get();
  ...
  public void testClusterMigrationWithReplicationBacklog() throws Exception {
    ...
    // verify that the producer1 is now is now connected to migrated cluster "r2" since backlog is
    // cleared.
  ...
  /**
   * This test validates that blue cluster first creates list of subscriptions into green cluster
   * so, green cluster will not lose the data if producer migrates.
   *
   * @throws Exception
   */
  @Test
  public void testClusterMigrationWithResourceCreated() throws Exception {
    ...
    PersistentTopic topic1 =
        (PersistentTopic) pulsar1.getBrokerService().getTopic(topicName, false).getNow(null).get();
    retryStrategically(
        (test) -> {
          try {
            topic1.checkClusterMigration().get();
    ...
    retryStrategically(
        ...
        10,
        500);
    PersistentTopic pulsar2Topic =
        (PersistentTopic) pulsar2.getBrokerService().getTopic(newTopic, false).getNow(null).get();
    retryStrategically(
        (test) -> {
          try {
  ...
  public void testNamespaceMigration(
      SubscriptionType subType, boolean isClusterMigrate, boolean isNamespaceMigrate)
      throws Exception {
    ...
    final String topicName2 =
        BrokerTestUtil.newUniqueName("persistent://" + namespaceNotToMigrate + "/migrationTopic");
    ...
    Producer<byte[]> blueProducerNs1_1 =
        client1
            .newProducer()
            .topic(topicName)
            .enableBatching(false)
            .producerName("blue-producer-ns1-1")
            .messageRoutingMode(MessageRoutingMode.SinglePartition)
            .create();
    Consumer<byte[]> blueConsumerNs1_1 =
        client1
            .newConsumer()
            .topic(topicName)
    ...
    Producer<byte[]> blueProducerNs2_1 =
        client1
            .newProducer()
            .topic(topicName2)
            .enableBatching(false)
            .producerName("blue-producer-ns2-1")
            .messageRoutingMode(MessageRoutingMode.SinglePartition)
            .create();
    Consumer<byte[]> blueConsumerNs2_1 =
        client1
            .newConsumer()
            .topic(topicName2)
            .subscriptionType(subType)
            .subscriptionName("s1")
            .subscribe();
    AbstractTopic blueTopicNs2_1 =
        (AbstractTopic) pulsar1.getBrokerService().getTopic(topicName2, false).getNow(null).get();
    ...
    Producer<byte[]> greenProducerNs1_1 =
        client2
            .newProducer()
            .topic(topicName)
            .enableBatching(false)
            .producerName("green-producer-ns1-1")
            .messageRoutingMode(MessageRoutingMode.SinglePartition)
            .create();
    AbstractTopic greenTopicNs1_1 =
        (AbstractTopic) pulsar2.getBrokerService().getTopic(topicName, false).getNow(null).get();
    assertFalse(greenTopicNs1_1.getProducers().isEmpty());

    // green cluster - namespace2 - producer/consumer
    Producer<byte[]> greenProducerNs2_1 =
        client2
            .newProducer()
            .topic(topicName2)
            .enableBatching(false)
            .producerName("cluster2-nm1")
            .messageRoutingMode(MessageRoutingMode.SinglePartition)
            .create();
    AbstractTopic greenTopicNs2_1 =
        (AbstractTopic) pulsar2.getBrokerService().getTopic(topicName2, false).getNow(null).get();
    ...
    // producer is disconnected from blue for namespace1 as cluster or namespace migration is
    // enabled
    ...
    Producer<byte[]> blueProducerNs1_2 =
        client1
            .newProducer()
            .topic(topicName)
            .enableBatching(false)
            .producerName("blue-producer-ns1-2")
            .messageRoutingMode(MessageRoutingMode.SinglePartition)
            .create();
    ...
    Consumer<byte[]> blueConsumerNs1_2 =
        client1
            .newConsumer()
            .topic(topicName)
            .subscriptionType(subType)
            .subscriptionName("s2")
            .subscribe();
    Consumer<byte[]> blueConsumerNs2_2 =
        client1
            .newConsumer()
            .topic(topicName2)
            .subscriptionType(subType)
    ...
    consumerM =
        client1
            ...
            .subscribe();
    retryStrategically(
        (t) -> pulsar1.getBrokerService().getTopicReference(newTopicName).isPresent(), 5, 100);
    pulsar2.getBrokerService().getTopicReference(newTopicName).get().checkClusterMigration().get();
    retryStrategically(
        (t) ->
            pulsar2.getBrokerService().getTopicReference(newTopicName).isPresent()
  ...
  public void testMigrationWithReader() throws Exception {
    ...
    // after consuming all messages, reader should have disconnected from cluster-1 and reconnect
    // with cluster-2
    retryStrategically((test) -> !topic2.getSubscriptions().isEmpty(), 10, 500);
    assertFalse(topic2.getSubscriptions().isEmpty());
    assertTrue(topic1.getSubscriptions().isEmpty());
  ...
  @Test(dataProvider = "NamespaceMigrationTopicSubscriptionTypes")
  public void testNamespaceMigrationWithReplicationBacklog(
      SubscriptionType subType, boolean isClusterMigrate, boolean isNamespaceMigrate)
      throws Exception {
    ...
    Consumer<byte[]> blueConsumerReplicationNs1 =
        client3
            .newConsumer()
            .topic(topicName)
            .subscriptionType(subType)
    ...
    Consumer<byte[]> blueConsumerReplicationNs2 =
        client3
            .newConsumer()
            .topic(topicName2)
            .subscriptionType(subType)
    ...
    Producer<byte[]> greenProducerNs2_1 =
        client2
            ...
            .topic(topicName2)
            .enableBatching(false)
            .producerName("green-producer-ns2-1")
            .messageRoutingMode(MessageRoutingMode.SinglePartition)
            .create();
    AbstractTopic greenTopicNs2 =
        (AbstractTopic) pulsar2.getBrokerService().getTopic(topicName2, false).getNow(null).get();
    ...
    // verify that the producer1 is now is now connected to migrated cluster green since backlog is
    // cleared.
