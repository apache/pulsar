...
public class ClusterMigrationTest {
    ...
    public void testClusterMigration(boolean persistent, SubscriptionType subType) throws Exception {
        ...
        final String topicName = BrokerTestUtil.newUniqueName(
                (persistent ? "persistent" : "non-persistent") + "://" + namespace + "/migrationTopic");
        ...
        Consumer<byte[]> consumer1 = client1.newConsumer()
                ...
                .subscriptionName("s1")
                .subscribe();
        AbstractTopic topic1 = (AbstractTopic) pulsar1.getBrokerService()
        ...
        Producer<byte[]> producer2 = client2.newProducer()
                ...
                .messageRoutingMode(MessageRoutingMode.SinglePartition)
                .create();
        AbstractTopic topic2 = (AbstractTopic) pulsar2.getBrokerService()
                .getTopic(topicName, false)
                .getNow(null)
                .get();
        ...
        Producer<byte[]> producer3 = client1.newProducer()
                .topic(topicName)
                .enableBatching(false)
        ...
        String diffTopic = BrokerTestUtil.newUniqueName(
                (persistent ? "persistent" : "non-persistent") + "://" + namespace + "/migrationTopic");
        Consumer<byte[]> consumerDiff = client1.newConsumer()
                .topic(diffTopic)
                .subscriptionType(subType)
                .subscriptionName("s1-d")
                .subscribe();
        Producer<byte[]> producerDiff = client1.newProducer()
                .topic(diffTopic)
                .enableBatching(false)
                .producerName("cluster1-d")
                .messageRoutingMode(MessageRoutingMode.SinglePartition)
                .create();
        AbstractTopic topicDiff = (AbstractTopic) pulsar1.getBrokerService()
                .getTopic(diffTopic, false)
                .getNow(null)
                .get();
        ...
        // restart broker-1
        broker1.restart();
        Producer<byte[]> producer4 = client1.newProducer()
                .topic(topicName)
                .enableBatching(false)
                .producerName("cluster1-4")
                .messageRoutingMode(MessageRoutingMode.SinglePartition)
                .create();
        Consumer<byte[]> consumer3 = client1.newConsumer()
                .topic(topicName)
                .subscriptionType(subType)
                .subscriptionName("s3")
                .subscribe();
    ...
    @Test(dataProvider = "TopicsubscriptionTypes")
    public void testClusterMigrationWithReplicationBacklog(boolean persistent, SubscriptionType subType)
            throws Exception {
        ...
        // cluster-3 consumer
        Consumer<byte[]> consumer3 = client3.newConsumer()
                .topic(topicName)
                .subscriptionType(subType)
                .subscriptionName("s1")
                .subscribe();
