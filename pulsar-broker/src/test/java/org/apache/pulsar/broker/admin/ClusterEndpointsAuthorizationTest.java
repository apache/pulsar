...
public class ClusterEndpointsAuthorizationTest extends MockedPulsarStandalone {
  ...
  public void testGetCluster() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowClusterOperationAsync(
            eq(clusterName), eq(ClusterOperation.GET_CLUSTER), any(), any(), any());
  ...
  public void testCreateCluster() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowClusterOperationAsync(
            eq(clusterName), eq(ClusterOperation.CREATE_CLUSTER), any(), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
  ...
  public void testUpdateCluster() {
    ...
    try {
      superUserAdmin
          .clusters()
          .updateCluster(clusterName, ClusterData.builder().serviceUrl("aaa").build());
    ...
    verify(spyAuthorizationService)
        .allowClusterOperationAsync(
            eq(clusterName), eq(ClusterOperation.UPDATE_CLUSTER), any(), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    // ---- test nobody
    Assert.assertThrows(
  ...
  public void testSetPeerClusterNames() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowClusterOperationAsync(
            eq(clusterName), eq(ClusterOperation.UPDATE_PEER_CLUSTER), any(), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    // ---- test nobody
    Assert.assertThrows(
  ...
  public void testGetPeerCluster() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowClusterOperationAsync(
            eq(clusterName), eq(ClusterOperation.GET_PEER_CLUSTER), any(), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    // ---- test nobody
    Assert.assertThrows(
  ...
  public void testDeleteCluster() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowClusterOperationAsync(
            eq(clusterName), eq(ClusterOperation.DELETE_CLUSTER), any(), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    // ---- test nobody
    Assert.assertThrows(
  ...
  public void testGetBrokerWithNamespaceIsolationPolicy() throws PulsarAdminException {
    ...
    superUserAdmin
        .clusters()
        .getBrokerWithNamespaceIsolationPolicy(clusterName, getPulsarService().getBrokerId());
    ...
    try {
      superUserAdmin
          ...
          .updateNamespaceIsolationPolicy(
              clusterName, "test", NamespaceIsolationData.builder().build());
    } catch (Throwable ignore) {
  ...
  public void testSetFailureDomain() throws PulsarAdminException {
    ...
    superUserAdmin
        .clusters()
        .updateFailureDomain(clusterName, "test", FailureDomain.builder().build());

    // test allow cluster operation
    verify(spyAuthorizationService)
        .allowClusterOperationAsync(
            eq(clusterName), eq(ClusterOperation.UPDATE_FAILURE_DOMAIN), any(), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    // ---- test nobody
    Assert.assertThrows(
  ...
  public void testGetFailureDomains() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowClusterOperationAsync(
            eq(clusterName), eq(ClusterOperation.GET_FAILURE_DOMAIN), any(), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    // ---- test nobody
    Assert.assertThrows(
  ...
  public void testDeleteFailureDomain() throws PulsarAdminException {
    ...
    verify(spyAuthorizationService)
        .allowClusterOperationAsync(
            eq(clusterName), eq(ClusterOperation.DELETE_FAILURE_DOMAIN), any(), any(), any());
    // fallback to superuser
    verify(spyAuthorizationService).isSuperUser(any(), any());
    // ---- test nobody
    Assert.assertThrows(
