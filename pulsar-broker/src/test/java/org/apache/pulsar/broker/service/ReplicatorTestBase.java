...
public abstract class ReplicatorTestBase extends TestRetrySupport {
  ...
  protected void setup() throws Exception {
    ...
    assertEquals(
        admin2.clusters().getCluster(cluster1).getBrokerServiceUrl(),
        pulsar1.getBrokerServiceUrl());
    assertEquals(
        admin2.clusters().getCluster(cluster2).getBrokerServiceUrl(),
        pulsar2.getBrokerServiceUrl());
    assertEquals(
        admin2.clusters().getCluster(cluster3).getBrokerServiceUrl(),
    ...
    assertEquals(
        admin2.clusters().getCluster(cluster1).getBrokerServiceUrlTls(),
        pulsar1.getBrokerServiceUrlTls());
    assertEquals(
        admin2.clusters().getCluster(cluster2).getBrokerServiceUrlTls(),
        pulsar2.getBrokerServiceUrlTls());
    assertEquals(
        admin2.clusters().getCluster(cluster3).getBrokerServiceUrlTls(),
        pulsar3.getBrokerServiceUrlTls());
    assertEquals(
        admin2.clusters().getCluster(cluster4).getBrokerServiceUrlTls(),
    ...
    admin1
        ...
        .setNamespaceReplicationClusters(
            "pulsar/global/ns", Sets.newHashSet(cluster1, cluster2, cluster3));

    Thread.sleep(100);
    log.info("--- ReplicatorTestBase::setup completed ---");
  }

  private PulsarService buildPulsarService(
      ServiceConfiguration config, InMemoryMetricReader metricReader) {
    return new PulsarService(
        config,
        new WorkerConfig(),
        Optional.empty(),
