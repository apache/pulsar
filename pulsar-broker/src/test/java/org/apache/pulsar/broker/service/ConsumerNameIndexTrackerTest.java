...
public class ConsumerNameIndexTrackerTest {
  ...
  public void testTrackingReturnsStableIndexWhenRemovedAndAddedInSameOrder() {
    List<ConsumerIdentityWrapper> consumerIdentityWrappers =
        IntStream.range(0, 100)
            .mapToObj(i -> mockConsumer())
            .map(ConsumerIdentityWrapper::new)
            .toList();
    Map<ConsumerIdentityWrapper, Integer> trackedIndexes =
        consumerIdentityWrappers.stream()
            .collect(
                Collectors.toMap(
                    wrapper -> wrapper,
                    wrapper -> tracker.increaseConsumerRefCountAndReturnIndex(wrapper)));
  ...
  public void testTrackingMultipleNames() {
    List<ConsumerIdentityWrapper> consumerIdentityWrappers =
        IntStream.range(0, 100)
            .mapToObj(i -> mockConsumer("consumer" + i))
            .map(ConsumerIdentityWrapper::new)
            .toList();
    consumerIdentityWrappers.forEach(
        wrapper -> tracker.increaseConsumerRefCountAndReturnIndex(wrapper));
