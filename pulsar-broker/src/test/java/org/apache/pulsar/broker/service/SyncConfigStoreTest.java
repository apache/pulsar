...
public class SyncConfigStoreTest extends GeoReplicationWithConfigurationSyncTestBase {
  ...
  private static final String SYNC_EVENT_TOPIC =
      TopicDomain.persistent.value() + "://" + SYSTEM_NAMESPACE + "/__sync_config_meta";
  ...
  public void testDynamicEnableConfigurationMetadataSyncEventTopic() throws Exception {
            ...
            () -> {
              boolean shouldShutdownConfigurationMetadataStore =
                  WhiteboxImpl.getInternalState(
                      pulsar1, "shouldShutdownConfigurationMetadataStore");
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              assertEquals(
                  pulsar1.getConfig().getConfigurationMetadataSyncEventTopic(), SYNC_EVENT_TOPIC);
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              assertEquals(
                  synchronizerStarted.getState(), PulsarMetadataEventSynchronizer.State.Closed);
