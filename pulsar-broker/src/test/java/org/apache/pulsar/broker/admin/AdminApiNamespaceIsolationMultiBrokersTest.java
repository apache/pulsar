...
public class AdminApiNamespaceIsolationMultiBrokersTest extends MultiBrokerBaseTest {
  ...
  public void setupClusters() throws Exception {
    ...
    localAdmin
        .clusters()
        .createCluster(
            "cluster-1", ClusterData.builder().serviceUrl(localBrokerWebService).build());
    remoteAdmin
        .clusters()
        .createCluster(
            "cluster-2", ClusterData.builder().serviceUrl(remoteBrokerWebService).build());
    TenantInfoImpl tenantInfo =
        new TenantInfoImpl(Set.of(""), Set.of("test", "cluster-1", "cluster-2"));
