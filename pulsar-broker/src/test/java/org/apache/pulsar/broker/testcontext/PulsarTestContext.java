 ...
 *
 * <p>There are 2 types of Pulsar unit tests that use a PulsarService:
 *
 * <ul>
 *   <li>Some Pulsar unit tests use a PulsarService that isn't started
 *   <li>Some Pulsar unit tests start the PulsarService and use less mocking
 * </ul>
 *
 * This class can be used to set up a PulsarService that can be used in both types of tests.
 *
 * <p>There are few motivations for PulsarTestContext:
 *
 * <ul>
 *   <li>It reduces the reliance on Mockito for hooking into the PulsarService for injecting mocks
 *       or customizing the behavior of some collaborators. Mockito is not thread-safe and some
 *       mocking operations get corrupted. Some examples of the issues:
 *       https://github.com/apache/pulsar/issues/13620,
 *       https://github.com/apache/pulsar/issues/16444 and
 *       https://github.com/apache/pulsar/issues/16427.
 *   <li>Since the Mockito issue causes test flakiness, this change will improve reliability.
 *   <li>It makes it possible to use composition over inheritance in test classes. This can help
 *       reduce the dependency on deep test base cases hierarchies.
 *   <li>It reduces code duplication across test classes.
 * </ul>
 *
...
public class PulsarTestContext implements AutoCloseable {
  ...
  /**
   * Create a builder for a PulsarTestContext that creates a PulsarService that is started.
   *
  ...
  /**
   * Create a builder for a PulsarTestContext that creates a PulsarService that cannot be started.
   * Some tests need to create a PulsarService that cannot be started. This was added when this type
   * of tests were migrated to use PulsarTestContext.
   *
  ...
  /**
   * Create a ServerCnx instance that has a Mockito spy that is recording invocations. This is
   * useful for tests for ServerCnx.
   *
   * @return a ServerCnx instance
   */
  public ServerCnx createServerCnxSpy() {
    return BrokerTestUtil.spyWithClassAndConstructorArgsRecordingInvocations(
        ServerCnx.class, getPulsarService());
  }
   ...
   *
   * <p>The builder is created with Lombok. That is the reason why the builder source code doesn't
   * show all behaviors
   */
  public static class Builder {
    ...
    /**
     * Some settings like the broker shutdown timeout and thread pool sizes are overridden if the
     * provided values are the default values. This is used to run tests with smaller thread pools
     * and shorter timeouts by default. You can use
     *
     * <pre>{@code .configCustomizer(null)}</pre>
     *
    ...
    protected void defaultOverrideServiceConfiguration(ServiceConfiguration svcConfig) {
      ...
      if (svcConfig.getBrokerShutdownTimeoutMs()
              == unconfiguredDefaults.getBrokerShutdownTimeoutMs()
      ...
      if (svcConfig.getNumOrderedExecutorThreads()
          == unconfiguredDefaults.getNumOrderedExecutorThreads()) {
        // use a single threaded ordered executor by default
        svcConfig.setNumOrderedExecutorThreads(1);
      }
      if (svcConfig.getNumExecutorThreadPoolSize()
          == unconfiguredDefaults.getNumExecutorThreadPoolSize()) {
      ...
      if (svcConfig.getManagedLedgerCacheSizeMB()
          == unconfiguredDefaults.getManagedLedgerCacheSizeMB()) {
        svcConfig.setManagedLedgerCacheSizeMB(8);
      }

      if (svcConfig.getTopicLoadTimeoutSeconds()
          == unconfiguredDefaults.getTopicLoadTimeoutSeconds()) {
        svcConfig.setTopicLoadTimeoutSeconds(10);
      }
    }

    /**
     * Internal method used in the {@link StartableCustomBuilder} to override the default value for
     * the broker shutdown timeout.
     *
    ...
    /**
     * Configure the PulsarService instance and the PulsarService collaborator objects to use
     * Mockito spies by default.
     *
     * @see SpyConfig
     * @return the builder
     */
    ...
    /**
     * Customize the ServiceConfiguration object that is used to configure the PulsarService
     * instance.
     *
    ...
    /**
     * Override configuration values in the ServiceConfiguration instance as a last step. There are
     * default overrides provided by {@link
     * PulsarTestContext.Builder#defaultOverrideServiceConfiguration(ServiceConfiguration)} that can
     * be disabled by using
     *
     * <pre>{@code .configOverride(null)}</pre>
     *
    ...
    /**
     * Chains closing of the other PulsarTestContext to this one. The other PulsarTestContext will
     * be closed when this one is closed.
     */
    ...
    /**
     * Configure this PulsarTestContext to use a mock ZooKeeper instance which is shared for both
     * the local and configuration metadata stores.
     *
     * @return the builder
     */
     ...
     *
     * @param useSeparateGlobalZk if true, the global (configuration) zookeeper will be a separate
     *     instance
     * @return the builder
     */
    ...
    /**
     * Configure this PulsarTestContext to use a test ZooKeeper instance which is shared for both
     * the local and configuration metadata stores.
     *
     * @return the builder
     */
    ...
    /**
     * Applicable only when PulsarTestContext is not startable. This will configure mocks for
     * PulsarTestResources and related classes.
     *
     * @return the builder
     */
    ...
    /**
     * Applicable only when PulsarTestContext is not startable. This will configure mocks for
     * PulsarTestResources and related collaborator instances. The {@link NamespaceResources} and
     * {@link TopicResources} instances will use the provided {@link MetadataStore} instance.
     *
    ...
    /**
     * Applicable only when PulsarTestContext is not startable. This will configure the {@link
     * BookKeeper} and {@link ManagedLedgerFactory} instances to use for creating a {@link
     * ManagedLedgerStorage} instance for PulsarService.
     *
     ...
     * @return the builder
     */
    public Builder managedLedgerClients(
    ...
    /**
     * Configures a function to use for customizing the {@link BrokerService} instance when it gets
     * created.
     *
     * @return the builder
     */
    public Builder brokerServiceCustomizer(
   ...
   *
   * <p>With Lombok, it is necessary to extend the generated Builder class for adding customizations
   * related to instantiation and completing the builder.
   */
  abstract static class AbstractCustomBuilder extends Builder {
    ...
    private static void initializeZookeeper(ZooKeeper zk)
    ...
    private TestZKServer createTestZookeeper(int sessionTimeout) throws Exception {
      ...
      try (ZooKeeper zkc =
          new ZooKeeper(testZKServer.getConnectionString(), sessionTimeout, event -> {})) {
    ...
    private void initializeCommonPulsarServices(SpyConfig spyConfig) {
      ...
      if (super.bookKeeperClient == null && super.managedLedgerStorage != null) {
        bookKeeperClient(
            super.managedLedgerStorage.getStorageClasses().stream()
                .filter(BookkeeperManagedLedgerStorageClass.class::isInstance)
                .map(BookkeeperManagedLedgerStorageClass.class::cast)
                .map(BookkeeperManagedLedgerStorageClass::getBookKeeperClient)
                .findFirst()
                .get());
      }
        ...
        if (super.mockZooKeeper != null) {
          MetadataStoreExtended mockZookeeperMetadataStore =
              createMockZookeeperMetadataStore(
                  super.mockZooKeeper, super.config, MetadataStoreConfig.METADATA_STORE);
          ...
          MetadataStoreExtended testZookeeperMetadataStore =
              createTestZookeeperMetadataStore(
                  super.testZKServer, super.config, MetadataStoreConfig.METADATA_STORE);
          if (super.localMetadataStore == null) {
          ...
          try {
            MetadataStoreExtended store =
                MetadataStoreFactoryImpl.createExtended(
                    "memory:local",
    ...
    private MetadataStoreConfig createMetadataStoreConfig(
        ServiceConfiguration config, String metadataStoreName) {
    ...
    private MetadataStoreExtended createMockZookeeperMetadataStore(
        MockZooKeeper mockZooKeeper, ServiceConfiguration config, String metadataStoreName) {
      ...
      ZKMetadataStore zkMetadataStore =
          new ZKMetadataStore(
              mockZooKeeperSession, createMetadataStoreConfig(config, metadataStoreName));
      registerCloseable(
          () -> {
      ...
      MetadataStoreExtended nonClosingProxy =
          NonClosingProxyHandler.createNonClosingProxy(
              zkMetadataStore, MetadataStoreExtended.class);
      return nonClosingProxy;
    }

    @SneakyThrows
    private MetadataStoreExtended createTestZookeeperMetadataStore(
        TestZKServer zkServer, ServiceConfiguration config, String metadataStoreName) {
      MetadataStoreExtended store =
          MetadataStoreExtended.create(
              "zk:" + zkServer.getConnectionString(),
  ...
  static class StartableCustomBuilder extends AbstractCustomBuilder {
    ...
    protected void initializePulsarServices(SpyConfig spyConfig, Builder builder) {
      ...
      if (builder.compactionServiceFactory == null
        ...
        compactionServiceFactory =
            new MockPulsarCompactionServiceFactory(spyConfig, builder.compactor);
      }
    ...
    protected long resolveBrokerShutdownTimeoutMs() {
      ...
      // this reduces the chances that some threads of the PulsarTestsContexts of subsequent test
      // executions
      ...
      if (builder.managedLedgerStorage == null) {
        ...
        managedLedgerStorage(
            spyConfig
                ...
                .spy(
                    PulsarTestContext.createManagedLedgerStorage(
                        builder.bookKeeperClient, mlFactoryMock)));
      }
  ...
  @NonNull
  private static ManagedLedgerStorage createManagedLedgerStorage(
      BookKeeper bookKeeperClient, ManagedLedgerFactory managedLedgerFactory) {
