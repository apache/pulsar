...
public class AdminApiTransactionMultiBrokerTest extends TransactionTestBase {
   ...
   *
   * <p>1. Set up 16 broker and create 3 transaction coordinator topic. 2. The 3 transaction
   * coordinator topic will be assigned to these brokers through some kind of load-balancing
   * strategy. (In current implementations, they tend to be assigned to a broker.) 3. Find a broker
   * x which is not the owner of the transaction coordinator topic. 4. Create a admin connected to
   * broker x, and use the admin to call ` getCoordinatorInternalStats`.
   */
  @Test
  ...
  public void testGetTransactionBufferInternalStatsInMultiBroker() throws Exception {
    ...
    assertTrue(
        stats.segmentInternalStats.managedLedgerName.contains(
            SystemTopicNames.TRANSACTION_BUFFER_SNAPSHOT_SEGMENTS));
    assertNotNull(stats.segmentIndexInternalStats);
    assertTrue(
        stats.segmentIndexInternalStats.managedLedgerName.contains(
