...
public class TransactionMetadataStoreServiceTest extends BrokerTestBase {
  ...
  public void testAddAndRemoveTransactionMetadataStore() throws Exception {
    ...
    admin
        .lookups()
        .lookupTopic(SystemTopicNames.TRANSACTION_COORDINATOR_ASSIGN.getPartition(0).toString());
  ...
  public void testNewTransaction() throws Exception {
    ...
    checkTransactionMetadataStoreReady(
        (MLTransactionMetadataStore)
            pulsar
                .getTransactionMetadataStoreService()
                .getStores()
                .get(TransactionCoordinatorID.get(0)));
    checkTransactionMetadataStoreReady(
        (MLTransactionMetadataStore)
            pulsar
                .getTransactionMetadataStoreService()
                .getStores()
                .get(TransactionCoordinatorID.get(1)));
    checkTransactionMetadataStoreReady(
        (MLTransactionMetadataStore)
            pulsar
                .getTransactionMetadataStoreService()
                .getStores()
                .get(TransactionCoordinatorID.get(2)));
    TxnID txnID0 =
  ...
  private void checkoutTimeout(
      ConcurrentSkipListMap<Long, Pair<TxnMeta, List<Position>>> txnMap, int time) {
  ...
  private void checkTransactionMetadataStoreReady(
      MLTransactionMetadataStore transactionMetadataStore) throws NoSuchMethodException {
