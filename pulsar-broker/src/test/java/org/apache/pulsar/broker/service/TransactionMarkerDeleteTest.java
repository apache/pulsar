...
public class TransactionMarkerDeleteTest extends TransactionTestBase {
  ...
  public void testMarkerDeleteTimes() throws Exception {
    ...
    persistentSubscription.acknowledgeMessage(
        Collections.singletonList(position), AckType.Individual, Collections.emptyMap());
  ...
  public void testMarkerDelete() throws Exception {
    ...
    Producer<byte[]> producer =
        pulsarClient.newProducer().sendTimeout(0, TimeUnit.SECONDS).topic(topicName).create();
        ...
        .untilAsserted(
            () ->
                assertEquals(
                    admin
                        .topics()
                        .getInternalStats(topicName)
                        .cursors
                        .get(subName)
                        .markDeletePosition,
                    PositionFactory.create(msgId4.getLedgerId(), msgId4.getEntryId() + 2)
                        .toString()));
  }
