...
public class BucketDelayedDeliveryTrackerTest extends AbstractDeliveryTrackerTest {
  ...
  public Object[][] provider(Method method) throws Exception {
    ...
    doReturn("persistent://public/default/testDelay" + " / " + cursor.getName())
        .when(dispatcher)
        .getName();
  ...
  public void testRecoverSnapshot(BucketDelayedDeliveryTracker tracker) throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              Assert.assertTrue(
                  tracker.getImmutableBuckets().asMapOfRanges().values().stream()
    ...
    BucketDelayedDeliveryTracker tracker2 =
        new BucketDelayedDeliveryTracker(
            dispatcher,
            timer,
            1000,
  ...
  public void testRoaringBitmapSerialize() {
    ...
    RoaringBitmap roaringBitmap2 =
        new ImmutableRoaringBitmap(ByteBuffer.wrap(array)).toRoaringBitmap();
  ...
  public void testWithCreateFailDowngrade(BucketDelayedDeliveryTracker tracker) {
    ...
    Awaitility.await()
        .untilAsserted(() -> assertEquals(0, tracker.getImmutableBuckets().asMapOfRanges().size()));
