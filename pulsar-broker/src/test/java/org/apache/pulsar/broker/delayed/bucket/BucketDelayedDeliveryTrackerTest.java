...
public class BucketDelayedDeliveryTrackerTest extends AbstractDeliveryTrackerTest {
    ...
    public Object[][] provider(Method method) throws Exception {
        ...
        return switch (methodName) {
            ...
            case "testAddMessageWithDeliverAtTimeAfterNowAfterTickTimeFrequencyWithStrict",
                    "testRecoverSnapshot" -> new Object[][] {
                {
                    new BucketDelayedDeliveryTracker(
                            dispatcher,
                            timer,
    ...
    public void testRecoverSnapshot(BucketDelayedDeliveryTracker tracker) {
        ...
        Awaitility.await().untilAsserted(() -> {
            Assert.assertTrue(tracker.getImmutableBuckets().asMapOfRanges().values().stream()
                    .noneMatch(
                            x -> x.merging || !x.getSnapshotCreateFuture().get().isDone()));
        });
    ...
    public void testWithCreateFailDowngrade(BucketDelayedDeliveryTracker tracker) {
        ...
        Awaitility.await()
                .untilAsserted(() -> assertEquals(
                        0, tracker.getImmutableBuckets().asMapOfRanges().size()));
