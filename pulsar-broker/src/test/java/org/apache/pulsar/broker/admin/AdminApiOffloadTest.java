...
public class AdminApiOffloadTest extends MockedPulsarServiceBaseTest {
  ...
  private void testOffload(String topicName, String mlName) throws Exception {
    ...
    assertEquals(
        admin.topics().offloadStatus(topicName).getStatus(),
    ...
    BrokerOpenTelemetryTestUtil.assertMetricLongSumValue(
        ...
        attributes,
        actual -> assertThat(actual).isZero());
    ...
    assertEquals(
        admin.topics().offloadStatus(topicName).getStatus(), LongRunningProcessStatus.Status.ERROR);
    Assert.assertTrue(
        admin.topics().offloadStatus(topicName).getLastError().contains("Some random failure"));
    ...
    MessageId firstUnoffloaded =
        admin.topics().offloadStatus(topicName).getFirstUnoffloadedMessage();
  ...
  public void testOffloadPoliciesApi() throws Exception {
    ...
    OffloadPoliciesImpl offloadPolicies =
        (OffloadPoliciesImpl) admin.topics().getOffloadPolicies(topicName);
    ...
    Awaitility.await()
        .untilAsserted(() -> assertNotNull(admin.topics().getOffloadPolicies(topicName)));
    ...
    Awaitility.await()
        .untilAsserted(() -> assertNull(admin.topics().getOffloadPolicies(topicName)));
  ...
  public void testOffloadPoliciesAppliedApi() throws Exception {
    ...
    pulsarClient.newProducer().topic(topicName).create().close();
    OffloadPoliciesImpl offloadPolicies =
        (OffloadPoliciesImpl) admin.topics().getOffloadPolicies(topicName, true);
    OffloadPoliciesImpl brokerPolicies =
        OffloadPoliciesImpl.mergeConfiguration(
            null, null, pulsar.getConfiguration().getProperties());
    ...
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(
                    admin.namespaces().getOffloadPolicies(myNamespace), namespacePolicies));
  ...
  public void testSetNamespaceOffloadPoliciesFailByReadOnly() throws Exception {
    ...
    try {
      pulsar
          .getConfigurationMetadataStore()
          .put(NamespaceResources.POLICIES_READONLY_FLAG_PATH, "0".getBytes(), Optional.empty())
          .join();
      ...
      if (setNsPolicyReadOnlySuccess) {
        pulsar
            .getConfigurationMetadataStore()
            .delete(NamespaceResources.POLICIES_READONLY_FLAG_PATH, Optional.empty())
            .join();
      }
    }
  }
  ...
  public void testSetTopicOffloadPolicies() throws Exception {
    ...
    for (int a = 1; a <= 5; a++) {
      try {
        ...
        assertEquals(
            policies.getManagedLedgerOffloadThresholdInSeconds(),
      ...
      for (int i = 0; i < partitionNum; i++) {
        PersistentTopic topic =
            (PersistentTopic)
                pulsar
                    .getBrokerService()
                    .getTopicIfExists(TopicName.get(topicName).getPartition(i).toString())
                    .get()
                    .get();
        assertNotNull(topic.getManagedLedger().getConfig().getLedgerOffloader());
        assertEquals(
            topic.getManagedLedger().getConfig().getLedgerOffloader().getOffloadDriverName(),
            "NullLedgerOffloader");
      }
    } else {
      PersistentTopic topic =
      ...
      for (int i = 0; i < partitionNum; i++) {
        PersistentTopic topic =
            (PersistentTopic)
                pulsar
                    .getBrokerService()
                    .getTopic(TopicName.get(topicName).getPartition(i).toString(), false)
                    .get()
                    .get();
        assertNotNull(topic.getManagedLedger().getConfig().getLedgerOffloader());
        assertEquals(
            topic.getManagedLedger().getConfig().getLedgerOffloader().getOffloadDriverName(), "S3");
      }
    } else {
      PersistentTopic topic =
          (PersistentTopic) pulsar.getBrokerService().getTopic(topicName, false).get().get();
