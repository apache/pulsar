...
public class SubscribeRateTest extends BrokerTestBase {
  ...
  public void testBrokerLevelSubscribeRateDynamicUpdate() throws Exception {
    ...
    admin
        .brokers()
        .updateDynamicConfiguration("subscribeThrottlingRatePerConsumer", ratePerConsumer + "");
    Awaitility.await()
        .untilAsserted(() -> Assert.assertTrue(topicRef.getSubscribeRateLimiter().isPresent()));
    SubscribeRateLimiter limiter = topicRef.getSubscribeRateLimiter().get();
    Assert.assertEquals(
        limiter.getSubscribeRate().subscribeThrottlingRatePerConsumer, ratePerConsumer);
    Assert.assertEquals(limiter.getSubscribeRate().ratePeriodInSecond, 30);

    // subscribeRatePeriodPerConsumerInSecond
    admin
        .brokers()
        .updateDynamicConfiguration("subscribeRatePeriodPerConsumerInSecond", ratePeriod + "");
    Awaitility.await()
        .untilAsserted(
