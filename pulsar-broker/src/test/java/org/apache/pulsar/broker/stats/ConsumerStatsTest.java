...
public class ConsumerStatsTest extends ProducerConsumerBase {
  ...
  @Test
  public void testConsumerStatsOnZeroMaxUnackedMessagesPerConsumer()
    ...
    final String topicName =
        "persistent://my-property/my-ns/testConsumerStatsOnZeroMaxUnackedMessagesPerConsumer";
    ...
    Assert.assertEquals(
        stats.getSubscriptions().entrySet().iterator().next().getValue().getConsumers().size(), 1);
    Assert.assertFalse(
        stats
            .getSubscriptions()
            .entrySet()
  ...
  @Test
  public void testAckStatsOnPartitionedTopicForExclusiveSubscription()
      throws PulsarAdminException, PulsarClientException, InterruptedException {
    final String topic =
        "persistent://my-property/my-ns/testAckStatsOnPartitionedTopicForExclusiveSubscription";
    ...
    for (int i = 0; i < 3; i++) {
      ...
      Assert.assertEquals(stats.getSubscriptions().size(), 1);
      Assert.assertEquals(
          stats.getSubscriptions().entrySet().iterator().next().getValue().getConsumers().size(),
          1);
      Assert.assertEquals(
          stats
              .getSubscriptions()
  ...
  public void testConsumerStatsOutput(boolean classicDispatchers, SubscriptionType subscriptionType)
    ...
    if (subscriptionType == SubscriptionType.Key_Shared) {
      if (classicDispatchers) {
        expectedFields.addAll(List.of("readPositionWhenJoining", "keyHashRanges"));
      } else {
        expectedFields.addAll(List.of("drainingHashes", "keyHashRangeArrays"));
      }
    }
    final String topicName =
    ...
    String topicStatsUri =
        String.format(
            "%s/admin/v2/%s/stats", pulsar.getWebServiceAddress(), topicName.replace("://", "/"));
  ...
  private void testMessageAckRateMetric(String topicName, boolean exposeTopicLevelMetrics)
    ...
    String rateOutMetricName =
        exposeTopicLevelMetrics ? "pulsar_consumer_msg_rate_out" : "pulsar_rate_out";
  ...
  public void testAvgMessagesPerEntry() throws Exception {
    ...
    EntryFilterWithClassLoader loader =
        spyWithClassAndConstructorArgs(
            EntryFilterWithClassLoader.class, filter, narClassLoader, false);
  ...
  public void testNonPersistentTopicSharedSubscriptionUnackedMessages() throws Exception {
    ...
    List<? extends ConsumerStats> consumers =
        topicStats.getSubscriptions().get(subName).getConsumers();
  ...
  public void testKeySharedDrainingHashesConsumerStats() throws Exception {
    ...
    assertThat(c1Stats.getDrainingHashes())
        .extracting(DrainingHash::getHash)
        .containsExactlyInAnyOrderElementsOf(c2HashesByStats);
    assertThat(c1Stats.getDrainingHashes())
        .extracting(DrainingHash::getHash, DrainingHash::getUnackMsgs)
        .containsExactlyInAnyOrderElementsOf(
            c1DrainingHashesExpected.entrySet().stream()
    ...
    assertThat(c1Stats.getDrainingHashesUnackedMessages())
        .isEqualTo(expectedDrainingHashesUnackMessages);
        ...
        .untilAsserted(
            () -> {
              SubscriptionStats stats =
                  admin.topics().getStats(topic).getSubscriptions().get(subscriptionName);
              assertThat(stats.getConsumers().get(0).getDrainingHashes())
                  .isNotEmpty()
                  .allSatisfy(
                      dh -> {
                        assertThat(dh)
                            .extracting(DrainingHash::getBlockedAttempts)
        ...
        Awaitility.await()
            ...
            .untilAsserted(
                () -> {
                  SubscriptionStats stats =
                      admin.topics().getStats(topic).getSubscriptions().get(subscriptionName);
                  assertThat(stats.getConsumers().get(0))
                      .satisfies(
                          consumerStats -> {
                            assertThat(consumerStats)
                                .describedAs(
                                    "Consumer stats should have one draining hash %s",
                                    consumerStats)
  ...
  private String findConsumerNameForHash(SubscriptionStats subscriptionStats, int hash) {
    return findConsumerForHash(subscriptionStats, hash)
        .map(ConsumerStats::getConsumerName)
        .orElse(null);
  }

  private Optional<? extends ConsumerStats> findConsumerForHash(
      SubscriptionStats subscriptionStats, int hash) {
