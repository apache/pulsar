...
public class KeySharedSubscriptionBrokerCacheTest extends ProducerConsumerBase {
  ...
  public static Object[] createTestInstances() {
    return KeySharedImplementationType.generateTestInstances(
        KeySharedSubscriptionBrokerCacheTest::new);
  }
  ...
  @SneakyThrows
  private StickyKeyDispatcher getStickyKeyDispatcher(String topic, String subscription) {
  ...
  public void testReplayQueueReadsGettingCached(KeySharedImplementationType impl) throws Exception {
    ...
    BlockingQueue<Pair<Consumer<Integer>, Message<Integer>>> unackedMessages =
    ...
    MessageListener<Integer> messageHandler =
        (consumer, msg) -> {
    ...
    pulsarTestContext
        .getMockBookKeeper()
        .setReadHandleInterceptor(
            (ledgerId, firstEntry, lastEntry, entries) -> {
              log.error(
                  "Attempting to read from BK when cache should be used. {}:{} to {}:{}",
                  ledgerId,
                  firstEntry,
                  ledgerId,
                  lastEntry);
    ...
    StickyKeyDispatcher dispatcher = getStickyKeyDispatcher(topic, SUBSCRIPTION_NAME);
    ...
    try {
      assertSoftly(
          softly -> {
            ...
            softly.assertThat(cacheStats.getCacheHitsTotal()).as("cache hits").isGreaterThan(0);
            softly.assertThat(cacheStats.getCacheMissesTotal()).as("cache misses").isEqualTo(0);
            softly
                .assertThat(cacheStats.getNumberOfCacheEvictions())
                .as("cache evictions")
                .isEqualTo(0);
          });
    } finally {
