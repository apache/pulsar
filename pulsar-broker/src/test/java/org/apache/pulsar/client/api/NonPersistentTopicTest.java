...
public class NonPersistentTopicTest extends ProducerConsumerBase {
  ...
  public Object[][] getLoadManager() {
    return new Object[][] {
      {SimpleLoadManagerImpl.class.getCanonicalName()},
  ...
  public void testNonPersistentPartitionsAreNotAutoCreatedWhenThePartitionedTopicDoesNotExist()
      throws Exception {
    ...
    try {
      ...
      assertThrows(
          ...
          () ->
              pulsarClient
                  .newConsumer()
                  .topic(topicPartitionName)
                  .subscriptionName("sub-issue-9173")
                  .subscribe());
  ...
  @Test(timeOut = 90000 /* 1.5mn */)
  public void testAutoCreateNonPersistentPartitionsWhenThePartitionedTopicExists()
      throws Exception {
    final boolean defaultAllowAutoTopicCreation = conf.isAllowAutoTopicCreation();
    try {
      // Given the auto topic creation is disabled
      ...
      // When subscribe, then a sub-consumer is created for each partition which means the
      ...
      // When produce, a sub-producer is created for each partition which means the partitions are
      // created
  ...
  @Test(dataProvider = "subscriptionType")
  public void testPartitionedNonPersistentTopicWithTcpLookup(SubscriptionType type)
      throws Exception {
    log.info("-- Starting {} test --", methodName);
  ...
  /**
   * It verifies that broker doesn't dispatch messages if consumer runs out of permits filled out
   * with messages
   */
  @Test(dataProvider = "subscriptionType")
  ...
  /**
   * Verifies that broker should failed to publish message if producer publishes messages more than
   * rate limit
   */
  @Test
  public void testProducerRateLimit() throws Exception {
    ...
    try {
      ...
      @Cleanup
      Consumer<byte[]> consumer =
          pulsarClient.newConsumer().topic(topic).subscriptionName("subscriber-1").subscribe();
      ...
      assertMetricLongSumValue(
          metrics,
          OpenTelemetryProducerStats.MESSAGE_DROP_COUNTER,
          attributes,
  ...
  public void testMultipleSubscription() throws Exception {
    ...
    final String topic = "non-persistent://my-property/my-ns/unacked-topic";
    ConsumerImpl<byte[]> consumer1Shared =
        (ConsumerImpl<byte[]>)
            pulsarClient
                .newConsumer()
                .topic(topic)
                .subscriptionName("subscriber-shared")
                .subscriptionType(SubscriptionType.Shared)
                .subscribe();

    ConsumerImpl<byte[]> consumer2Shared =
        (ConsumerImpl<byte[]>)
            pulsarClient
                .newConsumer()
                .topic(topic)
    ...
    ConsumerImpl<byte[]> consumer1FailOver =
        (ConsumerImpl<byte[]>)
            pulsarClient
                .newConsumer()
                .topic(topic)
                .subscriptionName("subscriber-fo")
                .subscriptionType(SubscriptionType.Failover)
                .subscribe();

    ConsumerImpl<byte[]> consumer2FailOver =
        (ConsumerImpl<byte[]>)
            pulsarClient
                .newConsumer()
                .topic(topic)
  ...
  public void testReplicator() throws Exception {
    ...
    try {
      ...
      ConsumerImpl<byte[]> consumer2 =
          (ConsumerImpl<byte[]>)
              client1
                  .newConsumer()
                  .topic(globalTopicName)
                  .subscriptionName("subscriber-2")
                  .subscribe();

      ConsumerImpl<byte[]> repl2Consumer =
          (ConsumerImpl<byte[]>)
              client2
                  .newConsumer()
                  .topic(globalTopicName)
                  .subscriptionName("subscriber-1")
                  .subscribe();
      ConsumerImpl<byte[]> repl3Consumer =
          (ConsumerImpl<byte[]>)
              client3
                  .newConsumer()
                  .topic(globalTopicName)
      ...
      NonPersistentTopic topicRef =
          (NonPersistentTopic)
              replication.pulsar1.getBrokerService().getTopicReference(globalTopicName).get();
      NonPersistentReplicator replicatorR2 =
          (NonPersistentReplicator) topicRef.getPersistentReplicator("r2");
      NonPersistentReplicator replicatorR3 =
          (NonPersistentReplicator) topicRef.getPersistentReplicator("r3");
      assertNotNull(topicRef);
  ...
  public void testLoadManagerAssignmentForNonPersistentTestAssignment(String loadManagerName)
    ...
    try {
      ...
      try {
        Producer<byte[]> producer =
            pulsarClient.newProducer().topic(topicName).createAsync().get(1, TimeUnit.SECONDS);
        producer.close();
  ...
  /**
   * verifies: broker should reject non-persistent topic loading if broker is not enable for
        ...
        executor.submit(
            () -> {
              try {
                ...
                if (count > totalProduceMessages * 0.2
                    && msgId != null
  ...
  class ReplicationClusterManager {
    ...
    ExecutorService executor =
        new ThreadPoolExecutor(5, 20, 30, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
    ...
    void setupReplicationCluster() throws Exception {
      ...
      // NOTE: we have to instantiate a new copy of System.getProperties() to make sure pulsar1 and
      // pulsar2 have
      // completely
      ...
      config1.setConfigurationMetadataStoreUrl(
          "zk:127.0.0.1:" + globalZkS.getZookeeperPort() + "/foo");
      ...
      config2.setConfigurationMetadataStoreUrl(
          "zk:127.0.0.1:" + globalZkS.getZookeeperPort() + "/foo");
      ...
      config3.setConfigurationMetadataStoreUrl(
          "zk:127.0.0.1:" + globalZkS.getZookeeperPort() + "/foo");
      ...
      admin1
          .namespaces()
          .setNamespaceReplicationClusters("pulsar/global/ns", Sets.newHashSet("r1", "r2", "r3"));
      ...
      assertEquals(
          admin2.clusters().getCluster("r1").getBrokerServiceUrl(), pulsar1.getBrokerServiceUrl());
      assertEquals(
          admin2.clusters().getCluster("r2").getBrokerServiceUrl(), pulsar2.getBrokerServiceUrl());
      assertEquals(
          admin2.clusters().getCluster("r3").getBrokerServiceUrl(), pulsar3.getBrokerServiceUrl());
      Thread.sleep(100);
