...
public class RawReaderTest extends MockedPulsarServiceBaseTest {
    ...
    public void testFlowControl() throws Exception {
        ...
        for (Future<RawMessage> f : futures) {
            try (RawMessage m = f.get(1, TimeUnit.SECONDS)) {
                ...
                Assert.assertTrue(
                        keys.add(key), "Received duplicated key '" + key + "' : already received keys = " + keys);
    ...
    public void testBatchingExtractKeysAndIds() throws Exception {
        ...
        try (RawMessage m = reader.readNextAsync().get()) {
            ...
            Assert.assertTrue(
                    idsAndKeys.get(0).getLeft().compareTo(idsAndKeys.get(1).getLeft()) < 0);
            Assert.assertTrue(
                    idsAndKeys.get(1).getLeft().compareTo(idsAndKeys.get(2).getLeft()) < 0);
    ...
    public void testBatchingRebatch() throws Exception {
        ...
        try (RawMessage m1 = reader.readNextAsync().get()) {
            RawMessage m2 = RawBatchConverter.rebatchMessage(m1, (key, id) -> key.equals("key2"))
                    .get();
            List<ImmutableTriple<MessageId, String, Integer>> idsAndKeys =
    ...
    public void testAcknowledgeWithProperties() throws Exception {
        ...
        Awaitility.await()
                .untilAsserted(() -> Assert.assertEquals(
                        ledger.openCursor(subscription).getProperties().get("foobar"), Long.valueOf(0xdeadbeefdecaL)));
    }
