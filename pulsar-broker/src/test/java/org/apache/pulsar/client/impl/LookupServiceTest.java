...
public class LookupServiceTest extends ProducerConsumerBase {
  ...
  public void testGetTopicsOfGetTopicsResult(boolean isUsingHttpLookup) throws Exception {
    ...
    Collection<String> topics =
        lookupService
            .getTopicsUnderNamespace(
                NamespaceName.get("public/default"), Mode.PERSISTENT, "public/default/.*", null)
            .join()
            .getTopics();
    ...
    assertTrue(
        nonPartitionedOrPartitionTopics.contains(
            TopicName.get(partitionedTopic).getPartition(0).toString()));
    assertTrue(
        nonPartitionedOrPartitionTopics.contains(
            TopicName.get(partitionedTopic).getPartition(1).toString()));
    assertTrue(
        nonPartitionedOrPartitionTopics.contains(
            TopicName.get(partitionedTopic).getPartition(2).toString()));
  ...
  public void testGetPartitionedTopicMetadataByPulsarClient(boolean isUsingHttpLookup)
    ...
    assertThat(
            lookupService.getPartitionedTopicMetadata(
                TopicName.get(partitionedTopicWithPartitionIndex), false))
        .succeedsWithin(3, TimeUnit.SECONDS)
        .matches(n -> n.partitions == 0);
  }
  ...
  public void testGetPartitionedTopicMedataByAdmin() throws PulsarAdminException {
    ...
    assertThrows(
        ...
        () -> admin.topics().getPartitionedTopicMetadata(nonPartitionedTopic));
    assertThrows(
        PulsarAdminException.NotFoundException.class,
        () -> admin.topics().getPartitionedTopicMetadata(partitionedTopic));
    assertThrows(
        PulsarAdminException.NotFoundException.class,
    ...
    assertEquals(
        admin.topics().getPartitionedTopicMetadata(partitionedTopicWithPartitionIndex).partitions,
        0);
  }
}
