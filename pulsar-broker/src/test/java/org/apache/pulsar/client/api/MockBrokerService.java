...
public class MockBrokerService {
  ...
  private class genericResponseHandler extends AbstractHandler {
    ...
    private final PartitionedTopicMetadata multiPartitionedTopicMetadata =
        new PartitionedTopicMetadata(4);
  ...
  private class MockServerCnx extends PulsarDecoder {
    ...
    protected void handleGetOrCreateSchema(CommandGetOrCreateSchema commandGetOrCreateSchema) {
      ...
      // default
      ctx.writeAndFlush(
          Commands.newGetOrCreateSchemaResponse(
      ...
      bootstrap.childHandler(
          new ChannelInitializer<SocketChannel>() {
            ...
            public void initChannel(SocketChannel ch) throws Exception {
              ch.pipeline()
                  .addLast(
                      "frameDecoder", new LengthFieldBasedFrameDecoder(MaxMessageSize, 0, 4, 0, 4));
  ...
  public String getBrokerAddress() {
    return String.format(
        "pulsar://localhost:%d", ((InetSocketAddress) listenChannel.localAddress()).getPort());
  }
