...
/**
 * Different with {@link org.apache.pulsar.client.api.SimpleProducerConsumerTest}, this class can
 * visit the variables of {@link ConsumerImpl} or {@link ProducerImpl} which have protected or
...
public class ProducerConsumerInternalTest extends ProducerConsumerBase {
  ...
  public void testSameProducerRegisterTwiceWithSpecifiedProducerName() throws Exception {
    ...
    ProducerImpl producer =
        (ProducerImpl) pulsarClient.newProducer().producerName(pName).topic(topicName).create();
    ServiceProducer serviceProducer = getServiceProducer(producer, topicName);
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              assertEquals(
                  producer.getState().toString(), "Ready", "The producer registration failed");
            });
  }
        ...
        .untilAsserted(
            () -> {
              assertFalse(
                  serverCnx
                      .getProducers()
                      .containsKey(serviceProducer.getServiceProducer().getProducerId()));
            });
  }

  @Test(groups = "flaky")
  public void testExclusiveConsumerWillAlwaysRetryEvenIfReceivedConsumerBusyError()
      throws Exception {
    final String topicName = BrokerTestUtil.newUniqueName("persistent://my-property/my-ns/tp_");
    ...
    // Make the second subscribe runs after the broker removing the old consumer, then it will
    // receive
  ...
  public void testSendTimerCheckForBatchContainer(BatcherBuilder batcherBuilder) throws Exception {
    ...
    ((ProducerImpl<byte[]>) producer)
        .triggerSendTimer(); // t+1000ms && t+2000ms: run() will be called again
        ...
        .untilAsserted(
            () -> {
              ...
              PersistentTopicInternalStats internalStats =
                  admin.topics().getInternalStatsAsync(topicName).get();
