...
public class ClientErrorsTest {
  ...
  public void testMockBrokerService() throws PulsarClientException {
    ...
    try {
      Consumer<byte[]> consumer =
          client
              .newConsumer()
              .topic("persistent://prop/use/ns/t1")
  ...
  private void producerCreateFailWithoutRetry(String topic) throws Exception {
    ...
    mockBrokerService.setHandleProducer(
        (ctx, producer) -> {
          if (counter.incrementAndGet() == 2) {
            // piggyback unknown error to relay assertion failure
            ctx.writeAndFlush(
                Commands.newError(
                    producer.getRequestId(), ServerError.UnknownError, ASSERTION_ERROR));
            return;
          }
          ctx.writeAndFlush(
              Commands.newError(producer.getRequestId(), ServerError.AuthorizationError, "msg"));
        });
  ...
  private void producerCreateSuccessAfterRetry(String topic) throws Exception {
    ...
    mockBrokerService.setHandleProducer(
        (ctx, producer) -> {
          if (counter.incrementAndGet() == 2) {
            ctx.writeAndFlush(
                Commands.newProducerSuccess(
                    producer.getRequestId(), "default-producer", SchemaVersion.Empty));
            return;
          }
          ctx.writeAndFlush(
              Commands.newError(producer.getRequestId(), ServerError.ServiceNotReady, "msg"));
        });
  ...
  private void producerCreateFailAfterRetryTimeout(String topic) throws Exception {
    ...
    mockBrokerService.setHandleCloseProducer(
        (ctx, closeProducer) -> {
    ...
    // There is a small race condition here because the producer's timeout both fails the client
    // creation
  ...
  private void producerCreatedThenFailsRetryTimeout(String topic) throws Exception {
    ...
    mockBrokerService.setHandleProducer(
        (ctx, producer) -> {
          ...
          if (producerCount == 1) {
            ctx.writeAndFlush(
                Commands.newProducerSuccess(
                    producer.getRequestId(), "producer1", SchemaVersion.Empty));
            // Trigger reconnect
    ...
    // Create producer should succeed then upon closure, it should reattempt creation. The first
    // request will
    // time out, which triggers CloseProducer. The client might send the third Producer command
    // before the
  ...
  private void consumerCreatedThenFailsRetryTimeout(String topic) throws Exception {
    ...
    mockBrokerService.setHandleSubscribe(
        (ctx, subscribe) -> {
    ...
    // The first
    // request will time out, which triggers CloseConsumer. The client might send the third
    // Subscribe command before
  ...
  public void testProducerFailDoesNotFailOtherProducer() throws Exception {
    producerFailDoesNotFailOtherProducer(
        "persistent://prop/use/ns/t1", "persistent://prop/use/ns/t2");
  }

  @Test
  public void testPartitionedProducerFailDoesNotFailOtherProducer() throws Exception {
    producerFailDoesNotFailOtherProducer(
        "persistent://prop/use/ns/part-t1", "persistent://prop/use/ns/part-t2");
  }
  ...
  private void producerFailDoesNotFailOtherProducer(String topic1, String topic2) throws Exception {
    ...
    ProducerBase<byte[]> producer1 =
        (ProducerBase<byte[]>) client.newProducer().topic(topic1).create();
  ...
  private void producerContinuousRetryAfterSendFail(String topic) throws Exception {
    ...
    mockBrokerService.setHandleProducer(
        (ctx, producer) -> {
          ...
          ctx.writeAndFlush(
              Commands.newError(producer.getRequestId(), ServerError.PersistenceError, "msg"));
        });
    ...
    mockBrokerService.setHandleSend(
        (ctx, send, headersAndPayload) -> {
          ...
          if (msgCounter.incrementAndGet() == 1) {
            ctx.writeAndFlush(
                Commands.newSendError(0, 0, ServerError.PersistenceError, "Send Failed"));
            return;
          }
  ...
  public void testLookupWithDisconnection() throws Exception {
    ...
    mockBrokerService.setHandleLookup(
        (ctx, lookup) -> {
          ...
          ctx.writeAndFlush(
              Commands.newLookupResponse(
                  mockBrokerService.getBrokerAddress(),
                  null,
                  true,
                  LookupType.Connect,
                  lookup.getRequestId(),
    ...
    mockBrokerService.setHandleSubscribe(
        (ctx, subscribe) -> {
          if (counter.incrementAndGet() == 2) {
            ...
            ctx.writeAndFlush(
                Commands.newError(
                    subscribe.getRequestId(), ServerError.UnknownError, ASSERTION_ERROR));
            return;
          }
          ctx.writeAndFlush(
              Commands.newError(subscribe.getRequestId(), ServerError.PersistenceError, "msg"));
        });
  ...
  private void subscribeSuccessAfterRetry(String topic) throws Exception {
    ...
    mockBrokerService.setHandleSubscribe(
        (ctx, subscribe) -> {
          ...
          ctx.writeAndFlush(
              Commands.newError(subscribe.getRequestId(), ServerError.ServiceNotReady, "msg"));
        });
  ...
  private void subscribeFailAfterRetryTimeout(String topic) throws Exception {
    ...
    PulsarClient client =
        PulsarClient.builder()
            .serviceUrl(mockBrokerService.getBrokerAddress())
            .operationTimeout(200, TimeUnit.MILLISECONDS)
            .build();
    final AtomicInteger counter = new AtomicInteger(0);
  ...
  public void testSubscribeFailDoesNotFailOtherConsumer() throws Exception {
    subscribeFailDoesNotFailOtherConsumer(
        "persistent://prop/use/ns/t1", "persistent://prop/use/ns/t2");
  }
  ...
  private void subscribeFailDoesNotFailOtherConsumer(String topic1, String topic2)
      throws Exception {
    @Cleanup
    PulsarClient client =
        PulsarClient.builder().serviceUrl(mockBrokerService.getBrokerAddress()).build();
    ...
    mockBrokerService.setHandleSubscribe(
        (ctx, subscribe) -> {
          if (counter.incrementAndGet() == 2) {
            // fail second producer
            ctx.writeAndFlush(
                Commands.newError(subscribe.getRequestId(), ServerError.AuthorizationError, "msg"));
            return;
          }
          ctx.writeAndFlush(Commands.newSuccess(subscribe.getRequestId()));
        });

    ConsumerBase<byte[]> consumer1 =
        (ConsumerBase<byte[]>)
            client.newConsumer().topic(topic1).subscriptionName("sub1").subscribe();

    ConsumerBase<byte[]> consumer2 = null;
    try {
      consumer2 =
          (ConsumerBase<byte[]>)
              client.newConsumer().topic(topic2).subscriptionName("sub1").subscribe();
      fail("Should have failed");
    } catch (Exception e) {
      // ok
    }
  ...
  // failed to connect to partition at initialization step if a producer which connects to broker as
  // lazy-loading mode
  @Test
  public void testPartitionedProducerFailOnInitialization() throws Throwable {
    ...
    try {
      client
          ...
          .topic("persistent://prop/use/ns/multi-part-t1")
          .create();
  ...
  // failed to connect to partition at sending step if a producer which connects to broker as
  // lazy-loading mode
  @Test
  public void testPartitionedProducerFailOnSending() throws Throwable {
    ...
    try {
      ...
      assertEquals(
          producer.getProducers().size(), client.getPartitionsForTopic(topicName).get().size());
  ...
  // if a producer which doesn't connect as lazy-loading mode fails to connect while creating
  ...
  // if a consumer fails to subscribe while creating partitioned consumer, it should close all
  ...
  public void testFlowSendWhenPartitionedSubscribeCompletes() throws Exception {
    ...
    mockBrokerService.setHandleFlow(
        (ctx, sendFlow) -> {
          if (subscribed.get() != 4) {
            fail.set(true);
          }
  ...
  public void testProducerReconnect() throws Exception {
    ...
    mockBrokerService.setHandleConnect(
        (ctx, connect) -> {
          channelCtx.set(ctx);
    ...
    mockBrokerService.setHandleProducer(
        (ctx, produce) -> {
          ctx.writeAndFlush(
              Commands.newProducerSuccess(
                  produce.getRequestId(), "default-producer", SchemaVersion.Empty));
        });

    mockBrokerService.setHandleSend(
        (ctx, sendCmd, headersAndPayload) -> {
          msgSent.set(true);
          ctx.writeAndFlush(Commands.newSendReceipt(0, 0, 0, 1, 1));
        });
  ...
  public void testCommandErrorMessageIsNull() throws Exception {
    ...
    mockBrokerService.setHandleProducer(
        (ctx, producer) -> {
          try {
            ctx.writeAndFlush(
                Commands.newError(producer.getRequestId(), ServerError.AuthorizationError, null));
          } catch (Exception e) {
