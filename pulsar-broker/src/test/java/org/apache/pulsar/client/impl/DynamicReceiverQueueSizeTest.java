...
public class DynamicReceiverQueueSizeTest extends MockedPulsarServiceBaseTest {
  ...
  public void testConsumerImpl() throws PulsarClientException {
    ...
    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(consumer.getTotalIncomingMessages(), 5));
    ...
    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(consumer.getTotalIncomingMessages(), 10));
    Assert.assertEquals(consumer.getAvailablePermits(), 0);
    ...
    Awaitility.await()
        .untilAsserted(() -> Assert.assertEquals(consumer.getTotalIncomingMessages(), 3));
  }
