...
public class ServiceUrlProviderTest extends ProducerConsumerBase {
  ...
  public void testCreateClientWithServiceUrlProvider() throws Exception {
    ...
    PulsarClient client =
        PulsarClient.builder()
            ...
            .build();
    Assert.assertTrue(
        ((PulsarClientImpl) client).getConfiguration().getServiceUrlProvider()
  ...
  public void testCreateClientWithAutoChangedServiceUrlProvider() throws Exception {
    ...
    log.info(
        "Service url : producer = {}, consumer = {}",
        producer.getClient().getLookup().getServiceUrl(),
        consumer.getClient().getLookup().getServiceUrl());

    Assert.assertEquals(
        producer.getClient().getLookup().getServiceUrl(), pulsarService1.getBrokerServiceUrl());
    Assert.assertEquals(
        consumer.getClient().getLookup().getServiceUrl(), pulsarService1.getBrokerServiceUrl());

    log.info(
        "Changing service url from {} to {}",
        pulsarService1.getBrokerServiceUrl(),
        pulsarService2.getBrokerServiceUrl());

    serviceUrlProvider.onServiceUrlChanged(pulsarService2.getBrokerServiceUrl());
    log.info(
        "Service url changed : producer = {}, consumer = {}",
        producer.getClient().getLookup().getServiceUrl(),
        consumer.getClient().getLookup().getServiceUrl());
    Assert.assertEquals(
        producer.getClient().getLookup().getServiceUrl(), pulsarService2.getBrokerServiceUrl());
    Assert.assertEquals(
        consumer.getClient().getLookup().getServiceUrl(), pulsarService2.getBrokerServiceUrl());
    producer.close();
    consumer.close();
  }
