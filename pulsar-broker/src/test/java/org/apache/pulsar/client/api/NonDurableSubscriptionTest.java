...
public class NonDurableSubscriptionTest extends ProducerConsumerBase {
    ...
    brokerService.setPulsarChannelInitializerFactory(
        (_pulsar, opts) -> {
          return new PulsarChannelInitializer(_pulsar, opts) {
            @Override
            protected ServerCnx newServerCnx(PulsarService pulsar, String listenerName)
                throws Exception {
  ...
  public void testSameSubscriptionNameForDurableAndNonDurableSubscription() throws Exception {
    ...
    try {
      @Cleanup
      Consumer<String> durableConsumer =
          pulsarClient
              .newConsumer(Schema.STRING)
              .topic(topicName)
              .subscriptionMode(SubscriptionMode.Durable)
  ...
  public void testNonDurableSubscriptionRecovery(SubscriptionType subscriptionType)
      throws Exception {
    log.info("testing {}", subscriptionType);
    String topicName =
        "persistent://my-property/my-ns/nonDurable-sub-recorvery-" + subscriptionType;
    // 1 setup producer、consumer
    @Cleanup
    Producer<String> producer = pulsarClient.newProducer(Schema.STRING).topic(topicName).create();
    @Cleanup
  ...
  public void testHasMessageAvailableIfIncomingQueueNotEmpty() throws Exception {
    ...
    Reader<String> reader =
        pulsarClient
            ...
            .topic(topicName)
            .receiverQueueSize(1)
            .subscriptionName(nonDurableCursor)
            .startMessageId(MessageIdImpl.earliest)
            .create();
    Producer<String> producer = pulsarClient.newProducer(Schema.STRING).topic(topicName).create();
    ...
    // Since there is one message in the incoming queue, so the method "reader.hasMessageAvailable"
    // should return
    // true.
  ...
  public void testInitReaderAtSpecifiedPosition() throws Exception {
    ...
    Reader<String> reader1 =
        pulsarClient
            ...
            .subscriptionName(s1)
            .receiverQueueSize(0)
            .startMessageId(startMessageId1)
            .create();
    ManagedLedgerInternalStats.CursorStats cursor1 =
        admin.topics().getInternalStats(topicName).cursors.get(s1);
    log.info(
        "cursor1 readPosition: {}, markDeletedPosition: {}",
        cursor1.readPosition,
        cursor1.markDeletePosition);
    ...
    Reader<String> reader2 =
        pulsarClient
            ...
            .subscriptionName(s2)
            .receiverQueueSize(0)
            .startMessageId(startMessageId2)
            .create();
    ManagedLedgerInternalStats.CursorStats cursor2 =
        admin.topics().getInternalStats(topicName).cursors.get(s2);
    log.info(
        "cursor2 readPosition: {}, markDeletedPosition: {}",
        cursor2.readPosition,
        cursor2.markDeletePosition);
    ...
    Reader<String> reader3 =
        pulsarClient
            ...
            .subscriptionName(s3)
            .receiverQueueSize(0)
            .startMessageId(startMessageId3)
            .create();
    ManagedLedgerInternalStats.CursorStats cursor3 =
        admin.topics().getInternalStats(topicName).cursors.get(s3);
    log.info(
        "cursor3 readPosition: {}, markDeletedPosition: {}",
        cursor3.readPosition,
        cursor3.markDeletePosition);
    ...
    String s4 = "s4";
    MessageIdImpl startMessageId4 =
        new MessageIdImpl(currentLedger.getId() + 1, Long.MAX_VALUE, -1);
    Reader<String> reader4 =
        pulsarClient
            ...
            .subscriptionName(s4)
            .receiverQueueSize(0)
            .startMessageId(startMessageId4)
            .create();
    ManagedLedgerInternalStats.CursorStats cursor4 =
        admin.topics().getInternalStats(topicName).cursors.get(s4);
    log.info(
        "cursor4 readPosition: {}, markDeletedPosition: {}",
        cursor4.readPosition,
        cursor4.markDeletePosition);
    ...
    Reader<String> reader5 =
        pulsarClient
            ...
            .subscriptionName(s5)
            .receiverQueueSize(0)
            .startMessageId(startMessageId5)
            .create();
    ManagedLedgerInternalStats.CursorStats cursor5 =
        admin.topics().getInternalStats(topicName).cursors.get(s5);
    log.info(
        "cursor5 readPosition: {}, markDeletedPosition: {}",
        cursor5.readPosition,
        cursor5.markDeletePosition);
    ...
    Reader<String> reader6 =
        pulsarClient
            ...
            .subscriptionName(s6)
            .receiverQueueSize(0)
            .startMessageId(startMessageId6)
            .create();
    ManagedLedgerInternalStats.CursorStats cursor6 =
        admin.topics().getInternalStats(topicName).cursors.get(s6);
    log.info(
        "cursor6 readPosition: {}, markDeletedPosition: {}",
        cursor6.readPosition,
        cursor6.markDeletePosition);
    ...
    String s7 = "s7";
    MessageIdImpl startMessageId7 =
        new MessageIdImpl(ledgers.get(ledgers.size() - 1), Long.MAX_VALUE, -1);
    Reader<String> reader7 =
        pulsarClient
            ...
            .subscriptionName(s7)
            .receiverQueueSize(0)
            .startMessageId(startMessageId7)
            .create();
    ManagedLedgerInternalStats.CursorStats cursor7 =
        admin.topics().getInternalStats(topicName).cursors.get(s7);
    log.info(
        "cursor7 readPosition: {}, markDeletedPosition: {}",
        cursor7.readPosition,
        cursor7.markDeletePosition);
    ...
    Reader<String> reader8 =
        pulsarClient
            ...
            .subscriptionName(s8)
            .receiverQueueSize(0)
            .startMessageId(startMessageId8)
            .create();
    ManagedLedgerInternalStats.CursorStats cursor8 =
        admin.topics().getInternalStats(topicName).cursors.get(s8);
    log.info(
        "cursor8 readPosition: {}, markDeletedPosition: {}",
        cursor8.readPosition,
        cursor8.markDeletePosition);
    ...
    Reader<String> reader10 =
        pulsarClient
            .newReader(Schema.STRING)
            .topic(topicName)
            .subscriptionName(s10)
            .receiverQueueSize(0)
            .startMessageId(startMessageId10)
            .create();
    ManagedLedgerInternalStats.CursorStats cursor10 =
        admin.topics().getInternalStats(topicName).cursors.get(s10);
    log.info(
        "cursor10 readPosition: {}, markDeletedPosition: {}",
        cursor10.readPosition,
        cursor10.markDeletePosition);
  ...
  private Position parseReadPosition(ManagedLedgerInternalStats.CursorStats cursorStats) {
    String[] ledgerIdAndEntryId = cursorStats.readPosition.split(":");
    return PositionFactory.create(
        Long.valueOf(ledgerIdAndEntryId[0]), Long.valueOf(ledgerIdAndEntryId[1]));
  }
  ...
  public void testReaderInitAtDeletedPosition() throws Exception {
    ...
    MessageIdImpl startMessageId =
        new MessageIdImpl(
            msgIdInDeletedLedger4.getLedgerId(), msgIdInDeletedLedger4.getEntryId(), -1);
    Reader<String> reader =
        pulsarClient
            .newReader(Schema.STRING)
            .topic(topicName)
        ...
        .untilAsserted(
            () -> {
              SubscriptionStats subscriptionStats =
                  admin.topics().getStats(topicName, true, true, true).getSubscriptions().get("s1");
              ...
              Position expectedMarkDeletedPos =
                  PositionFactory.create(
                      msgIdInDeletedLedger5.getLedgerId(), msgIdInDeletedLedger5.getEntryId());
  ...
  public void testTrimLedgerIfNoDurableCursor() throws Exception {
    ...
    Consumer<String> consumer =
        pulsarClient
            ...
            .topic(topicName)
            .receiverQueueSize(1)
            .subscriptionName(durableCursor)
            .subscribe();
    consumer.close();
        ...
        .untilAsserted(
            () -> {
              SubscriptionStats subscriptionStats =
                  admin
                      .topics()
                      .getStats(topicName, true, true, true)
                      .getSubscriptions()
                      .get(nonDurableCursor);
              log.info("backlog size: {}", subscriptionStats.getMsgBacklog());
              assertEquals(subscriptionStats.getMsgBacklog(), 0);
              ManagedLedgerInternalStats.CursorStats cursorStats =
    ...
    // The following tests are designed to verify the api "getNumberOfEntries" and "consumedEntries"
    // still work
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              // Since there is one message has been pulled into the incoming queue of reader. There
              // is only one messages
    ...
    // Note: "topic_internal_stat.cursor.messagesConsumedCounter" means how many messages were acked
    // on this
    //   cursor. The similar one "topic_stats.lastConsumedTimestamp" means the last time of sending
    // messages to
    //   the consumer.
