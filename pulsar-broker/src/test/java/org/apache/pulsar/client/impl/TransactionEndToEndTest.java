...
public class TransactionEndToEndTest extends TransactionTestBase {
  ...
  private void testFilterMsgsInPendingAckStateWhenConsumerDisconnect(boolean enableBatch)
      throws Exception {
    final String topicName =
        NAMESPACE1 + "/testFilterMsgsInPendingAckStateWhenConsumerDisconnect-" + enableBatch;
    final int count = 10;

    @Cleanup Producer<Integer> producer = null;
    if (enableBatch) {
      producer =
          pulsarClient
              .newProducer(Schema.INT32)
  ...
  private void testMsgsInPendingAckStateWouldNotGetTheConsumerStuck() throws Exception {
    ...
    @Cleanup
    Producer<Integer> producer = pulsarClient.newProducer(Schema.INT32).topic(topicName).create();
    @Cleanup
    Consumer<Integer> consumer =
        pulsarClient
            .newConsumer(Schema.INT32)
  ...
  private void produceCommitTest(boolean enableBatch) throws Exception {
    ...
    Consumer<byte[]> consumer =
        pulsarClient.newConsumer().topic(TOPIC_OUTPUT).subscriptionName("test").subscribe();
                  ...
                  if (topicFuture != null) {
                    ...
                    if (topicOptional.isPresent()) {
                      ...
                      if (!markDeletePosition.equals(lastConfirmedEntry)) {
                        ...
                        // when delete commit marker operation is processing, next delete operation
                        // will not do again
                        // when delete commit marker operation finish, it can run next delete commit
                        // marker operation
                        // so this test may not delete all the position in this manageLedger.
                        Position markerPosition =
                            ((ManagedLedgerImpl)
                                    persistentSubscription.getCursor().getManagedLedger())
                                .getNextValidPosition(markDeletePosition);
                        // marker is the lastConfirmedEntry, after commit the marker will only be
                        // write in
  ...
  private void testAckWithTransactionReduceUnAckMessageCount(boolean enableBatch) throws Exception {
    ...
    final String topicName =
        NAMESPACE1 + "/testAckWithTransactionReduceUnAckMessageCount-" + enableBatch;
    @Cleanup
    Consumer<byte[]> consumer =
        pulsarClient
            .newConsumer()
    ...
    for (int i = 0; i < getPulsarServiceList().size(); i++) {
      CompletableFuture<Optional<Topic>> topicFuture =
          getPulsarServiceList().get(i).getBrokerService().getTopic(topic, false);
      ...
      if (topicFuture != null) {
        ...
        if (topicOptional.isPresent()) {
          PersistentSubscription persistentSubscription =
              (PersistentSubscription) topicOptional.get().getSubscription(subName);
          assertEquals(
              persistentSubscription.getConsumers().get(0).getUnackedMessages(), messageCount / 2);
          flag = true;
        }
      }
    }
  ...
  protected void txnAckTest(
      boolean batchEnable, int maxBatchSize, SubscriptionType subscriptionType) throws Exception {
  ...
  public void testAfterDeleteTopicOtherTopicCanRecover() throws Exception {
    ...
    Consumer<String> consumer =
        pulsarClient.newConsumer(Schema.STRING).topic(topicTwo).subscriptionName(sub).subscribe();
    String content = "test";
    producer.send(content);
    assertEquals(
        consumer.receive(waitTimeForCanReceiveMsgInSec, TimeUnit.SECONDS).getValue(), content);
    ...
    for (int i = 0; i < getPulsarServiceList().size(); i++) {
      ...
      if (topicFuture != null) {
        ...
        if (topicOptional.isPresent()) {
          ...
          Position lastConfirmedEntry =
              persistentSubscription.getCursor().getManagedLedger().getLastConfirmedEntry();
          exist = true;
          if (!markDeletePosition.equals(lastConfirmedEntry)) {
            // this because of the transaction commit marker have't delete
            // delete commit marker after ack position
            // when delete commit marker operation is processing, next delete operation will not do
            // again
            // when delete commit marker operation finish, it can run next delete commit marker
            // operation
            // so this test may not delete all the position in this manageLedger.
            Position markerPosition =
                ((ManagedLedgerImpl) persistentSubscription.getCursor().getManagedLedger())
                    .getNextValidPosition(markDeletePosition);
  ...
  private void txnCumulativeAckTest(
      boolean batchEnable, int maxBatchSize, SubscriptionType subscriptionType) throws Exception {
    String normalTopic = NAMESPACE1 + "/normal-topic";
    ...
    for (int retryCnt = 0; retryCnt < 2; retryCnt++) {
      ...
      for (int i = 0; i < messageCnt; i++) {
        ...
        log.info(
            "receive msgId abort: {}, retryCount : {}, count : {}",
      ...
      try {
        ...
        Assert.assertTrue(
            e.getCause() instanceof PulsarClientException.TransactionConflictException);
      }
            ...
            .acknowledgeCumulativeAsync(
                DefaultImplementation.getDefaultImplementation()
                    .newMessageId(
                        ...
                        ((MessageIdImpl) message.getMessageId()).getEntryId() - 1,
                        -1),
                abortTxn)
            .get();
        fail("not ack conflict ");
  ...
  public Transaction getTxn() throws Exception {
    return pulsarClient.newTransaction().withTransactionTimeout(10, TimeUnit.MINUTES).build().get();
  }
  ...
  public void txnMetadataHandlerRecoverTest() throws Exception {
    ...
    @Cleanup
    PulsarClientImpl recoverPulsarClient =
        (PulsarClientImpl)
  ...
  public void produceTxnMessageOrderTest() throws Exception {
    ...
    for (int ti = 0; ti < 10; ti++) {
      Transaction txn =
          pulsarClient.newTransaction().withTransactionTimeout(2, TimeUnit.SECONDS).build().get();
  ...
  public void produceAndConsumeCloseStateTxnTest() throws Exception {
    ...
    try {
      ...
      assertTrue(
          e.getCause() instanceof TransactionCoordinatorClientException.InvalidTxnStatusException);
    }
        ...
        .forEach(
            pulsarService -> {
              if (pulsarService
                  ...
                  .getStores()
                  .containsKey(
                      TransactionCoordinatorID.get(
                          ((TransactionImpl) timeoutTxn).getTxnIdMostBits()))) {
                transactionMetadataStore.set(
                    pulsarService
                        .getTransactionMetadataStoreService()
                        .getStores()
                        .get(
              ...
              try {
                transactionMetadataStore
                    ...
                    .getTxnMeta(
                        new TxnID(
                            ((TransactionImpl) timeoutTxn).getTxnIdMostBits(),
                            ((TransactionImpl) timeoutTxn).getTxnIdLeastBits()))
                    .get();
                return false;
              } catch (Exception e) {
                return true;
    ...
    Constructor<TransactionImpl> constructor =
        transactionClass.getDeclaredConstructor(
            PulsarClientImpl.class, long.class, long.class, long.class);
  ...
  public void testTxnTimeoutAtTransactionMetadataStore() throws Exception {
    ...
    long timeoutCountOriginal =
        transactionMetadataStores.stream()
            .mapToLong(store -> store.getMetadataStoreStats().timeoutCount)
            .sum();
    TxnID txnID =
        pulsarServiceList
            .get(0)
            .getTransactionMetadataStoreService()
            .newTransaction(new TransactionCoordinatorID(0), 1, null)
            .get();
    Awaitility.await()
        .until(
            () -> {
  ...
  public void transactionTimeoutTest() throws Exception {
    ...
    Transaction consumeTimeoutTxn =
        pulsarClient.newTransaction().withTransactionTimeout(7, TimeUnit.SECONDS).build().get();
  ...
  public void txnTransactionRedeliverNullDispatcher(CommandAck.AckType ackType) throws Exception {
    ...
    if (ackType == CommandAck.AckType.Individual) {
      consumer.acknowledgeAsync(
          consumer.receive(waitTimeForCanReceiveMsgInSec, TimeUnit.SECONDS).getMessageId(), txn);
    } else {
  ...
  public void oneTransactionOneTopicWithMultiSubTest() throws Exception {
    ...
    for (int i = 0; i < getPulsarServiceList().size(); i++) {
      ...
      if (transactionMetadataStoreService
          .getStores()
          .containsKey(TransactionCoordinatorID.get(txn.getTxnIdMostBits()))) {
        List<TransactionSubscription> list =
  ...
  public void testTxnTimeOutInClient() throws Exception {
    ...
    Producer<String> producer =
        pulsarClient
            .newProducer(Schema.STRING)
            .producerName("testTxnTimeOut_producer")
            .topic(topic)
            .sendTimeout(0, TimeUnit.SECONDS)
            .enableBatching(false)
            .create();
    ...
    Consumer<String> consumer =
        pulsarClient
            ...
            .consumerName("testTxnTimeOut_consumer")
            .topic(topic)
            .subscriptionName("testTxnTimeOut_sub")
            .subscribe();
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              Assert.assertEquals(
                  ((TransactionImpl) transaction).getState(), TransactionImpl.State.TIME_OUT);
            });
  ...
  public void testSendTxnMessageTimeout() throws Exception {
    ...
    ProducerImpl<byte[]> producer =
        (ProducerImpl<byte[]>)
            pulsarClient.newProducer().topic(topic).sendTimeout(1, TimeUnit.SECONDS).create();
    ...
    completableFuture.complete(new ProducerResponse("test", 1, "1".getBytes(), Optional.of(30L)));
  ...
  public void testAckWithTransactionReduceUnackCountNotInPendingAcks() throws Exception {
    final String topic =
        "persistent://" + NAMESPACE1 + "/testAckWithTransactionReduceUnackCountNotInPendingAcks";
    final String subName = "test";
    @Cleanup
    ProducerImpl<byte[]> producer =
        (ProducerImpl<byte[]>)
    ...
    // remove the message from the pendingAcks, in fact redeliver will remove the messageId from the
    // pendingAck
    getPulsarServiceList()
        .get(0)
        .getBrokerService()
  ...
  public void testSendTxnAckMessageToDLQ() throws Exception {
    ...
    assertEquals(
        ...
        new String(
            deadLetterConsumer
                .receive(waitTimeForCanReceiveMsgInSec, TimeUnit.SECONDS)
                .getValue()));
  ...
  public void testSendTxnAckBatchMessageToDLQ() throws Exception {
    ...
    // consumer receive the batch message two the second time, redeliverCount = 1, also can be
    // received
    consumer.acknowledgeAsync(message.getMessageId(), transaction).get();
