...
public class SimpleProduceConsumeIoTest extends ProducerConsumerBase {
  ...
  protected void setup() throws Exception {
    ...
    singleConnectionPerBrokerClient =
        (PulsarClientImpl)
            PulsarClient.builder().connectionsPerBroker(1).serviceUrl(lookupUrl.toString()).build();
  }
  ...
  /**
   * 1. Create a producer with a pooled connection. 2. When executing "producer.connectionOpened",
   * the pooled connection has been closed due to a network issue. 3. Verify: the producer can be
  ...
  public void testUnstableNetWorkWhenCreatingProducer() throws Exception {
    ...
    ProducerImpl p =
        (ProducerImpl) singleConnectionPerBrokerClient.newProducer().topic(topic).create();
    ClientCnx cnx = p.getClientCnx();
    p.close();

    // 1. Create a new producer with the pooled connection(since there is a pooled connection, the
    // new producer
    // will reuse it).
        ...
        .untilAsserted(
            () -> {
              ...
              assertTrue(
                  "Ready"
                      .equals(
                          WhiteboxImpl.getInternalState(p2FutureWrap.get().join(), "state")
                              .toString()));
            });
