...
public class SubscriptionPauseOnAckStatPersistTest extends ProducerConsumerBase {
  ...
  private void cancelPendingRead(String tpName, String cursorName) throws Exception {
    ...
    if (dispatcher instanceof PersistentDispatcherMultipleConsumers) {
      Method cancelPendingRead =
          PersistentDispatcherMultipleConsumers.class.getDeclaredMethod(
              "cancelPendingRead", new Class[] {});
      ...
      Method cancelPendingRead =
          PersistentDispatcherSingleActiveConsumer.class.getDeclaredMethod(
              "cancelPendingRead", new Class[] {});
      cancelPendingRead.setAccessible(true);
      cancelPendingRead.invoke(dispatcher, new Object[] {});
    }
  ...
  private Object[][] skipTypes() {
    return new Object[][] {
      {SkipType.SKIP_ENTRIES}, {SkipType.CLEAR_BACKLOG}, {SkipType.SEEK}, {SkipType.RESET_CURSOR}
    };
  }
  ...
  private ReceivedMessages<String> ackOddMessagesOnly(Consumer<String>... consumers)
      throws Exception {
  ...
  private void enableDispatcherPauseOnAckStatePersistentAndCreateTopic(
      String tpName, TypeOfUpdateTopicConfig type) throws Exception {
    if (type == TypeOfUpdateTopicConfig.BROKER_CONF) {
      admin
          .brokers()
          .updateDynamicConfiguration("dispatcherPauseOnAckStatePersistentEnabled", "true");
      ...
      admin
          .namespaces()
          .setDispatcherPauseOnAckStatePersistent(TopicName.get(tpName).getNamespace());
    }
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              if (type == TypeOfUpdateTopicConfig.BROKER_CONF) {
                Assert.assertTrue(
                    pulsar.getConfig().isDispatcherPauseOnAckStatePersistentEnabled());
              } else if (type == TypeOfUpdateTopicConfig.TOPIC_LEVEL_POLICY) {
                Assert.assertTrue(
                    policies.getDispatcherPauseOnAckStatePersistentEnabled().getTopicValue());
                Assert.assertTrue(
                    admin
                        .topicPolicies()
                        .getDispatcherPauseOnAckStatePersistent(tpName, false)
                        .join());
              }
            });
  }

  private void disableDispatcherPauseOnAckStatePersistent(
      String tpName, TypeOfUpdateTopicConfig type) throws Exception {
    if (type == TypeOfUpdateTopicConfig.BROKER_CONF) {
      admin
          .brokers()
          .updateDynamicConfiguration("dispatcherPauseOnAckStatePersistentEnabled", "false");
    } else if (type == TypeOfUpdateTopicConfig.TOPIC_LEVEL_POLICY) {
      admin.topicPolicies().removeDispatcherPauseOnAckStatePersistent(tpName).join();
    } else if (type == TypeOfUpdateTopicConfig.NAMESPACE_LEVEL_POLICY) {
      admin
          .namespaces()
          .removeDispatcherPauseOnAckStatePersistent(TopicName.get(tpName).getNamespace());
    }
    Awaitility.await()
        .untilAsserted(
            () -> {
  ...
  public void testBrokerDynamicConfig(TypeOfUpdateTopicConfig type) throws Exception {
    ...
    Consumer<String> c1 =
        pulsarClient
            ...
            .receiverQueueSize(incomingQueueSize)
  ...
  public void testUnPauseOnSkipEntries(SkipType skipType) throws Exception {
    ...
    // Since the message "specifiedMessage1" might be skipped, we send a new message to verify the
    // result.
  ...
  private void skipMessages(String tpName, String subscription, SkipType skipType, Consumer c)
      throws Exception {
    PersistentTopic persistentTopic =
        (PersistentTopic) pulsar.getBrokerService().getTopic(tpName, false).join().get();
  ...
  public void testSingleConsumerDispatcherWillNotPause(SubscriptionType subscriptionType)
      throws Exception {
    final String tpName = BrokerTestUtil.newUniqueName("persistent://public/default/tp");
    final String subscription = "s1";
    final int msgSendCount = MAX_UNACKED_RANGES_TO_PERSIST * 4;
  ...
  @Test(dataProvider = "multiConsumerSubscriptionTypes")
  public void testPauseOnAckStatPersistNotAffectReplayRead(SubscriptionType subscriptionType)
      throws Exception {
    final String tpName = BrokerTestUtil.newUniqueName("persistent://public/default/tp");
    final String subscription = "s1";
    final int msgSendCount = MAX_UNACKED_RANGES_TO_PERSIST * 4;
    ...
    Assert.assertEquals(
        messageCountAckedByC2, msgSendCount - messageCountAckedByC1 + specifiedMessageCount);
  ...
  @Test(dataProvider = "multiConsumerSubscriptionTypes")
  public void testNeverCallCursorIsCursorDataFullyPersistableIfDisabledTheFeature(
      SubscriptionType subscriptionType) throws Exception {
    final String tpName = BrokerTestUtil.newUniqueName("persistent://public/default/tp");
    ...
    final ManagedLedgerImpl ml =
        (ManagedLedgerImpl) pulsar.getDefaultManagedLedgerFactory().open(mlName);
