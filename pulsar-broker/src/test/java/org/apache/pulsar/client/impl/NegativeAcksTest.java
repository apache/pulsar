...
public class NegativeAcksTest extends ProducerConsumerBase {
  ...
  public void testNegativeAcks(
    ...
    Producer<String> producer =
        pulsarClient.newProducer(Schema.STRING).topic(topic).enableBatching(batching).create();
  ...
  public static Object[][] variationsBackoff() {
    return new Object[][] {
      // batching / partitions / subscription-type / min-nack-time-ms/ max-nack-time-ms /
      // ack-timeout
      {false, false, SubscriptionType.Shared, 100, 1000},
      {false, false, SubscriptionType.Failover, 100, 1000},
      {false, true, SubscriptionType.Shared, 100, 1000},
  ...
  public void testNegativeAcksWithBackoff(
    ...
    MultiplierRedeliveryBackoff backoff =
  ...
  /**
   * If we nack multiple messages in the same batch with different redelivery delays, the messages
   * should be redelivered with the correct delay. However, all messages are redelivered at the same
   * time.
   *
   * @throws Exception
   */
  ...
  public void testMultiTopicConsumerConcurrentRedeliverAndReceive() throws Exception {
    ...
    // Make sure the message redelivery is completed. The incoming queue will be cleaned up during
    // the redelivery.
  ...
  /**
   * When negativeAckPrecisionBitCnt is greater than 0, the lower bits of the redelivery time will
   * be truncated to reduce the memory occupation. If set to k, the redelivery time will be bucketed
   * by 2^k ms, resulting in the redelivery time could be earlier(no later) than the expected time
   * no more than 2^k ms.
   *
   * @throws Exception if an error occurs
   */
  @Test(dataProvider = "negativeAckPrecisionBitCnt")
  public void testConfigureNegativeAckPrecisionBitCnt(int negativeAckPrecisionBitCnt)
      throws Exception {
