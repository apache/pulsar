...
public class ProducerSemaphoreTest extends ProducerConsumerBase {
  ...
  public void testProducerSemaphoreAcquireAndRelease()
    ...
    try {
      ...
      Assert.assertEquals(
          producer.getSemaphore().get().availablePermits(), pendingQueueSize - messages);
    ...
    try {
      ...
      Assert.assertEquals(
          producer.getSemaphore().get().availablePermits(), pendingQueueSize - messages / 2);
      Assert.assertFalse(producer.isErrorStat());
    } finally {
      producer.getClientCnx().channel().config().setAutoRead(true);
    }
  ...
  /**
   * We use semaphore to limit the pending send, so we must ensure that the thread of sending
   * message never block at the pending message queue. If not, the dead lock might occur. Here is
   * the related issue to describe the dead lock happens
   * {https://github.com/apache/pulsar/issues/5585}
   */
  @Test(timeOut = 30000)
  public void testEnsureNotBlockOnThePendingQueue() throws Exception {
    ...
    try {
      ...
      try {
        ...
        Assert.assertEquals(
            ee.getCause().getClass(), PulsarClientException.ProducerQueueIsFullError.class);
        Assert.assertEquals(producer.getSemaphore().get().availablePermits(), 0);
        Assert.assertFalse(producer.isErrorStat());
      }
    } finally {
