...
public class PatternConsumerBackPressureMultipleConsumersTest extends MockedPulsarServiceBaseTest {
    ...
    private static class SharedClientResources implements AutoCloseable {
        ...
        public SharedClientResources() {
            this.ioEventLoopGroup = EventLoopUtil.newEventLoopGroup(Runtime.getRuntime().availableProcessors(), false,
        ...
        public PulsarClientImpl.PulsarClientImplBuilder useSharedResources(
                PulsarClientImpl.PulsarClientImplBuilder builder) {
            builder.eventLoopGroup(ioEventLoopGroup).internalExecutorProvider(internalExecutorProvider)
                    .externalExecutorProvider(externalExecutorProvider)
                    .scheduledExecutorProvider(scheduledExecutorProvider).timer(timer)
                    .lookupExecutorProvider(lookupExecutorProvider);
            return builder;
        }
