...
public class TopicsConsumerImplTest extends ProducerConsumerBase {
                    ...
                    index ->
                        consumer
                            ...
                            .exceptionally(
                                ex -> {
                                  log.warn(
                                      "receive index: {}, failed receive message {}",
                                      index,
                                      ex.getMessage());
                                  ex.printStackTrace();
                                  return null;
  ...
  public void testConsumerUnackedRedelivery() throws Exception {
    ...
    // 4. Receiver receives the message, not ack, Unacked Message Tracker size should be
    // totalMessages.
    Message<byte[]> message = consumer.receive();
    while (message != null) {
      assertTrue(message instanceof TopicMessageImpl);
    ...
    // 5. Blocking call, redeliver should kick in, after receive and ack, Unacked Message Tracker
    // size should be 0.
    ...
    multiTopicsConsumer
        ...
        .handle(
            (res, exception) -> {
              assertTrue(exception instanceof PulsarClientException.AlreadyClosedException);
              assertEquals(
                  exception.getMessage(), "Already subscribed to " + topicNameWithNamespace);
              return null;
            })
        .get();
    multiTopicsConsumer
        ...
        pulsarClient
            ...
            .messageListener(
                (c1, msg) -> {
                  ...
                  log.info(
                      "Received message [{}] in the listener, latch: {}",
                      receivedMessage,
                      latch.getCount());
   ...
   *
   * <p>Steps: 1. Create a consumer with 2 topics, and each topic has 2 partitions: xx-partition-0,
   * xx-partition-1. 2. update topics to have 3 partitions. 3. trigger partitionsAutoUpdate. this
   * should be done automatically, this is to save time to manually trigger. 4. produce message to
   * xx-partition-2 again, and verify consumer could receive message.
   */
  @Test(timeOut = 30000)
  ...
  @Test(timeOut = testTimeout)
  public void testConsumerDistributionInFailoverSubscriptionWhenUpdatePartitions()
      throws Exception {
    ...
    for (TopicStats value : stats.getPartitions().values()) {
      for (SubscriptionStats subscriptionStats : value.getSubscriptions().values()) {
        assertTrue(
            subscriptionStats.getActiveConsumerName().equals(consumer_1.getConsumerName())
                || subscriptionStats.getActiveConsumerName().equals(consumer_2.getConsumerName()));
        activeConsumers.putIfAbsent(
            subscriptionStats.getActiveConsumerName(), new AtomicInteger(0));
  ...
  public void testDefaultBacklogTTL() throws Exception {
    ...
    admin
        .clusters()
        .createCluster("use", ClusterData.builder().serviceUrl(brokerUrl.toString()).build());
  ...
  public void testGetLastMessageId() throws Exception {
    ...
    IntStream.range(0, 2)
        .forEach(i -> topics.add(topicName2 + TopicName.PARTITIONED_TOPIC_SUFFIX + i));
    IntStream.range(0, 3)
        .forEach(i -> topics.add(topicName3 + TopicName.PARTITIONED_TOPIC_SUFFIX + i));
    ...
    assertEquals(
        msgIds.stream().map(TopicMessageId::getOwnerTopic).collect(Collectors.toSet()), topics);
  ...
  @Test(timeOut = testTimeout)
  public void testSubscriptionMustCompleteWhenOperationTimeoutOnMultipleTopics()
    ...
    PulsarClient client =
        PulsarClient.builder()
            ...
            .operationTimeout(
                2,
                TimeUnit
                    .MILLISECONDS) // Set this very small so the operation timeout can be triggered
            .build();
  ...
  public void testPartitionsUpdatesForMultipleTopics() throws Exception {
    ...
    final String topicName1 =
        "persistent://public/default/testPartitionsUpdatesForMultipleTopics-1";
  ...
  public void testTopicsDistribution() throws Exception {
    ...
    eventListener
        .getActiveConsumers()
        .forEach((k, v) -> assigned.compute(v, (t, c) -> c == null ? 1 : ++c));
  ...
  public void testSeekToNewerPosition(boolean seekByFunction) throws Exception {
    final var topic1 =
        TopicName.get(newTopicName())
            .toString()
            .replace("my-property", "public")
            .replace("my-ns", "default");
    final var topic2 =
        TopicName.get(newTopicName())
            .toString()
            .replace("my-property", "public")
    ...
    final var consumer3 =
        pulsarClient
            ...
            .messageListener(
                (MessageListener<String>) (__, msg) -> valuesInListener.add(msg.getValue()))
            .ackTimeout(1, TimeUnit.SECONDS)
            .subscriptionInitialPosition(SubscriptionInitialPosition.Earliest)
            .subscribe();
    ...
    } // else: consumer3 has passed messages to the listener before seek, in this case we cannot
      // assume anything
