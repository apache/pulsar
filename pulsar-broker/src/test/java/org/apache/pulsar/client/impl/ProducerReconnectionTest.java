...
public class ProducerReconnectionTest extends ProducerConsumerBase {
  ...
  public void testConcurrencyReconnectAndClose() throws Exception {
    ...
    ProducerBuilderImpl<byte[]> producerBuilder =
            ...
            doAnswer(
                    invocation -> {
                      ...
                      if (reconnectionStartTrigger.get()) {
                        log.info(
                            "[testConcurrencyReconnectAndClose] verified state for reconnection");
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              ...
              assertTrue(
                  state1 == HandlerState.State.Closed || state1 == HandlerState.State.Closing);
            });
