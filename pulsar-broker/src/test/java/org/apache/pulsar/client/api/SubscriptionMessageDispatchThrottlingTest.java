...
public class SubscriptionMessageDispatchThrottlingTest
    extends AbstractMessageDispatchThrottlingTest {
  ...
  private void testMessageNotDuplicated(SubscriptionType subscription) throws Exception {
    ...
    final String topicName =
        BrokerTestUtil.newUniqueName("persistent://" + namespace + "/throttlingAll");
    final String subName = "my-subscriber-name-" + subscription;

    DispatchRate subscriptionDispatchRate =
        DispatchRate.builder()
            .dispatchThrottlingRateInMsg(-1)
            .dispatchThrottlingRateInByte(subRate)
            .ratePeriodInSecond(1)
            .build();
    DispatchRate topicDispatchRate =
        DispatchRate.builder()
            .dispatchThrottlingRateInMsg(-1)
    ...
    Awaitility.await()
        ...
        .untilAsserted(
            () -> {
              ...
              Assert.assertTrue(
                  topicDispatchRateLimiter != null
                      && topicDispatchRateLimiter.getDispatchRateOnByte() > 0);
              Assert.assertTrue(
                  subDispatchRateLimiter != null
                      && subDispatchRateLimiter.getDispatchRateOnByte() > 0);
            });

    Assert.assertEquals(
        admin.namespaces().getSubscriptionDispatchRate(namespace).getDispatchThrottlingRateInByte(),
        subRate);
    Assert.assertEquals(
        admin.namespaces().getDispatchRate(namespace).getDispatchThrottlingRateInByte(), topicRate);
    ...
    Thread.sleep(2000);
    // If this assertion failed, please alert we may have some regression cause message dispatch was
    // duplicated.
  ...
  private void testDispatchRate(
      SubscriptionType subscription, int brokerRate, int topicRate, int subRate, int expectRate)
      throws Exception {
  ...
  private void updateBrokerDispatchThrottlingRateInBytes(long bytes) throws PulsarAdminException {
    admin
        .brokers()
        .updateDynamicConfiguration("dispatchThrottlingRateInByte", Long.toString(bytes));
  ...
  /**
   * Verify whether the broker level rate-limiting is throttle message-dispatching based on
   * byte-rate or not
   *
   * <pre>
   ...
   */
  @Test(dataProvider = "subscriptions", timeOut = 30000)
  public void testBrokerBytesRateLimitingReceiveAllMessagesAfterThrottling(
      SubscriptionType subscription) throws Exception {
    log.info("-- Starting {} test --", methodName);

    final String namespace1 = "my-property/throttling_ns1";
    final String topicName1 =
        BrokerTestUtil.newUniqueName("persistent://" + namespace1 + "/throttlingAll");
    final String namespace2 = "my-property/throttling_ns2";
    final String topicName2 =
        BrokerTestUtil.newUniqueName("persistent://" + namespace2 + "/throttlingAll");
    final String subName = "my-subscriber-name-" + subscription;
    ...
    Awaitility.await()
        .untilAsserted(
            () -> {
              Assert.assertEquals(
                  pulsar.getConfiguration().getDispatchThrottlingRateInByte(), byteRate);
            });
  ...
  public void testClusterRateLimitingConfiguration(SubscriptionType subscription) throws Exception {
    ...
    admin
        ...
        .updateDynamicConfiguration(
            "dispatchThrottlingRatePerSubscriptionInMsg", Integer.toString(messageRate));
    ...
    Assert.assertNotEquals(
        pulsar.getConfiguration().getDispatchThrottlingRatePerSubscriptionInMsg(), initValue);
    ...
    admin
        .brokers()
        .updateDynamicConfiguration(
            "dispatchThrottlingRatePerSubscriptionInMsg", Integer.toString(initValue));
  ...
  public void testClusterPolicyOverrideConfiguration() throws Exception {
    ...
    admin
        .brokers()
        .updateDynamicConfiguration(
            "dispatchThrottlingRatePerSubscriptionInMsg", Integer.toString(clusterMessageRate));
    // sleep incrementally as zk-watch notification is async and may take some time
    for (int i = 0; i < 5; i++) {
      if (pulsar.getConfiguration().getDispatchThrottlingRatePerSubscriptionInMsg() == initValue) {
        Thread.sleep(50 + (i * 10));
    ...
    Consumer<byte[]> consumer1 =
        pulsarClient.newConsumer().topic(topicName1).subscriptionName(subName1).subscribe();
    ...
    Consumer<byte[]> consumer2 =
        pulsarClient.newConsumer().topic(topicName2).subscriptionName(subName2).subscribe();
  ...
  public void testClosingRateLimiter(SubscriptionType subscription) throws Exception {
    ...
    final String namespace = "my-property/throttling_ns";
    final String topicName =
        "persistent://" + namespace + "/closingSubRateLimiter" + subscription.name();
