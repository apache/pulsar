...
public class RetryTopicTest extends ProducerConsumerBase {
   ...
   * to the retry topic. The delay queue feature is only supported in shared and key-shared
   * subscription type. As a result, the subscription type of the retry topic should be shared or
   * key-shared.
   *
   * @throws Exception
   */
        ...
        pulsarClient
            ...
            .deadLetterPolicy(
                DeadLetterPolicy.builder()
                    ...
                    .maxRedeliverCount(Integer.MAX_VALUE)
                    .build())
            .subscribe();
    @Cleanup
    Consumer<GenericRecord> retryTopicConsumer =
        pulsarClient
            .newConsumer(Schema.AUTO_CONSUME())
            .topic(retryTopic)
  ...
  public void testRetryTopicProperties() throws Exception {
    ...
    do {
      ...
      if (message.hasProperty(RetryMessageUtil.SYSTEM_PROPERTY_RECONSUMETIMES)) {
        ...
        retryMessageIds.add(
            message.getProperty(RetryMessageUtil.SYSTEM_PROPERTY_ORIGIN_MESSAGE_ID));
      }
      consumer.reconsumeLater(message, 1, TimeUnit.SECONDS);
      totalReceived++;
    } while (totalReceived < sendMessages * (maxRedeliveryCount + 1));
  ...
  // Issue 9327: do compatibility check in case of the default retry and dead letter topic name
  // changed
  @Test
  ...
  public void testRetryTopicException() throws Exception {
    ...
    Message<byte[]> message = consumer.receive();
    log.info(
        "consumer received message : {} {}", message.getMessageId(), new String(message.getData()));
    try {
      consumer.reconsumeLater(message, 1, TimeUnit.SECONDS);
  ...
  public void testRetryProducerWillCloseByConsumer() throws Exception {
    ...
    final Consumer<String> consumer =
        pulsarClient
            ...
            .enableRetry(true)
            .deadLetterPolicy(
                DeadLetterPolicy.builder().deadLetterTopic(topicDLQ).maxRedeliverCount(2).build())
            .receiverQueueSize(100)
            .subscriptionInitialPosition(SubscriptionInitialPosition.Earliest)
            .subscribe();
    ...
    // Verify: "retryLetterProducer" and "deadLetterProducer" will be closed by "consumer.close()",
    // so these two
  ...
  public void testCloseRetryLetterTopicProducerOnExceptionToPreventProducerLeak() throws Exception {
    ...
    try {
      ...
      assertThat(
              pulsar.getBrokerService().getTopicReference(retryTopic).get().getProducers().size())
          .describedAs(
              "producer count of retry topic %s should be <= 1 so that it doesn't leak producers",
              retryTopic)
    ...
    assertThat(pulsar.getBrokerService().getTopicReference(retryTopic).get().getProducers().size())
        .describedAs("producer count of retry topic %s should be 0 here", retryTopic)
        .isEqualTo(0);
  }
}
