...
public class RetryTopicTest extends ProducerConsumerBase {
    ...
    public void testAutoConsumeSchemaRetryLetter() throws Exception {
        ...
        Consumer<GenericRecord> consumer = pulsarClient
                ...
                .subscribe();
        @Cleanup
        Consumer<GenericRecord> retryTopicConsumer = pulsarClient
                .newConsumer(Schema.AUTO_CONSUME())
                .topic(retryTopic)
        ...
        for (int i = 0; i < sendMessages; i++) {
            if (i % 2 == 0) {
                ...
                foo.field2 = i + "";
                messageIds.add(
                        producer.newMessage(Schema.AVRO(Foo.class)).value(foo).send());
            } else {
                FooV2 foo = new FooV2();
                foo.field1 = i + "";
                foo.field2 = i + "";
                foo.field3 = i + "";
                messageIds.add(
                        producer.newMessage(Schema.AVRO(FooV2.class)).value(foo).send());
            }
        }
        producer.close();
