  ...
  public void testPtUpdateNotSharedMode() throws Throwable {
    ...
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(
                    ((TopicMetadata) field.get(producerImplExclusive)).numPartitions(), 3));
    ...
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(
                    ((TopicMetadata) field.get(producerImplWaitForExclusive)).numPartitions(), 4));
