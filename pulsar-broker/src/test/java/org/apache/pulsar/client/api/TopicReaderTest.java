...
public class TopicReaderTest extends ProducerConsumerBase {
    ...
    public void testMultiReaderOnLatestMessage(boolean startInclusive, int numOfMessages) throws Exception {
        final String topicName =
                "persistent://my-property/my-ns/testMultiReaderOnLatestMessage" + System.currentTimeMillis();
        admin.topics().createPartitionedTopic(topicName, 3);
        final int halfOfMsgs = numOfMessages / 2;
    ...
    public void testDefaultCryptoKeyReader() throws Exception {
        final String topic =
                "persistent://my-property/my-ns/test-reader-default-crypto-key-reader" + System.currentTimeMillis();
        ...
        Reader<byte[]> reader2 = pulsarClient
                ...
                .create();
        Reader<byte[]> reader3 = pulsarClient
                .newReader()
                .topic(topic)
                .startMessageId(MessageId.latest)
                .defaultCryptoKeyReader(privateKeyFileMap)
                .create();
        Reader<byte[]> reader4 = pulsarClient
                .newReader()
                .topic(topic)
                .startMessageId(MessageId.latest)
    ...
    public void testHasMessageAvailable() throws Exception {
        ...
        for (MessageId id : allIds) {
            reader = (ReaderImpl<byte[]>) pulsarClient
                    .newReader()
                    .topic(topicName)
                    .startMessageId(id)
                    .startMessageIdInclusive()
    ...
    public void testHasMessageAvailableOnEmptyTopic() throws Exception {
        ...
        @Cleanup
        Reader<String> r2Inclusive = pulsarClient
                .newReader(Schema.STRING)
                .topic(topic)
                .startMessageId(MessageId.latest)
                .startMessageIdInclusive()
