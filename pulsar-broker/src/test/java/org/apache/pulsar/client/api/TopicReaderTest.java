...
public class TopicReaderTest extends ProducerConsumerBase {
  ...
  public void testSimpleReader() throws Exception {
    Reader<byte[]> reader =
        pulsarClient
            .newReader()
            .topic("persistent://my-property/my-ns/testSimpleReader")
  ...
  public void testMultipleReaders() throws Exception {
    Producer<byte[]> producer =
        pulsarClient
            .newProducer()
            .topic("persistent://my-property/my-ns/testMultipleReaders")
            .create();
    for (int i = 0; i < 10; i++) {
      String message = "my-message-" + i;
      producer.send(message.getBytes());
  ...
  public void testReaderOnLatestMessage(boolean startInclusive, int numOfMessages)
      throws Exception {
    ...
    ReaderBuilder<byte[]> readerBuilder =
        pulsarClient.newReader().topic(topicName).startMessageId(MessageId.latest);
  ...
  @Test(dataProvider = "variationsForResetOnLatestMsg")
  public void testMultiReaderOnLatestMessage(boolean startInclusive, int numOfMessages)
      throws Exception {
  ...
  public void testReaderOnSpecificMessage() throws Exception {
    Producer<byte[]> producer =
        pulsarClient
            .newProducer()
            .topic("persistent://my-property/my-ns/testReaderOnSpecificMessage")
            .create();
    ...
    Reader<byte[]> reader =
        pulsarClient
            .newReader()
            .topic("persistent://my-property/my-ns/testReaderOnSpecificMessage")
            .startMessageId(messageIds.get(4))
            .create();

    // Publish more messages and verify the readers only sees messages starting from the intended
    // message
    Message<byte[]> msg = null;
    Set<String> messageSet = new HashSet<>();
    for (int i = 5; i < 10; i++) {
  ...
  public void testHasMessageAvailable(boolean enableBatch, boolean startInclusive)
      throws Exception {
  ...
  public void testHasMessageAvailable() throws Exception {
    ...
    // For batch-messages with single message, the type of client messageId should be the same as
    // that of broker
    ...
    // For batch-message with multi messages, the type of client messageId should be the same as
    // that of broker
    for (MessageId id : allIds) {
      reader =
          (ReaderImpl<byte[]>)
    ...
    producer = pulsarClient.newProducer().enableBatching(false).topic(topicName).create();
    messageId = (MessageIdImpl) producer.send("non-batch".getBytes());
    assertFalse(messageId instanceof BatchMessageIdImpl);
    assertTrue(messageId instanceof MessageIdImpl);
    reader =
        (ReaderImpl<byte[]>)
            pulsarClient.newReader().topic(topicName).startMessageId(messageId).create();
    MessageId lastMessageId = reader.getConsumer().getLastMessageId();
  ...
  public void testMultiReaderIsAbleToSeekWithTimeOnBeginningOfTopic() throws Exception {
    ...
    for (int i = 0; i < numOfMessage; i++) {
      ...
      String receivedMessage = new String(message.getData());
      Assert.assertTrue(
          messageSetA.add(receivedMessage), "Received duplicate message " + receivedMessage);
    }
    ...
    for (int i = 0; i < numOfMessage; i++) {
      ...
      String receivedMessage = new String(message.getData());
      Assert.assertTrue(
          messageSetB.add(receivedMessage), "Received duplicate message " + receivedMessage);
    }
  ...
  public void testReaderStartMessageIdAtExpectedPos(
    ...
    ReaderBuilder<byte[]> readerBuilder =
        pulsarClient.newReader().topic(topicName).startMessageId(resetPos.get());
    ...
    for (int i = 0; i < numTopic; i++) {
      assertTrue(
          readers
              ...
              .get()
              .getTopic()
              .startsWith(MultiTopicsConsumerImpl.DUMMY_TOPIC_NAME_PREFIX));
      readers.get(i).get().close();
      producers.get(i).close();
    }
  }
