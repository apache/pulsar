...
public class AutoCloseUselessClientConSupports extends MultiBrokerBaseTest {
  ...
  /**
   * To ensure that connection release is triggered in a short time, call manually. Why provide this
   * method: prevents instability on one side
   */
  protected void trigReleaseConnection(PulsarClientImpl pulsarClient)
    ...
    Awaitility.waitAtMost(Duration.ofSeconds(5))
        .until(
            () -> {
              for (CompletableFuture<ClientCnx> clientCnxWrapFuture :
                  pulsarClient.getCnxPool().getConnections()) {
  ...
  /**
   * Create connections to all brokers using the "Unload Bundle" If can't create it in a short time,
   * use direct implements {@link #connectionToEveryBroker} Why provide this method: prevents
   * instability on one side
   */
          ...
          .until(
              () -> {
                ...
                for (PulsarAdmin pulsarAdmin : super.getAllAdmins()) {
                  pulsarAdmin
                      .namespaces()
                      .unloadNamespaceBundle("public/default", "0x00000000_0xffffffff");
                  Thread.sleep(20);
                }
                return false;
              });
  ...
  /**
   * Create connections directly to all brokers Why provide this method: prevents instability on one
   * side
   */
  ...
  /** Ensure producer and consumer works */
  protected void ensureProducerAndConsumerWorks(
      Producer producer_1, Producer producer_2, Consumer consumer)
      throws PulsarClientException, ExecutionException, InterruptedException {
  ...
  protected void ensureTransactionWorks(
      PulsarClientImpl pulsarClient, Producer producer, Consumer consumer)
      throws PulsarClientException, ExecutionException, InterruptedException {
    ...
    Message message = (Message) consumer.receiveAsync().get();
    Assert.assertEquals(
        new String(message.getData(), StandardCharsets.UTF_8), messageContent_before);
    producer
    ...
    Assert.assertEquals(
        new String(message_tx.getData(), StandardCharsets.UTF_8), messageContent_tx);
