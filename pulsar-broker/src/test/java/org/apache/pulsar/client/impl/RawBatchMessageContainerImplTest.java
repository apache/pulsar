...
public class RawBatchMessageContainerImplTest {
  ...
  public void setEncryptionAndCompression(boolean encrypt, boolean compress) {
    ...
    if (encrypt) {
      ...
      encryptKeys =
          Map.of(
              key,
              new EncryptionContext.EncryptionKey(
                  publicKeyInfo.getKey(), publicKeyInfo.getMetadata()));
    } else {
      msgCrypto = null;
  ...
  public void testToByteBufWithCompressionAndEncryption() throws IOException {
    ...
    ByteBuf uncompressed =
        codec.decode(Unpooled.wrappedBuffer(decrypted), metadata.getUncompressedSize());
    SingleMessageMetadata messageMetadata = new SingleMessageMetadata();

    ByteBuf payload1 =
        Commands.deSerializeSingleMessageInBatch(uncompressed, messageMetadata, 0, 2);
    ByteBuf payload2 =
        Commands.deSerializeSingleMessageInBatch(uncompressed, messageMetadata, 1, 2);
