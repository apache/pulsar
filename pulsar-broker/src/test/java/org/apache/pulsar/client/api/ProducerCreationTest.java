...
public class ProducerCreationTest extends ProducerConsumerBase {
  ...
  public void testExactlyOnceWithProducerNameSpecified(TopicDomain domain)
            ...
            pulsarClient
                ...
                .topic(
                    TopicName.get(
                            ...
                            "public",
                            "default",
                            "testGeneratedNameProducerReconnect")
                        .toString())
                .create();
  ...
  @Test(dataProvider = "topicDomainProvider")
  public void testInitialSubscriptionCreation(TopicDomain domain)
      throws PulsarClientException, PulsarAdminException {
    final String initialSubscriptionName = "init-sub";
    final TopicName topic =
        TopicName.get(domain.value(), "public", "default", "testInitialSubscriptionCreation");

    // Should not create initial subscription when the initialSubscriptionName is null or empty
    Producer<byte[]> nullInitSubProducer =
        ((ProducerBuilderImpl<byte[]>) pulsarClient.newProducer())
    ...
    Producer<byte[]> emptyInitSubProducer =
        ((ProducerBuilderImpl<byte[]>) pulsarClient.newProducer())
  ...
  @Test
  public void testCreateInitialSubscriptionOnPartitionedTopic()
      throws PulsarAdminException, PulsarClientException {
    final TopicName topic =
        TopicName.get(
            "persistent", "public", "default", "testCreateInitialSubscriptionOnPartitionedTopic");
    final String initialSubscriptionName = "init-sub";
  ...
  @Test
  public void testCreateInitialSubscriptionWhenExisting()
      throws PulsarClientException, PulsarAdminException {
    final TopicName topic =
        TopicName.get(
            "persistent", "public", "default", "testCreateInitialSubscriptionWhenExisting");
    final String initialSubscriptionName = "init-sub";
    admin.topics().createNonPartitionedTopic(topic.toString());
    admin
        .topics()
        .createSubscription(topic.toString(), initialSubscriptionName, MessageId.earliest);
    Producer<byte[]> producer =
        ((ProducerBuilderImpl<byte[]>) pulsarClient.newProducer())
            .initialSubscriptionName(initialSubscriptionName)
            .topic(topic.toString())
  ...
  public void testCreateProducerWhenTopicTypeMismatch(TopicDomain domain)
    ...
    assertThrows(
        ...
        () -> {
          ...
          Producer<byte[]> ignored =
              pulsarClient
                  .newProducer()
                  .topic(TopicName.get(nonPartitionedTopic).getPartition(2).toString())
                  .create();
        });
    ...
    // Works fine because the lookup can help our to find all the topics.
    {
      ...
      Producer<byte[]> ignored =
          pulsarClient
              .newProducer()
              .topic(TopicName.get(partitionedTopic).getPartitionedTopicName())
              .create();
    }
    ...
    assertThrows(
        ...
        () -> {
          ...
          Producer<byte[]> ignored =
              pulsarClient
                  .newProducer()
                  .topic(TopicName.get(partitionedTopic).getPartition(100).toString())
                  .create();
        });
  }
  ...
  private void testCreateProducerWhenSinglePartitionIsDeleted(
      TopicDomain domain, boolean allowAutoTopicCreation)
      throws PulsarAdminException, PulsarClientException {
