...
public class MessagePayloadProcessorTest extends ProducerConsumerBase {
  ...
  protected void setup() throws Exception {
    ...
    admin
        .tenants()
        .createTenant(
            "public", new TenantInfoImpl(Sets.newHashSet("appid"), Sets.newHashSet("test")));
  ...
  public void testDefaultProcessor(
      int numPartitions, boolean enableBatching, int batchingMaxMessages) throws Exception {
    final String topic =
        "testDefaultProcessor-" + numPartitions + "-" + enableBatching + "-" + batchingMaxMessages;
    ...
    // Each buffer's refCnt is 2 because after retrieving the refCnt, there will be two release for
    // the ByteBuf:
  ...
  public void testCustomProcessor(final int numMessages, final int batchingMaxMessages)
      throws Exception {
    final String topic =
        "persistent://public/default/testCustomProcessor-"
            + numMessages
            + "-"
            + batchingMaxMessages;
    ...
    final String messagePrefix = "msg-";
    final CustomBatchProducer producer =
        new CustomBatchProducer(persistentTopic, batchingMaxMessages);
    for (int i = 0; i < numMessages; i++) {
