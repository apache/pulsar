...
public class MessageChecksumTest extends BrokerTestBase {
   ...
   *
   * <p>This test case verifies that a client is able to send messages to an older broker version
   * (<= v1.14, protocol version <= v5) in a mixed environment of broker versions (<= v1.14 & >=
   * v1.15)
   *
   * <p>This test case makes the assumption that the message checksum is ignored if a tampered
   * message can be read by the consumer in the test.
   *
   * <p>Scenario behind this test case:
   *
   * <p>MixedVersionScenario.CONNECTED_TO_NEW_THEN_OLD_VERSION A Pulsar client produces the message
   * while connected to a broker that supports checksums. While sending the message to the broker is
   * pending, the connection breaks and the client connects to another broker that doesn't support
   * message checksums. In this case, the client should remove the message checksum before resending
   * it to the broker. original PR https://github.com/apache/pulsar/pull/43
   *
   * <p>MixedVersionScenario.CONNECTED_TO_OLD_THEN_NEW_VERSION A Pulsar client produces the message
   * while connected to a broker that doesn't support checksums. While sending the message to the
   * broker is pending, the connection breaks and the client connects to another broker that
   * supports message checksums. In this case, the client should remove the message checksum before
   * resending it to the broker. original PR https://github.com/apache/pulsar/pull/89
   */
  @Test(dataProviderClass = EnumValuesDataProvider.class, dataProvider = "values")
  public void testChecksumCompatibilityInMixedVersionBrokerCluster(
      MixedVersionScenario mixedVersionScenario) throws Exception {
    // GIVEN
    ...
    // And
    // the producer disconnects from the broker and the test client is put in a mode where
    // reconnecting is rejected
    pulsarTestClient.disconnectProducerAndRejectReconnecting(producer);

    // And
    // when the the message is tampered by changing the last byte to '3'. This corrupts the already
    // calculated
    // checksum.
    ((TypedMessageBuilderImpl<byte[]>) messageBuilder)
        .getContent()
        .put(messageBytes.length - 1, (byte) '3');
    ...
    // THEN
    try {
      // sending of tampered message should not fail since the client is expected to remove the
      // checksum from the
  ...
  private void makeClientAssumeThatItsConnectedToBrokerWithoutChecksumSupport() {
    // make the client think that the connected broker is of version which doesn't support checksum
    // validation
    ((PulsarTestClient) pulsarClient)
        .setOverrideRemoteEndpointProtocolVersion(ProtocolVersion.v5.getValue());
  }
  ...
  public void testTamperingMessageIsDetected() throws Exception {
    ...
    OpSendMsg op =
        OpSendMsg.create(
            LatencyHistogram.NOOP, (MessageImpl<byte[]>) msgBuilder.getMessage(), cmd, 1, null);
