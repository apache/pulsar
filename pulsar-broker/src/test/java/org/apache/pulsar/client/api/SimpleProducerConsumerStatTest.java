...
public class SimpleProducerConsumerStatTest extends ProducerConsumerBase {
  ...
  public void testSyncProducerAndConsumer(int batchMessageDelayMs, int ackTimeoutSec)
      throws Exception {
    ...
    ProducerBuilder<byte[]> producerBuilder =
        pulsarClient.newProducer().topic("persistent://my-property/tp1/my-ns/my-topic1");
  ...
  @Test(dataProvider = "batch_with_timeout")
  public void testAsyncProducerAndAsyncAck(int batchMessageDelayMs, int ackTimeoutSec)
      throws Exception {
    log.info("-- Starting {} test --", methodName);
    ConsumerBuilder<byte[]> consumerBuilder =
        pulsarClient
  ...
  @Test(dataProvider = "batch_with_timeout")
  public void testAsyncProducerAndReceiveAsyncAndAsyncAck(
      int batchMessageDelayMs, int ackTimeoutSec) throws Exception {
    log.info("-- Starting {} test --", methodName);
    ConsumerBuilder<byte[]> consumerBuilder =
        pulsarClient
            .newConsumer()
  ...
  public void testMessageListener(int batchMessageDelayMs) throws Exception {
    ...
    ProducerBuilder<byte[]> producerBuilder =
        pulsarClient.newProducer().topic("persistent://my-property/tp1/my-ns/my-topic3");
    if (batchMessageDelayMs != 0) {
      producerBuilder
          .enableBatching(true)
          .batchingMaxPublishDelay(batchMessageDelayMs, TimeUnit.MILLISECONDS)
  ...
  @Test
  public void testBatchMessagesRateOut()
  ...
  public void testProducerPendingQueueSizeStats(boolean batchingEnabled) throws Exception {
    ...
    @Cleanup Producer<byte[]> producer = producerBuilder.enableBatching(batchingEnabled).create();
  ...
  public void testPartitionTopicStats() throws Exception {
    ...
    for (int i = 0; i < numMessages; i++) {
      ...
      log.info("Received message: [{}]", receivedMessage);
      Assert.assertTrue(
          messageSet.add(receivedMessage), "Received duplicate message " + receivedMessage);
    }
  ...
  @Test
  public void testRetryLetterAndDeadLetterStats()
