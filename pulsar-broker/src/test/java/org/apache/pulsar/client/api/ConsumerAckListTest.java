...
public class ConsumerAckListTest extends ProducerConsumerBase {
  ...
  private void ackListMessage(boolean isBatch, boolean isPartitioned, boolean ackReceiptEnabled)
      throws Exception {
  ...
  public void testAckMessageInAnotherTopic() throws Exception {
    ...
    @Cleanup
    final Consumer<String> allTopicsConsumer =
        pulsarClient.newConsumer(Schema.STRING).topic(topics).subscriptionName("sub1").subscribe();
    Consumer<String> partialTopicsConsumer =
        pulsarClient
            .newConsumer(Schema.STRING)
