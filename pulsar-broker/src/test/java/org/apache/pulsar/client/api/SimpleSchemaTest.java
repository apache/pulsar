...
public class SimpleSchemaTest extends ProducerConsumerBase {
  ...
  protected void setup() throws Exception {
    ...
    admin
        ...
        .setSchemaCompatibilityStrategy(
            NAMESPACE_ALWAYS_COMPATIBLE, SchemaCompatibilityStrategy.ALWAYS_COMPATIBLE);
  ...
  public void newProducerWithoutSchemaOnTopicWithSchema() throws Exception {
    ...
    try (Producer<byte[]> p = pulsarClient.newProducer(Schema.BYTES).topic(topic).create()) {
      if (!schemaValidationEnforced) {
        ...
        Assert.fail(
            "Shouldn't be able to connect to a schema'd topic with no schema"
      ...
      if (schemaValidationEnforced) {
        ...
        Assert.fail(
            "Shouldn't throw IncompatibleSchemaException"
    ...
    org.apache.avro.Schema v1SchemaAvroNative =
    ...
    try (Producer<byte[]> p =
        pulsarClient.newProducer(Schema.NATIVE_AVRO(v1SchemaAvroNative)).topic(topic).create()) {
      p.send("junkdata".getBytes(UTF_8));
    } catch (PulsarClientException e) {
      assertTrue(e.getCause() instanceof SchemaSerializationException);
    }
  ...
  public void newProducerForMessageSchemaOnTopicWithMultiVersionSchema() throws Exception {
    ...
    Schema<V1Data> v1Schema = Schema.AVRO(V1Data.class);
    byte[] v1SchemaBytes = v1Schema.getSchemaInfo().getSchema();
    AvroWriter<V1Data> v1Writer =
        new AvroWriter<>(new Parser().parse(new ByteArrayInputStream(v1SchemaBytes)));
    Schema<V2Data> v2Schema = Schema.AVRO(V2Data.class);
    byte[] v2SchemaBytes = v2Schema.getSchemaInfo().getSchema();
    AvroWriter<V2Data> v2Writer =
        new AvroWriter<>(new Parser().parse(new ByteArrayInputStream(v2SchemaBytes)));
    try (Producer<V1Data> ignored = pulsarClient.newProducer(v1Schema).topic(topic).create()) {}
    ...
    try (Producer<byte[]> p =
            pulsarClient.newProducer(Schema.AUTO_PRODUCE_BYTES()).topic(topic).create();
        Consumer<V2Data> c =
            pulsarClient.newConsumer(v2Schema).topic(topic).subscriptionName("sub1").subscribe()) {
      Assert.expectThrows(SchemaSerializationException.class, () -> p.send(contentV1));

      p.newMessage(Schema.AUTO_PRODUCE_BYTES(Schema.AVRO(V1Data.class))).value(contentV1).send();
      p.send(contentV2);
      ...
      try {
        ...
        if (schemaValidationEnforced) {
          Assert.fail(
              "Shouldn't be able to send to a schema'd topic with no schema"
                  + " if SchemaValidationEnabled is enabled");
        }
  ...
  public void testProducerConnectStateWhenRegisteringSchema() throws Exception {
    ...
    Producer producer =
        client.newProducer(Schema.AUTO_PRODUCE_BYTES()).enableBatching(false).topic(topic).create();
  ...
  @Test
  public void newNativeAvroProducerForMessageSchemaOnTopicWithMultiVersionSchema()
      throws Exception {
    String topic = NAMESPACE + "/schema-test";
    Schema<V1Data> v1Schema = Schema.AVRO(V1Data.class);
    byte[] v1SchemaBytes = v1Schema.getSchemaInfo().getSchema();
    ...
    Schema<V2Data> v2Schema = Schema.AVRO(V2Data.class);
    byte[] v2SchemaBytes = v2Schema.getSchemaInfo().getSchema();
    org.apache.avro.Schema v2SchemaAvroNative =
    ...
    try (Producer<byte[]> ignored =
        pulsarClient.newProducer(Schema.NATIVE_AVRO(v1SchemaAvroNative)).topic(topic).create()) {}
    try (Producer<byte[]> p =
        pulsarClient.newProducer(Schema.NATIVE_AVRO(v2SchemaAvroNative)).topic(topic).create()) {
      p.send(contentV2);
    }
    try (Producer<byte[]> p =
            pulsarClient.newProducer(Schema.NATIVE_AVRO(v1SchemaAvroNative)).topic(topic).create();
  ...
  public void newProducerForMessageSchemaOnTopicInitialWithNoSchema() throws Exception {
    ...
    Assert.assertEquals(
        allSchemas, Arrays.asList(v1Schema.getSchemaInfo(), v2Schema.getSchemaInfo()));
  }
  ...
  public void newNativeAvroProducerForMessageSchemaWithBatch() throws Exception {
    ...
    @Cleanup
    Consumer<byte[]> c =
        pulsarClient.newConsumer(Schema.BYTES).topic(topic).subscriptionName("sub1").subscribe();
    @Cleanup
    Producer<byte[]> p =
        pulsarClient
    ...
    try (Producer<byte[]> p = pulsarClient.newProducer().topic(topic).create();
        Consumer<V1Data> c =
            pulsarClient
                ...
                .subscribe()) {
      Assert.fail(
          "Shouldn't be able to consume with a schema from a topic which has no schema set");
    } catch (PulsarClientException e) {
      Assert.assertTrue(e instanceof IncompatibleSchemaException);
    }
  }
  ...
  public void testAutoKeyValueConsume(boolean batching, KeyValueEncodingType keyValueEncodingType)
      throws Exception {
    String topic =
        NAMESPACE + "/schema-test-auto-keyvalue-consume-" + batching + "-" + keyValueEncodingType;
    ...
    try (Consumer<KeyValue<GenericRecord, V1Data>> c3before =
        pulsarClient
            .newConsumer(
      ...
      try (Producer<KeyValue<V1Data, V1Data>> p =
              pulsarClient.newProducer(pojoSchema).topic(topic).enableBatching(batching).create();
          ...
          Consumer<KeyValue<GenericRecord, GenericRecord>> c1 =
              pulsarClient
                  .newConsumer(
                      Schema.KeyValue(
                          Schema.AUTO_CONSUME(), Schema.AUTO_CONSUME(), keyValueEncodingType))
                  .topic(topic)
                  .subscriptionName("sub1")
                  .subscriptionInitialPosition(SubscriptionInitialPosition.Earliest)
                  .subscribe();
          Consumer<KeyValue<V1Data, V1Data>> c2 =
              pulsarClient
                  .newConsumer(
                      Schema.KeyValue(
    ...
    try (Consumer<KeyValue<GenericRecord, V2Data>> c3before =
            pulsarClient
                .newConsumer(
                    Schema.KeyValue(
                        Schema.AUTO_CONSUME(), Schema.AVRO(V2Data.class), keyValueEncodingType))
                .topic(topic)
                .subscriptionName("sub3b")
                .subscribe();
        Producer<KeyValue<V2Data, V2Data>> p =
            pulsarClient.newProducer(pojoSchemaV2).topic(topic).enableBatching(batching).create();
        Consumer<GenericRecord> c0 =
            pulsarClient
                .newConsumer(Schema.AUTO_CONSUME())
                .topic(topic)
        ...
        Consumer<KeyValue<GenericRecord, GenericRecord>> c1 =
            pulsarClient
                ...
                .subscriptionName("sub1")
                .subscribe();
        Consumer<KeyValue<V2Data, V2Data>> c2 =
            pulsarClient
                .newConsumer(
                    Schema.KeyValue(
        ...
        Consumer<KeyValue<V2Data, GenericRecord>> c4 =
            pulsarClient
                .newConsumer(
                    Schema.KeyValue(
                        Schema.AVRO(V2Data.class), Schema.AUTO_CONSUME(), keyValueEncodingType))
                .topic(topic)
                .subscriptionName("sub4")
                .subscribe()) {
  ...
  public void testAutoKeyValueConsumeGenericObject() throws Exception {
    ...
    Schema<KeyValue<V1Data, V1Data>> pojoSchema =
        Schema.KeyValue(
            Schema.AVRO(V1Data.class), Schema.AVRO(V1Data.class), KeyValueEncodingType.SEPARATED);
    ...
    try (Producer<KeyValue<V1Data, V1Data>> p =
      ...
      for (int i = 0; i < numMessages; i++) {
        ...
        assertNotNull(keyValueSchema.getValueSchema());
        assertTrue(
            keyValueSchema.getKeySchema().getSchemaInfo().getSchemaDefinition().contains("V1Data"));
        assertTrue(
            keyValueSchema
                .getValueSchema()
                .getSchemaInfo()
  ...
  public void testGetSchemaByVersion()
    ...
    @Cleanup
    Producer p1 = pulsarClient.newProducer(Schema.AVRO(V1Data.class)).topic(topic).create();

    @Cleanup
    Producer p2 = pulsarClient.newProducer(Schema.AVRO(V2Data.class)).topic(topic).create();
    ...
    Assert.assertTrue(
        httpLookupService
            .getSchema(TopicName.get(topic), ByteBuffer.allocate(8).putLong(0).array())
            .get()
            .isPresent());
    Assert.assertTrue(
        httpLookupService
            .getSchema(TopicName.get(topic), ByteBuffer.allocate(8).putLong(1).array())
            .get()
            .isPresent());
    Assert.assertTrue(
        binaryLookupService
  ...
  public void testAutoKeyValueConsumeGenericObjectNullValues(KeyValueEncodingType encodingType)
      throws Exception {
    String topic =
        NAMESPACE
            + "/schema-test-auto-keyvalue-"
            + encodingType
            + "-null-value-consume-"
            + UUID.randomUUID();
  ...
  public void testConsumeAvroMessagesWithoutSchema() throws Exception {
    ...
    final String topic =
        NAMESPACE + "/test-consume-avro-messages-without-schema-" + UUID.randomUUID();
    final Schema<V1Data> schema = Schema.AVRO(V1Data.class);
    final Consumer<V1Data> consumer =
        pulsarClient
            .newConsumer(schema)
            .topic(topic)
            .subscriptionName("sub")
    ...
    for (int i = 0; i < numMessages; i++) {
      ...
      assertEquals(
          msg.getReaderSchema().orElse(Schema.BYTES).getSchemaInfo(), schema.getSchemaInfo());
      consumer.acknowledge(msg);
    }
