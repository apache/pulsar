...
public class TlsHostVerificationTest extends TlsProducerConsumerBase {
  ...
  protected void customizeMainPulsarTestContextBuilder(PulsarTestContext.Builder builder) {
    builder.configCustomizer(
        config -> {
          ...
          // Note that if you are on a Mac, you'll need to run the following to make loopback work
          // for 127.0.0.2
  ...
  public void testTlsHostVerificationAdminClient() throws Exception {
    ...
    Assert.assertTrue(
        pulsar.getWebServiceAddressTls().startsWith("https://127.0.0.2:"),
        "Test relies on this address");
    @Cleanup
    PulsarAdmin adminClientTls =
        PulsarAdmin.builder()
    ...
    try {
      ...
      // The test was previously able to fail here, but that is not the right way for the test to
      // pass.
      // If you hit this error and are running on OSX, you may need to run "sudo ifconfig lo0 alias
      // 127.0.0.2 up"
