...
public class ProduceWithMessageIdTest extends ProducerConsumerBase {
  ...
  public void testSend() throws Exception {
    long ledgerId = 123;
    long entryId = 456;
    mockBrokerService.setHandleSend(
        (ctx, send, headersAndPayload) -> {
          ...
          ctx.writeAndFlush(
              Commands.newSendReceipt(
                  send.getProducerId(), send.getSequenceId(), 0, ledgerId, entryId));
        });
  ...
  public void sendWithCallBack() throws Exception {
    ...
    Assert.assertEquals(
        opSendMsgStats.getUncompressedSize(), totalUncompressedSize + INITIAL_BATCH_BUFFER_SIZE);
