...
public class LookupRetryTest extends MockedPulsarServiceBaseTest {
  ...
  protected BrokerService customizeNewBrokerService(BrokerService brokerService) {
    brokerService.setPulsarChannelInitializerFactory(
        (_pulsar, opts) ->
            new PulsarChannelInitializer(_pulsar, opts) {
              @Override
  ...
  private static class ErrorByTopicServerCnx extends ServerCnx {
    ...
    ErrorByTopicServerCnx(
        PulsarService pulsar, ConcurrentHashMap<String, Queue<LookupError>> failureMap) {
      super(pulsar);
    ...
    @Override
    protected void handlePartitionMetadataRequest(
      ...
      if (error == LookupError.TOO_MANY) {
        ...
        ctx.writeAndFlush(
            newPartitionMetadataResponse(ServerError.TooManyRequests, "too many", requestId));
    ...
    protected void handleLookup(CommandLookupTopic lookup) {
      ...
      if (error == LookupError.TOO_MANY) {
        ...
        ctx.writeAndFlush(
            newLookupErrorResponse(ServerError.TooManyRequests, "too many", requestId));
      } else if (error == LookupError.TIMEOUT) {
        // do nothing
      } else if (error == null || error == LookupError.OK) {
