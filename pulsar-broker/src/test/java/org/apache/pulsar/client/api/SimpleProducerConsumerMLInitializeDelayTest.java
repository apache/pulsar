...
public class SimpleProducerConsumerMLInitializeDelayTest extends ProducerConsumerBase {
  ...
  public void testConsumerListMatchesConsumerSet() throws Exception {
    ...
    Awaitility.await()
        .atMost(Duration.ofSeconds(loadMLDelayMillis * 3))
        .ignoreExceptions()
        .untilAsserted(
            () -> {
              ...
              log.info(
                  "consumerSet_size: {}, consumerList_size: {}",
                  consumerSet.size(),
                  consumerList.size());
  ...
  public void testConcurrentlyOfPublishAndSwitchLedger() throws Exception {
    ...
    // Inject a delay for switching ledgers, so publishing requests will be push in to the pending
    // queue.
    AtomicInteger delayTimes = new AtomicInteger();
    mockZooKeeper.delay(
        10,
