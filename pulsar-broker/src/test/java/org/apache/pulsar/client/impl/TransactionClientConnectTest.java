...
public class TransactionClientConnectTest extends TransactionTestBase {
  ...
  public void tryCommandReconnect(
      Callable<CompletableFuture<?>> callable1, Callable<CompletableFuture<?>> callable2)
      throws Exception {
    start();
    try {
      callable1.call().get();
  ...
  public void testTransactionAbortToTxnAsyncReconnect() throws Exception {
    ...
    Callable<CompletableFuture<?>> callable1 =
        () -> transactionCoordinatorClient.abortAsync(new TxnID(0, 0));
    Callable<CompletableFuture<?>> callable2 =
        () -> transactionCoordinatorClient.abortAsync(new TxnID(0, 1));
  ...
  public void testTransactionCommitToTxnAsyncReconnect() throws Exception {
    TransactionCoordinatorClientImpl transactionCoordinatorClient =
        ((PulsarClientImpl) pulsarClient).getTcClient();
    Callable<CompletableFuture<?>> callable1 =
        () -> transactionCoordinatorClient.commitAsync(new TxnID(0, 0));
    Callable<CompletableFuture<?>> callable2 =
        () -> transactionCoordinatorClient.commitAsync(new TxnID(0, 1));
    tryCommandReconnect(callable1, callable2);
  }
  ...
  public void testHandlerStateChangeToReady() throws Exception {
    ...
    Assert.assertEquals(
        transactionMetaStoreHandler.getConnectHandleState(), HandlerState.State.Ready);
  ...
  public void fence(TransactionMetadataStoreService transactionMetadataStoreService)
      throws Exception {
  ...
  public void unFence(TransactionMetadataStoreService transactionMetadataStoreService)
      throws Exception {
    Field field = ManagedLedgerImpl.class.getDeclaredField("state");
    field.setAccessible(true);
    field.set(
  ...
  public void waitToReady() throws Exception {
    ...
    Map<TransactionCoordinatorID, TransactionMetadataStore> stores =
        (Map<TransactionCoordinatorID, TransactionMetadataStore>)
            field1.get(transactionMetadataStoreService);
    Awaitility.await()
        .until(
            () -> {
