...
public class BrokerServiceLookupTest extends ProducerConsumerBase implements ITest {
   ...
   *
   * <p>1. Broker1 is a leader 2. Lookup request reaches to Broker2 which redirects to leader
   * (Broker1) with authoritative = false 3. Leader (Broker1) finds out least loaded broker as
   * Broker2 and redirects request to Broker2 with authoritative = true 4. Broker2 receives final
   * request to own a bundle with authoritative = true and client connects to Broker2
   *
   * @throws Exception
   */
  @Test(timeOut = 30_000)
  public void testMultipleBrokerLookup() throws Exception {
    ...
    PulsarTestContext pulsarTestContext2 =
        createAdditionalPulsarTestContext(conf2, builder -> builder.enableOpenTelemetry(true));
    ...
    // Disable collecting topic stats during this test, as it deadlocks on access to map
    ...
    doAnswer(
        ...
        .doCallRealMethod()
        .when(lookupRequestSemaphoreSpy)
        .tryAcquire();
    lookupRequestSemaphoreField.set(
        pulsar.getBrokerService(), new AtomicReference<>(lookupRequestSemaphoreSpy));

    var topicLoadRequestSemaphoreField =
        BrokerService.class.getDeclaredField("topicLoadRequestSemaphore");
    topicLoadRequestSemaphoreField.setAccessible(true);
    var topicLoadRequestSemaphoreSpy =
        spy(pulsar2.getBrokerService().getTopicLoadRequestSemaphore().get());
    ...
    doAnswer(
        ...
        .doCallRealMethod()
        .when(topicLoadRequestSemaphoreSpy)
        .tryAcquire();
    topicLoadRequestSemaphoreField.set(
        pulsar2.getBrokerService(), new AtomicReference<>(topicLoadRequestSemaphoreSpy));
    ...
    @Cleanup var consumer = consumerFuture.get();
    @Cleanup
    var producer =
        pulsarClient.newProducer().topic("persistent://my-property/my-ns/my-topic1").create();
  ...
  /**
   * Usecase: Redirection due to different cluster 1. Broker1 runs on cluster: "use" and Broker2
   * runs on cluster: "use2" 2. Broker1 receives "use2" cluster request => Broker1 reads "/clusters"
   * from global-zookeeper and redirects request to Broker2 which serves "use2" 3. Broker2 receives
   * redirect request and own namespace bundle
   *
   * @throws Exception
   */
  ...
  public void testMultipleBrokerDifferentClusterLookup() throws Exception {
    ...
    @Cleanup PulsarTestContext pulsarTestContext2 = createAdditionalPulsarTestContext(conf2);
    PulsarService pulsar2 = pulsarTestContext2.getPulsarService();
    pulsar.getLoadManager().get().writeLoadReportOnZookeeper();
    pulsar2.getLoadManager().get().writeLoadReportOnZookeeper();
  ...
  /**
   * Create #PartitionedTopic and let it served by multiple brokers which requires a. tcp
   * partitioned-metadata-lookup b. multiple topic-lookup c. partitioned producer-consumer
   *
   * @throws Exception
   */
  @Test
  public void testPartitionTopicLookup() throws Exception {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      log.debug("Received message: [{}]", receivedMessage);
      assertTrue(
          messageSet.add(receivedMessage), "Message " + receivedMessage + " already received");
    }
  ...
  /**
   * 1. Start broker1 and broker2 with tls enable 2. Hit HTTPS lookup url at broker2 which redirects
   * to HTTPS broker1
   *
   * @throws Exception
   */
  ...
  public void testWebserviceServiceTls() throws Exception {
    ...
    // hit broker2 url
    URLConnection con =
        new URL(pulsar2.getWebServiceAddressTls() + lookupResourceUrl).openConnection();
  ...
  public void testSplitUnloadLookupTest() throws Exception {
    ...
    Set<String> serviceUnits1 =
        pulsar.getNamespaceService().getOwnedServiceUnits().stream()
            .map(nb -> nb.toString())
    ...
    for (int i = 0; i < retry; i++) {
      if (pulsar2.getNamespaceService().getBundle(topicName).equals(bundleInBroker2)
          && i != retry - 1) {
        Thread.sleep(200);
      } else {
        break;
    ...
    Consumer<byte[]> consumer2 =
        pulsarClient.newConsumer().topic(topic2).subscriptionName("my-subscriber-name").subscribe();

    NamespaceBundle bundleInBroker1AfterSplit =
        pulsar2.getNamespaceService().getBundle(TopicName.get(topic2));
  ...
  public void testModularLoadManagerSplitBundle() throws Exception {
    ...
    try {
      ...
      Awaitility.await().untilAsserted(() -> pulsar2.getLeaderElectionService().isLeader());

      ModularLoadManagerImpl loadManager =
          (ModularLoadManagerImpl)
              ((ModularLoadManagerWrapper) pulsar2.getLoadManager().get()).getLoadManager();
      ...
      Awaitility.await()
          .untilAsserted(
              () -> {
                assertNotEquals(
                    pulsar2.getNamespaceService().getBundle(topicName), bundleInBroker2);
              });

      // Unload the NamespacePolicies and AntiAffinity check.
      String currentBroker = pulsar.getBrokerId();
      assertTrue(
          loadManager.shouldNamespacePoliciesUnload(
              namespace, "0x00000000_0xffffffff", currentBroker));
      assertTrue(
          loadManager.shouldAntiAffinityNamespaceUnload(
              namespace, "0x00000000_0xffffffff", currentBroker));
      ...
      Awaitility.await()
          .untilAsserted(
              () -> {
                NamespaceBundle bundleInBroker1AfterSplit =
                    pulsar2.getNamespaceService().getBundle(TopicName.get(topic3));
  ...
  public void testMergeLookupRequests() throws Exception {
    ...
    log.info(
        "lookup count before unload: {}, after unload: {}",
        lookupCountBeforeUnload,
        lookupCountAfterUnload);
    assertTrue(
        lookupCountAfterUnload < lookupCountBeforeUnload * 2,
  ...
  private int calculateLookupRequestCount() throws Exception {
    int failures =
        CollectorRegistry.defaultRegistry
            .getSampleValue("pulsar_broker_lookup_failures_total")
            .intValue();
    int answers =
        CollectorRegistry.defaultRegistry
            .getSampleValue("pulsar_broker_lookup_answers_total")
            .intValue();
  ...
  private PartitionedTopicMetadata getPartitionedMetadata(
      AsyncHttpClient httpClient, URL url, String path) throws Exception {
              ...
              .execute(
                  new AsyncCompletionHandler<Response>() {
                    ...
                    public void onThrowable(Throwable t) {
                      log.warn(
                          "[{}] Failed to perform http request: {}", requestUrl, t.getMessage());
              ...
              if (response.getStatusCode() != HttpURLConnection.HTTP_OK) {
                ...
                future.completeExceptionally(
                    new PulsarClientException(
                        "HTTP get request failed: " + response.getStatusText()));
                return;
              }

              PartitionedTopicMetadata data =
  ...
  private AsyncHttpClient getHttpClient(String version) {
    ...
    confBuilder.setKeepAliveStrategy(
        new DefaultKeepAliveStrategy() {
          @Override
                    ...
                    .hasHistogramSatisfying(
                        histogram ->
                            histogram.hasPointsSatisfying(
                                point ->
                                    point.hasAttributes(
                                        NamespaceService.PULSAR_LOOKUP_RESPONSE_FAILURE_ATTRIBUTES),
                                point ->
                                    point.hasAttributes(
                                        NamespaceService
                                            .PULSAR_LOOKUP_RESPONSE_BROKER_ATTRIBUTES))));
  ...
  // TODO: This test is disabled since it's invalid. The test fails for both TestZKServer and
  // MockZooKeeper.
  ...
  public void testLookupConnectionNotCloseIfFailedToAcquireOwnershipOfBundle() throws Exception {
    ...
    final var lock =
        pulsar
            ...
            .acquireLock(ServiceUnitUtils.path(bundle), new NamespaceEphemeralData())
            .join();
    lock.updateValue(null);
    log.info("Updated bundle {} with null", bundle.getBundleRange());

    // wait for the system topic reader to __change_events is closed, otherwise the test will be
    // affected
    Thread.sleep(500);

    final var future = pulsarClientImpl.getLookup().getBroker(TopicName.get(tpName));
    final var cnx =
        pulsarClientImpl.getCnxPool().getConnections().stream()
            .findAny()
