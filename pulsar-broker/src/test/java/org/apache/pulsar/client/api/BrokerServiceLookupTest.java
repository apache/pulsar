...
public class BrokerServiceLookupTest extends ProducerConsumerBase {
    ...
    private InetSocketAddress resolveHost(LookupService lookupService) {
        if(!(lookupService instanceof BinaryProtoLookupService)) {
            throw new IllegalArgumentException("Only support for BinaryProtoLookupService");
        }
    ...
    public void testLookupConnectionNotCloseIfGetUnloadingExOrMetadataEx() throws Exception {
        ...
        assertTrue(lookupService instanceof BinaryProtoLookupService);
        ClientCnx lookupConnection = pulsarClientImpl.getCnxPool().getConnection(resolveHost(lookupService)).join();
