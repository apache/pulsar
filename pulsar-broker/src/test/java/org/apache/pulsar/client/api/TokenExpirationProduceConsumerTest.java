...
public class TokenExpirationProduceConsumerTest extends TlsProducerConsumerBase {
  ...
  protected void setup() throws Exception {
    ...
    admin
        ...
        .createTenant(
            tenant,
            new TenantInfoImpl(
                Sets.newHashSet("appid1", "appid2"), Sets.newHashSet(configClusterName)));
  ...
  public String getExpireToken(String role, Date date) {
    return Jwts.builder().setSubject(role).signWith(SECRET_KEY).setExpiration(date).compact();
  }
  ...
  public void testNonPersistentTopic() throws Exception {
    ...
    @Cleanup PulsarClient pulsarClient = getClient(ADMIN_TOKEN);
  ...
  public void testTokenExpirationProduceConsumer() throws Exception {
    ...
    final Consumer<byte[]> consumer =
        pulsarClient.newConsumer().topic(topic).subscriptionName("test-token").subscribe();
    ...
    Awaitility.await()
        ...
        .untilAsserted(
            () -> {
              assertThrows(
                  PulsarClientException.TimeoutException.class,
                  () -> {
