...
public class AuthorizationProducerConsumerTest extends ProducerConsumerBase {
    ...
    public void testProducerAndConsumerAuthorization() throws Exception {
        ...
        @Cleanup
        PulsarAdmin admin = spy(PulsarAdmin.builder()
        ...
        try {
            producer = pulsarClientInvalidRole
                    .newProducer()
                    .topic("persistent://my-property/my-ns/my-topic")
                    .create();
            Assert.fail("should have failed with authorization error");
    ...
    public void testSubscriberPermission() throws Exception {
        ...
        @Cleanup
        PulsarAdmin superAdmin = spy(PulsarAdmin.builder()
                .serviceHttpUrl(brokerUrl.toString())
                .authentication(adminAuthentication)
                .build());
        ...
        @Cleanup
        PulsarAdmin tenantAdmin = spy(PulsarAdmin.builder()
                .serviceHttpUrl(brokerUrl.toString())
        ...
        @Cleanup
        PulsarAdmin sub1Admin = spy(PulsarAdmin.builder()
                .serviceHttpUrl(brokerUrl.toString())
        ...
        superAdmin
                .tenants()
                .createTenant("my-property", new TenantInfoImpl(Sets.newHashSet(tenantRole), Sets.newHashSet("test")));
        ...
        try {
            ...
            assertTrue(e.getMessage().startsWith("Unauthorized to validateTopicOperation for operation [GET_STATS]"));
        }
        try {
        ...
        tenantAdmin.topics().grantPermission(topicName, subscriptionRole, Collections.singleton(AuthAction.consume));

        replacePulsarClient(
                PulsarClient.builder().serviceUrl(pulsar.getBrokerServiceUrl()).authentication(authentication));
        ...
        try {
            ...
            assertTrue(e.getMessage()
                    .startsWith("Unauthorized to validateNamespaceOperation for operation [UNSUBSCRIBE]"));
        }
        try {
            ...
            assertTrue(
                    e.getMessage().startsWith("Unauthorized to validateNamespaceOperation for operation [GET_TOPICS]"));
        }
        try {
            ...
            assertTrue(e.getMessage()
                    .startsWith("Role sub1-role has not the 'package' permission to do the packages operations"));
        }
        ...
        tenantAdmin
                .namespaces()
                .grantPermissionOnSubscription(namespace, subscriptionName, Collections.singleton(otherPrincipal));
    ...
    public void testClearBacklogPermission() throws Exception {
        ...
        superAdmin
                .topics()
                .grantPermission(topicName, subscriptionRole, Sets.newHashSet(AuthAction.produce, AuthAction.consume));
        replacePulsarClient(
                PulsarClient.builder().serviceUrl(pulsar.getBrokerServiceUrl()).authentication(subAdminAuthentication));

        @Cleanup
        Producer<byte[]> batchProducer = pulsarClient
                .newProducer()
                .topic(topicName)
        ...
        try {
            ...
            assertTrue(e.getMessage()
                    .startsWith("Unauthorized to validateNamespaceOperation for operation [CLEAR_BACKLOG]"));
        }

        superAdmin
                .namespaces()
                .grantPermissionOnNamespace(namespace, subscriptionRole, Sets.newHashSet(AuthAction.consume));
        ...
        superAdmin
                .namespaces()
                .grantPermissionOnNamespace(namespace, subscriptionRole, Sets.newHashSet(AuthAction.produce));
        assertEquals(sub1Admin.topics().getPartitionedTopicList(namespace), Lists.newArrayList(topicName));
    ...
    public void testSchemaCompatibilityStrategyPermission() throws Exception {
        ...
        @Cleanup
        PulsarAdmin generalAdmin = spy(PulsarAdmin.builder()
                .serviceHttpUrl(brokerUrl.toString())
        ...
        superAdmin.topics().grantPermission(topicName, generalRole, Collections.singleton(AuthAction.produce));
        ...
        try {
            ...
            assertTrue(e.getMessage()
                    .startsWith("Unauthorized to validateTopicPolicyOperation " + "for operation [WRITE] on topic ["
                            + topicName + "] on policy [SCHEMA_COMPATIBILITY_STRATEGY]"));
        }
        try {
            ...
            assertTrue(e.getMessage()
                    .startsWith("Unauthorized to validateTopicPolicyOperation " + "for operation [READ] on topic ["
                            + topicName + "] on policy [SCHEMA_COMPATIBILITY_STRATEGY]"));
        }
        try {
    ...
    public void testUpdateTopicPropertiesAuthorization() throws Exception {
        ...
        try {
            ...
            assertTrue(e.getMessage()
                    .startsWith("Unauthorized to validateTopicOperation for operation [UPDATE_METADATA]"));
        }
    ...
    public void testRevokePermission() throws Exception {
        ...
        Assert.assertFalse(authorizationService.canConsume(topicName, role, null, "sub1"));
        authorizationService
                .grantPermissionAsync(topicName, actions, role, "auth-json")
                .get();
        authorizationService
                .grantPermissionAsync(topicName, actions, role2, "auth-json")
                .get();
        Assert.assertTrue(authorizationService.canProduce(topicName, role, null));
        Assert.assertTrue(authorizationService.canConsume(topicName, role, null, "sub1"));
        ...
        authorizationService
                .grantPermissionAsync(namespaceName, actions, role, null)
                .get();
        Assert.assertTrue(authorizationService
        ...
        Assert.assertFalse(authorizationService
                .allowNamespaceOperationAsync(namespaceName, NamespaceOperation.GET_TOPIC, role, null)
                .get());
        log.info("-- Exiting {} test --", methodName);
    }
    ...
    public void testAuthData() throws Exception {
        ...
        Assert.assertEquals(
                TestAuthorizationProviderWithGrantPermission.authenticationData.getCommandData(), "prod-auth");
        Assert.assertTrue(
                authorizationService.canConsume(topicName, role, new AuthenticationDataCommand("cons-auth"), "sub1"));
        Assert.assertEquals(
                TestAuthorizationProviderWithGrantPermission.authenticationData.getCommandData(), "cons-auth");
    ...
    public void testPermissionForProducerCreateInitialSubscription() throws Exception {
        ...
        @Cleanup
        PulsarClient pulsarClientProducerRole = PulsarClient.builder()
                .serviceUrl(lookupUrl)
