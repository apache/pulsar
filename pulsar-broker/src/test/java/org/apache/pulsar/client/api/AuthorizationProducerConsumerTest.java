...
public class AuthorizationProducerConsumerTest extends ProducerConsumerBase {
  ...
  public void testProducerAndConsumerAuthorization() throws Exception {
    ...
    @Cleanup
    PulsarClient pulsarClientInvalidRole =
        PulsarClient.builder()
            .serviceUrl(lookupUrl)
            .operationTimeout(1000, TimeUnit.MILLISECONDS)
    ...
    Producer<byte[]> producer =
        pulsarClient.newProducer().topic("persistent://my-property/my-ns/my-topic").create();
    consumer.close();
    producer.close();
  ...
  public void testSubscriberPermission() throws Exception {
    ...
    PulsarAdmin tenantAdmin =
        spy(
            PulsarAdmin.builder()
                .serviceHttpUrl(brokerUrl.toString())
                .authentication(tenantAdminAuthentication)
                .build());
    ...
    // subscriptionRole doesn't have topic-level authorization, so it will fail to get topic
    // stats-internal info
    try {
      ...
      assertTrue(
          e.getMessage()
              .startsWith("Unauthorized to validateTopicOperation for operation [GET_STATS]"));
    }
    try {
      sub1Admin.topics().getBacklogSizeByMessageId(topicName, MessageId.earliest);
      fail("should have failed with authorization exception");
    } catch (Exception e) {
      assertTrue(e.getMessage().startsWith("Unauthorized to validateTopicOperation for operation"));
    }
    ...
    tenantAdmin
        .topics()
        .grantPermission(topicName, subscriptionRole, Collections.singleton(AuthAction.consume));
    ...
    Consumer<byte[]> consumer2 =
        pulsarClient.newConsumer().topic(topicName).subscriptionName(subscriptionName2).subscribe();
    consumer.close();
    consumer2.close();
    ...
    // now, subscriptionRole have consume authorization on topic, so it will successfully get topic
    // internal stats
    ...
    tenantAdmin
        .topics()
        .expireMessages(topicName, subscriptionName, new MessageIdImpl(-1, -1, -1), true);
    ...
    // subscriptionRole doesn't have namespace-level authorization, so it will fail to unsubscribe
    // namespace
    try {
    ...
    try {
      ...
      assertTrue(
          e.getMessage()
              .startsWith("Unauthorized to validateNamespaceOperation for operation [GET_TOPICS]"));
    }
    try {
    ...
    tenantAdmin
        ...
        .grantPermissionOnNamespace(
            namespace, subscriptionRole, Sets.newHashSet(AuthAction.consume, AuthAction.packages));

    // now, subscriptionRole have consume authorization on namespace, so it will successfully
    // unsubscribe namespace
    sub1Admin
        .namespaces()
        .unsubscribeNamespaceBundle(namespace, "0x00000000_0xffffffff", subscriptionName2);
    ...
    // grant subscription access to specific different role and only that role can access the
    // subscription
    String otherPrincipal = "Principal-1-to-access-sub";
    tenantAdmin
        .namespaces()
        .grantPermissionOnSubscription(
            namespace, subscriptionName, Collections.singleton(otherPrincipal));
    ...
    Assert.assertEquals(
        tenantAdmin.namespaces().getPermissionOnSubscription(namespace), permissionOnSubscription);

    // now, subscriptionRole doesn't have subscription level access so, it will fail to access
    // subscription
    try {
      sub1Admin.topics().resetCursor(topicName, subscriptionName, 10);
    ...
    superAdmin
        .namespaces()
        .grantPermissionOnSubscription(
            namespace, subscriptionName, Sets.newHashSet(otherPrincipal, subscriptionRole));
    TreeMap<String, Set<String>> permissionOnSubscription1 = new TreeMap<>();
    permissionOnSubscription1.put(
        subscriptionName, Sets.newHashSet(otherPrincipal, subscriptionRole));
    Assert.assertEquals(
        tenantAdmin.namespaces().getPermissionOnSubscription(namespace), permissionOnSubscription1);
    ...
    superAdmin
        .namespaces()
        .revokePermissionOnSubscription(namespace, subscriptionName, subscriptionRole);
  ...
  public void testClearBacklogPermission() throws Exception {
    ...
    assertEquals(
        tenantAdmin.topics().getPartitionedTopicList(namespace), Lists.newArrayList(topicName));

    // grant topic consume&produce authorization to the subscriptionRole
    superAdmin
        .topics()
        .grantPermission(
            topicName, subscriptionRole, Sets.newHashSet(AuthAction.produce, AuthAction.consume));
    replacePulsarClient(
        PulsarClient.builder()
            .serviceUrl(pulsar.getBrokerServiceUrl())
            .authentication(subAdminAuthentication));
    ...
    assertEquals(
        sub1Admin
            .topics()
            .getStats(topicName + "-partition-0")
            .getSubscriptions()
    ...
    superAdmin
        .namespaces()
        .grantPermissionOnNamespace(
            namespace, subscriptionRole, Sets.newHashSet(AuthAction.consume));
    // now, subscriptionRole have consume authorization on namespace, so it will successfully clear
    // backlog
    assertEquals(
        sub1Admin.topics().getPartitionedTopicList(namespace), Lists.newArrayList(topicName));
    sub1Admin.namespaces().clearNamespaceBundleBacklog(namespace, "0x00000000_0xffffffff");
    assertEquals(
        sub1Admin
            .topics()
    ...
    superAdmin
        .namespaces()
        .grantPermissionOnNamespace(
            namespace, subscriptionRole, Sets.newHashSet(AuthAction.produce));
    assertEquals(
        sub1Admin.topics().getPartitionedTopicList(namespace), Lists.newArrayList(topicName));
  ...
  public void testSchemaCompatibilityStrategyPermission() throws Exception {
    ...
    superAdmin
        .topics()
        .grantPermission(topicName, generalRole, Collections.singleton(AuthAction.produce));
    replacePulsarClient(
        PulsarClient.builder()
            .serviceUrl(pulsar.getBrokerServiceUrl())
            .authentication(generalAdminAuthentication));

    @Cleanup Producer<byte[]> batchProducer = pulsarClient.newProducer().topic(topicName).create();
    batchProducer.close();

    // generalRole doesn't have permission to access topic policy, so it will fail to write/read
    // topic policy
    try {
      generalAdmin
          .topicPolicies()
          .setSchemaCompatibilityStrategy(topicName, SchemaCompatibilityStrategy.ALWAYS_COMPATIBLE);
      fail("should have failed with authorization exception");
    } catch (Exception e) {
      assertTrue(
          e.getMessage()
              .startsWith(
                  "Unauthorized to validateTopicPolicyOperation "
                      + "for operation [WRITE] on topic ["
                      + topicName
                      + "] on policy [SCHEMA_COMPATIBILITY_STRATEGY]"));
    }
    try {
    ...
    // The superUser or tenantAdministrator can access topic policy, so it can successfully
    // write/read topic policy
    superAdmin
        .topicPolicies()
        .setSchemaCompatibilityStrategy(topicName, SchemaCompatibilityStrategy.BACKWARD_TRANSITIVE);
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(
  ...
  public void testUpdateTopicPropertiesAuthorization() throws Exception {
    ...
    Awaitility.await()
        .untilAsserted(
            () -> assertEquals(superAdmin.topics().getProperties(topicName).get("key1"), "value1"));
    topicProperties.put("key1", "value2");
    tenantAdmin.topics().updateProperties(topicName, topicProperties);
    Awaitility.await()
        .untilAsserted(
            () ->
                assertEquals(tenantAdmin.topics().getProperties(topicName).get("key1"), "value2"));
  ...
  public void testSubscriptionPrefixAuthorization() throws Exception {
    ...
    Consumer<byte[]> consumer =
        pulsarClient
            .newConsumer()
            .topic("persistent://prop-prefix/ns/t1")
            .subscriptionName(clientRole + "-sub1")
            .subscribe();
    consumer.close();
  ...
  public void testRevokePermission() throws Exception {
    ...
    AuthorizationService authorizationService =
        new AuthorizationService(conf, pulsar.getPulsarResources());
    ...
    Assert.assertTrue(
        authorizationService
            .allowNamespaceOperationAsync(namespaceName, NamespaceOperation.GET_TOPIC, role, null)
            .get());
  ...
  public void testAuthData() throws Exception {
    ...
    Assert.assertTrue(
        authorizationService.canProduce(
            topicName, role, new AuthenticationDataCommand("prod-auth")));
    Assert.assertEquals(
        TestAuthorizationProviderWithGrantPermission.authenticationData.getCommandData(),
        "prod-auth");
    Assert.assertTrue(
        authorizationService.canConsume(
            topicName, role, new AuthenticationDataCommand("cons-auth"), "sub1"));
    Assert.assertEquals(
        TestAuthorizationProviderWithGrantPermission.authenticationData.getCommandData(),
        "cons-auth");
  ...
  public void testPermissionForProducerCreateInitialSubscription() throws Exception {
    ...
    admin
        .topics()
        .grantPermission(
            topic, producerRole, Sets.newHashSet(AuthAction.produce, AuthAction.consume));
    ...
    try {
      Producer<byte[]> invalidRoleProducer =
          ((ProducerBuilderImpl<byte[]>) pulsarClientInvalidRole.newProducer())
    ...
    // If the producer doesn't have permission to create the init sub, we should also avoid creating
    // the topic.
    Assert.assertFalse(admin.namespaces().getTopics(tn.getNamespace()).contains(tn.getLocalName()));

    @Cleanup
    PulsarClient pulsarClientProducerRole =
        PulsarClient.builder()
            .serviceUrl(lookupUrl)
            .authentication(authenticationProducerRole)
            .build();

    Producer<byte[]> producer =
        ((ProducerBuilderImpl<byte[]>) pulsarClientProducerRole.newProducer())
            .initialSubscriptionName(initialSubscriptionName)
            .topic(topic)
            .create();
    producer.close();
  ...
  public static class ClientAuthentication implements Authentication {
    ...
    public AuthenticationDataProvider getAuthData() throws PulsarClientException {
      AuthenticationDataProvider provider =
          new AuthenticationDataProvider() {
  ...
  public static class TestAuthenticationProvider implements AuthenticationProvider {
    ...
    public String authenticate(AuthenticationDataSource authData) throws AuthenticationException {
      return authData.getCommandData() != null
          ? authData.getCommandData()
          : authData.getHttpHeader("user");
    }
  }
  ...
  public static class TestAuthorizationProviderWithGrantPermission
    ...
    @Override
    public CompletableFuture<Void> grantPermissionAsync(
        NamespaceName namespace, Set<AuthAction> actions, String role, String authData) {
    ...
    @Override
    public CompletableFuture<Void> grantPermissionAsync(
        TopicName topicname, Set<AuthAction> actions, String role, String authData) {
      this.authDataJson = authData;
      grantRoles.add(role);
      return CompletableFuture.completedFuture(null);
    }
