...
public class MultiTopicsConsumerTest extends ProducerConsumerBase {
  ...
  public void testMultiTopicsConsumerCloses() throws Exception {
        ...
        new PulsarClientImpl(conf) {
          {
            ...
            internalExecutorServiceDelegate =
                mock(
                    ...
                    // the mock delegatesTo an existing instance. A delegate is sufficient for
                    // verifying
                    // method calls on the interface.
                    Mockito.withSettings()
                        .defaultAnswer(AdditionalAnswers.delegatesTo(internalExecutorService)));
          }
    ...
    Producer<byte[]> producer1 =
        client.newProducer().topic(topicNameBase + "1").enableBatching(false).create();
    @Cleanup
    Producer<byte[]> producer2 =
        client.newProducer().topic(topicNameBase + "2").enableBatching(false).create();
    @Cleanup
    Producer<byte[]> producer3 =
        client.newProducer().topic(topicNameBase + "3").enableBatching(false).create();

    Consumer<byte[]> consumer =
        client
            .newConsumer()
            .topics(
                Lists.newArrayList(topicNameBase + "1", topicNameBase + "2", topicNameBase + "3"))
  ...
  public void testShouldMaintainOrderForIndividualTopicInMultiTopicsConsumer()
    ...
    while (receivedCount < numPartitions * numMessages) {
      ...
      Assert.assertEquals(
          message.getValue().longValue(), receivedSequenceCounter.getAndIncrement());
    }
  ...
  public void testAcknowledgeWrongMessageId() throws Exception {
    ...
    // Multi-topics consumers can only acknowledge TopicMessageId, otherwise NotAllowedException
    // will be thrown
  ...
  public void testSeekCustomTopicMessageId(boolean messageIdFromProducer) throws Exception {
    ...
    @Cleanup
    final var consumer =
        pulsarClient.newConsumer(Schema.INT32).topic(topic).subscriptionName("sub").subscribe();
  ...
  public void testSameTopics() throws Exception {
    ...
    try {
      ...
      assertTrue(
          e.getMessage()
              .contains("Subscription topics include duplicate items" + " or invalid names"));
    }
    // cleanup.
