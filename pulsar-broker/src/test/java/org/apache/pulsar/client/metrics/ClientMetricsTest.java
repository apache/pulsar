...
public class ClientMetricsTest extends ProducerConsumerBase {
  ...
  private long getCounterValue(
      Map<String, MetricData> metrics, String name, Attributes expectedAttributes) {
  ...
  private void assertHistoCountValue(
      Map<String, MetricData> metrics,
      String name,
      long expectedCount,
      Attributes expectedAttributes) {
  ...
  private long getHistoCountValue(
      Map<String, MetricData> metrics, String name, Attributes expectedAttributes) {
    MetricData md = metrics.get(name);
    assertNotNull(md, "metric not found: " + name);
  ...
  public void testProducerMetrics() throws Exception {
    ...
    PulsarClient client =
        PulsarClient.builder().serviceUrl(pulsar.getBrokerServiceUrl()).openTelemetry(otel).build();
    ...
    Attributes nsAttrs =
        Attributes.builder()
            ...
            .build();
    Attributes nsAttrsSuccess =
        nsAttrs.toBuilder().put("pulsar.response.status", "success").build();
    ...
    assertHistoCountValue(
        metrics, "pulsar.client.producer.message.send.duration", 5, nsAttrsSuccess);
    assertHistoCountValue(metrics, "pulsar.client.producer.rpc.send.duration", 5, nsAttrsSuccess);
    assertCounterValue(
        metrics, "pulsar.client.producer.message.send.size", "hello".length() * 5, nsAttrs);
  ...
  public void testPublishFailedMetrics() throws Exception {
    ...
    Producer<String> producer =
        client.newProducer(Schema.STRING).topic(topic).sendTimeout(3, TimeUnit.SECONDS).create();
    ...
    Attributes nsAttrs =
        Attributes.builder()
            ...
            .put("pulsar.namespace", "my-property/my-ns")
            .build();
    Attributes nsAttrsFailure = nsAttrs.toBuilder().put("pulsar.response.status", "failed").build();

    assertCounterValue(metrics, "pulsar.client.producer.message.pending.count", 0, nsAttrs);
    assertCounterValue(metrics, "pulsar.client.producer.message.pending.size", 0, nsAttrs);
    assertHistoCountValue(
        metrics, "pulsar.client.producer.message.send.duration", 1, nsAttrsFailure);
  ...
  public void testConsumerMetrics() throws Exception {
    ...
    assertCounterValue(
        metrics, "pulsar.client.consumer.receive_queue.size", "hello".length() * 10, nsAttrs);
    ...
    assertCounterValue(
        metrics, "pulsar.client.consumer.receive_queue.size", "hello".length() * 7, nsAttrs);
    assertCounterValue(metrics, "pulsar.client.consumer.message.received.count", 3, nsAttrs);
    assertCounterValue(
        metrics, "pulsar.client.consumer.message.received.size", "hello".length() * 3, nsAttrs);
    ...
    assertCounterValue(
        metrics, "pulsar.client.consumer.receive_queue.size", "hello".length() * 8, nsAttrs);
