...
public class TableViewTest extends MockedPulsarServiceBaseTest {
  ...
  private static final String ECDSA_PUBLIC_KEY =
  ...
  private Set<String> publishMessages(
      String topic, int keyStartPosition, int count, boolean enableBatch, boolean enableEncryption)
      throws Exception {
  ...
  /**
   * Case1: 1. Slow down the rate of reading messages. 2. Send some messages 3. Call new `refresh`
   * API, it will wait for reading all the messages completed. Case2: 1. No new messages. 2. Call
   * new `refresh` API, it will be completed immediately. Case3: 1. multi-partition topic, p1, p2
   * has new message, p3 has no new messages. 2. Call new `refresh` API, it will be completed after
   * read new messages.
   */
  ...
  public void testRefreshAPI(int partition) throws Exception {
    ...
    @Cleanup TableView<byte[]> tv = pulsarClient.newTableView(Schema.BYTES).topic(topic).create();
    ...
    // This is a sync operation, so sleep in the listen action can slow down the reading rate of
    // messages.
    tv.listen(
        (k, v) -> {
          try {
    ...
    // After message sending completely, the table view will take at least 2 seconds to receive all
    // the messages.
  ...
  /**
   * Case1: 1. Slow down the read of reading messages. 2. Send some messages. 3. Call new `refresh`
   * API. 4. Close the reader of the tableview. 5. The refresh operation will be failed with a
   * `AlreadyClosedException`. Case2: 1. Close the reader of the tableview. 2. Call new `refresh`
   * API. 3. The refresh operation will be fail with a `AlreadyClosedException`.
   */
  @Test
  public void testRefreshTaskCanBeCompletedWhenReaderClosed() throws Exception {
    ...
    @Cleanup TableView<byte[]> tv1 = pulsarClient.newTableView(Schema.BYTES).topic(topic1).create();
    @Cleanup TableView<byte[]> tv2 = pulsarClient.newTableView(Schema.BYTES).topic(topic1).create();
  ...
  public void testTableView() throws Exception {
    ...
    TableView<byte[]> tv =
        pulsarClient
            .newTableViewBuilder(Schema.BYTES)
            .topic(topic)
  ...
  public void testPublishNullValue(String topicDomain) throws Exception {
    ...
    final TableView<String> tv =
        pulsarClient
            .newTableViewBuilder(Schema.STRING)
            .topic(topic)
            .autoUpdatePartitionsInterval(5, TimeUnit.SECONDS)
            .create();
    @Cleanup
  ...
  public void testAck(boolean partitionedTopic) throws Exception {
    ...
    if (partitionedTopic) {
      MultiTopicsReaderImpl<String> reader =
          ((CompletableFuture<MultiTopicsReaderImpl<String>>)
                  FieldUtils.readDeclaredField(tv1, "reader", true))
              .get();
      consumerBase = spy(reader.getMultiTopicsConsumer());
      FieldUtils.writeDeclaredField(reader, "multiTopicsConsumer", consumerBase, true);
    } else {
      ReaderImpl<String> reader =
          ((CompletableFuture<ReaderImpl<String>>)
                  FieldUtils.readDeclaredField(tv1, "reader", true))
              .get();
    ...
    Awaitility.await()
        ...
        .untilAsserted(
            () ->
                verify(consumerBase, times(msgCount))
                    .acknowledgeCumulativeAsync(any(MessageId.class)));
  }
  ...
  public void testTableViewTailMessageReadRetry() throws Exception {
    ...
    var reader =
        ((CompletableFuture<Reader<byte[]>>) FieldUtils.readDeclaredField(tv, "reader", true))
            .join();
    var consumer =
        spy((ConsumerImpl<byte[]>) FieldUtils.readDeclaredField(reader, "consumer", true));
  ...
  public void testBuildTableViewWithMessagesAlwaysAvailable() throws Exception {
    ...
    // Use mock reader to build tableview. In the old implementation, the readAllExistingMessages
    // method
    ...
    when(mockReader.getLastMessageIdsAsync())
        .thenReturn(CompletableFuture.completedFuture(lastMessageIds));
    ...
    when(mockReader.readNextAsync())
        .thenAnswer(
            invocation -> {
              ...
              if (localIndex >= 0) {
                when(message.getTopicName())
                    .thenReturn(lastMessageIds.get(localIndex).getOwnerTopic());
    ...
    @Cleanup
    TableViewImpl<byte[]> tableView =
        (TableViewImpl<byte[]>) pulsarClient.newTableView().topic(topic).createAsync().get();
    TableViewImpl<byte[]> mockTableView = spy(tableView);
    Method readAllExistingMessagesMethod =
        TableViewImpl.class.getDeclaredMethod("readAllExistingMessages", Reader.class);
    readAllExistingMessagesMethod.setAccessible(true);
    CompletableFuture<Reader<?>> future =
        (CompletableFuture<Reader<?>>)
            readAllExistingMessagesMethod.invoke(mockTableView, mockReader);
