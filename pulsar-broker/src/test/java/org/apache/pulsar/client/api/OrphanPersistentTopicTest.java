...
public class OrphanPersistentTopicTest extends ProducerConsumerBase {
  ...
  public void testNoOrphanTopicAfterCreateTimeout() throws Exception {
    ...
    for (int i = 0; i < 5; i++) {
      mockZooKeeper.delay(
          topicLoadTimeoutSeconds * 2 * 1000,
          (op, path) -> {
    ...
    // Load topic.
    CompletableFuture<Consumer<byte[]>> consumer =
        pulsarClient.newConsumer().topic(tpName).subscriptionName("my-sub").subscribeAsync();
  ...
  public void testCloseLedgerThatTopicAfterCreateTimeout() throws Exception {
    ...
    // The first topic load will trigger a timeout. When the topic closes, it will call
  ...
  public void testNoOrphanTopicIfInitFailed() throws Exception {
    ...
    // Load topic.
    Consumer consumer =
        pulsarClient.newConsumer().topic(tpName).subscriptionName("my-sub").subscribe();

    // Make the method `PersitentTopic.initialize` fail.
    Field fieldCompactionServiceFactory =
        PulsarService.class.getDeclaredField("compactionServiceFactory");
    ...
    // We do not know the next time of consumer reconnection, so wait for 2 minutes to avoid flaky.
    // It will be
    // very fast in normal.
    Awaitility.await()
        .ignoreExceptions()
  ...
  public void testTopicLoadAndDeleteAtTheSameTime(boolean injectTimeout) throws Exception {
            ...
            invocation -> {
              ...
              if (paramTp.toString().equalsIgnoreCase(tpName)
                ...
                pulsar
                    ...
                    .delete(TopicName.get(tpName).getPersistenceNamingEncoding());
              }
              return invocation.callRealMethod();
            })
        .when(namespaceService)
    ...
    // Verify: the consumer create failed due to pulsar does not allow to create topic
    // automatically.
    try {
