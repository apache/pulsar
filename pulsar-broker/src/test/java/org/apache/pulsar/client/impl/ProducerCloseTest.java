...
public class ProducerCloseTest extends ProducerConsumerBase {
  ...
  /** Param1: Producer enableBatch or not Param2: Send in async way or not */
  ...
  @Test(timeOut = 10_000)
  public void testProducerCloseFailsPendingBatchWhenPreviousStateNotReadyCallback()
      throws Exception {
    initClient();
    @Cleanup
    ProducerImpl<byte[]> producer =
    ...
    CompletableFuture<MessageId> completableFuture =
        producer.newMessage().value("test-msg".getBytes(StandardCharsets.UTF_8)).sendAsync();
    // By setting the state to Failed, the close method will exit early because the previous state
    // was not Ready.
  ...
  public void brokerCloseTopicTest(boolean enableBatch, boolean isAsyncSend) throws Exception {
    @Cleanup
    PulsarClient longBackOffClient =
    ...
    ProducerImpl<byte[]> producer =
        (ProducerImpl<byte[]>)
            longBackOffClient.newProducer().topic(topic).enableBatching(enableBatch).create();
