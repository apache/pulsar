...
public class ZeroQueueSizeTest extends BrokerTestBase {
  ...
  public void zeroQueueSizePartitionedTopicInCompatibility()
    ...
    try {
      consumer
          ...
          .handle(
              (ok, e) -> {
                if (e == null) {
                  // as zero receiverQueueSize doesn't support batch message, must receive exception
                  // at callback.
                  Assert.fail();
                }
                return null;
  ...
  public void testPauseAndResume() throws Exception {
    ...
    assertTrue(
        latch.get().await(2, TimeUnit.SECONDS), "Timed out waiting for message listener acks");
    Thread.sleep(2000);
  ...
  public void testPauseAndResumeNoReconnection() throws Exception {
    ...
    final Producer<Integer> producer =
        pulsarClient.newProducer(Schema.INT32).topic(topicName).enableBatching(false).create();
