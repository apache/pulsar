...
public class SchemaTypeCompatibilityCheckTest extends MockedPulsarServiceBaseTest {
  ...
  public void structTypeProducerProducerUndefinedCompatible() throws Exception {
    ...
    pulsarClient.newProducer(Schema.JSON(Schemas.PersonOne.class)).topic(topicName).create();
    ...
    assertTrue(
        t.getMessage()
            .contains("Incompatible schema: exists schema type JSON, new schema type AVRO"));
  }
  ...
  public void structTypeProducerConsumerUndefinedCompatible() throws Exception {
    ...
    Throwable t =
        expectThrows(
            PulsarClientException.IncompatibleSchemaException.class, consumerBuilder::subscribe);
    assertTrue(
        t.getMessage()
            .contains("Incompatible schema: exists schema type JSON, new schema type AVRO"));
  }
  ...
  public void primitiveTypeProducerProducerUndefinedCompatible() throws Exception {
    ...
    ProducerBuilder producerBuilder = pulsarClient.newProducer(Schema.STRING).topic(topicName);
    ...
    assertTrue(
        t.getMessage()
            .contains("Incompatible schema: exists schema type INT32, new schema type STRING"));
  }
  ...
  public void testAlwaysCompatible() throws Exception {
    ...
    // bytes[] schema and bytebuffer schema does not upload schema info to the
    // schema registry
