...
public class SimpleCacheTest {
  ...
  public void testExpire() throws InterruptedException {
    ...
    recentAccessedKey.forEach(
        key -> cache.get(key, () -> -1, expiredValues::add)); // access these keys

    Thread.sleep(300L);
    recentAccessedKey.forEach(
        key -> Assert.assertEquals(key + 100, cache.get(key, () -> -1, __ -> {})));
    allKeys.stream()
        .filter(key -> !recentAccessedKey.contains(key))
