...
public class PulsarFunctionTlsTest {
  ...
  void setup() throws Exception {
    ...
    for (int i = 0; i < BROKER_COUNT; i++) {
      ...
      WorkerConfig workerConfig =
          PulsarService.initializeWorkerConfigFromBrokerConfig(config, null);
      ...
      File packagePath =
          new File(PulsarSink.class.getProtectionDomain().getCodeSource().getLocation().getPath())
              .getParentFile();
      List<String> urlPatterns = List.of(packagePath.toURI() + ".*");
      ...
      pulsarServices[i] =
          new PulsarService(config, workerConfig, Optional.of(fnWorkerServices[i]), code -> {});
      pulsarServices[i].start();

      // Sleep until pulsarServices[0] becomes leader, this way we can spy namespace bundle
      // assignment easily.
    ...
    TenantInfo tenantInfo =
        TenantInfo.builder().allowedClusters(Collections.singleton(testCluster)).build();
  ...
  public void testFunctionsCreation() throws Exception {
    ...
    for (int i = 0; i < BROKER_COUNT; i++) {
                ...
                if (leaderService.isLeader()) {
                  ...
                  assertTrue(
                      workerInfo != null
                          && !workerInfo
                              .getWorkerId()
                              .equals(workerService.getWorkerConfig().getWorkerId()));
                }
              });
      pulsarAdmins[i].functions().createFunctionWithUrl(functionConfig, jarFilePathUrl);

      // Function creation is not strongly consistent, so this test can fail with a get that is too
      // eager and
      ...
      pulsarAdmins[i]
          .functions()
          .deleteFunction(config.getTenant(), config.getNamespace(), config.getName());
    }
  }
