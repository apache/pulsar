...
public class PulsarFunctionE2ESecurityTest {
  ...
  void setup(Method method) throws Exception {
    ...
    ClusterData clusterData =
        ClusterData.builder().serviceUrl(brokerWebServiceUrl.toString()).build();
  ...
  private PulsarWorkerService createPulsarFunctionWorker(ServiceConfiguration config) {
    ...
    List<String> urlPatterns =
        List.of(
            getPulsarApiExamplesJar().getParentFile().toURI() + ".*", "http://127\\.0\\.0\\.1:.*");
  ...
  public void testAuthorizationWithAnonymousUser() throws Exception {
    ...
    try (PulsarAdmin admin1 = spy(PulsarAdmin.builder().serviceHttpUrl(brokerServiceUrl).build())) {

      String jarFilePathUrl = getPulsarApiExamplesJar().toURI().toString();

      FunctionConfig functionConfig =
          createFunctionConfig(
              TENANT, NAMESPACE, functionName, sourceTopic, sinkTopic, subscriptionName);

      FunctionConfig functionConfig2 =
          createFunctionConfig(
              TENANT2, NAMESPACE, functionName, sourceTopic, sinkTopic, subscriptionName);
      ...
      superUserAdmin
          .namespaces()
          .grantPermissionOnNamespace(replNamespace, ANONYMOUS_ROLE, actions);
          ...
          retryStrategically(
              (test) -> {
                try {
                  return admin1
                              .functions()
                              .getFunctionStatus(TENANT, NAMESPACE, functionName)
                              .getNumRunning()
                          == 1
      ...
      try (Producer<String> producer =
        ...
        // validate pulsar-sink consumer has consumed all messages and delivered to Pulsar sink but
        // unacked
        // messages
        ...
        try {
          admin1
              .functions()
              .triggerFunction(TENANT2, NAMESPACE, functionName, sourceTopic, "foo", null);
        ...
        admin1
            .functions()
            .triggerFunction(TENANT, NAMESPACE, functionName, sourceTopic, "foo", null);
