...
public class PulsarWorkerAssignmentTest {
  ...
  public void testFunctionAssignments() throws Exception {
    ...
    FunctionConfig functionConfig =
        createFunctionConfig(
            tenant, namespacePortion, functionName, "my.*", sinkTopic, subscriptionName);
  ...
  public void testFunctionAssignmentsWithRestart() throws Exception {
    ...
    final FunctionRuntimeManager runtimeManager2 =
        functionsWorkerService.getFunctionRuntimeManager();
    retryStrategically(
        (test) -> {
          try {
    ...
    for (int i = 0; i < (totalFunctions - totalDeletedFunction); i++) {
      ...
      assertEquals(
          admin.functions().getFunction(tenant, namespacePortion, functionName).getLogTopic(),
          logTopic);
    }
  }
  ...
  protected static FunctionConfig createFunctionConfig(
    ...
    final String sourceTopicPattern =
        String.format("persistent://%s/%s/%s", tenant, namespace, sourceTopic);
