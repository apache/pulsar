...
public class WssClientSideEncryptUtils {
  ...
  public static byte[] calculateEncryptedKeyValue(
      MessageCryptoBc msgCrypto, CryptoKeyReader cryptoKeyReader, String publicKeyName)
  ...
  public static String toJSONAndBase64AndUrlEncode(Object obj)
      throws PulsarClientException.CryptoException {
    try {
      String json = ObjectMapperFactory.getMapper().getObjectMapper().writeValueAsString(obj);
      return urlEncode(base64Encode(json));
    } catch (JsonProcessingException e) {
      throw new PulsarClientException.CryptoException(
          String.format("Serialize object %s failed", obj));
    }
  }
  ...
  private static Cipher loadAndInitCipher(PublicKey pubKey)
      throws PulsarClientException.CryptoException,
  ...
  public static EncryptedPayloadAndParam encryptPayload(
      CryptoKeyReader cryptoKeyReader, MessageCryptoBc msgCrypto, byte[] payload, String keyName)
    ...
    msgCrypto.encrypt(
        Collections.singleton(keyName),
        cryptoKeyReader,
        () -> ignoredMessageMetadata,
        unEncryptedMessagePayload,
        encryptedMessagePayload);
    byte[] res = new byte[encryptedMessagePayload.remaining()];
    encryptedMessagePayload.get(res);
    return new EncryptedPayloadAndParam(
        WssClientSideEncryptUtils.base64Encode(res),
  ...
  public static byte[] decryptMsgPayload(
    ...
    for (Map.Entry<String, EncryptionContext.EncryptionKey> entry : encKeys.entrySet()) {
      EncryptionKeys encryptionKeys =
          messageMetadata
              .addEncryptionKey()
              .setKey(entry.getKey())
              .setValue(entry.getValue().getKeyValue());
  ...
  public static byte[] unCompressionIfNeeded(
      byte[] payloadBytes, EncryptionContext encryptionContext) throws IOException {
    if (encryptionContext.getCompressionType() != null
        && !org.apache.pulsar.client.api.CompressionType.NONE.equals(
            encryptionContext.getCompressionType())) {
  ...
  /**
   * Note: this method does not parse the message in its entirety; it only parses the payload of the
   * message.
   */
  public static List<ConsumerMessage> extractBatchMessagesIfNeeded(
      byte[] payloadBytes, EncryptionContext encryptionContext) throws IOException {
    ...
    if (encryptionContext.getBatchSize().isPresent()) {
      ...
      for (int i = 0; i < batchSize; i++) {
        ...
        ByteBuf singleMsgPayload =
            Commands.deSerializeSingleMessageInBatch(payload, singleMsgMetadata, i, batchSize);
