...
public class ClientSideEncryptionWssConsumer extends WebSocketAdapter implements Closeable {
  ...
  public ClientSideEncryptionWssConsumer(
  ...
  private URI buildConnectURL() throws PulsarClientException.CryptoException {
    ...
    final StringBuilder consumerUri =
        new StringBuilder(protocolAndHostPort)
            ...
            .append(subscriptionType.toString())
            .append("&")
            .append("cryptoFailureAction=CONSUME");
  ...
  public void onWebSocketText(String text) {
    ...
    try {
      ...
      if (msg.messageId == null) {
        log.error(
            "Consumer[{}-{}] Could not extract the response payload: {}",
            topicName,
            subscriptionName,
            text);
        return;
      ...
      byte[] decryptedPayload =
          WssClientSideEncryptUtils.decryptMsgPayload(
              msg.payload, msg.encryptionContext, cryptoKeyReader, msgCrypto);
      // Un-compression if needed.
      byte[] unCompressedPayload =
          WssClientSideEncryptUtils.unCompressionIfNeeded(decryptedPayload, msg.encryptionContext);
