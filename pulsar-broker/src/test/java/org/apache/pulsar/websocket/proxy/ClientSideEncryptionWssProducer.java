...
public class ClientSideEncryptionWssProducer extends WebSocketAdapter implements Closeable {
  ...
  public ClientSideEncryptionWssProducer(
      String webSocketProxyHost,
  ...
  private URI buildConnectURL() throws PulsarClientException.CryptoException {
    ...
    final byte[] keyValue =
        WssClientSideEncryptUtils.calculateEncryptedKeyValue(msgCrypto, cryptoKeyReader, keyName);
    EncryptionKey encryptionKey = new EncryptionKey();
    encryptionKey.setKeyValue(keyValue);
    encryptionKey.setMetadata(
        cryptoKeyReader.getPublicKey(keyName, Collections.emptyMap()).getMetadata());
    ...
    final StringBuilder producerUrL =
        new StringBuilder(protocolAndHostPort)
            .append("/ws/v2/producer/persistent/")
            .append(topicName)
            .append("?")
            .append("encryptionKeys=")
            .append(encryptionKeys);
  ...
  public synchronized MessageIdData sendMessage(ProducerMessage msg) throws Exception {
    ...
    byte[] compressedPayload =
        WssClientSideEncryptUtils.compressionIfNeeded(msg.compressionType, unCompressedPayload);
