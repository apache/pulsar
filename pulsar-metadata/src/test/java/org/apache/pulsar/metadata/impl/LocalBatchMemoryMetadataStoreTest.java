...
public class LocalBatchMemoryMetadataStoreTest {

    @Test
    public void testPrivateInstance() throws Exception {
        @Cleanup
        MetadataStore store1 = MetadataStoreFactory.create("batch-memory:local",
                MetadataStoreConfig.builder().build());

        @Cleanup
        MetadataStore store2 = MetadataStoreFactory.create("batch-memory:local",
                MetadataStoreConfig.builder().build());
    ...
    public void testSharedInstance() throws Exception {
        String url = "batch-memory:" + UUID.randomUUID();
    ...
    @Test
    public void testPrivateInstanceInBatch() throws Exception {
        @Cleanup
        MetadataStore store = MetadataStoreFactory.create("batch-memory:local",
                MetadataStoreConfig.builder().build());
    ...
    @Test
    public void testSharedInstanceInBatch() throws Exception {
        String url = "batch-memory:" + UUID.randomUUID();
        ...
        //store1 put
        {
            ...
            ((AbstractBatchedMetadataStore) store1).batchOperation(ops);
        ...
        // store2 get
        {
            ...
            //store2 get
            ((AbstractBatchedMetadataStore) store2).batchOperation(ops);
    ...
    public void testTransactionFailed() throws Exception {
        ...
        // batch put with bad version exception
        {
            ...
            Class<MetadataStoreException.BadVersionException> clazz = MetadataStoreException.BadVersionException.class;

            AtomicReference<Throwable> exceptionClazz = new AtomicReference<>();

            FutureUtil.waitForAll(ops.stream().map(MetadataOp::getFuture).collect(Collectors.toList()))
                    .exceptionally(e -> {
                        exceptionClazz.set(e.getCause().getCause());
                        return null;
                    });

            Assert.assertEquals(clazz, exceptionClazz.get().getClass());
        }
