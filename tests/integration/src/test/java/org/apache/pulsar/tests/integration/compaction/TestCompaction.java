...
public class TestCompaction extends PulsarTestSuite {
    ...
    public void testPublishCompactAndConsumeCLI(Supplier<String> serviceUrl) throws Exception {
        ...
        try (PulsarClient client =
            ...
            try (Producer<String> producer =
                ...
                producer.newMessage().key("key0").value("content0").send();
                producer.newMessage().key("key0").value("content1").send();
            }
    ...
    public void testPublishCompactAndConsumeRest(Supplier<String> serviceUrl) throws Exception {
        ...
        try (PulsarClient client =
            ...
            pulsarCluster.runAdminCommandOnAnyBroker("topics", "compaction-status", "-w", topic);
    ...
    public void testPublishCompactAndConsumePartitionedTopics(Supplier<String> serviceUrl) throws Exception {
        ...
        try (PulsarClient client =
            ...
            pulsarCluster.runAdminCommandOnAnyBroker("topics", "compact", topic + "-partition-0");
            pulsarCluster.runAdminCommandOnAnyBroker("topics", "compact", topic + "-partition-1");
            ...
            pulsarCluster.runAdminCommandOnAnyBroker("topics", "compaction-status", "-w", topic + "-partition-0");
            pulsarCluster.runAdminCommandOnAnyBroker("topics", "compaction-status", "-w", topic + "-partition-1");
    ...
    private static Map<Integer, String> consumeCompactedTopic(
            PulsarClient client, String topic, String subscription, int numKeys) throws PulsarClientException {
    ...
    private static void waitAndVerifyCompacted(
            PulsarClient client, String topic, String sub, String expectedKey, String expectedValue) throws Exception {
        for (int i = 0; i < 60; i++) {
            try (Consumer<String> consumer = client.newConsumer(Schema.STRING)
                    .topic(topic)
                    .readCompacted(true)
    ...
    public void testPublishWithAutoCompaction(Supplier<String> serviceUrl) throws Exception {
        ...
        pulsarCluster.runAdminCommandOnAnyBroker(
                "namespaces", "set-compaction-threshold", "--threshold", "1", namespace);
        ...
        try (PulsarClient client =
            ...
            try (Producer<String> producer =
                    client.newProducer(Schema.STRING).topic(topic).create()) {
                producer.newMessage().key("key0").value("content2").send();
            }
