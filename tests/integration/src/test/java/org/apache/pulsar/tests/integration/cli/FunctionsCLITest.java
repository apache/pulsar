...
public class FunctionsCLITest extends PulsarFunctionsTestBase {
    ...
    public String uploadFunction() throws Exception {
        String bkPkgPath =
                String.format("%s/%s/%s", "tenant-" + randomName(8), "ns-" + randomName(8), "fn-" + randomName(8));

        UploadDownloadCommandGenerator generator =
                UploadDownloadCommandGenerator.createUploader(PulsarCluster.ADMIN_SCRIPT, bkPkgPath);
        String actualCommand = generator.generateCommand();

        log.info(actualCommand);

        String[] commands = {"sh", "-c", actualCommand};
    ...
    public void testUploadDownload() throws Exception {
        ...
        UploadDownloadCommandGenerator generator =
                UploadDownloadCommandGenerator.createDownloader(localPkgFile, bkPkgPath);
        String actualCommand = generator.generateCommand();
        ...
        String[] diffCommand = {"diff", PulsarCluster.ADMIN_SCRIPT, localPkgFile};
