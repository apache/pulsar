...
public class PulsarIOSinkRunner extends PulsarIOTestRunner {
    ...
    public <T extends GenericContainer> void runSinkTester(SinkTester<T> tester, boolean builtin) throws Exception {
        ...
        final String inputTopicName = "test-sink-connector-" + tester.getSinkType() + "-" + functionRuntimeType
                + "-input-topic-" + PulsarTestBase.randomName(8);
        final String sinkName =
                "test-sink-connector-" + tester.getSinkType().name().toLowerCase() + "-" + functionRuntimeType
                        + "-name-" + PulsarTestBase.randomName(8);
        ...
        ensureSubscriptionCreated(
                inputTopicName, String.format("public/default/%s", sinkName), tester.getInputTopicSchema());
        ...
        try {
            ...
            if (tester instanceof JdbcPostgresSinkTester && !((JdbcPostgresSinkTester) tester).isKeyValueSchema()) {
                kvs = produceSchemaInsertMessagesToInputTopic(
                        inputTopicName, numMessages, AvroSchema.of(JdbcPostgresSinkTester.Foo.class));
                // wait for sink to process messages
                Failsafe.with(statusRetryPolicy)
                        .run(() -> waitForProcessingSinkMessages(tenant, namespace, sinkName, numMessages));

                // validate the sink result
                tester.validateSinkResult(kvs);

                kvs = produceSchemaUpdateMessagesToInputTopic(
                        inputTopicName, numMessages, AvroSchema.of(JdbcPostgresSinkTester.Foo.class));

                // wait for sink to process messages
                Failsafe.with(statusRetryPolicy)
                        .run(() -> waitForProcessingSinkMessages(tenant, namespace, sinkName, numMessages + 20));

                // validate the sink result
                tester.validateSinkResult(kvs);

                kvs = produceSchemaDeleteMessagesToInputTopic(
                        inputTopicName, numMessages, AvroSchema.of(JdbcPostgresSinkTester.Foo.class));

                // wait for sink to process messages
                Failsafe.with(statusRetryPolicy)
                        .run(() -> waitForProcessingSinkMessages(tenant, namespace, sinkName, numMessages + 20 + 20));
    ...
    protected void submitSinkConnector(
            @SuppressWarnings("rawtypes") SinkTester tester,
            String tenant,
            String namespace,
            String sinkName,
        ...
        if (tester.getSinkType() != SinkTester.SinkType.UNDEFINED) {
            commands = new String[] {
                ...
                "--sinkConfig",
                new Gson().toJson(tester.sinkConfig()),
                "--inputs",
                inputTopicName,
                "--ram",
    ...
    protected void updateSinkConnector(
            @SuppressWarnings("rawtypes") SinkTester tester,
            String tenant,
            String namespace,
            String sinkName,
    ...
    protected void getSinkInfoSuccess(
            @SuppressWarnings("rawtypes") SinkTester tester,
            String tenant,
            String namespace,
            String sinkName,
            ...
            throws Exception {
        String[] commands = {
            PulsarCluster.ADMIN_SCRIPT, "sink", "get", "--tenant", tenant, "--namespace", namespace, "--name", sinkName
        };
        ContainerExecResult result = pulsarCluster.getAnyWorker().execCmd(commands);
    ...
    protected void deleteSink(String tenant, String namespace, String sinkName) throws Exception {
        ...
        ContainerExecResult result = pulsarCluster.getAnyWorker().execCmd(commands);
        assertTrue(result.getStdout().contains("Deleted successfully"), result.getStdout());
    ...
    protected void waitForProcessingSinkMessages(String tenant, String namespace, String sinkName, int numMessages)
            throws Exception {
        final String[] commands = {
            PulsarCluster.ADMIN_SCRIPT,
            "sink",
    ...
    protected Map<String, String> produceSchemaInsertMessagesToInputTopic(
            String inputTopicName, int numMessages, Schema<Foo> schema) throws Exception {
        @Cleanup
        ...
        Producer<Foo> producer =
                client.newProducer(schema).topic(inputTopicName).create();
        ...
        for (int i = 0; i < numMessages; i++) {
            ...
            producer.newMessage().properties(properties).key(key).value(obj).send();
        }
        return kvs;
    }

    // This for JdbcPostgresSinkTester
    protected Map<String, String> produceSchemaUpdateMessagesToInputTopic(
            String inputTopicName, int numMessages, Schema<Foo> schema) throws Exception {
        @Cleanup
        PulsarClient client = PulsarClient.builder()
                .serviceUrl(pulsarCluster.getPlainTextServiceUrl())
