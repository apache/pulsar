...
public class ManagedCursorBatchAckRecoveryTest extends MockedBookKeeperTestCase {
    ...
    public void testPartialBatchAckPersistenceRetainsConfiguredCount(int batchCount, boolean metadataStore)
        ...
        if (metadataStore) {
            ...
            assertThat(persisted.getBatchedEntryDeletionIndexInfoCount())
            ...
            for (int i = 0; i < expectedPersistedCount; i++) {
                BatchedEntryDeletionIndexInfo indexInfo = persisted.getBatchedEntryDeletionIndexInfo(i);
    ...
    public void testMetadataStoreRecoveryPreservesAllCursorState() throws Exception {
        ...
        assertThat(persisted.getBatchedEntryDeletionIndexInfoCount()).isEqualTo(1);
        assertThat(deleteSet(persisted.getBatchedEntryDeletionIndexInfo(0))).containsExactly(ackSet);
    ...
    private static long[] deleteSet(BatchedEntryDeletionIndexInfo indexInfo) {
        long[] result = new long[indexInfo.getDeleteSetCount()];
        for (int i = 0; i < result.length; i++) {
            result[i] = indexInfo.getDeleteSet(i);
        }
        return result;
    }
