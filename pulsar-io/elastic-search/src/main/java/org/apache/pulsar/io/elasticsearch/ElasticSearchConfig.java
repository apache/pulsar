...
public class ElasticSearchConfig implements Serializable {
    ...
    @FieldDoc(
            ...
            defaultValue = "1000",
            help =
                    "The bulk flush interval flushing any bulk request pending if the interval passes. -1 or zero means the scheduled flushing is disabled.")
    private long bulkFlushIntervalInMs = 1000L;

    // connection settings, see
    // https://www.elastic.co/guide/en/elasticsearch/client/java-rest/current/java-rest-low-config.html
    ...
    @FieldDoc(
            defaultValue = "true",
            help =
                    "If stripNonPrintableCharacters is true, all non-printable characters will be removed from the document.")
    ...
    @FieldDoc(
            defaultValue = "false",
            help =
                    "When the message key schema is AVRO or JSON, copy the message key fields into the Elasticsearch _source.")
    ...
    public void validate() {
        ...
        if ((basicAuthSet && tokenAuthSet && apiKeySet)
            ...
            throw new IllegalArgumentException(
                    "Only one between basic/token/apiKey authentication mode must be configured.");
        }
