 ...
 * under the License.
 */
package org.apache.pulsar.io.ignite.sink;
...
import org.apache.pulsar.io.ignite.igniteAbstractConfig;
...
public class igniteSinkConfig extends igniteAbstractConfig implements Serializable {
    ...
    @FieldDoc(
        required = false,
        defaultValue = "1000L",
        help = "The ignite operation time in milliseconds")
    private long batchTimeMs = 1000L;

    @FieldDoc(
        required = false,
        defaultValue = "200",
        help = "The batch size of write to ignite database"
    )
    private int batchSize = 200;

    public static igniteSinkConfig load(String yamlFile) throws IOException {
        ObjectMapper mapper = new ObjectMapper(new YAMLFactory());
        return mapper.readValue(new File(yamlFile), igniteSinkConfig.class);
    }
