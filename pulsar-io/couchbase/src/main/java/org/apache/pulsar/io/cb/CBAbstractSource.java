...
public abstract class CBAbstractSource<T> extends PushSource<T> {
  ...
  private ConcurrentMap<Short, DcpSnapshotMarkerRequestMeta> snapshots = new ConcurrentHashMap();
  ...
  protected void process() {
    client.dataEventHandler(new DataEventHandler() {
      ...
      public void onEvent(ChannelFlowController flowController, ByteBuf event) {
        ...
        if (cbSourceConfig.getUseSnapshots()) {
          if (DcpMutationMessage.is(event)) {
            message = DcpMutationMessage.contentBytes(event);
            id = DcpMutationMessage.bySeqno(event);
          }
          else if (DcpDeletionMessage.is(event)) {
            message = MessageUtil.getContentAsByteArray(event);
            id = DcpDeletionMessage.bySeqno(event);
          }
        }
        else {
          message = new byte[event.readableBytes()];
          event.readBytes(message);
        }
    ...
    client.controlEventHandler(new ControlEventHandler() {
      ...
      public void onEvent(ChannelFlowController flowController, ByteBuf event) {
        try {
          ...
          if (DcpSnapshotMarkerRequest.is(event) && cbSourceConfig.getUseSnapshots()) {
            DcpSnapshotMarkerRequestMeta snapshot = new DcpSnapshotMarkerRequestMeta(event);

            DcpSnapshotMarkerRequestMeta existingSnapshot = snapshots.put(snapshot.getPartition(), snapshot);

            if (existingSnapshot != null) {
              log.warn("Incomplete snapshot detected: {}", existingSnapshot);
            }
          }

          if (RollbackMessage.is(event)) {
            final short vbucket = RollbackMessage.vbucket(event);
            final long seqNo = RollbackMessage.seqno(event);

            log.warn("Rolling back partition {} to seqno {}", vbucket, seqNo);
            ...
            client.rollbackAndRestartStream(vbucket, seqNo)
                .subscribe(new CompletableSubscriber() {
                  ...
                  public void onCompleted() {
                    log.info("Rollback for vbucket {} complete", vbucket);

                    // TODO: find if exclusive unsubscribe needed ?
                  ...
                  public void onError(Throwable throwable) {
                    log.error("Failed to rollback vbucket {} to seqNo {}", vbucket, seqNo, throwable);
                  }
          ...
          log.error("Exception in ControlEventHandler", throwable);
        }
        finally {
          //TODO: push to queue
          event.release();
