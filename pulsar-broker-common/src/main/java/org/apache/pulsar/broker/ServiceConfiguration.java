...
public class ServiceConfiguration implements PulsarConfiguration {
    ...
    @FieldContext(
            ...
            doc = "Whether we should enable fsync for local metadata store. It's supported by RocksdbMetadataStore "
                    + "for now. If this flag is true, metadata writes will be slower. "
                    + "If this flag is false, and the machine crashes, some recent metadata writes may be lost. "
                    + "Note that if it is just the process that crashes (i.e., the machine does not reboot), "
                    + "no writes will be lost even if it is false."
    )
