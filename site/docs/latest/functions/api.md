...
1. You can use the [`StringSerDe`](https://github.com/apache/incubator-pulsar/blob/master/pulsar-client-cpp/python/functions/serde.py#L70), which leaves the data unchanged. The `StringSerDe` is the **default**. Creating or running a function without explicitly specifying SerDe will mean that this option is used.
