...
public class LookupWithDiscoveryServiceHandler implements LookupProxyHandler {
    ...
    public LookupWithDiscoveryServiceHandler(ProxyService proxy, ProxyConnection proxyConnection) {
        this.service = proxy;
    ...
    public void handlePartitionMetadataResponse(CommandPartitionedTopicMetadata partitionMetadata) {
        ...
        service.getDiscoveryProvider().getPartitionedTopicMetadata(service, dn, proxyConnection.clientAuthRole)
                .thenAccept(metadata -> {
                    ...
                    proxyConnection.ctx()
                            .writeAndFlush(Commands.newPartitionMetadataResponse(metadata.partitions, clientRequestId));
                }).exceptionally(ex -> {
                    log.warn("[{}] Failed to get partitioned metadata for topic {} {}", clientAddress, dn,
                            ex.getMessage(), ex);
                    proxyConnection.ctx().writeAndFlush(Commands
                            .newPartitionMetadataResponse(ServerError.ServiceNotReady, ex.getMessage(), clientRequestId));
                    return null;
                });
    }

    private static final Logger log = LoggerFactory.getLogger(LookupWithDiscoveryServiceHandler.class);
}
