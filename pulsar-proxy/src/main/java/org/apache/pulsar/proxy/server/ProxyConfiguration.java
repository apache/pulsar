...
public class ProxyConfiguration implements PulsarConfiguration {
    ...
    @FieldContext(category = CATEGORY_SERVER, doc = "Metadata store session timeout in milliseconds.")
    private int metadataStoreSessionTimeoutMillis = 30_000;

    @FieldContext(category = CATEGORY_SERVER, doc = "Metadata store cache expiry time in seconds.")
    private int metadataStoreCacheExpirySeconds = 300;

    @FieldContext(category = CATEGORY_SERVER, doc = "Is metadata store read-only operations.")
    ...
    @Deprecated
    @FieldContext(category = CATEGORY_SERVER, deprecated = true, doc = "Is zooKeeper allow read-only operations.")
    ...
    @FieldContext(
            category = CATEGORY_BROKER_PROXY,
            doc = "Allowed broker target host names. " + "Supports multiple comma separated entries and a wildcard.")
    ...
    @FieldContext(category = CATEGORY_BROKER_PROXY, doc = "Allowed broker target ports")
    ...
    @FieldContext(
            category = CATEGORY_KEYSTORE_TLS,
            doc = "TLS KeyStore password for proxy, " + " used by the Pulsar proxy to authenticate with Pulsar brokers")
    @ToString.Exclude
    ...
    @FieldContext(
            category = CATEGORY_KEYSTORE_TLS,
            doc = "TLS TrustStore path for proxy, " + " used by the Pulsar proxy to authenticate with Pulsar brokers")
    ...
    @FieldContext(
            category = CATEGORY_SERVER,
            doc = "Capacity for thread pool queue in the HTTP server" + " Default is set to 8192.")
    private int httpServerThreadPoolQueueSize = 8192;

    @FieldContext(
            category = CATEGORY_SERVER,
            doc = "Capacity for accept queue in the HTTP server" + " Default is set to 8192.")
    private int httpServerAcceptQueueSize = 8192;

    @FieldContext(
            category = CATEGORY_SERVER,
            doc = "Maximum number of inbound http connections. " + "(0 to disable limiting)")
    ...
    @FieldContext(
            category = CATEGORY_SERVER,
            doc = "Number of threads used for Netty Acceptor." + " Default is set to `1`")
    ...
    public void setProperties(Properties properties) {
        ...
        groups.entrySet().forEach((e) -> {
            Map<String, String> keyToFullKey =
                    e.getValue().stream().collect(Collectors.toMap(m -> m.group(2), m -> m.group(0)));
            ...
            httpReverseProxyConfigs.add(new HttpReverseProxyConfig(
                    e.getKey(),
