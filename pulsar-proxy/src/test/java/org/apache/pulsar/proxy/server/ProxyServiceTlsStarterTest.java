...
public class ProxyServiceTlsStarterTest extends MockedPulsarServiceBaseTest {
    ...
    public void testProduceAndConsumeMessageWithWebsocket() throws Exception {
        ...
        producerSession.get().getRemote().sendString(ObjectMapperFactory.getInstance().writeValueAsString(produceRequest));
        assertTrue(consumerSocket.getResponse().contains("ping"));
        ProducerMessage message = ObjectMapperFactory.getInstance().readValue(consumerSocket.getResponse(), ProducerMessage.class);
