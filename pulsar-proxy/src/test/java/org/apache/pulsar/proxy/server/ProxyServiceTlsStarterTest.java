...
public class ProxyServiceTlsStarterTest extends MockedPulsarServiceBaseTest {
    ...
    public void testProduceAndConsumeMessageWithWebsocket() throws Exception {
        ...
        producerSession.get().getRemote().sendString(ObjectMapperFactory.getMapper().getWriter().writeValueAsString(produceRequest));
        assertTrue(consumerSocket.getResponse().contains("ping"));
        ProducerMessage message = ObjectMapperFactory.getMapper().getReader().readValue(consumerSocket.getResponse(), ProducerMessage.class);
