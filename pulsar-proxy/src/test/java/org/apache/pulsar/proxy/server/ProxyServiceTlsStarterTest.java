...
public class ProxyServiceTlsStarterTest extends MockedPulsarServiceBaseTest {
    ...
    public void testProduceAndConsumeMessageWithWebsocket() throws Exception {
        ...
        producerSession.get().getRemote().sendString(ObjectMapperFactory.getObjectMapper().writeValueAsString(produceRequest));
        assertTrue(consumerSocket.getResponse().contains("ping"));
        ProducerMessage message = ObjectMapperFactory.getObjectMapper().readValue(consumerSocket.getResponse(), ProducerMessage.class);
