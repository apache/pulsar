...
function ci_dependency_check() {
  _ci_gradle dependencyCheckAnalyze "$@"
}
...
function ci_move_test_reports() {
  (
    ...
    # Gradle stores test results in build/test-results/test/TEST-*.xml
    if [ -d test-reports ]; then
      ...
      # also check for integration test results
      find . -path '*/build/test-results/integrationTest/TEST-*.xml' -print0 | xargs -0 -r -n 1 mv -t test-reports --backup=numbered
    ...
    # aggregate all test-results directories into surefire-reports (keeping the name for CI compatibility)
...
ci_snapshot_pulsar_maven_artifacts() {
  ...
  mkdir -p build
  # For Gradle, find all built Pulsar jars in the Gradle cache and subproject build dirs
  {
    find $HOME/.gradle/caches/modules-*/files-*/org.apache.pulsar -name "*.jar" 2>/dev/null || true
    find . -path '*/build/libs/*.jar' -name "*.jar" 2>/dev/null || true
  } > /tmp/provided_pulsar_maven_artifacts
  )
}
...
_ci_upload_coverage_files() {
  ...
  local classpathFile="build/classpath_${testtype}_${testgroup}"

  # Find jacoco exec files in Gradle build directories
  local execFiles=$(find . -path "*/build/jacoco/*.exec" -printf "%P\n")
  if [[ -n "$execFiles" ]]; then
    # create temp file
    ...
    local projects=$({
      for execFile in $execFiles; do
        # Navigate up from build/jacoco to project dir
        local projectDir=$(dirname "$(dirname "$execFile")")
        echo "$projectDir"
      done
    } | sort | uniq)

    # iterate the projects and collect classpath via Gradle
    for project in $projects; do
      local projectName=$(echo "$project" | sed 's|/|:|g' | sed 's/^/:/; s/:$//')
      # find built jars in the project
      find "$project/build/libs" -name "*.jar" 2>/dev/null | { grep 'org/apache/pulsar' || true; } \
                                    | { tee -a $completeClasspathFile || true; } > build/classpath_$(basename $project) || true
    done
    ...
    # upload build/jacoco/*.exec, build/classes and any dependent jar files
    ...
    (
      ...
      ci_store_tar_to_github_actions_artifacts coverage_and_deps_${testtype}_${testgroup} \
                tar -I zstd -cPf - \
                  ...
                  $GITHUB_WORKSPACE/build/classpath_* \
                  $(find "$GITHUB_WORKSPACE" -path "*/build/jacoco/*.exec" -printf "%p\n") \
                  $(find "$GITHUB_WORKSPACE" -path "*/build/classes/java/main" -printf "%p\n" | sort | uniq) \
...
# creates an aggregated jacoco xml report for all projects that contain a build/jacoco/*.exec file
ci_create_test_coverage_report() {
  ...
  local execFiles=$(find . -path "*/build/jacoco/*.exec" -size +10c -printf "%P\n" )
  if [[ -n "$execFiles" ]]; then
    mkdir -p /tmp/jacocoDir
    if [ ! -f /tmp/jacocoDir/jacococli.jar ]; then
      # Retrieve jacoco version from Gradle build
      local jacoco_version
      jacoco_version=$(./gradlew properties --no-daemon -q 2>/dev/null | grep 'jacoco' | head -1 | awk '{print $NF}' || echo "0.8.12")
    ...
    # iterate projects
    for project in $projects; do
      local artifactId="$(basename "$project")"
      if [[ -d "$project/build/classes/java/main" && -d "$project/src/main/java" ]]; then
        mkdir -p "$classesDir/$project"
        cp -Rl "$project/build/classes/java/main" "$classesDir/$project/classes"
      ...
      if [[ -n "$(echo "$project" | grep -v -E "$excludeProjectsPattern")" ]]; then
        if [ -f "build/classpath_$artifactId" ]; then
          echo "Found cached classpath for $artifactId."
          cat "build/classpath_$artifactId" >> $completeClasspathFile
        fi
      else
    ...
    local sourcefilesArgs="--sourcefiles $({
      ...
      for artifactId in $(cat $completeClasspathFile  | sort | uniq | { grep -v -E "$excludeJarsPattern" || true; } | perl -p -e 's|.*/org/apache/pulsar/([^/]*)/.*|$1|'); do
        local project=$(find . -maxdepth 2 -name "$artifactId" -type d | head -1)
    ...
    set -x
    mkdir -p build/jacoco_test_coverage_report/html
    java -jar /tmp/jacocoDir/jacococli.jar report $execFiles \
          $classfilesArgs \
          --encoding UTF-8 --name "Apache Pulsar test coverage" \
          $sourcefilesArgs \
          --xml build/jacoco_test_coverage_report/jacoco.xml \
          --html build/jacoco_test_coverage_report/html \
          --csv build/jacoco_test_coverage_report/jacoco.csv
    set +x
...
ci_create_inttest_coverage_report() {
  ...
  if [[ -n "$(find /tmp/jacocoDir -name "*.exec" -print -quit)" ]]; then
    cd "$GITHUB_WORKSPACE"
    echo "Creating coverage report to build/jacoco_inttest_coverage_report"
    set -x
    mkdir -p build/jacoco_inttest_coverage_report
    ...
    # projects that aren't considered as production code
    local excludeProjectsPattern="testmocks|testclient|buildtools"
    ...
    java -jar /tmp/jacocoDir/jacococli.jar report /tmp/jacocoDir/*.exec \
      ...
      --xml build/jacoco_inttest_coverage_report/jacoco.xml \
      --html build/jacoco_inttest_coverage_report/html \
      --csv build/jacoco_inttest_coverage_report/jacoco.csv
    set +x
  else
