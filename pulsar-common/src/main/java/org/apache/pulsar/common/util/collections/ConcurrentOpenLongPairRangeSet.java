...
public class ConcurrentOpenLongPairRangeSet<T extends Comparable<T>> implements LongPairRangeSet<T> {
    ...
    protected final NavigableMap<Long, RoaringBitmap> rangeBitSetMap = new ConcurrentSkipListMap<>();
    ...
    public void addOpenClosed(long lowerKey, long lowerValueOpen, long upperKey, long upperValue) {
        ...
        if (lowerKey != upperKey) {
            ...
            if (isValid(lowerKey, lowerValue)) {
                RoaringBitmap rangeBitSet = rangeBitSetMap.get(lowerKey);
                ...
                if (rangeBitSet != null && (rangeBitSet.last() > lowerValueOpen)) {
                    int lastValue = (int) rangeBitSet.previousValue(rangeBitSet.last());
                    rangeBitSet.add((int) lowerValue, (int) Math.max(lastValue, lowerValue) + 1);
                }
            }
            // (2) set 0th-index to upper-index in upperRange.getKey()
            if (isValid(upperKey, upperValue)) {
                RoaringBitmap rangeBitSet = rangeBitSetMap.computeIfAbsent(upperKey, (key) -> createNewBitSet());
                rangeBitSet.add(0, upperValue + 1);
            }
            // No-op if values are not valid eg: if lower == LongPair.earliest or upper == LongPair.latest then nothing
            // to set
        } else {
            RoaringBitmap rangeBitSet = rangeBitSetMap.computeIfAbsent(lowerKey, (k) -> createNewBitSet());
            rangeBitSet.add(lowerValue,  upperValue + 1);
        }
    ...
    public boolean contains(long key, long value) {
        ...
        RoaringBitmap rangeBitSet = rangeBitSetMap.get(key);
        if (rangeBitSet != null) {
            return rangeBitSet.contains(getSafeEntry(value));
        }
        return false;
    }
    ...
    public Range<T> rangeContaining(long key, long value) {
        RoaringBitmap rangeBitSet = rangeBitSetMap.get(key);
        if (rangeBitSet != null) {
            if (!rangeBitSet.contains(getSafeEntry(value))) {
                // if position is not part of any range then return null
                return null;
            }
            int lowerValue = (int) (rangeBitSet.previousAbsentValue(getSafeEntry(value)) + 1);
            final T lower = consumer.apply(key, lowerValue);
            final T upper = consumer.apply(key,
                    Math.max(rangeBitSet.nextAbsentValue(getSafeEntry(value)) - 1, lowerValue));
    ...
    public Range<T> span() {
        ...
        Entry<Long, RoaringBitmap> firstSet = rangeBitSetMap.firstEntry();
        Entry<Long, RoaringBitmap> lastSet = rangeBitSetMap.lastEntry();
        int first = (int) firstSet.getValue().nextValue(0);
        int last = (int) lastSet.getValue().previousValue(lastSet.getValue().last());
    ...
    public void forEachRawRange(RawRangeProcessor processor) {
        ...
        rangeBitSetMap.forEach((key, set) -> {
            ...
            int first = (int) set.nextValue(0);
            int last = (int) set.previousValue(set.last());
            int currentClosedMark = first;
            while (currentClosedMark != -1 && currentClosedMark <= last) {
                int nextOpenMark = (int) set.nextAbsentValue(currentClosedMark);
                if (!processor.processRawRange(key, currentClosedMark - 1,
                        key, nextOpenMark - 1)) {
                    completed.set(true);
                    break;
                }
                currentClosedMark = (int) set.nextValue(nextOpenMark);
            }
        });
    }
    ...
    public Range<T> firstRange() {
        ...
        Entry<Long, RoaringBitmap> firstSet = rangeBitSetMap.firstEntry();
        int lower = (int) firstSet.getValue().nextValue(0);
        int upper = (int) Math.max(lower, firstSet.getValue().nextAbsentValue(lower) - 1);
    ...
    public Range<T> lastRange() {
        ...
        Entry<Long, RoaringBitmap> lastSet = rangeBitSetMap.lastEntry();
        int upper = (int) lastSet.getValue().previousValue(lastSet.getValue().last());
        int lower = (int) Math.min(lastSet.getValue().previousAbsentValue(upper), upper);
    ...
    public int cardinality(long lowerKey, long lowerValue, long upperKey, long upperValue) {
        NavigableMap<Long, RoaringBitmap> subMap = rangeBitSetMap.subMap(lowerKey, true, upperKey, true);
        ...
        subMap.forEach((key, bitset) -> {
            if (key == lowerKey || key == upperKey) {
                ...
                if (key == upperKey) {
                    temp.remove(Math.min(upperValue, temp.last()), temp.last());
                }
                v.add(temp.getCardinality());
            } else {
                v.add(bitset.getCardinality());
            }
        });
        return v.intValue();
    }
