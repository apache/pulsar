...
public class MessageParser {
    ...
    public static void parseMessage(
        ...
        try {
            ...
            try {
                ...
                log.warn(
                        "[{}] Failed to deserialize metadata for message {}:{} - Ignoring",
                        topicName,
                        ledgerId,
                        entryId);
                return;
            ...
            uncompressedPayload = uncompressPayloadIfNeeded(
            ...
            if (numMessages == 1 && !msgMetadata.hasNumMessagesInBatch()) {
                processor.process(RawMessageImpl.get(
                        refCntMsgMetadata, null, uncompressedPayload.retain(), ledgerId, entryId, 0));
            } else {
    ...
    public static ByteBuf uncompressPayloadIfNeeded(
            TopicName topic,
        ...
        if (payloadSize > maxMessageSize) {
            ...
            log.error("[{}] Got corrupted payload message size {} at {}:{}", topic, payloadSize, ledgerId, entryId);
            return null;
        }
    ...
    private static void receiveIndividualMessagesFromBatch(
        ...
        try {
            for (int i = 0; i < batchSize; ++i) {
                ...
                ByteBuf singleMessagePayload = Commands.deSerializeSingleMessageInBatch(
                        uncompressedPayload, singleMessageMetadata, i, batchSize);
                ...
                processor.process(RawMessageImpl.get(
                        msgMetadata, singleMessageMetadata, singleMessagePayload, ledgerId, entryId, i));
            }
        } catch (IOException e) {
