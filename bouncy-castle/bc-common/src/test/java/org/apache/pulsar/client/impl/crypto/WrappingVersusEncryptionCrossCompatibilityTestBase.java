...
public abstract class WrappingVersusEncryptionCrossCompatibilityTestBase {
    ...
    protected static byte[] hexStringToByteArray(String s/* s must be an even-length string. */) {
        int len = s.length();
...
//         .length*8,pubKey.getEncoded().length*8,bytesToHex(actual));
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException, NoSuchProviderException {
        ...
        for (int keySize : new int[]{128, 256}) {
            ...
            String keyAsHexString = bytesToHex(aesKey.getEncoded());
            System.out.printf("%s%dBitKey = %s%n", algorithm,
                    keySize,
                    keyAsHexString);
            assertEquals(aesKey.getEncoded(), hexStringToByteArray(keyAsHexString));
        }
