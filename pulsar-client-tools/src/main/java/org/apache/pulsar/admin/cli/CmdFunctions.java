...
@Slf4j
@Parameters(
        commandDescription = "Interface for managing Pulsar Functions "
...
public class CmdFunctions extends CmdBase {
    ...
    abstract class FunctionDetailsCommand extends BaseCommand {
        @Parameter(
                names = "--fqfn",
                description = "The Fully Qualified Function Name (FQFN) for the function" + " #Java, Python")
        protected String fqfn;
        ...
        @Parameter(
                names = {"-t", "--function-type"},
                description = "The built-in Pulsar Function type")
        protected String functionType;

        @Parameter(
                names = "--cleanup-subscription",
                description = "Whether delete the subscription " + "when function is deleted")
        protected Boolean cleanupSubscription;

        @Parameter(
                names = "--jar",
                description =
                        "Path to the JAR file for the function "
        ...
        @Parameter(
                names = {"-i", "--inputs"},
                description = "The input topic or "
        ...
        @Parameter(
                names = "--topicsPattern",
                description = "TopicsPattern to consume from list of topics "
        ...
        @Parameter(
                names = "--topics-pattern",
                description = "The topic pattern to consume from a list of topics "
        ...
        @Parameter(
                ...
                description = "The custom producer configuration (as a JSON string)" + " #Java")
        protected String producerConfig;
        // for backwards compatibility purposes
        @Parameter(
                names = "--logTopic",
                description = "The topic to which the logs of a Pulsar Function are produced",
                hidden = true)
        protected String deprecatedLogTopic;

        @Parameter(
                names = "--log-topic",
                description = "The topic to which the logs of a Pulsar Function are produced" + " #Java, Python, Go")
        protected String logTopic;

        @Parameter(
                names = {"-st", "--schema-type"},
                description = "The builtin schema type or "
        ...
        @Parameter(
                names = "--retainOrdering",
                description = "Function consumes and processes messages in order",
                hidden = true)
        ...
        @Parameter(
                names = "--retain-key-ordering",
                description = "Function consumes and processes messages in key order #Java")
        protected Boolean retainKeyOrdering;

        @Parameter(
                names = "--batch-builder",
                description = "BatcherBuilder provides two types of "
                        + "batch construction methods, DEFAULT and KEY_BASED. The default value is: DEFAULT")
        protected String batchBuilder;

        @Parameter(
                names = "--forward-source-message-property",
                description = "Forwarding input message's properties "
                        + "to output topic when processing (use false to disable it) #Java",
                arity = 1)
        protected Boolean forwardSourceMessageProperty = true;

        @Parameter(
                names = "--subs-name",
                description = "Pulsar source subscription name if user wants a specific "
                        + "subscription-name for input-topic consumer #Java, Python, Go")
        protected String subsName;

        @Parameter(
                names = "--subs-position",
                description = "Pulsar source subscription position if user wants to "
                        + "consume messages from the specified location #Java")
        protected SubscriptionInitialPosition subsPosition;

        @Parameter(
                names = "--skip-to-latest",
                description = "Whether or not the consumer skip to latest message " + "upon function instance restart",
                arity = 1)
        protected Boolean skipToLatest;

        @Parameter(
                names = "--parallelism",
                description = "The parallelism factor of a Pulsar Function "
        ...
        @Parameter(
                names = "--cpu",
                description =
                        "The cpu in cores that need to be allocated "
                                + "per function instance(applicable only to docker runtime) #Java(Process & K8s),Python(K8s),Go(K8s)")
        protected Double cpu;

        @Parameter(
                names = "--ram",
                description = "The ram in bytes that need to be allocated "
        ...
        @Parameter(
                names = "--disk",
                description =
                        "The disk in bytes that need to be allocated "
                                + "per function instance(applicable only to docker runtime) #Java(Process & K8s),Python(K8s),Go(K8s)")
        protected Long disk;
        // for backwards compatibility purposes
        ...
        @Parameter(
                names = "--sliding-interval-duration-ms",
        ...
        @Parameter(
                names = "--auto-ack",
                description = "Whether or not the framework acknowledges messages automatically" + " #Java, Python, Go",
                arity = 1)
    ...
    class LocalRunner extends FunctionDetailsCommand {
        ...
        @Parameter(
                names = "--state-storage-service-url",
                description =
                        "The URL for the state storage service " + "(the default is Apache BookKeeper) #Java, Python")
        ...
        // for backwards compatibility purposes
        @Parameter(
                names = "--hostname_verification_enabled",
                description = "Enable hostname verification",
                hidden = true)
        protected Boolean deprecatedTlsHostNameVerificationEnabled = null;

        @Parameter(
                names = "--hostname-verification-enabled",
                description = "Enable hostname verification" + " #Java, Python")
        ...
        @Parameter(
                names = "--secrets-provider-classname",
                description = "Whats the classname for secrets provider" + " #Java, Python")
        protected String secretsProviderClassName;

        @Parameter(
                names = "--secrets-provider-config",
                description = "Config that needs to be passed to secrets provider #Java, Python")
        protected String secretsProviderConfig;

        @Parameter(
                names = "--metrics-port-start",
                description = "The starting port range for metrics server" + " #Java, Python, Go")
    ...
    class RestartFunction extends FunctionCommand {
        ...
        @Parameter(
                names = "--instance-id",
                description = "The function instanceId " + "(restart all instances if instance-id is not provided)")
        protected String instanceId;
    ...
    class StopFunction extends FunctionCommand {
        ...
        @Parameter(
                names = "--instance-id",
                description = "The function instanceId " + "(stop all instances if instance-id is not provided)")
        protected String instanceId;
    ...
    class StartFunction extends FunctionCommand {
        ...
        @Parameter(
                names = "--instance-id",
                description = "The function instanceId " + "(start all instances if instance-id is not provided)")
        protected String instanceId;
    ...
    class StateGetter extends FunctionCommand {
        ...
        @Parameter(
                names = {"-w", "--watch"},
                description = "Watch for changes in the value associated with a key " + "for a Pulsar Function")
            ...
            do {
                try {
                    FunctionState functionState =
                            getAdmin().functions().getFunctionState(tenant, namespace, functionName, key);
                    Gson gson = new GsonBuilder().setPrettyPrinting().create();
    ...
    class StatePutter extends FunctionCommand {
        ...
        @Parameter(
                names = {"-s", "--state"},
                description = "The FunctionState that needs to be put",
                required = true)
        private String state = null;

        @Override
        void runCmd() throws Exception {
            FunctionState stateRepr = ObjectMapperFactory.getMapper().reader().readValue(state, FunctionState.class);
            getAdmin().functions().putFunctionState(tenant, namespace, functionName, stateRepr);
        }
    }
    ...
    class TriggerFunction extends FunctionCommand {
        ...
        @Parameter(
                names = "--topic",
                description = "The specific topic name that the function consumes from that"
    ...
    class DownloadFunction extends FunctionCommand {
        ...
        @Parameter(names = "--transform-function", description = "Download the transform Function of the connector")
    ...
    public class ListBuiltInFunctions extends BaseCommand {
        @Override
        void runCmd() throws Exception {
            getAdmin().functions().getBuiltInFunctions().forEach(function -> {
