...
public class CmdSinks extends CmdBase {
    ...
    abstract class SinkDetailsCommand extends BaseCommand {
        ...
        @Parameter(
                names = {"-t", "--sink-type"},
                description = "The sinks's connector provider")
        protected String sinkType;

        @Parameter(
                names = "--cleanup-subscription",
                description = "Whether delete the subscription " + "when sink is deleted")
        protected Boolean cleanupSubscription;

        @Parameter(
                names = {"-i", "--inputs"},
                description = "The sink's input topic or topics "
        ...
        @Parameter(
                names = "--subsName",
                description = "Pulsar source subscription name "
        ...
        @Parameter(
                names = "--subs-position",
                description = "Pulsar source subscription position "
        ...
        @Parameter(
                names = "--max-redeliver-count",
                description = "Maximum number of times that a message "
        ...
        @Parameter(
                names = {"-a", "--archive"},
                description = "Path to the archive file for the sink. It also supports "
        ...
        @Parameter(
                names = "--className",
                description = "The sink's class name if archive is file-url-path (file://)",
                hidden = true)
        ...
        @Parameter(
                names = "--auto-ack",
                description = "Whether or not the framework will automatically acknowledge messages",
                arity = 1)
        ...
        @Parameter(
                names = "--negative-ack-redelivery-delay-ms",
        ...
        @Parameter(
                names = "--transform-function-config",
                description = "Configuration of the transform function " + "applied before the Sink")
            ...
            if (!Utils.isFunctionPackageUrlSupported(sinkConfig.getArchive())
                ...
                if (!new File(sinkConfig.getArchive()).exists()) {
                    throw new IllegalArgumentException(
                            String.format("Sink Archive file %s does not exist", sinkConfig.getArchive()));
                }
            }
        }
    ...
    public class ListBuiltInSinks extends BaseCommand {
        @Override
        void runCmd() throws Exception {
            getAdmin().sinks().getBuiltInSinks().stream()
                    .filter(x -> isNotBlank(x.getSinkClass()))
                    .forEach(connector -> {
