...
public class PatternMultiTopicsConsumerImpl<T> extends MultiTopicsConsumerImpl<T> implements TimerTask {
    ...
    public PatternMultiTopicsConsumerImpl(
        ...
        this.recheckPatternTimeout =
                client.timer().newTimeout(this, Math.max(1, conf.getPatternAutoDiscoveryPeriod()), TimeUnit.SECONDS);
        this.watcherFuture = new CompletableFuture<>();
        if (subscriptionMode == Mode.PERSISTENT) {
            long watcherId = client.newTopicListWatcherId();
            new TopicListWatcher(
                    topicsChangeListener, client, topicsPattern, watcherId, namespaceName, topicsHash, watcherFuture);
            watcherFuture.thenAccept(__ -> recheckPatternTimeout.cancel()).exceptionally(ex -> {
    ...
    public void run(Timeout timeout) throws Exception {
        ...
        client.getLookup()
                .getTopicsUnderNamespace(namespaceName, subscriptionMode, topicsPattern.pattern(), topicsHash)
                .thenCompose(getTopicsResult -> {
                    ...
                    return updateSubscriptions(
                            topicsPattern, this::setTopicsHash, getTopicsResult, topicsChangeListener, oldTopics);
                })
                .exceptionally(ex -> {
                ...
                .thenAccept(__ -> {
                    ...
                    this.recheckPatternTimeout = client.timer()
                            .newTimeout(
                                    PatternMultiTopicsConsumerImpl.this,
                                    Math.max(1, conf.getPatternAutoDiscoveryPeriod()),
                                    TimeUnit.SECONDS);
                });
    }

    static CompletableFuture<Void> updateSubscriptions(
            Pattern topicsPattern,
    ...
    private class PatternTopicsChangedListener implements TopicsChangedListener {
        ...
        public CompletableFuture<Void> onTopicsAdded(Collection<String> addedTopics) {
            ...
            List<CompletableFuture<Void>> futures = Lists.newArrayListWithExpectedSize(partitionedTopics.size());
            addTopicPartitionedName.forEach(partitionedTopic ->
                    futures.add(subscribeAsync(partitionedTopic, false /* createTopicIfDoesNotExist */)));
            FutureUtil.waitForAll(futures)
