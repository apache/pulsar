...
/**
 * v5-native HTTP-basic authentication (PIP-478). A single-pass credential served over both transports:
 * the binary protocol sends {@code userId:password} as {@code auth_data}, and HTTP sends
 * {@code Authorization: Basic <base64(userId:password)>}. The built-in v4 {@code AuthenticationBasic}
 * drives this body on the async binary path.
 *
 * <p>Although the v5 {@code Authentication} SPI deliberately does not extend {@link Serializable}, this
 * concrete built-in body is serializable so the v4 {@code AuthenticationBasic} shim (whose interface
 * requires {@code Serializable}, for Functions/connector frameworks) round-trips.
 */
public class BasicAuthenticationV5 implements SinglePassAuthentication, Serializable {
    ...
    public CompletableFuture<BinaryAuthData> getAuthDataAsync(AuthenticationCallContext ctx) {
        return CompletableFuture.completedFuture(
                new BinaryAuthData(userInfo().getBytes(StandardCharsets.UTF_8)));
    }
