...
public class MessageIdCompareToTest  {
    ...
    public void compareToSymmetricTest() {
        ...
        try {
            simpleMessageId.compareTo(batchMessageId1);
            fail();
        } catch (Exception e) {
            assertTrue(e instanceof UnsupportedOperationException);
            assertEquals(e.getMessage(), "org.apache.pulsar.client.impl.MessageIdImpl "
                    + "can't compare with org.apache.pulsar.client.impl.BatchMessageIdImpl "
                    + "when they have the same `LedgerId` and `EntryId`.");
        }
