...
DECLARE_LOG_OBJECT();
    ...
    std::string PublisherStatsImpl::latencyToString(const LatencyAccumulator& obj) {
        ...
        std::stringstream os;
        os <<   "Latencies [ 50pct: " << latencies[0] / 1e3 <<
                ", 90pct: " << latencies[1] / 1e3 <<
                ", 99pct: " << latencies[2] / 1e3 <<
                ", 99.9pct: " << latencies[3] / 1e3 <<
                "]";
        return os.str();
    }

    double PublisherStatsImpl::differenceInMicros(timespec& start, timespec& end) {
        return (end.tv_sec - start.tv_sec) * 1e6 + (end.tv_sec - start.tv_sec) / 1e3;
    }

    PublisherStatsImpl::PublisherStatsImpl(std::string producerStr, DeadlineTimerPtr timer, uint64_t publishDelayInMs)
    : numMsgsSent_(0),
    numBytesSent_(0),
    numAcksReceived_(0),
    totalMsgsSent_(0),
    ...
    statsInterval_(publishDelayInMs),
    mutex_(),
    ...
    void PublisherStatsImpl::flushAndReset(const boost::system::error_code& ec) {
        ...
        timer_->expires_from_now(boost::posix_time::milliseconds(statsInterval_));
        timer_->async_wait(
