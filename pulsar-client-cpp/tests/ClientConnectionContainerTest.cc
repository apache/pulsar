...
TEST(ClientConnectionContainerTest, basicWorking) {
	...
	for (int i = 0; i<rnumber; i++) {
		ASSERT_EQ(container.getNext(), i % 3);
	}

	// Test remove functionality
	int sum = 0;
	for (int i = 0; i<3; i++) {
		sum += container.getNext();
...
TEST(ClientConnectionContainerTest, removeOperation) {
	...
	ASSERT_EQ(container.getNext(), 2);
	ASSERT_EQ(container.getNext(), 3);
	ASSERT_EQ(container.getNext(), 2);
	...
	ASSERT_TRUE(container.remove());
	ASSERT_EQ(container.getNext(), 6);
	ASSERT_EQ(container.getNext(), 7);
...
TEST(ClientConnectionContainerTest, exceptions) {
