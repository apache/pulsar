...
|saslJaasClientAllowedIds|This is a regexp, which limits the range of possible IDs. These IDs can connect to the broker using SASL. By default, it is set to `SaslConstants.JAAS_CLIENT_ALLOWED_IDS_DEFAULT`, which is ".*pulsar.*", so only clients whose ID contains 'pulsar' are allowed to connect.|N/A|
