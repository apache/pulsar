...
|loadBalancerLoadManagerStrategy | The load manager strategy of load balance. <br /><br />Available values: <li>`LeastLongTermMessageRate`: selects a broker based on which one has the least long term message rate.</li><li>`ThresholdLoadManagerStrategy`: randomly selects a broker from those having resource usage below average load.</li> |`LeastLongTermMessageRate`
