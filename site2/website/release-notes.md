...
### 2.3.2 &mdash; 2019-04-12 <a id="2.3.2"></a>

#### Fixes

* Validate admin operation on topic with authoritative parameter
* fix bug with source local run
* fix cannot use size (type _Ctype_int) as type _Ctype_ulong 
* Fix segfault in c++ producer
* AlwaysCompatible doesn't use AlwaysSchemaValidator in 2.3.1 component/schemaregistry 
* Avoid potentially blocking method during topic ownership check 
* [pulsar-broker]Fix: client-producer can't reconnect due to failed producer-future on cnx cache
* Removing # TYPE comment from topic metrics in Prometheus
* For functions metrics in prometheus also remove TYPE
* Fix: set receive queue size for sinks
* Fix: Exception when switch cluster from auth enabled to auth disabled
* Fix update cli source sink 
...
* [pulsar-function] fix worker-stats broken admin-api
* fix errors in sql doc
* Fix the swagger files generated by removing troublesome class
* [pulsar-function] fix broken backward compatibility with v1-namespace while registering function
* Revert dup consumer and related code 
...
* [pulsar-function] support bookie authentication from function-worker
* Optimizing performance for Pulsar function archive download
* allow users to update output topics for functions and sources
* improve data-generator source performance
* [client] Set actual topic name to partitioned consumer
* ack records in datagenerator print sink 
* [security] Upgrade athenz libraries
* [python client] Handle subrecords in JsonSchema encoding 
* [Issue 3987][pulsar-broker]Handle config is null when create tenant 
* Add bookkeeper client version constraint
* Improve error handling for triggering function when there is a schema mismatch
* [pulsar-broker] add producer/consumer id in error-logging 
