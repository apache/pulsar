...
1. The application uses a schema instance to construct a producer instance. 
   ...
   Take AVRO as an example, Pulsar extracts schema definition from the POJO class and construct the `SchemaInfo` that the producer needs to pass to a broker when it connects.
