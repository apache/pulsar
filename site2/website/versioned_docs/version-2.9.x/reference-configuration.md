...
|isDelayedDeliveryDeliverAtTimeStrict|Control whether the deliverAt time is strictly followed when using `InMemoryDelayedDeliveryTrackerFactory` (the default `DelayedDeliverTrackerFactory`). By default (`false`), messages are sent to consumers when their deliverAt time is within `tickTimeMillis` from now, which reduces the overhead on the broker of maintaining the delayed index for a potentially short time period. When it's set to `true`, messages will only be delivered when the deliverAt time has passed, which can be as late as the deliverAt time plus the `tickTimeMillis` for the topic plus `delayedDeliveryTickTimeMillis`.|false|
