...
public class PrometheusTextFormat {
    ...
    /**
     * Writes everything registered in the Prometheus Java client registry, which is where the JVM metrics and the
     * Netty direct-memory gauges live. The client library's own text format writer is used here rather than a
     * hand-rolled one so that every snapshot type is rendered exactly as the Prometheus exposition format specifies.
     */
    static void writeMetricsCollectedByPrometheusClient(Writer w, PrometheusRegistry registry) throws IOException {
        MetricSnapshots snapshots = registry.scrape();
        ByteArrayOutputStream buffer = new ByteArrayOutputStream();
        PrometheusTextFormatWriter.create().write(buffer, snapshots);
        w.write(buffer.toString(StandardCharsets.UTF_8));
    }
