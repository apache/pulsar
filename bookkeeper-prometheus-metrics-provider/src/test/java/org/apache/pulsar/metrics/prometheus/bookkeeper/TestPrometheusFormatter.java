...
public class TestPrometheusFormatter {
    ...
    public void testWriteMetricsCollectedByPrometheusClient() {
        // A dedicated registry rather than the default one, so this test does not collide with any other test that
        // registers the same JVM metrics.
        PrometheusRegistry registry = new PrometheusRegistry();
        JvmMetrics.builder().register(registry);
        GaugeWithCallback.builder()
                .name("jvm_memory_direct_bytes_used")
                .help("-")
                .callback(callback -> callback.call(1.0))
                .register(registry);
        GaugeWithCallback.builder()
                .name("jvm_memory_direct_bytes_max")
                .help("-")
                .callback(callback -> callback.call(100.0))
                .register(registry);
        ...
        try {
            ...
            // Prometheus Java client 1.x renamed several JVM metrics relative to the legacy simpleclient
            // hotspot exports: jvm_memory_pool_bytes_committed is now jvm_memory_pool_committed_bytes, and
            // the process counter is exposed under its full process_cpu_seconds_total name.
            assertTrue(output.contains("# TYPE jvm_memory_pool_committed_bytes gauge"));
            assertTrue(output.contains("# TYPE jvm_memory_used_bytes gauge"));
            assertTrue(output.contains("# TYPE process_cpu_seconds_total counter"));
        } catch (Exception e) {
            fail();
        }
    ...
    private static Multimap<String, Metric> parseMetrics(String metrics) {
        ...
        // Values may be negative and may carry an exponent, e.g. 1.0E-5.
        Pattern pattern = Pattern.compile("^(\\w+)(\\{([^\\}]*)\\})?\\s(-?[\\d\\w\\.+-]+)(\\s(\\d+))?$");
        // Label values are quoted strings and may contain anything except a quote. jvm_runtime_info, for
        // example, reports values such as "OpenJDK Runtime Environment" and "21.0.11+10-LTS".
