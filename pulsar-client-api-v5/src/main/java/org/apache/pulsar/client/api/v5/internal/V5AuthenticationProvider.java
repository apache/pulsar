 ...
 * turned into a v5 one, and there are two ways to do that. The built-in credential shims (token, basic,
 * OAuth2, Athenz, SASL) already <em>have</em> a v5-native body — the v4 class is a thin compatibility
 * surface over it — so they hand that body over through this interface. Everything else is wrapped by
