...
{
  "msgRateIn": 12000.0,
  "byteRateIn": 64000000.0,
  "msgRateOut": 48000.0,
  "byteRateOut": 256000000.0
}
```

| Field | Source on the owning broker | Meaning for auto split/merge |
|-------|------------------------------|---------------------------|
| `msgRateIn` / `byteRateIn` | segment topic `TopicStats` (60s rolling) | ingest load |
| `msgRateOut` / `byteRateOut` | segment topic `TopicStats` | dispatch/fanout load (high for topics with many subscriptions) |
...
(b) Load-driven (if (a) didn't fire):
      candidate segments = active segments where ANY of:
        ...
        - byteRateIn > splitBytesRateInThreshold
        - msgRateOut  > splitMsgRateOutThreshold
        - byteRateOut> splitBytesRateOutThreshold
...
candidate pairs = adjacent ACTIVE segment pairs where BOTH segments satisfy,
    ...
    - byteRateIn < mergeBytesRateInThreshold
    - msgRateOut  < mergeMsgRateOutThreshold
    - byteRateOut< mergeBytesRateOutThreshold
