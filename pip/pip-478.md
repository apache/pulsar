...
- **Defaults.** Both provider keys are unset on every surface, and unset means today's behaviour. Under this
  ...
  only, so aarch64 (Apple silicon, ARM servers) and s390x would fail at startup. Availability is therefore
  tested rather than assumed — Conscrypt's own `checkAvailability()`, which is what fails on those platforms.
...
- **Security-relevant behavior changes (5.0), surfaced here so they aren't buried in the removal inventory:**
  ...
  - **The `Conscrypt` web-listener provider defaults become conditional.** `webServiceTlsProvider` (broker
    ...
    Conscrypt has no native library (`conscrypt-openjdk-uber` ships x86_64 only: aarch64 and s390x). The
    default is kept and made conditional: unset resolves to Conscrypt where it is available and to the JVM
    default elsewhere. **Operationally this means x86_64 web listeners now genuinely run on Conscrypt where
    they previously ran on the JVM default**, which is the behaviour the shipped configuration always
    described. An operator who prefers the JDK provider sets it explicitly.
