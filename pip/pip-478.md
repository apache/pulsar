...
- **Defaults.** Both provider keys are unset on every surface, and unset means today's behaviour. Under this
  ...
  legacy `Conscrypt` defaults untenable, so this PIP **removes** them: `webServiceTlsProvider` on
  `ServiceConfiguration` / `ProxyConfiguration` and the WebSocket proxy's web-listener `tlsProvider` all ship
  unset. Under PIP-337 those defaults were inert — the broker passed `webServiceTlsProvider` to Jetty's
  `SslContextFactory`, which never reaches the built `SSLContext` that `DefaultPulsarSslFactory` supplies
  with no provider — so no deployment has actually been running on Conscrypt, and dropping the default
  changes nothing operationally. Keeping it, on the other hand, would break the web listener out of the box
  wherever Conscrypt cannot load: `conscrypt-openjdk-uber` ships native libraries for x86_64 only, so
  aarch64 (Apple silicon, ARM servers) and s390x would fail at startup. An operator who wants Conscrypt sets
  it explicitly and gets the loud failure when it is unavailable, which is the point of pinning.
...
- **Security-relevant behavior changes (5.0), surfaced here so they aren't buried in the removal inventory:**
  ...
  - **The `Conscrypt` web-listener provider defaults are removed.** `webServiceTlsProvider` (broker and
    proxy) and the WebSocket proxy's web-listener `tlsProvider` shipped a default of `Conscrypt` that was
    **inert** under PIP-337 — it reached only Jetty's `SslContextFactory`, never the `SSLContext` that
    `DefaultPulsarSslFactory` built with no provider. This PIP makes those keys authoritative on the JSSE
    axis, so the defaults are dropped to unset rather than becoming real on upgrade: `conscrypt-openjdk-uber`
    ships native libraries for x86_64 only, and a retained default would fail the web listener at startup on
    aarch64 and s390x. Unset selects the JVM default `SSLContext` provider, which is what deployments have
    effectively been running; a deployment that genuinely wants Conscrypt sets the key explicitly and gets a
    loud failure where the provider cannot be loaded.
