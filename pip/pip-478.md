...
4. **Round cap.** A non-refresh exchange is bounded at 10 challenge rounds (`ClientCnx.MAX_AUTH_CHALLENGE_ROUNDS`); exceeding it fails the connection rather than looping forever, mirroring the HTTP driver's cap. A REFRESH resets the counter, since it begins a fresh exchange. **This is a behaviour change from v4**, which had no cap: a mechanism that legitimately needs more than 10 rounds — an unusually long GSSAPI negotiation — now fails where it previously completed.
5. **Overlapping challenges.** A non-refresh challenge arriving while a round is already in flight is dropped, so a broker cannot interleave two conversations on one connection. A REFRESH is never dropped: it supersedes the round in progress.
...
- **`PulsarTlsFactory`** (interface, in `org.apache.pulsar.tls`) — the v5 TLS SPI. The framework requests an instance of a well-known class for a `TlsPurpose` (described next); the factory returns a configured instance when it supports the combination, or `Optional.empty()` when it does not:
  ...
  **Ownership and disposal.** The component that creates a factory owns and closes it: the default factory, and any custom factory instantiated from configuration, are closed by the framework when the owning client or service closes. A factory *instance* supplied programmatically to the v5 builder is **adopted** — the client closes it on `PulsarClient.close()`, matching the v4 precedent for builder-supplied `Authentication` instances. Adoption is a hand-over rather than a share, so one instance serves one client: a builder still builds as many clients as the application wants (each takes its own copy of the configuration, and each composes its own factory from a `tlsPolicy(...)`), but a `build()` that would hand an already-adopted instance to a second client — or, on the admin builder a server component attaches one to, to a second `PulsarAdmin` — is **rejected**, because `initialize` is called exactly once and `close` at most once per factory and the second one would otherwise be left serving TLS from a factory the first closed. A build that *failed* is no different: an instance is spent from the moment the framework takes it, and the framework closes what it initialized — a factory whose `initialize` failed, or which failed the client's fail-fast probe, is closed there and then, before any client exists to own it, so an implementation must tolerate releasing a state it never finished building. A build that failed *before* the framework took the instance — a missing service URL, say — leaves it untouched and re-usable. Handles follow their consumer: server-side subscriptions are disposed when the listener or web service stops; a client connection disposes its one-shot handle when the connection closes. A leaked (undisposed) handle costs at most a factory-side cache/refresh registration — the default factory shares one `TlsMaterialSource` per purpose, so a leak never duplicates file watchers and is bounded.
...
**One factory per client, on both paths.** `initialize(...)` is called exactly once per `PulsarClient` / `PulsarAdmin`, and `close()` exactly once, however many internal transports share it — a `PulsarAdmin` in particular drives more than one HTTP connector. Custom factories may therefore hold per-instance resources (HSM sessions, rotation tasks) without defending against repeated initialization, and whoever hands a factory *in* — the broker's and the functions worker's admin attach in tree, `tlsFactory(...)` for an application — transfers ownership with it, must not close it itself, and must hand each instance to one client or admin only: a builder rejects a second `build()` from a factory it has already handed over, since the alternative is a second component initializing a live instance and a third-party factory being closed twice.
...
- `LegacyV4CredentialAdapter implements BinaryAuthDataProvider, BinaryAuthChallengeHandler, HttpAuthHeadersProvider` — the default for any plugin whose method name is neither `"tls"` nor `"sasl"` (e.g. `AuthenticationToken`). It advertises **both binary capabilities unconditionally**, which is what preserves v4 parity: under v4 every plugin could be handed a challenge, so withholding the challenge capability from a plugin that did not *look* multi-round would silently break third-party plugins that are. Only the HTTP capability is decided by probing (`hasDataForHttp()`), *post-start* on the blocking executor. The binary credential is rendered through `d.authenticate(AuthData.INIT_AUTH_DATA)` rather than `getCommandData()`, so a plugin that computes its initial frame in `authenticate(...)` — the shape SASL-style plugins use — is served correctly.
- `LegacyV4ChallengeResponseAdapter implements BinaryAuthDataProvider, BinaryAuthChallengeHandler` — selected for the method name `"sasl"`. It produces the initial frame via `getAuthDataAsync(...)` and routes binary challenges through `respondToChallengeAsync(...)`, driving the v4 `authenticate(AuthData)` method across rounds (the per-exchange v4 provider is kept in the call-context state slot). **Consequence of name-based routing:** a third-party challenge-response plugin whose method name is *not* `"sasl"` is routed to the credential adapter instead. This is **not** a functional limitation: that adapter advertises `BinaryAuthChallengeHandler` too and drives the plugin's multi-round `authenticate(AuthData)` flow through the same call-context state slot. The routing decides which adapter serves the plugin, not whether its challenge rounds work.
...
3. **Refresh is the plugin's concern.** The framework has no built-in refresh; proactive renewal and token reuse are internal to the authentication implementation. A short-lived-credential plugin (e.g. `AuthenticationOAuth2`, `AuthenticationAthenz`) renews its credential internally and returns the current one whenever the framework next calls its async credential method — and the broker's `CommandAuthChallenge` refresh sentinel simply triggers that same call. The legacy adapter therefore just re-invokes the wrapped v4 plugin per request; no refresh metadata crosses the capability surface.
   ...
   **Sentinel delta for bridged v4 plugins — a behaviour change.** Because binary routing rule 2 makes a broker-pushed `REFRESH` start a *fresh* exchange rather than a challenge round, the bridged plugin's refresh call arrives as `authenticate(AuthData.INIT_AUTH_DATA)`, where the v4 client passed `authenticate(AuthData.REFRESH_AUTH_DATA)`. **The `"PulsarAuthRefresh"` sentinel never reaches a bridged v4 plugin at all.** For every plugin whose `authenticate(...)` ignores its argument on the credential path — all of the built-ins, and any plugin that simply returns its current credential — this is invisible. It is observable only for a third-party v4 plugin that *branches* on `REFRESH_AUTH_DATA`, for instance to force a renewal it would otherwise skip; such a plugin sees a refresh as an initial credential request and must instead renew on its own schedule (which invariant 3 already asks of it). This is a deliberate consequence of unifying refresh onto the single "produce your current credential" path, not an oversight: keeping the sentinel would mean routing it into a conversation the refresh has just terminated.
...
**HTTP challenge routing (normative).** The driver runs the challenge exchange **ahead of** the real request, as a side band, rather than reacting to the real request's `401`. This is what v4's `authenticationStage(...)` did and what the SASL-over-HTTP flow needs: the negotiation is completed first, and the validated result is what the real request carries.

1. **Applicability** → the driver engages only for a plugin exposing **both** `capability(HttpAuthChallengeHandler.class)` and `capability(HttpAuthHeadersProvider.class)`. A plugin exposing neither, or only one, is not driven: its request is issued with whatever headers `getHttpHeadersAsync(ctx)` supplies, or none.
2. **Side-band exchange** → the driver issues bodiless `GET`s to the original URI, calling `respondToHttpChallengeAsync(ctx)` for each round. `HttpAuthCallContext.serverChallengeHeaders()` is empty on the first round and carries the server's previous response headers thereafter. The exchange is bounded by a round cap; exceeding it fails the request.
3. **Real request** → once the exchange completes, `getHttpHeadersAsync(ctx)` composes the real request's headers from the state the exchange established (for SASL, the validated role token).

*A note on why this is not `401`-reactive:* the driver never sees the real request's response, so it cannot resubmit it. Making the exchange reactive would be a larger change — the two client APIs would each have to surrender their response handling to the driver — and would gain nothing for the one plugin that uses it.
...
Dependency note: the v4 `pulsar-client` module gains **three** `api` dependencies — `pulsar-client-api-v5`, `pulsar-tls-factory-api`, and `pulsar-http-client-api` — because the migrated built-in v4 plugins delegate to v5-native bodies (in-scope item #7) and surface the TLS/HTTP SPI types on their exported ABI. The shaded v4 artifacts consequently carry the v5 auth API, the TLS-factory API and the HTTP-client API as **compile-scope dependencies** of their dependency-reduced POM, exactly as they already carry `pulsar-client-api` — not bundled inside the jar. These are API modules a plugin author compiles against (a custom `PulsarTlsFactory`, a v5 `Authentication`), so bundling a second copy would put two versions of the same interface on that author's class path; and being Pulsar's own packages (`org.apache.pulsar.client.api.v5.*`, `org.apache.pulsar.tls.*`, `org.apache.pulsar.http.*`) they are never relocated, so the declared artifact and the shaded jar agree on the class names.
...
- **Both columns ship on every row.** The JCA axis is honoured end to end in the SPI and the default
  factory — `TlsPolicy.jcaProvider()` feeds every material load — and each surface in the table carries the
  key, wired at the same site as its JSSE counterpart. One asymmetry is deliberate: the JSSE axis has a
  web-listener default (Conscrypt where usable) and is therefore resolved through
  `resolveWebJsseProvider` / `resolveJsseProvider`, whereas the JCA axis has no default at all — unset
  means the JVM provider search order everywhere — so it is read straight from the configuration.
...
- **An unregistered `BCJSSE` is constructed from the class path, and not necessarily in FIPS mode.** Name resolution prefers a provider the operator has already registered in the JVM (`Security.getProvider(name)`) and only then falls back to `ServiceLoader`, so a statically registered instance always wins — that is the mechanism by which a mode fixed at construction, such as `new BouncyCastleJsseProvider("fips:BCFIPS")`, reaches Pulsar, and it is not shadowed by a no-arg instance of the same name. The residual hazard is the step after those two: `bctls` ships no `ServiceLoader` entry, so when *nothing* has registered `BCJSSE`, Pulsar constructs and registers it itself — bound to FIPS only when the resolved BouncyCastle JCA provider is `BCFIPS`, and in the default (non-FIPS) mode otherwise. Because BouncyCastle's JSSE provider registers under the plain name `BCJSSE` either way and the fallback construction is logged only at INFO, `jsseProvider=BCJSSE` resolves successfully in both cases. A FIPS audit must therefore confirm that the FIPS-mode instance was registered in `java.security` before Pulsar started, not merely that the key is configured (mechanism: [provider-name resolution](#redesigned-pip-337-ssl-provider-pulsartlsfactory)).
...
- Existing applications using `pulsar-client-api` (v4) need **no source changes** — except the rare application that called the removed PIP-337 `sslFactoryPlugin(...)` / `sslFactoryPluginParams(...)` builder methods, which no longer compile and must move to `tlsFactoryClassName(...)` / `tlsFactoryConfig(...)` (see below) — but 5.0 carries several **runtime behavior deltas** they should review before upgrading:
  ...
  - **TLS material loads eagerly at client build on the v5-builder TLS path** (the fail-fast contract) — when the application adopts a factory with `tlsFactory(...)` or sets a policy with `tlsPolicy(...)`, a misconfigured cert/key/trust path surfaces at `PulsarClient` build instead of on first connect. A plain v4 client keeps 4.x's lazy loading: the probe is deliberately scoped to the v5-builder path so that an unmodified v4 application which tolerated an unused, badly-configured TLS field still starts.
