...
- **Defaults.** Both provider keys are unset on every surface, and unset means today's behaviour. The one
  ...
  `Conscrypt`. The broker reads it — `WebService` passes it to Jetty's `SslContextFactory` — but under
  PIP-337 it never reaches the built `SSLContext`, which `DefaultPulsarSslFactory` supplies with no
  provider, so the default is inert today; the proxy's web listener reads `tlsProvider` instead. Under
  this PIP the `WEB` purpose honours it: `webServiceTlsProvider` / `webServiceTlsProtocols` /
  `webServiceTlsCiphers` take precedence over the binary-listener keys and fall back to them when unset.)
