...
The lifecycle is: the framework constructs a single `AuthenticationInitContext` per `PulsarClient` and calls `initializeAsync(ctx)` once, and the implementation may retain references for the lifetime of the client. When the plugin is configured through the v5 builder it is initialized **eagerly at client build**, on the application thread, so a misconfiguration fails the build. The framework's own drivers — which also carry a v4 built-in plugin that delegates to a v5 body, where there is no such build-time hook — initialize **on first use** instead, and there the call may land on a Netty event loop: the framework does not wait for the future, it chains the connection's authentication rounds onto it. Hence the "must not block the calling thread" clause above; a body that does its I/O inline before returning the future would stall every connection sharing that event loop. Initialization failures are not cached, so a transient failure recovers on reconnect rather than poisoning the plugin for the client's lifetime. `Authentication.close()` releases any resources the implementation acquired; the framework owns and closes the shared services (HTTP clients, scheduler, blocking executor). Per-call `AuthenticationCallContext` instances are cheap to allocate.
