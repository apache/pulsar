...
- **What is implemented, as of the final PR in the series.** The **JSSE column** ships on every row except
  `pulsar-perf` / `pulsar-testclient` (no flags were added) and the WebSocket proxy's *outbound*
  `brokerClientJsseProvider` (its web-listener `jsseProvider` does ship). The **JCA column** ships in the SPI
  and the default factory — `TlsPolicy.jcaProvider()` is honoured for every material load — but has **no
  configuration key on any surface**, so today it is reachable only programmatically through the v5 builder.
  Closing that is an open decision, not an oversight to be papered over by deleting the column.
