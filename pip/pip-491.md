...
active, the in-scope Java path must conserve `P` and settle each unit to that source as the corresponding command or
message reaches a terminal outcome. The command does not carry `P`, so the client has to infer it from `ack_set` and
the payload. If the payload cannot be parsed or only part of a batch enters the message lifecycle, the client can
return a different count from the one used by the broker.
...
the in-scope Java native-message path must conserve `P` and settle it to the source as command and message units reach
terminal outcomes. When that broker-consumer incarnation is removed, its debt disappears with it, so the client
discards old credit rather than moving it to a replacement.
...
- Finalize `P` once on the broker after final admission.
- Reuse that result for consumer accounting, command serialization, and every dispatcher aggregate debited for the
  send.
- Conserve `P` in the standard typed Java `ConsumerImpl` native-batch path across successful delivery, client-side
  skips, and checksum, metadata, decompression, and batch-deserialization failures.
...
- `RawReader` and raw-entry consumption paths that bypass the standard typed `ConsumerImpl` message lifecycle.
...
- Changes to asynchronous broker `CommandFlow` processing and consumer-removal ordering.
...
- Using finalized `P` to change `avgMessagesPerEntry`, its exported statistics, or dispatcher read-sizing heuristics.
- New user-facing Java client APIs, configuration, CLI commands, metrics, or broad consumer/dispatcher refactoring.

These exclusions do not change wire compatibility. Specialized payload paths retain their current path-specific
return cardinality, while the ownership boundary below prevents an additional generic return. Known adjacent work is
listed in Appendix B.

# High Level Design

For a send containing one or more entries, the broker produces one finalized `P` for every entry admitted for asynchronous
sender handoff. It also calculates the sum of those per-command values. The per-command values and their sum become the only
...
to a message or delegated specialized owner. The same finalized broker value and the ownership split are shown below.
...
For each command processed entirely through the in-scope Java native-message path, without delegation to an
out-of-scope specialized owner, while its source consumer remains active, the accounting invariant is:

```text
command-owned units + live-message-owned units + credit settled to the source incarnation == P
```

Settled credit includes credit retained locally for an existing threshold, pause, or demand gate as well as credit
already written in `CommandFlow`. After every in-scope message unit reaches a terminal outcome, the first two terms
are zero and the client has settled exactly `P` to the source incarnation. If that incarnation disappears first, its
remaining local credit is discarded rather than transferred to a replacement.
...
| --- | --- |
| Broker finalization | Decide which entries pass final admission, calculate one `P` per admitted command, and calculate their sum. An entry rejected there produces neither a command nor a debit. All covered broker counters and serialization consume this result. |
...
| Java command path | Resolve and validate `P`, create `remaining = P`, transfer one unit to each accepted native message, and return every unit that was neither transferred nor delegated. |
...
```text
entry rejected by final admission => no command and no debit
ack_set is empty     => P = B
ack_set is non-empty => P = deliverable set-bit count within B
```

Finalization does not change the debit of a valid command emitted both before and after this change: one for a
non-batched message, `B` for a complete batch, and the deliverable `ack_set` cardinality for a partial batch. An entry
rejected by final admission does not contribute a debit.

The finalized `P` and the serialized `B` and `ack_set` describe the same admitted delivery snapshot. No downstream
layer independently rewrites one of those values or recomputes `P` from mutable or recycled inputs.
...
the finalized logical-message count. The finalized per-entry values and sum remain valid through asynchronous
serialization and aggregate accounting, even after the sender recycles existing batch-size and `ack_set` inputs. The
concrete carrier is an implementation choice, and downstream layers do not recompute the values.

All `Consumer` send entry points, including overloads carrying consumer epochs or sticky-key hashes, make the same
finalized values and sum available to internal callers that own aggregate accounting; the method shape is an
implementation choice.

The debit belongs to the source broker-consumer incarnation before asynchronous sender handoff. A later batch-compatibility
abort in `PulsarCommandSenderImpl` can close the connection after debit without emitting a command; that outcome relies
on existing teardown, rather than the zero-debit admission rule. This PIP does not add new handling for asynchronous
network-write failure or speculative re-crediting after an ambiguous write; existing transport cleanup remains unchanged.
...
the entries admitted for sender handoff:
...
A present `message_permits` value is authoritative for flow accounting once validated. Java interprets the protobuf
`uint32` using unsigned conversion and accepts an explicit value only in `1..Integer.MAX_VALUE`. An explicit value
outside that range is a connection-level protocol error: the client returns no Flow credit and closes the source
connection.
...
out-of-range set bit, or native expansion beyond `P` is malformed payload. Validation occurs before accepting a native
message whenever the required metadata is available. When a range-valid explicit `P` is present, it remains the
accounting authority: units already transferred remain owned by their message lifecycles, and the command terminal
action returns the remaining command-owned units, so the total settlement is exactly `P`. This corrupted-data outcome
does not by itself close or recreate the source consumer, or close the pooled connection.
...
available to checksum and metadata-failure paths. When the field is absent, compatibility fallback and
corrupted-message handling remain best effort. The final fallback cannot recover a complete-batch debit from an old
broker when failure occurs before metadata parsing, so the client retains the legacy one-permit behavior. This PIP
does not introduce a consumer or pooled-connection close for old-protocol or payload-derived ambiguity; exact
conservation for that command requires an upgraded broker.
...
Existing direct `increaseAvailablePermits` call sites reached by the in-scope path are rewritten to consume the
corresponding command- or message-owned units, or are removed in favor of the terminal drain. They do not remain as
independent returns alongside that drain.

If native expansion would exceed `P`, the client stops accepting further messages and follows the malformed-payload
outcome above.
...
| --- | --- |
| Explicit `P` is zero or its unsigned value exceeds `Integer.MAX_VALUE` | Return no credit and close the source connection. |
| Invalid `B`, `ack_set`, or expansion with a range-valid explicit `P` | Handle the command as corrupted data; transferred messages and the terminal budget settle exactly `P`; keep the source consumer and pooled connection open. |
| Invalid `B`, `ack_set`, or expansion without an explicit `P` | Follow compatibility fallback and best-effort corrupted-message handling; do not introduce a source-consumer or pooled-connection close. |
...
accumulator. Permit ownership follows the accepted delivery lifecycle across in-scope interceptor or parent wrapping
and replacement; terminal handling must not depend on the public `Message` still being the original `MessageImpl`.
Reconnect or `CommandCloseConsumer` first invalidates the old state and installs a replacement, even when the pooled
`ClientCnx` is unchanged, before enqueueing the replacement Subscribe. It never copies the old accumulator into the
replacement.
...
Pause can delay writing accumulated credit; resume restores its eligibility under the existing Flow scheduling policy
without manufacturing another return.
...
A zero-queue non-batched receive has `P = 1`. Its terminal outcome settles one demand-gated unit to the source state,
but does not itself write another Flow for synchronous or asynchronous receive. The next receive demand flushes that
settled unit instead of granting an additional unit. A listener represents continuing demand and retains its existing
post-callback Flow behavior. If command processing settles `P` before satisfying an outstanding zero-queue demand,
all `P` units are flushed for that same demand; this restores the failed command's debit without creating additional
demand. If an accepted non-batched message terminates before satisfying that demand, its one unit is likewise reused
immediately. Only the `P = 1` unit from a successfully satisfied receive is held for the next demand. Native batch
delivery remains unsupported by that specialization; rejection closes the consumer and sends no Flow credit for the
rejected command. A stale zero-queue message is released without crediting a replacement incarnation.
...
assembly, a crypto failure action, or a custom `MessagePayloadProcessor` takes ownership, that path becomes the sole
permit-return owner. That transfer is a single terminal transition completed before invoking the specialized handler
or exposing any callback or output; afterward, generic cleanup returns zero even if specialized processing throws or
produces partial output. The existing specialized return cardinality may be less than, equal to, or greater than `P`;
for example, crypto `DISCARD` can return `B` for a partial delivery with `P < B`. This existing mismatch remains out
of scope, and this PIP guarantees only that no additional generic-budget return is introduced. Existing returns remain
source-incarnation-bound. Crypto
`FAIL` continues to return no permit while retaining the message for redelivery. A failure before ownership transfers
to one of these paths remains command-owned and follows the normal terminal action.
...
Beyond generated protocol accessors, no user-facing Java client API, configuration, CLI option, or metric is added.
Broker finalization does not reprice emitted commands. Entries rejected by final admission or having `P = 0` no
longer contribute phantom consumer-permit, unacked-message, or dispatcher-aggregate accounting;
`avgMessagesPerEntry` and its dispatcher read-sizing uses remain unchanged.
...
and verify that repeated corrupted batches carrying a range-valid explicit `P` do not stall a Shared consumer.
...
return more than `P` for one command.

# Backward & Forward Compatibility

| Broker | Java client | Behavior |
| --- | --- | --- |
| New | New | Exact accounting from explicit `P` for the in-scope standard typed Java `ConsumerImpl` path. |
...
Payload-derived inconsistency on an old-broker command does not introduce a new pooled-connection close. The fallback
remains best effort, so exact exceptional-path conservation still requires an upgraded broker and Java client.
...
can remain connected to upgraded brokers. Broker finalization, every covered consumer and aggregate debit, and wire
emission are enabled together in one broker release; the Java client can roll independently through optional-field
fallback.
...
- **Always disconnect on ambiguous payload failure:** discards the debt but disrupts every producer and consumer on a
  multiplexed connection. Disconnect remains appropriate when an explicit `message_permits` value is zero or above
  `Integer.MAX_VALUE`, not for normal data corruption or old-protocol ambiguity.
...
- invalid explicit zero or unsigned range returns no Flow credit and closes the source connection;
- with a range-valid explicit `P`, invalid `B`, `ack_set`, inconsistent metadata, or native expansion settles exactly
  `P` without closing the source consumer or pooled connection; without an explicit `P`, the same payload failures
  retain best-effort compatibility behavior and do not disconnect the source consumer or unrelated connection users;
- old-version clients and protobuf decoders, Pulsar proxy forwarding, and a simulated field-stripping intermediary;
- filtering or pending-ack rejection, including `P = 0`, produces no command, consumer or unacked-message debit,
  dispatcher-aggregate debit, or pending-ack ownership;
...
- a pending Flow update combined with a finalized send and consumer removal preserves the merged #26289 accounting,
  including negative removal balances, in both modern and classic persistent Shared dispatchers;
...
- multi-topic and partitioned parent forwarding, and interceptor wrapping or replacement, preserve the retained source
  ownership with and without listener-mode deferral;
- after a zero-queue receive is satisfied, synchronous and asynchronous terminal handling sends no Flow before the next
  receive demand and that demand sends exactly one unit; a command failure before satisfying the pending demand
  flushes its full `P`, and a pre-delivery message failure reuses its one unit, for that same demand; listener mode
  preserves continuous demand, and native batch rejection remains unsupported;
...
- chunk, crypto-failure, and custom processor paths produce no additional generic-budget return after ownership
  transfer, including failure or partial output after transfer and cases where their existing cardinality differs from
  `P`; successful decryption returning to the built-in path uses exact `P`, and any return remains
  source-incarnation-bound;
- deterministically generated native batches whose settled source credit equals the explicit `P` sum after all
  delivered messages and immediate skips reach terminal handling; after a test-controlled accumulator drain, the
  decoded `CommandFlow.messagePermits` sum has the same value; and
- repeated corrupted batches carrying a range-valid explicit `P` on a persistent Shared subscription without dispatch
  stall.
...
- further changes to asynchronous `CommandFlow` ordering and consumer removal;
- Key_Shared hash draining and admission policy;
- custom `MessagePayloadProcessor` output cardinality;
- exact permit-return cardinality within encrypted-payload and chunk-assembly paths;
...
output-cardinality semantics are not redefined by this PIP. The Flow-removal race in
[issue #26288](https://github.com/apache/pulsar/issues/26288) was addressed by the merged
[PR #26289](https://github.com/apache/pulsar/pull/26289). Its pending Flow accounting is the baseline the finalized send
debit must preserve: for persistent Shared and Key_Shared dispatchers, completed updates under the dispatcher monitor
maintain `totalAvailablePermits == sum(messagePermits - pendingDispatcherFlowPermits)` over registered consumers.
This PIP preserves that baseline without redesigning Flow processing.
...
* Mailing List discussion thread: https://lists.apache.org/thread/bor1ykqtxk2csrjx4tfqzc47t2r0r19n
