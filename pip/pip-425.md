...
- Fallback: If the availableAddressPool ever becomes empty, the client will fall back to trying a random endpoint from the allAddressPool. This prevents a deadlock where the client cannot connect because all known endpoints are simultaneously in quarantine.
...
```java
     ...
     * @param serviceUrlQuarantineInitDuration the initial quarantine duration for unavailable endpoint. Defaults to 60 seconds.
     ...
     * <p>
     * A value of 0 means don't quarantine any endpoints even if they fail.
     * @param serviceUrlQuarantineMaxDuration the maximum quarantine duration for unavailable endpoint. Defaults to 1 day.
     * @param unit the time unit for the quarantine duration
     * @return the client builder instance
     */
