...
└── org.apache.pulsar.functions.windowing
    ...
    ├── WindowFunctionExecutor<T,X> // executor bridging runtime and user function (T=input, X=output)
...
- Expose the `Window<T>` interface in the public API (including `getNew()`, `getExpired()`, and timestamp methods), with explicit Javadoc for **object lifetime** (valid only during `process()`) and **list mutability** (callers must not modify returned lists; the executor snapshots the ack list so mutation cannot reach the ack path).
...
- Deliver `onActivation` when the current window is empty but expired events remain, without invoking `process(Collection, …)`.
...
The new overload is a default method that forwards a non-empty `window.get()` to the existing abstract `process(Collection<Record<X>>, WindowContext)` method, and returns `null` when `get()` is empty:

```java
default T process(Window<Record<X>> window, WindowContext context) throws Exception {
    List<Record<X>> current = window.get();
    if (current.isEmpty()) {
        return null;
    }
    return process(current, context);
}
```

This keeps `WindowFunction` a valid `@FunctionalInterface`. The runtime instantiates window functions by class name, so a deployed `WindowFunction` is always a class and can override the new overload; the lambda restriction applies only to in-process/test usage.
...
#### Empty-but-expired delivery

`TimeTriggerPolicy` fires `onTrigger()` on a fixed-rate timer even after traffic stops. When the last events leave the window, `WindowManager` already drains them into `expired`, then skips `onActivation` because `events` is empty. `WindowFunctionExecutor.onExpiry` is a no-op, so those expirations never reach user code. `WindowManagerTest.testTimeBasedWindowExpiry` asserts this today.

After this PIP, `WindowManager` calls `onActivation` when `events` is empty but `expired` is not. The executor still calls `process(Window, …)`:

- A class that overrides `process(Window, …)` receives `get()` empty and `getExpired()` populated, so it can apply the eviction (for example decrement a running sum).
- A class that does not override it hits the default method, which sees empty `get()` and returns `null`. The original `process(Collection, …)` is not called.
- A bare `java.util.function.Function` is not invoked either; the executor returns `null` when `get()` is empty.

### Design tradeoff

Keeping `process(Collection, …)` as the single abstract method is what makes the change source- and binary-compatible. The cost is that an incremental implementation must still declare a `process(Collection, …)` body. The runtime never calls that body unless `process(Window, …)` delegates to it.

That collection method should contain the full-window computation. `process(Window, …)` should call it when incremental state is missing (first activation after construction or restart). When a class overrides `process(Window, …)`, that override is the only runtime entry point.
...
flowchart TD
    ...
    subgraph executor ["process(Window, WindowContext) dispatch"]
        P1["WindowFunction class override<br/>→ process(window, context)"]
        P2["WindowFunction default<br/>→ process(get()) if non-empty, else null"]
        P3["bareWindowFunction<br/>→ apply(values) if get() non-empty"]
    end
...
public interface Window<T> {
     ...
     * Callers must not modify it. Mutation does not affect the runtime's live
     * window state or the records the executor will ack.
     * </p>
     * <p>
     * May be empty when this activation exists only to report expirations
     * (see {@link #getExpired()}).
     * </p>
     */
    List<T> get();
     ...
     * </p>
     * <p>
     * When the last events leave the window, {@link #get()} may be empty while
     * this list is not.
     * </p>
     */
    List<T> getExpired();
     ...
     * as supplied; it does not derive timestamps itself.
     * </p>
     * <p>
     * For time-based windows this is the window's end boundary: the trigger
     * policy's reference time (watermark in event-time mode, processing time in
     * processing-time mode). For count-based windows this is the trigger policy's
     * reference timestamp taken at trigger time, not a window boundary, and
     * {@link #getStartTimestamp()} is {@code null}.
     * </p>
     */
     ...
     * for count-based windows (no time-based window length).
     * </p>
     * <p>
...
| `windowing/WindowFunctionExecutor.java` | import `org.apache.pulsar.functions.api.Window`; snapshot the ack list before `process()` |
...
The lists stay mutable. `get()` currently returns the same `tuples` list that `processWindow()` later iterates to ack records under `ATLEAST_ONCE`, so `window.get().clear()` would silently skip those acks. Wrapping the getters in `Collections.unmodifiableList` would turn that into `UnsupportedOperationException` for any existing `process(Collection, …)` implementation that already mutates its input, including via the default `process(Window)` overload. Instead, `processWindow()` copies `tuples` into a separate ack list **before** calling `process()`, and acks from that copy. Mutation of `get()` / `getNew()` / `getExpired()` then cannot reach the ack path or `WindowManager`'s live event store. Javadoc still tells callers not to modify the lists; that rule is advisory, not load-bearing.
...
public interface WindowFunction<X, T> {
     ...
     * implementations continue to work without source changes. Incremental
     * implementations should put the full-window computation here and have
     * {@link #process(Window, WindowContext)} fall back to it when incremental
     * state is not available.</p>
     *
     * <p>The runtime does not invoke this method when the current window is
     * empty.</p>
     *
     ...
     *
     * <p>The default implementation preserves existing behavior: if
     * {@link Window#get()} is empty it returns {@code null} without calling
     * {@link #process(Collection, WindowContext)}; otherwise it delegates with
     * {@link Window#get()}.</p>
     *
     * <p>Implementations can override this method to access newly added events
     * ({@link Window#getNew()}) and expired events ({@link Window#getExpired()}).
     * When this method is overridden, it is the runtime entry point; the
     * collection-based method is invoked only if this override delegates to it.</p>
     *
...
The collection method holds the full-window computation. The window overload uses `getNew()` / `getExpired()` when a running sum is already in memory, and falls back to the collection method on the first activation after construction or restart. The runtime invokes only `process(Window, …)`. The incremental update is correct when `slidingInterval ≤ windowLength`. `slidingInterval > windowLength` remains a legal config; in that case a record can expire between triggers and appear only in `getExpired()`, so a decrement-on-expiry aggregate would drift.
...
public class SlidingWindowSumFunction implements WindowFunction<Integer, Integer> {
    ...
    public Integer process(Collection<Record<Integer>> input, WindowContext context) {
        int sum = 0;
        for (Record<Integer> record : input) {
            sum += record.getValue();
        }
        return sum;
    }
    ...
    public Integer process(Window<Record<Integer>> window, WindowContext context) throws Exception {
        if (runningSum == null) {
            Collection<Record<Integer>> current = window.get();
            if (current.isEmpty()) {
                runningSum = 0L;
                return 0;
            }
            runningSum = process(current, context).longValue();
...
### Change 3: Deliver empty-but-expired activations in `WindowManager`

**Path**: `pulsar-functions/instance/src/main/java/org/apache/pulsar/functions/windowing/WindowManager.java`

Today `onTrigger()` drains `expiredEvents` and then calls `onActivation` only when `events` is non-empty. If the window has drained, the expired list is dropped. `WindowFunctionExecutor.onExpiry` is a no-op, so user code never sees those events.

Call `onActivation` when `events` is empty but `expired` is not. `prevWindowEvents` stays empty after the existing `clear()` when there is nothing left in the window. `triggerPolicy.reset()` still runs on every trigger.

```java
prevWindowEvents.clear();
if (!events.isEmpty() || !expired.isEmpty()) {
    if (!events.isEmpty()) {
...
Fully empty triggers (`events` and `expired` both empty) still skip `onActivation`.

### Change 4: Update `WindowFunctionExecutor`

**Path**: `pulsar-functions/instance/src/main/java/org/apache/pulsar/functions/windowing/WindowFunctionExecutor.java`

No new field or user-function detection path is needed. `initializeUserFunction()` continues to accept `WindowFunction` exactly as it does today. The class remains `WindowFunctionExecutor<T, X>` (T=input, X=output); this PIP does not rename its type parameters. `onExpiry` stays a no-op.

Two call-site changes:

1. `processWindow()` snapshots the ack list before calling `process()`, so user mutation of `window.get()` cannot skip `ATLEAST_ONCE` acks. When `tuples` is empty the ack copy is empty. This activation does not ack `getExpired()`. Under `ATLEAST_ONCE`, a record is acked only if some activation included it in a non-empty `get()`.

```java
List<Record<T>> ackTuples = new ArrayList<>(tuples);
output = this.process(
        new WindowImpl<>(tuples, newTuples, expiredTuples, getWindowStartTs(referenceTime), referenceTime),
        new WindowContextImpl(context));
// ack from ackTuples, not tuples
```

2. `process(Window<Record<T>>, WindowContext)` passes the full window view to `WindowFunction`. Bare functions return `null` when `get()` is empty:
...
Promoted from the internal package. The internal `org.apache.pulsar.functions.windowing.Window` type is removed (see Change 1); instance code imports the public interface directly. Marked `@InterfaceStability.Evolving` (same as `StateStore` / `StateStoreContext`); `WindowFunction` stays `@InterfaceStability.Stable`.

| Method | Description |
|--------|-------------|
| `List<T> get()` | All events in the current window. Returns a mutable list that is a **snapshot** for this activation; callers must not modify it or retain it past `process()`. Mutation does not affect live window state or acks. May be empty when the activation exists only to report expirations. |
| `List<T> getNew()` | Events added since the last trigger. Same lifetime and mutability contract as `get()`. |
| `List<T> getExpired()` | Events removed since the last trigger. Same lifetime and mutability contract as `get()`. When the window has drained, this list may be non-empty while `get()` is empty. |
| `Long getStartTimestamp()` | Window start time for this activation. Set by `WindowFunctionExecutor` to `getEndTimestamp() - windowLengthDurationMs` when a time-based window length is configured; `null` for count-based windows. For sliding windows, this bounds the current activation's window interval (window length), not the sliding step. |
| `Long getEndTimestamp()` | Window end time for this activation. Set by `WindowFunctionExecutor` from the trigger policy's reference time. For time-based windows this is the window's end boundary (watermark in event-time mode, processing time in processing-time mode). For count-based windows this is the trigger policy's reference timestamp taken at trigger time, not a window boundary, and `getStartTimestamp()` is `null`. `WindowImpl` stores the value as supplied. |
...
| **List mutability** | `get()`, `getNew()`, and `getExpired()` return mutable `List` instances that are **snapshots for the current activation**. Callers **must not modify** them. `processWindow()` copies `tuples` before `process()`, so mutation cannot skip `ATLEAST_ONCE` acks or update `WindowManager` live state. Javadoc on each getter states this explicitly. |
| **Empty-but-expired delivery** | If `get()` is empty and `getExpired()` is not, `WindowManager` still calls `onActivation`. The executor calls `process(Window, …)` only. The default `process(Window, …)` returns `null` without calling `process(Collection, …)`. Bare `Function` implementations are not invoked. |
...
| Method | Description |
|--------|-------------|
| `T process(Collection<Record<X>> input, WindowContext context)` | Existing abstract method. Existing implementations continue to target this method. Not invoked when `window.get()` is empty. |
| `default T process(Window<Record<X>> window, WindowContext context)` | New default method. Returns `null` when `get()` is empty; otherwise delegates to `process(window.get(), context)` unless a class override provides incremental access (see **Design tradeoff**). |
...
No new metrics are introduced by this change. Event classification (`get()`, `getNew()`, `getExpired()`) is unchanged.
...
The selected design avoids those costs by extending the existing `WindowFunction` interface with a default overload. Existing validation and type inference continue to work because users still implement `WindowFunction`. Deployed functions are instantiated by class name, so they can override `process(Window<Record<X>>, WindowContext)`.
...
**Fully backward compatible for collection-based implementations.** The `WindowFunctionExecutor.initializeUserFunction()` detection path for `WindowFunction` is unchanged. Existing class implementations use the new default method, which delegates to the existing collection-based method when `get()` is non-empty. Class implementations that override both methods use the `Window<Record<X>>` overload as the runtime entry point.

Empty-but-expired activations are new `onActivation` calls, but they do not enter `process(Collection, …)`. The default `process(Window, …)` returns `null`, matching today's skip (no user-function invocation, no output). Implementations that override `process(Window, …)` start seeing those activations; that is opt-in to the new overload.

The runtime instantiates window functions by class name, so a deployed `WindowFunction` is always a class and can override the new overload; the lambda restriction applies only to in-process/test usage.

Today `process(Collection<Record<X>>)` receives the same `tuples` list that `processWindow()` later iterates to ack records under `ATLEAST_ONCE`. Mutating that collection already drops acks with no error. After this change the executor acks from a copy taken before `process()`, so existing implementations that mutate the input collection keep compiling and running; they no longer lose acks as a side effect. Lists remain mutable; wrapping them in `Collections.unmodifiableList` was rejected because it would throw `UnsupportedOperationException` from existing `process(Collection, …)` bodies.
...
- The runtime already classifies `getNew()` and `getExpired()` on every trigger. This PIP exposes those lists through the public API, and additionally delivers `onActivation` when only expirations remain (today that list is discarded).
...
- Intended test coverage: extend `WindowFunctionExecutorTest` / `WindowManagerTest` to verify (1) delegation of the default `process(Window, ...)` overload to `process(Collection, ...)` when the window method is not overridden and `get()` is non-empty, (2) correct invocation of the overridden `process(Window, ...)` path for class-based implementations (including visibility of `getNew()` / `getExpired()` within the call), (3) that `ATLEAST_ONCE` acks still run after the user mutates `window.get()`, (4) empty-but-expired: `onActivation` fires; only `process(Window)` runs; default / bare `Function` do not.
