...
The collection method holds the full-window computation. The window overload uses `getNew()` / `getExpired()` when a running sum is already in memory, and falls back to the collection method on the first activation after construction or restart. The runtime invokes only `process(Window, …)`.
