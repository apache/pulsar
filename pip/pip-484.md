...
That method should contain the full-window computation. `process(Window, …)` should call it when incremental state is missing (first activation after construction or restart). When a class overrides `process(Window, …)`, that override is the only runtime entry point.
...
public class SlidingWindowSumFunction implements WindowFunction<Integer, Integer> {
    ...
    public Integer process(Window<Record<Integer>> window, WindowContext context) throws Exception {
        if (runningSum == null) {
            runningSum = process(window.get(), context).longValue();
...
No new field or user-function detection path is needed. `initializeUserFunction()` continues to accept `WindowFunction` exactly as it does today. The class remains `WindowFunctionExecutor<T, X>` (T=input, X=output); this PIP does not rename its type parameters.

Two call-site changes:

1. `processWindow()` snapshots the ack list before calling `process()`, so user mutation of `window.get()` cannot skip `ATLEAST_ONCE` acks:
...
2. `process(Window<Record<T>>, WindowContext)` passes the full window view to `WindowFunction` (see **Design tradeoff** above):
...
| Method | Description |
|--------|-------------|
| `List<T> get()` | All events in the current window. Returns a mutable list that is a **snapshot** for this activation; callers must not modify it or retain it past `process()`. Mutation does not affect live window state or acks. |
...
| Method | Description |
|--------|-------------|
| `T process(Collection<Record<X>> input, WindowContext context)` | Existing abstract method. Existing implementations continue to target this method. |
...
- Intended test coverage: extend `WindowFunctionExecutorTest` to verify (1) delegation of the default `process(Window, ...)` overload to `process(Collection, ...)` when the window method is not overridden, (2) correct invocation of the overridden `process(Window, ...)` path for class-based implementations (including visibility of `getNew()` / `getExpired()` within the call), and (3) that `ATLEAST_ONCE` acks still run after the user mutates `window.get()`.
