...
public interface ServiceUnitStateTableView extends Closeable {
    ...
    /**
     * Tries to put one item into the tableview. This will update the item in the persistent store.
...
public interface MetadataStoreTableView<T> {
    ...
    /**
     * Tries to delete one item into the tableview. This will delete the item in the metadata store.
     * All peer tableviews (including the local one) will be notified and be eventually consistent with this
     * put value.
     * <p>
