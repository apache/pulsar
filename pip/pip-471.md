...
which makes them harder to alert on and harder to compare with allowed authorization traffic.
...
Operators need a low-cardinality, broker-native signal that shows whether authorization checks are allowed or denied.
...
- Alerting on spikes in authorization denials.
- Comparing authorization denials against allowed authorizations.
- Distinguishing authentication failures from authorization denials.
...
A denial-only metric is also not sufficient. Operators often need allowed, denied, and error counts together to
...
- Record allowed, denied, and errored authorization operations.
...
- whether the result was allowed, denied, or errored
...
Invalid original-principal combinations in proxied authorization flows are counted as authorization denials because the
...
| Method | Behavior |
|---|---|
| `recordAllowed(resourceType, operation)` | Increments the Prometheus counter with `result="allowed"` and adds `1` to the OpenTelemetry counter with `pulsar.authorization.result="allowed"`. |
| `recordDenied(resourceType, operation)` | Increments the Prometheus counter with `result="denied"` and adds `1` to the OpenTelemetry counter with `pulsar.authorization.result="denied"`. |
...
helper records an allowed result. If the provider returns `false`, the helper records a denied result. If the provider
future completes exceptionally, the helper records an error because authorization evaluation failed before a boolean
...
for proxied requests, it records a denied result directly and returns a completed `false` future. Existing
...
| Value | Meaning |
|---|---|
| `allowed` | The authorization request was allowed. |
| `denied` | The authorization request was denied or rejected by authorization handling. |
...
Operators should monitor absolute authorization denials and errors, plus the relationship between denied and allowed
results.
Recommended patterns include:

- Alert on sustained increases in `result="denied"`.
- Alert on sustained increases in `result="error"`, which can indicate authorization provider failures or outages.
- Build dashboards that show `allowed`, `denied`, and `error` together by `resource_type`.
- Investigate rollout regressions by comparing denial rates before and after authorization policy changes.
- Correlate authorization denials with authentication metrics to distinguish authentication incidents from
  authorization incidents.

This proposal enables ratio-based alerting because allowed, denied, and error outcomes are reported in the same metric
family.
...
Failed proxy original-principal validation is counted as an authorization denial because the broker rejects the request
...
- Denial-only counter:
  Rejected because operators often need allowed, denied, and error counts to interpret changes correctly and to build
  ratio-based alerts.
