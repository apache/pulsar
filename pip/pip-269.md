...
- `mark deleted position`(the messages before this position were all acknowledged)
...
- A [compensation mechanism](https://github.com/apache/pulsar/issues/6403) of **Message Replay Read**: For some unknown reason, the rule "reads below Read Position is called **Message Replay Read**" be broken, causing the message which equals `read position` was put into the **Replay Queue**, which leads to repeated consumption. The [compensation mechanism](https://github.com/apache/pulsar/issues/6403) moves the `read position` of the Cursor to the next position to avoid repeated consumption.
- Something wrong makes there a position the same as `read position` in the replay queue,  provides an additional mechanism to increase the `read position` before reading to avoid repeated read(read by Message Replay Read once and read by Sequential Read once).
