...
### Dispatcher(a component used to delivery messages to the client) works with Cursor
...
In the messages delivery scenario, if **Sequential Read** and **Cursor Position Reset** are executed concurrently, the consumption will be out of order,  also leading to consumption being stuck. For example:
...
### Geo Replicator(a component used to replicate messages to the standby cluster) works with Cursor
...
**There have three scenarios that make messages out of order**
#### scenario-1
