...
<del>
For the **Scenario-1** and **Scenario-2**(see Motivation), discard the in-flight reading.
- Discard reading if a **Cursor Position Reset*** is in-progress.
- If a in-flight reading is complete after a new **Cursor Position Reset*** started, discard it.
...
- For the **Scenario-3**(see Motivation), we should do these two things:
  ...
  - Prevent more than one **Cursor Position Reset** executed at the same time(Current Proposal does not consider this optimization, there will be some bug fixes after this PIP)
