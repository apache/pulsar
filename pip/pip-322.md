...
achieve high performance and low overhead. 
There is no separate scheduled task to add tokens. Instead, new tokens
are calculated based on the time passed since the last update and added
to the current tokens balance while ordinary operations happen.

The token balance is updated once in every interval of the configured
