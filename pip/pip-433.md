...
#### Topic auto-creation by Rest API if enabled Geo replication.

The source broker will copy the Rest requests of creates partitioned topic to the remote cluster, if you have already enabled a namespace-level Geo Replication.

The source broker will do the following things when you try to enable a topic-level Geo-Replication, and you did not enable a namespace-level Gep Replication.
- The source broker check whether the partitioned topic exists under the remote cluster
- If yes, the source broker compares both partitions count, enable the topic-level replication if both clusters have the same value of partitions; otherwise, you will get a bad request error.
- If not, it will create the partitioned topic with the same partitions count automatically under the remote cluster.

#### Topic auto-creation by clients if enabled Geo replication.
...
- `Source broker`: starts geo replicator when a sub-topic is loading up. 
  - The geo replicator maintains a internal producer for the topic under the remote cluster.
  ...
  - **(Highlight)** Otherwise, prints error logs and stop to start.

#### Schemas replication if enabled Geo replication.
The internal producer of geo replicator starts with a `auto-produce` schema, copies new schema it reads form the source cluster to the remote cluster, and it will be stuck once a schema is incompatible to the remote cluster.

# Motivation

#### Issue 1: conflict topic creation if enabled Geo-Replication
**Steps to reproduce the issue**
- Configurations of both source cluster and remote cluster 
  - `allowAutoTopicCreation`: `true`
  - `allowAutoTopicCreationType`: `non-partitioned`
  - `defaultNumPartitions`: `2`
- The namespace `public/default` exists, you have not enable Geo-Replication for the namespace.
- Start a producer for a topic `persistent://public/default/topic` on the source cluster.
  - It triggers a partitioned topic with `2` partitions creation.
- Enable namespace-level Geo-Replication for the namespace `public/default`.
- Start geo replicator for the exists sub-topic `public/default/topic-partition-0`
...
#### Issue 2: replication is stuck because remote side does not allow to update schema
**Steps to reproduce the issue**
- The topic`public/default/topic` is enabled replication.
- Users controls topic schema manually, do not allow auto update schema by consumers/producers for both clusters.
- The internal producer is stuck after user set a new schema on the source cluster.
- However, the user wants to allow replicator to copy schema to the remote cluster.

# Goals

1. Add a optional choice: always allow the replicator to register schemas if compatible, even if users set `set-is-allow-auto-update-schema` to false.
2. Add a optional choice: always allow the replicator to trigger a topic auto-creation, including partitioned topics, even if users have disabled topic auto-creation.
...
3. Checks compatibility between two clusters when enabling namespace-level replication, it includes follows
...
#### Regarding the Goal 1: "always allow the replicator to register schemas if compatible"
 ...
 * @param expectedPartitionsAutoCreated take effects when {@param metadataAutoCreationEnabled} is true, broker will
...
pulsar-admin namespaces set-auto-topic-creation 
  ...
  -n, --num-partitions=<defaultNumPartitions>
                        Default number of partitions of topic to be
...
Add a new param for the exits pulsar-admin command `pulsar-admin namespaces get-auto-topic-creation`
...
```shell
--adjust-namespace-level-policies-automatically broker use the policies that applied on the source cluster as a standard
                                                value, to adjust remote cluster policies, including auto-creation
                                                policies, topic partitions of exists topics, and schema update policies.
```

Add a new param for the exists pulsar-admin command `pulsar-admin namespaces set-clusters` to preview what will be modified for the new feature `--adjust-namespace-level-policies-automatically`
