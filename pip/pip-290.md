...
# High-Level Design
**For WSS producers**
- Support parameters `encryptionKeyValues` and `encryptionKeyMetadata` when producers are registering. Web Socket Proxy Server marks the Producer registered with parameter `encryptionKeyValues` as Client-Side Encryption Producer and discards server-side batch messages, server-side compression, and server-side encryption.
  - The parameter `encryptionKeys` was supported before. See [feature](https://github.com/apache/pulsar/pull/16234).
  - Since the order of producer operation for message payloads is `build batch messages payload --> compression --> encryption,` users need to handle Compression themselves if they need this feature(this PIP does not support batch message sending).

**For WSS consumers**
- Instead of printing a warning level log, just print an info level log when delivering the un-decrypted messages to users if the consumer parameter `cryptoFailureAction` is to `CONSUME.` You can get more detail about the policy `CONSUME` [here](https://github.com/apache/pulsar/pull/2024) and the footnote-1<sup>[1]</sup>.
  - Since the order of consumer operation for message payload is `deencryption --> un-compression --> extract the batched messages`, users need to handle Extract Batch Messages and Un-compression themselves if they need these features.
...
####Endpoint: producer connect 
Add query params below: 
| param name | description|
| --- | --- |
| `encryptionKeyValues` | The secret key generated by the client-side producer |
| `encryptionKeyMetadata` | The properties of encryption that need to be passed to the consumer |

####Endpoint: publish messages 
Add query params below: 
| param name | description|
| --- | --- |
...
| `encryptionParam` | Initialization Vector used when the client encrypts |
