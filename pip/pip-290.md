...
**For WSS consumers**: Users can set the parameter `cryptoFailureAction` to `CONSUME` to receive the undecrypted message payload directly(it was supported before). 

# Detailed Design
**For the producers marked as Client-Side Encryption Producer**: 

- forcefully set the component `CryptoKeyReader` to `DummyCryptoKeyReaderImpl`. For `DummyCryptoKeyReaderImpl`, it doesn't provide any public key or private key, and just returns `null`.
- forcefully set the component `MessageCrypto` to `WSSDummyMessageCryptoImpl` to skip the message Server-Side encryption. For `WSSDummyMessageCryptoImpl`, it is a new implementation of `MessageCrypto`, it only handles the message metadata and does not precisely encrypt.
...
**For the client-side encryption consumers**: to avoid too many warning logs: after setting the config `cryptoFailureAction` of the consumer is `CONSUME`, just print an `INFO` level log when receiving an encrypted message if the consumer could not decrypt it(the original log level is `WARN`).
