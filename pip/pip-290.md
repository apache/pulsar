...
The Producer will composite several message payloads into a batched message payload if the producer is enabled batch; the Producer will compress the batched message payload to a compressed payload if enabled compression; After the previous two steps; the Producer encrypts the compressed payload to an encrypted payload. If anyone wants to decrypt the encrypted payload himself, he should also uncompress the compressed message payload and extract the batched message payload himself because encryption is the last step when sending messages.
...
When the internal consumer of Web Socket Proxy Server receives a message, if the message metadata indicates that the message is encrypted, the consumer will add Encrypt Context into the response for the WSS consumer. 
...
- `MessageCrypto`: a tool interface to encrypt and decrypt the message payload, add and extract encryption information for message metadata.
...
# Goals
Provide a way to make Web Socket Proxy Server just passes encrypt information to the client, and WSS producer and WSS consumer did encrypt and decrypt themselves.

Note: I want to cherry-pick this feature into `branch-2.11`.

# High-Level Design
**For WSS producer**
- Support parameters `encryptionKeyValues` and `encryptionKeyMetadata` when producers are registering. Web Socket Proxy Server marks the Producer registered with parameter `encryptionKeyValues` as Client-Side Encryption Producer, and discard server-side batch messages, server-side compression and server-side encryption.
  - the parameter `encryptionKeys` was supported before. See: [feature](https://github.com/apache/pulsar/pull/16234).
  - Since the order of producer operation for message payloads is `build batch message --> comprassion --> encryption`, users need to handle Batch Messages and Compression themselves if they need these features.
- Instead of printing a warning level log, just print an info level log when delivering the un-decrypted messages to users if the consumer parameter `cryptoFailureAction` to `CONSUME.` You can get more detail of the policy `CONSUME` [here](https://github.com/apache/pulsar/pull/2024) and the footnote-1<sup>[1]</sup>.
  - Since the order of consumer operation for message payloads is `deencryption --> un-compression --> extract the batched messages`, users need to handle Extract Batch Messages and Un-compression themselves if they need these features.


Provide a config `enabledE2ECompressionAndEncryption` for the Web Socket Proxy Server to change the behavior like the below:
- Users do the encryption themself before the WSS producer sends messages, and the internal producer of the Web Socket Proxy Server just adds the encryption metadata to the message and does not encrypt the message payload.
-  The internal consumer of the Web Socket Proxy Server just transports the messages to the WSS consumer and does not do decryption; users do the decryption themself after receiving messages from the WSS consumer.
...
# Detailed Design
- Add a `CryptoKeyReader` implementation named `DummyCryptoKeyReaderImpl`, it doesn't provide any public key or private ket, it just returns `null`.
...
- For the client-side encryption consumers
  - Nothing has changed except that the parameter `cryptoFailureAction` is required to set `CONSUME.`
Note: this proposal does not intend to support the two features "Null value messages" and "Chunked messages", please do not use these two features.
...
public void sendMessage(byte[] payload, String msgKey) {
    ...
    msg.encryptionParam = encryptionParam;
