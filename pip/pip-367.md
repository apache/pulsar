# PIP-367: Add a contrib repo for Pulsar
...
Pulsar Contributor Repository is a fork of Pulsar Main Repository, which contains all the core functions of Pulsar and
allows contributors to add customized functions according to their needs.

Anyone can create a branch to add their own features and new functions without anyone's permission. After review, new
functions and optimizations will be maintained in the function list. Users can choose according to their needs and then
customize a release.
...
The Contributor Repository is different from the main contribution process of Pulsar. The Contributor Repository should
take a more flexible approach to handle the integration of new functions to reduce potential functional conflicts:
...
2. **Potential conflicting functions**:

- For new functions or optimizations that may conflict with other functions, we recommend developing them on separate
  branches. This helps us isolate changes and ensure the stability of the main branch.

3. **Branch review and merge**:

- After strict code review, confirm that the conflict-free branches can be merged into the main branch. At the same
  time, update the function document of the main branch, including the newly merged functions. After code review, the
  branches that may conflict now or in the future will update the new functions and commit ids to the function document
  of the main branch.

4. **User-defined integration**:

- Users can select the functions they need according to the description in the function document and integrate them into
  their own fork for customized release.

5. **Version release**

- Maintainers regularly release the main branch content of the contributor's repository and announce the newly added
  features and optimizations in the community for those who need them to choose.
