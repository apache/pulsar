...
public interface BrokerInterceptor extends AutoCloseable {
     ...
     * If the future completes with {@code Optional.empty()}, proceed to the next interceptor or fall back to broker's default logic to list all topics.
     */
    default CompletableFuture<Optional<TopicListingResult>> onGetTopicsOfNamespace(
            ...
            AsyncDualMemoryLimiter.LimitType.HEAP_MEMORY, isPermitRequestCancelled, initialPermits -> {
                ...
                return interceptorFuture.thenCompose(interceptorResult -> {
                    ...
                    if (interceptorResult != null && interceptorResult.isPresent()) {
                        // Not support in Topic List Watcher mode
                        if (enableTopicListWatcher) {
                            return CompletableFuture.failedFuture(
                                new UnsupportedOperationException(
                                    "Customizable topic listing is not supported when "
                                    + "enableBrokerTopicListWatcher is true"));
                        }
