...
public class KubernetesSecretsTokenAuthProviderTest {
    ...
    public void testConfigureAuthDataStatefulSet() {
        ...
        FunctionAuthData functionAuthData =
                FunctionAuthData.builder().data("foo".getBytes()).build();
        kubernetesSecretsTokenAuthProvider.configureAuthDataStatefulSet(statefulSet, Optional.of(functionAuthData));

        Assert.assertEquals(
                statefulSet.getSpec().getTemplate().getSpec().getVolumes().size(), 1);
        Assert.assertEquals(
                statefulSet
                        .getSpec()
                        .getTemplate()
    ...
    public void testCacheAuthData() throws ApiException {
        CoreV1Api coreV1Api = mock(CoreV1Api.class);
        doReturn(new V1Secret())
                .when(coreV1Api)
                .createNamespacedSecret(anyString(), any(), anyString(), anyString(), anyString(), anyString());
        KubernetesSecretsTokenAuthProvider kubernetesSecretsTokenAuthProvider =
                new KubernetesSecretsTokenAuthProvider();
        kubernetesSecretsTokenAuthProvider.initialize(coreV1Api, null, (fd) -> "default");
        ...
        Optional<FunctionAuthData> functionAuthData =
                kubernetesSecretsTokenAuthProvider.cacheAuthData(funcDetails, new AuthenticationDataSource() {
                    @Override
        ...
        Assert.assertTrue(
                StringUtils.isNotBlank(new String(functionAuthData.get().getData())));
    }
    ...
    public void configureAuthenticationConfig() {
        ...
        FunctionAuthData functionAuthData =
                FunctionAuthData.builder().data("foo".getBytes()).build();
        kubernetesSecretsTokenAuthProvider.configureAuthenticationConfig(
                authenticationConfig, Optional.of(functionAuthData));
    ...
    public void testUpdateAuthData() throws Exception {
        ...
        functionAuthData = kubernetesSecretsTokenAuthProvider.updateAuthData(
                funcDetails, existingFunctionAuthData, new AuthenticationDataSource() {
                    @Override
                    public boolean hasDataFromCommand() {
                        return true;
