...
public class ProcessRuntimeTest {
    ...
    class TestSecretsProviderConfigurator implements SecretsProviderConfigurator {
        ...
        public void configureKubernetesRuntimeSecretsProvider(
                V1PodSpec podSpec, String functionsContainerName, FunctionDetails functionDetails) {}

        @Override
        public void configureProcessRuntimeSecretsProvider(
                ProcessBuilder processBuilder, FunctionDetails functionDetails) {}
    ...
    private ProcessRuntimeFactory createProcessRuntimeFactory(
            String extraDependenciesDir, String webServiceUrl, boolean exposePulsarAdminClientEnabled) {
        ...
        processRuntimeFactory.initialize(
                workerConfig,
                null,
    ...
    private void verifyJavaInstance(InstanceConfig config, Path depsDir, String webServiceUrl) throws Exception {
        List<String> args;
        try (MockedStatic<SystemUtils> systemUtils =
                Mockito.mockStatic(SystemUtils.class, Mockito.CALLS_REAL_METHODS)) {
            systemUtils
                    .when(() -> SystemUtils.isJavaVersionAtLeast(JavaVersion.JAVA_9))
                    .thenReturn(true);
            ProcessRuntime container =
                    factory.createContainer(config, userJarFile, userJarFile, userJarFile, userJarFile, 30L);
