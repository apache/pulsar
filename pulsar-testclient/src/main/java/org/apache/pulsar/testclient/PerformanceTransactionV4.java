...
/**
 * Runs the {@code transaction} benchmark with the v4 ({@code pulsar-client-original}) client and the
 * v4 transaction coordinator, which stays a live broker code path alongside the v5 one (PIP-473 P5.4).
 *
 * <p>Used for {@code persistent://} and unprefixed topics, and for any topics with
 * {@code --client-api V4}. The benchmark itself comes from {@link PerformanceTransactionBase} and only the
 * client bindings differ. Two things it measures that the V5 client cannot: the acknowledgement round
 * trip (v4 {@code acknowledgeAsync} completes when the broker has replied, whereas V5's
 * {@code acknowledge} is a synchronous void), and real {@code Exclusive}/{@code Failover}/
 * {@code Key_Shared} subscription types.
 */
public class PerformanceTransactionV4 extends PerformanceTransactionBase<PulsarClient, Producer<byte[]>,
    ...
    public PerformanceTransactionV4(PerformanceTransaction arguments) {
        super(arguments);
    }
