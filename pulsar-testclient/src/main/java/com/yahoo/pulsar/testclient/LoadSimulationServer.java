...
/**
 * LoadSimulationServer is used to simulate client load by maintaining producers and consumers for topics.
 * Instances of this class are controlled across a network via LoadSimulationController.
 */
public class LoadSimulationServer {
    ...
    // A TradeUnit is a Consumer and Producer pair. The rate of message consumption as well as size may be changed at
    ...
    private static class TradeUnit {
        ...
        // Creating a byte[] for every message is stressful for a client machine, so in order to ensure that any
        // message size may be sent/changed while reducing object creation, the byte[] is wrapped in an AtomicReference.
        ...
        public TradeUnit(final TradeConfiguration tradeConf, final PulsarClient client,
            ...
            // Add a byte[] of the appropriate size if it is not already present in the cache.
        ...
        // Attempt to create a Producer indefinitely. Useful for ensuring messages continue to be sent after broker
        // restarts occur.
        ...
        public void start() throws Exception {
            ...
            while (!stop.get()) {
                ...
                final Function<Throwable, ? extends MessageId> exceptionHandler = e -> {
                    // Unset the well flag in the case of an exception so we can try to get a new Producer.
                ...
                producer.closeAsync();
                if (!stop.get()) {
                    // The Producer failed due to an exception: attempt to get another producer.
    ...
    // Decode TradeConfiguration fields common for topic creation and modification.
    ...
    private void handle(final byte command, final DataInputStream inputStream, final DataOutputStream outputStream)
        ...
        switch(command) {
            case CHANGE_COMMAND:
                // Change the topic's settings if it exists. Report whether the topic was found on this server.
                ...
                break;
            case STOP_COMMAND:
                // Stop the topic if it exists. Report whether the topic was found, and whether it was already stopped.
                tradeConf.topic = inputStream.readUTF();
                if (topicsToTradeUnits.containsKey(tradeConf.topic)) {
            ...
            case CHANGE_GROUP_COMMAND:
                // Change the settings of all topics belonging to a group. Report the number of topics changed.
            ...
            case STOP_GROUP_COMMAND:
                // Stop all topics belonging to a group. Report the number of topics stopped.
                decodeGroupOptions(tradeConf, inputStream);
                // See if a topic belongs to this tenant and group using this regex.
    ...
    public void run() throws Exception {
        ...
        while (true) {
            // Technically, two controllers can be connected simultaneously, but non-sequential handling of commands
