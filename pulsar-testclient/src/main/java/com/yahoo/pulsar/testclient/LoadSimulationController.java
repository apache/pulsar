 ...
 * On each of these machines, get them to listen via pulsar-perf simulation-server --port <chosen port> --service-url
 * <broker service url> 4. Start the controller with pulsar-perf simulation-controller --cluster <cluster name>
 * --servers <comma separated list of <hostname>:<port> --server-port <chosen port> 5. You will get a shell on the
 * controller, where you can use the commands trade, change, stop, trade_group, change_group, stop_group. You can enter
 * "help" to see the syntax for the commands. Note that tenant, namespace, and topic refer to
 * persistent://cluster/tenant/namespace/topic/bundle. For instance, to start trading for topic with destination
 * persistent://mycluster/mytenant/mynamespace/mytopic/bundle at rate 200 msgs/s, you would type "trade mytenant
 * mynamespace mytopic --rate 200". The group commands also refer to a "group_name" parameter. This is a string that is
 * prefixed to the namespaces when trade_group is invoked so they may be identified by other group commands. At the
 * moment, groups may not be modified after they have been created via trade_group.
 *
 */
